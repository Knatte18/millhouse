...
- **Requirements:**
  ...
  5. Update the module-level Public API docstring entry for `recreate_active_junction` (currently at line ~51 with the stale signature `(wiki_path, slug, mill_dir)`) to match the new signature: `recreate_active_junction(slug, hub_root, container_path) -> None`. The public-API list must stay self-consistent with the actual function signature.
...
- **Deletes:** none
- **Requirements:**
  1. `test_write_initial_status`: change the assertion `status_path != repo / "status.md"` to `status_path != repo / "task" / "status.md"`. Also update the git log path-filter argument from `"status.md"` to `"task/status.md"` (`["git", "-C", str(repo), "log", "--oneline", "task/status.md"]`) — Card 4 changes the committed file path; the path filter must match. The expected commit message (`"spawn: init status for task-one"`) is unchanged.
  2. `test_write_initial_status_forced_failure_raises_runtime_error`: update the error-substring check from `"git add status.md failed"` to `"git add task/status.md failed"` — Card 4 req 1 changes the error string prefix; the substring check must match.
...
- **Requirements:**
  ...
  **test-millpy-claim.py:**
  3. In `test_main_happy_path` (the test with all helper call assertions): change the comment on line ~241 from `"Verify new signature: (slug, mill_dir, container_path)"` to `"Verify new signature: (slug, hub_root, container_path)"`. Change `expected_mill_dir = Path("/fake/repo") / ".millhouse"` to `expected_hub_root` set to whatever `resolve_hub_path()` is mocked to return in that test. Change the assertion `rac_call.args[1] != expected_mill_dir` to `rac_call.args[1] != expected_hub_root` and update the error message to name `hub_root`. Both standard claim fixture blocks (lines ~83/88 and ~136/141) currently set `resolve_git_root` and `resolve_hub_path` to the same `Path("/fake/repo")`. Update the `resolve_hub_path` mock in **both** blocks to return a distinct path (e.g. `Path("/fake/repo/subdir")`) so the test catches the `git_root` vs `resolve_hub_path()` distinction. `expected_hub_root` then becomes `Path("/fake/repo/subdir")`.
