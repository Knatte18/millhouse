...
- **Requirements:**
  ...
  5. Update the import line to add `write_wiki_active_task_md` to the imports from `_spawn_core`. (Do not add a test for it here — it requires a real wiki git repo; the mill-spawn integration is tested in test-millpy-spawn.py.)
- **Commit:** `test(test-spawn-core): task/status.md path; hub_root signature for recreate_active_junction`
