...
Mirrors the `_review_plan` integration in `_review_code.run`: ERROR-handling parity (no-raise pattern), `deletes_union` plumbing, `## Intentionally deleted` surface, and timeout plumbing for both per-batch and holistic calls. No mid-round resume here — code review is single-scope per call (one batch OR holistic, not both) so there's no in-call resume window. Depends on `review-plan-integration` because Card 21 reads `cfg["llm"]["holistic_timeout"]` — that key is added to `wiki/config.yaml` by B04 Card 13. Without the dependency edge B05 could land first and break with `KeyError`. Both batches consume the `content-helpers` foundation directly.
