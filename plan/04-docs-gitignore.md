...
- **Modifies:**
  - `plugins/mill/skills/mill-add/SKILL.md`
  - `plugins/mill/skills/mill-setup/SKILL.md`
  - any other operational SKILL.md file under `plugins/mill/skills/` whose Bash command examples reference source-tree paths instead of `${CLAUDE_PLUGIN_ROOT}`
- **Creates:** none
- **Requirements:** In `mill-add/SKILL.md` and `mill-setup/SKILL.md`, locate the operational Bash invocation examples (the lines that show the `uv run` or `python` command Claude Code is supposed to run when executing the skill). For each, ensure the project path and script path resolve via `${CLAUDE_PLUGIN_ROOT}`, not via `plugins/mill/...`. If a wrong/right example pair is appropriate (i.e. the skill currently demonstrates an invocation pattern), add a short comment block — keep it to two fenced bash examples, labeled WRONG and RIGHT, mirroring the CLAUDE.md style. For every other mill skill listed under Reads: scan for `plugins/mill/scripts` or `plugins/codeguide/scripts` substrings; classify each hit as descriptive prose (keep, no change), file-path reference inside Reads/Modifies/Creates lists or other markdown (keep, no change), or operational Bash command (FIX — replace with `${CLAUDE_PLUGIN_ROOT}/scripts/...`). The hit list from the discussion's verify pass: `mill-add/SKILL.md` lines 8/122/151 (descriptive — keep), `mill-resume/SKILL.md` line 127 (descriptive — keep), `mill-skills-index/SKILL.md` line 20 (verify whether it is operational; if so, fix). Re-grep the listed skills as part of this card to catch anything the discussion missed.
