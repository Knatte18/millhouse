...
- **Modifies:**
  ...
  - `plugins/mill/scripts/millpy-spawn.py`
- **Creates:** none
- **Requirements:** In every caller, delete the explicit `_wiki.acquire_lock(...)` / `_wiki.release_lock(...)` calls and the surrounding `try/finally` boilerplate. Add `slug=<value>` kwarg to every `_wiki.sync_pull(wiki_path)` and `_wiki.write_commit_push(wiki_path, paths, msg)` call. Slug values: `_spawn_core.py:448-477` and `:599-609` use the existing local `slug` / `merged_slug` variables; `millpy-add.py:170-204` uses `args.slug`; `millpy-cleanup.py:423,443,463` uses literal `"mill-cleanup"`; `millpy-abandon.py:94-103` uses local `slug`; `millpy-claim.py:179` uses literal `"mill-claim"` (the early sync_pull happens before any slug is picked); `millpy-spawn.py:119` uses literal `"mill-spawn"` (same reason). After this card lands, no script outside `_wiki.py` references `_wiki.acquire_lock` / `_wiki.release_lock`.
