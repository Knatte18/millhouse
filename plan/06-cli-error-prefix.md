...
- **Deletes:** none
- **Requirements:** Create a new module `plugins/mill/scripts/_review_cli.py`. Module docstring: `"""Shared CLI helpers for the review subsystem. Today exposes one helper: print_error(exc) — formats a ReviewError with an uppercase ERROR: prefix and an optional one-line hint when the message is from resolve_ref_paths. Used by millpy-review-discussion.py, millpy-review-plan.py, millpy-review-code.py."""`. Public API: `print_error(exc: ReviewError) -> None`. Implementation: import `sys` and `from _review_common import ReviewError`; emit the prefixed line via `print(f"ERROR: {exc}", file=sys.stderr)` (no `\n` in the format string — `print` appends one); if `str(exc).startswith("[resolve_ref_paths]")`, additionally emit `print("Hint: check the plan card referencing this file; if the file is intentionally deleted, list it under Deletes: in that card.", file=sys.stderr)`. Use `print(..., file=sys.stderr)` rather than direct `sys.stderr.write` so trailing newlines stay single (Card 25 test (a) asserts a single trailing newline). No `if __name__ == "__main__":` block (per CLAUDE.md). No other public symbols.
