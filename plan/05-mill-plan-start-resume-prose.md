...
- **Modifies:**
  - `plugins/mill/skills/mill-plan/SKILL.md`
- **Creates:** none
- **Requirements:** Replace `1. wiki.sync_pull() on the wiki clone.` with the concrete invocation: `1. Resolve the wiki path via _paths.resolve_wiki_path(_paths.resolve_git_root(Path.cwd())) and call _wiki.sync_pull(wiki_path, slug="mill-plan").` Add the canonical signature line: `signature: _wiki.sync_pull(wiki_path: Path, *, slug: str) -> None`. In step 1.5 (validator-fix commit, around line 93), replace `mill-plan commits the fix(es) to plan files via _wiki.write_commit_push(wiki_path, [f"active/{slug}/plan/"], f"mill-plan: validator-fix pass for {slug}")` with `mill-plan commits the fix(es) on the task branch: git -C <worktree> add plan/ && git -C <worktree> commit -m "mill-plan: validator-fix pass for {slug}"` (no push; matches the rest of mill-plan's task-branch commit pattern). The commit message keyword `validator-fix` is preserved to distinguish from `plan-fix-r{N}` LLM-fix-pass commits. **Also fix step 4b's fixer-report path:** the current SKILL.md step 4b writes the report to `` `<WIKI_PATH>/active/<slug>/reviews/<YYYYMMDD-HHMMSS>-plan-fix-r<N>.md` `` — replace with `` `reviews/<YYYYMMDD-HHMMSS>-plan-fix-r<N>.md` (worktree root) ``, matching step 4c's pattern. **Broad sweep criterion:** search the file for any occurrence of `` `<WIKI_PATH>/active/ `` prose paths (not just API call sites), in addition to sweeping for `_wiki.write_commit_push` references; convert any found to their worktree-root equivalents. Do not use "none expected" hedges — at least one bad prose path (step 4b's reviews path) is now known to exist.
