...
- **Modifies:**
  ...
  - `plugins/mill/skills/mill-cleanup/SKILL.md`
- **Creates:** none
- **Requirements:** Each of these 12 skills has a `## Run it` (or similarly-named) section containing one or more `python ${CLAUDE_PLUGIN_ROOT}/scripts/millpy-X.py [args]` invocations in a fenced bash block. Note: `mill-worktree/SKILL.md` has three (`create`, `remove`, `list`); the others have one each. Replace EVERY invocation in each file with `uv run --project "${CLAUDE_PLUGIN_ROOT}" "${CLAUDE_PLUGIN_ROOT}/scripts/millpy-X.py" [args]` — preserve all argument flags exactly. No other text changes per skill in this card. After edits, grep verification: `Grep -P 'python\s+\$\{CLAUDE_PLUGIN_ROOT\}'` across the 12 files returns zero matches.
...
- **Modifies:**
  - `plugins/mill/skills/mill-go/SKILL.md`
- **Creates:** none
- **Requirements:** (1) Replace `python plugins/mill/scripts/millpy-review-code.py …` (line ~96) with `uv run --project "${CLAUDE_PLUGIN_ROOT}" "${CLAUDE_PLUGIN_ROOT}/scripts/millpy-review-code.py" …`. Apply the same transformation to every other invocation in the file (line ~131 etc.). (2) Audit helper-call examples in prose against the real function signatures (verified by `inspect.signature` on each `_*.py` helper): `_status.append_phase(status_path, phase, timestamp) -> None`, `_status.set_batch_field(status_path, name, key, value) -> None`, `_wiki.write_commit_push(wiki_path, relative_paths, commit_msg)` (3-arg), `_wiki.acquire_lock(wiki_path, slug, timeout_seconds=30)`, `_wiki.release_lock(wiki_path) -> None`, **`_tasks_md.set_phase(text: str, slug: str, phase: str | None) -> str`** (NB: takes the file's text as the first arg, not a Path; returns the rewritten text — does NOT mutate in place). The current mill-go SKILL.md call `_tasks_md.set_phase(home_path, slug, "done")` is WRONG against this signature — fix the SKILL.md prose to instruct reading `home_path.read_text(...)`, calling `_tasks_md.set_phase(text, slug, "done")`, and writing the returned string back via `home_path.write_text(...)`. Also: `_notify.notify(...)` (verify against `_notify.py`). (3) Replace any `plugins/mill/scripts/...` repo-relative path in invocation lines with `${CLAUDE_PLUGIN_ROOT}/scripts/...`. Prose mentions ("`_wiki.write_commit_push` is the canonical writer") stay as-is.
...
- **Modifies:**
  - `plugins/mill/skills/mill-resume/SKILL.md`
- **Creates:** none
- **Requirements:** (1) Replace the broken `PYTHONPATH=<scripts-dir> python -m millpy.entrypoints.regenerate_sidebar` (line ~147) with `uv run --project "${CLAUDE_PLUGIN_ROOT}" python -c "from pathlib import Path; import _sidebar; _sidebar.regenerate(Path(r'<wiki-dir>').resolve())"`. (2) **Phase 8 has TWO bugs to fix at lines 134-135** — a broken import AND a reversed-argument call: (a) replace `from millpy.core.junction import create as junction_create` (line 134) with `from _junction import create as junction_create` — `millpy` package does not exist (verified by grep). (b) the call on line 135 — `junction_create(new_worktree / ".millhouse" / "wiki", wiki_clone_path)` — has its arguments REVERSED against `_junction.create(target: Path, link_path: Path)`. The verified signature: `target` is the existing directory the junction POINTS AT; `link_path` is the location WHERE the junction is created. Rewrite the call to `junction_create(wiki_clone_path, new_worktree / ".millhouse" / "wiki")`. Note: card 18's grep sweep does not catch positional-argument reversal, so fixing this is solely card 15's responsibility. (3) Migrate any other invocations in the file to `uv run --project "${CLAUDE_PLUGIN_ROOT}" …`. (4) Audit any other helper-call examples; verify `_sidebar.regenerate(wiki_path)` is the real signature.
- **Commit:** `mill-resume(skill): fix broken millpy refs + migrate to uv run`
...
- **Modifies:**
  - `plugins/mill/skills/mill-skills-from-scripts/SKILL.md`
- **Creates:** none
- **Requirements:** (1) Replace `python ${CLAUDE_PLUGIN_ROOT}/scripts/millpy-<X>.py …` (line ~51) with `uv run --project "${CLAUDE_PLUGIN_ROOT}" "${CLAUDE_PLUGIN_ROOT}/scripts/millpy-<X>.py" …`. Same for `python ${CLAUDE_PLUGIN_ROOT}/scripts/millpy-skills-index.py` (line ~61). The `<X>` is a meta-token in the skill prose (not a real shell variable) — preserve it. (2) Remove the line `sys.path.insert(0, '${CLAUDE_PLUGIN_ROOT}/scripts')` from any "How to invoke the helper" Python block — redundant once the global PYTHONPATH env var is set by mill-setup.
...
- **Requirements:** Run grep verification across every file under `plugins/mill/skills/` matching `*/SKILL.md` (40+ files including style/workflow/conversation that this batch does not otherwise touch) to confirm the migration is complete and consistent. The card itself does not modify any file directly — if a grep below catches an issue, the implementer fixes it by amending the appropriate earlier card's commit (cards 7-17, depending on which file fails). The audit is verification-only at this card; modifications belong to whichever card owns the failing file.
  ...
  - Grep `from\s+millpy\.` → must be zero. Catches Python `import` statements inside fenced code blocks that reference the non-existent `millpy` package (e.g. mill-resume Phase 8's `from millpy.core.junction import create as junction_create`, fixed in card 15).
