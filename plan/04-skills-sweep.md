...
- **Modifies:**
  - `plugins/mill/skills/mill-resume/SKILL.md`
- **Creates:** none
- **Requirements:** (1) Replace the broken `PYTHONPATH=<scripts-dir> python -m millpy.entrypoints.regenerate_sidebar` (line ~147) with `uv run --project "${CLAUDE_PLUGIN_ROOT}" python -c "from pathlib import Path; import _sidebar; _sidebar.regenerate(Path(r'<wiki-dir>').resolve())"`. (2) **Phase 8 also has a broken import** at line 134: `from millpy.core.junction import create as junction_create`. The `millpy` package does not exist (verified by grep across `plugins/mill/`). Replace this import with the flat-helper pattern used elsewhere in the codebase: `from _junction import create as junction_create` (or fold the call into a `uv run --project "${CLAUDE_PLUGIN_ROOT}" python -c "..."` bash invocation if that's how Phase 8 is structured). Verify `_junction.create` is the real API (`grep -n "^def create" plugins/mill/scripts/_junction.py`). (3) Migrate any other invocations in the file to `uv run --project "${CLAUDE_PLUGIN_ROOT}" …`. (4) Audit any other helper-call examples; verify `_sidebar.regenerate(wiki_path)` is the real signature.
