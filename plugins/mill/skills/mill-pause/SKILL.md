...
- **If an Agent-mode dispatch (implementer/reviewer/fixer) is in flight:** wait for its terminal event (the SubagentHandback message or the `<task-notification>`, whichever arrives first), then run the CLI's `--stage finalize` call so the round is correctly recorded in status.md/reviews, then stop cleanly — do not dispatch the next round or batch. Calling `TaskStop` on a running Agent-mode dispatch is **forbidden**: the round is not resumable from a kill, and `discover_round` would just re-run it from scratch on the next `/mill-go` or `/mill-plan`, silently discarding the in-flight work.
