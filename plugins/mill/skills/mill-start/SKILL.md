...
- Before the loop, initialise: `prev_blocking_titles: set[str] = set()`, `extension_used: bool = False`, `parent_escalated_start_cap: bool = False` and `parent_retry_round: int | None = None`.
...
- At the end of each REQUEST_CHANGES round (after committing and pushing fixes): (1) read the round's `findings` list from the JSON envelope -- this list is post-ceiling, so a finding the ceiling demoted to NIT is not present as BLOCKING and correctly does not count toward non-progress -- and take the `title` of every entry whose `severity` is `BLOCKING` into `current_blocking_titles`;
  ...
  otherwise (overlap exists, or `extension_used` is already `True`): **if `auto_approve_on_cap` is `True`:** treat this occurrence as an implicit approval instead of halting — this round's BLOCKING findings already had fixes applied to `<discussion_path>` (per the auto-resolve rule above), so accept that edited state: call `_status.append_phase(status_path, f"discussion-fix-r{N}", _timestamp.now_utc_iso())`, then `_status.append_phase(status_path, "discussed", _timestamp.now_utc_iso())`; single git commit covering `<discussion_path>`, `<reviews_dir>/`, `<status_path>`, `_mill/briefs/` with message `"mill-start: discussion-fix round {N} for {slug} (auto-approved on round-cap exhaustion, config auto_approve_on_cap)"`; push; report the Handoff completion message directly (do not re-run Phase: Handoff's status-append/commit) and break the loop — do NOT run round N+1. **Otherwise** (`auto_approve_on_cap` is `False`, today's behavior; interactive mode never reaches this branch): first, when `parent_escalated_start_cap` is false, set it true and load the `ask-thread` skill with site `start-cap`, reason `f"auto: discussion review gaps unresolved after {N} rounds"`, actions `approve,retry,halt`.
  ...
  On `halt`: the existing halt below, with `halt_suffix` appended to the `_status.set_blocked` reason.
  The halt: call `_status.set_blocked(status_path, f"auto: discussion review gaps unresolved after {N} rounds", timestamp=_timestamp.now_utc_iso())`, then `if [ -d <worktree>/_mill/briefs ]; then git -C <worktree> add _mill/briefs/; fi && git -C <worktree> add <status_path> && git -C <worktree> commit -m "mill-start: blocked (auto: discussion review gaps unresolved) for <slug>" && git -C <worktree> push`, then halt — do NOT proceed to Handoff;
...
- Because `extension_used` was just set to `True` in this iteration, the next iteration's Step 2 dispatch (the discussion-review prepare call) for this extension round MUST pass `--max-rounds <max_review_rounds + 1>` (Agent-mode: as `<args>`;
  ...
  When `parent_retry_round` is set and the round being dispatched is `>= parent_retry_round`, pass `--max-rounds <round>` (the round being dispatched) instead — the extension round may already be spent, so the cap is computed from the current round.
...
- `round_n == 1`: load the `orch-wait` skill via the Skill tool now and follow it in place of the normal Step 2 dispatch — it notifies `parent_thread` (when set) that `discussion.md` is ready with one `SendMessage` naming `/orch-review <slug>`, then handles the wait, the substitute review, and handing back an envelope shaped exactly like Step 2's own output.
  The notification needs the spawner to pass `--parent <name>`; without it, or when the message fails, the orchestrator arms `/orch-review <slug>` by hand as before. Resume this SKILL's Phase: Discussion Review at step 3 with that envelope.
...
2. Load config — deep-merge `<hub_root>/mill-config.yaml` (shared hub overlay) with `.millhouse/config.local.yaml` (gitignored worktree overlay).
   ...
   Read `roles.discussion-review.holistic.min_rounds` as `min_review_rounds` (default `1` when absent — see "Convergence gate" in Phase: Discussion Review below).
   Read `roles.discussion-review.holistic.auto_approve_on_cap` as `auto_approve_on_cap` (default `False` when absent) — only consulted by the `--auto`/`--orch` non-progress-check branch in "Phase: Discussion Review — `--auto` changes" above; interactive mode never reads it, since an operator is already present to decide. Under `--orch`, also initialise `prev_verdict_was_approve` per "## Orch mode"'s Convergence-gate addendum. `signature: _config.load_config(hub_root: Path, worktree_root: Path) -> dict`
...
This is the one site in mill with no brief, no resume requirement, and no per-role model tier to lose from forking — but it is NOT a site with no tool restriction to lose: a fork inherits the parent's full tool access exactly as documented in `${CLAUDE_PLUGIN_ROOT}/skills/mill-go-base/SKILL.md`'s "Why not fork?" paragraph, and a live incident (#919) showed an in-context research fork, dispatched mid-`--auto` session for a narrowly-scoped read-only investigation, instead inherited the full session context and autonomously executed the entire downstream pipeline — writing and committing discussion.md, writing a full plan, running a plan-fix round, and dispatching a live reviewer, all pushed to origin before the operator noticed.
...
2. **Dispatch mode:** Resolve dispatch mode via `_agent_dispatch.resolve_dispatch_mode(cfg)`.
   ...
   **Subprocess/psmux branch — Background the CLI via `millpy-bg`:**
   ...
   > Only when this round is the Auto mode non-progress-extension round (per the rule in "Phase: Discussion Review — `--auto` changes" above), append ` --max-rounds <max_review_rounds + 1>` to the inner `millpy-review-discussion.py` invocation below; when `parent_retry_round` is set and this round is `>= parent_retry_round`, append ` --max-rounds <round>` instead, which supersedes `<max_review_rounds + 1>`; omit it on every other round.
