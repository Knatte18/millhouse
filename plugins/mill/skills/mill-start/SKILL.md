...
- If gaps remain after `max_review_rounds`: call `_status.set_blocked(status_path, f"auto: discussion review gaps unresolved after {N} rounds", timestamp=_timestamp.now_utc_iso())`, then `git -C <worktree> add _mill/status.md && git commit -m "mill-start: blocked (auto: discussion review gaps unresolved) for <slug>" && git push`, then halt with that message. Do NOT proceed to Handoff.
