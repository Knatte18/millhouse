...
Language-specific skills build on these.
...
- **Test behavior, not implementation.**
  Tests should verify observable outcomes, not internal state or implementation details.
  If you refactor the internals, the tests should still pass.
- **Every test path.**
  Happy path is the minimum.
  Always cover:
...
- **All existing tests must pass.**
  ...
  Run the full suite, not just new/related tests.
- **No shallow tests.**
  A test that asserts "function returned something" is worthless.
...
- **Strict equality.**
  Prefer exact equality over loose containment checks.
...
- **Last resort.**
  Prefer fakes, stubs, or in-memory implementations over mocking frameworks.
- **Never mock your own code.**
  Mock only external dependencies you do not control.
...
- **Terminology matters:**
  ...
  - Use the correct term.
    Do not call everything a "mock".

## Naming

- Test names describe **behavior**, not implementation.
  The name should read as a sentence describing what is expected.
