...
Then run a full validator re-run: call `_plan_validate.run` with the identical 7 keyword arguments (`root`, `git_root`, `wiki_root`, `skip_checks=plan_skip_checks`, `parent_branch`, `max_cards_per_batch`, `max_batch_context_tokens`) that Phase: Plan's own self-validate call already uses (all already bound/resolvable at this point in Phase: Plan Review).
