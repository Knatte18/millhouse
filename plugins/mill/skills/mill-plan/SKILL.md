...
6. **Max-rounds escape** (only when round counter exhausts without APPROVE, BLOCKINGs still remain, non-progress did not fire, the operator has not given a live step-6-waiver instruction, AND `auto_approve_on_cap` is not `True` — see "Live operator waiver of step 6" above; when either the live waiver or `auto_approve_on_cap: true` applies, the implicit-approve-at-cap path documented there fires instead of this halt): `_status.set_blocked(status_path, f"max-rounds exhausted after {N} rounds, {M} BLOCKINGs remain", timestamp=ts)`;
   commit and push;
