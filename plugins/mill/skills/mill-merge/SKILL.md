...
1.5. **Path Setup.** `cfg` was loaded in step 1; `container_path` and `slug` are in scope from Step 1. (If the stale-worktree edge above already derived `worktree_root`/`status_path` inline, this step is a no-op re-derivation — the computation is idempotent.) Derive:
   ```python
