...
Public API:
    ...
    dict.
        Used by interactive launchers that bypass run().
"""
...
# Session-scoped markers a parent Claude Code process injects into its own environment.
# A child `claude` process spawned with these still present believes it is a sub-session of the parent and disables transcript saving.
# Persistent user config like CLAUDE_CODE_USE_BEDROCK/_VERTEX is deliberately excluded — this is an exact-match allowlist, not a CLAUDE_CODE_ prefix strip.
...
def run(
    ...
    *,
    ...
    Args:
        ...
        cwd: Working directory for the child. ``None`` inherits the caller's cwd.
        ...
        stdout: Override for the stdout stream.
            When None (default), ``subprocess.PIPE`` is used and output is captured.
...
Returns the Popen handle.
    ...
    if os.name == "nt":
        # Two-stage launch: cmd.exe shim dispatches via `start /B`, escaping the parent's Win32 Job Object.
        # The empty "" is the required title argument for `start` — omitting it causes the first real argument to be misinterpreted when the path contains spaces.
        # DETACHED_PROCESS is intentionally omitted — see comment above on CREATE_NO_WINDOW being sufficient and DETACHED_PROCESS causing a console flash when combined with CREATE_NO_WINDOW.
        popen_kwargs["creationflags"] = (
