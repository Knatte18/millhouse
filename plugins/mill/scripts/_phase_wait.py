...
mill-go waiting for mill-plan to reach ``planned``) instead of returning control to the operator and
requiring a manual re-invocation.
This module is pure string-building and predicate logic only -- it performs no file I/O, network
access, or subprocess call of its own.
The caller is responsible for actually running the returned poll script (via the harness ``Monitor``
tool) and for reading ``status.md``.
...
def build_wait_command(
    ...
    ``Path.write_text`` with no ``newline=`` override, so a Windows-authored ``status.md`` can carry
    CRLF line endings;
    GNU/POSIX ``grep``'s trailing ``$`` anchor does not match immediately before a ``\\r``, so an
    un-piped ``grep`` would never see ``READY``/``blocked`` on such a file and the wait would
    silently run out the full ``giveup_s`` clock.
    Stripping ``\\r`` first makes the trailing-``$`` anchor reliable regardless of which platform
    wrote the file.
    Both ``grep`` patterns are anchored with a trailing ``$`` as defensive future-proofing against a
    hypothetical future phase value that string-prefix-extends an existing target (e.g.
    a future ``planned-v2`` falsely matching an unanchored ``grep "^phase: planned"``).

    This function never reads or knows about ``pipeline.entry_wait_timeout_minutes`` -- the
    ...
    Args:
        ...
        ready_phase: The single-word phase value to wait for (always one of the literal values
            ``planned`` / ``discussed`` in this codebase, never attacker- or user-supplied).
...
def matches_wait_trigger(
    ...
    or if it full-matches (via ``re.fullmatch``, never ``str.startswith``) any pattern in
    ``regex_patterns``.
    The parameter is named ``regex_patterns`` rather than ``prefix_patterns`` because the values it
    holds in this codebase's actual usage (``^plan-review-r\\d+$``, ``^plan-fix-r\\d+$``) are
