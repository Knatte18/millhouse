...
def main(argv: list[str] | None = None) -> int:
    ...
    # Pick the task. --slug short-circuits to single;
    # multi fires from the numbered prompt when the user enters comma-separated indices.
    try:
    ...
    # A surviving remote branch from a previous aborted spawn would cause 'git worktree add -b' to succeed locally but 'git push --set-upstream' to fail later with a non-fast-forward error.
    # Catching it here lets the operator clean up (e.g.
    # via teardown) before any artifact is created.
    # Exit-code semantics from git ls-remote: 0 -> ref found (branch already exists on remote -> abort) 2 -> ref not found (genuine absent -> proceed normally) other non-zero -> network/config problem -> soft skip (do not block spawn)
    if not args.dry_run:
    ...
    # Capture the hub's current branch BEFORE creating the new worktree so we can record it in status.md as the parent — mill-merge / mill-cleanup read this to know where to merge back to.
    try:
    ...
    # Each step that successfully creates an artifact pushes a cleanup callable onto _cleanup_stack.
    # On any exception the stack is drained in reverse (last-created artifact is removed first) before the error is re-raised, leaving no partial state on disk, in git, or in the wiki.
    # Rollback scope (MULTI mode): multi_select_groom_then_claim runs BEFORE this span.
    # Restoring the absorbed source slugs is out of scope here;
    # wiki.set_phase(slug, None) only clears the merged task's [active] marker.
    # The orphaned-active reconciliation backstop in batch 2 converges any residual state.
    # --------------------------------------------------------------------------
    ...
    try:
        # Create the worktree.
        # git refuses if the target path exists, so we only ensure the PARENT exists.
        # Any failure here surfaces as a WorktreeError with captured stderr.
        ...
        # Propagate .millhouse/ minus task/worktree-specific subtrees.
        # The excluded names cover (a) the scratch area whose contents belong to the parent clone's last run and (b) junctions that must be recreated per-worktree.
        ...
        # Create remaining junctions/hardlinks from junctions config for the new worktree (.wiki, .portals). _setup.create_hub_links uses the token-scope filter so that entries requiring <SLUG> are only created in slug-bearing (task) worktrees.
        ...
        # .active is task-scoped and points to <hub>/_mill/ -- created explicitly rather than via mill-config.yaml junctions block so it is not auto-created in non-task worktrees.
        ...
        # Pick a colour + write .vscode/settings.json.
        ...
        # Revert the wiki claim that was made before this span (or the merged slug's active marker in MULTI mode).
        # set_phase(slug, None) is best-effort -- a failure here is printed but does not re-raise.
        try:
