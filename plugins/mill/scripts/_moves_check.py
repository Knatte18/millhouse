...
def planned_rename_findings(
    ...
    code-block::
        ...
        **Location:** `src` -> `dst`
        **Issue:** git diff --find-renames did not report this pair as a rename (R status);
            it may have landed as separate add+delete.
        **Fix:** Confirm ``git mv`` was used with surgical edits to preserve history;
            a full rewrite would warrant BLOCKING by the LLM reviewer.
    ...
    # Each rename line has a status code starting with 'R' followed by an optional similarity percentage (e.g. 'R100', 'R030'), then two tab- separated path columns: old path and new path.
    detected_renames: set[tuple[str, str]] = set()
    for raw_line in name_status_text.splitlines():
        # Strip trailing \r so CRLF-terminated Windows output is handled identically to LF-terminated output.
        line = raw_line.rstrip("\r")
        if not line:
            continue
        parts = line.split("\t")
        # A valid rename line has at least three tab-separated fields: status code, old path, and new path.
        if len(parts) >= 3 and parts[0].startswith("R"):
            old_path = parts[1].strip()
            new_path = parts[2].strip()
            detected_renames.add((old_path, new_path))

    # For each planned move not found among detected renames, produce one advisory NIT finding block.
    # The NIT is purely informational: it surfaces when the planned move's similarity fell below git's threshold (which can happen legitimately for seam extractions) and prompts the LLM reviewer to confirm ``git mv`` was used.
    findings: list[str] = []
