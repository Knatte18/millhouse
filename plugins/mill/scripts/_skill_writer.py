...
# Skill names (with hyphen, no `py` infix) whose SKILL.md is hand-written and must survive regeneration.
# Today only `mill-add` qualifies — its body is judgment-heavy (slug derivation, summary writing, proposal-extraction heuristics). `mill-skills-index` is also hand-written but is excluded one layer up (not in SHORTCUT_SCRIPTS), so does NOT appear here.
...
def iter_target_scripts(plugins_root: Path) -> list[Path]:
    ...
    Returns:
        ...
        Order matches ``SHORTCUT_SCRIPTS``.
        Paths are NOT verified to exist on disk — callers that care can ``.exists()`` each entry.
    """
    result: list[Path] = []
