...
.millhouse/agents.local.yaml (per-hub overrides), and legacy wiki/agents.yaml or wiki/reviewers.yaml
(for in-flight branches before migration).

Public API:
    ReviewerError — raised on every validation/resolution failure.
    load(hub_dir: Path) -> dict[str, dict] Load and validate plugin template + local overlay +
    legacy wiki fallback.
    Returns name → raw spec dict.
    resolve(registry: dict, name: str) -> dict Resolve a reviewer name to a fully-flattened spec
    dict.
    Special case: "test_stub" returns a synthetic spec without consulting the registry.
    resolve_reviewer_override(registry: dict, name: str, *, reject_non_claude: bool) -> dict Resolve
    a `--reviewer` per-round override name, rejecting cluster types, specs with no model (e.g.
    test_stub),
    and (when reject_non_claude) any model whose family is not recognized by
    _agent_dispatch.model_to_tier.
    resolve_role(cfg: dict, registry: dict, role: str, scope: str) -> dict | None Read
    cfg.roles.<role>.<scope>.reviewer and resolve via registry.
    Returns None if reviewer is null or rounds is 0. validate_role_refs(cfg: dict, registry: dict)
    -> None Walk cfg.roles.<role>.<scope>.reviewer for every (role, scope) pair;
    confirm each non-null name resolves.
    Raises ReviewerError listing all failures.
    tier_rank(spec: dict) -> tuple[int, int] | None Rank a resolved single/claude spec as
    (family_rank, effort_rank) for comparison.
    Returns None for cluster types, non-Claude providers, or unrecognized model families.
    fixer_weaker_than_reviewer_warning(fixer_spec, reviewer_spec, *, fixer_name, reviewer_name,
    scope) -> str | None Return a stderr-ready warning string when the reviewer's tier outranks the
    fixer's tier;
    returns None when either side is not comparable or the fixer is at least as strong as the
    reviewer.
"""
...
def load(hub_dir: Path) -> dict[str, dict]:
    ...
    Validates: all names match [a-z0-9_-]+, no duplicate names in each source file, every entry has
    a known type, required fields per type, cluster use: references resolve to type=single only, no
    cycles in the use: graph, and entries with `extends: <name>` are resolved top-down at load time
    (single-string form only; cluster entries may neither extend nor be extended; cycle detection
    raises with the chain).
...
def resolve(registry: dict, name: str) -> dict:
    ...
    Special case: name == "test_stub" returns {"type": "single", "provider": "test_stub", "tooluse":
    False} without consulting the registry.

    For type=single: returns a copy of the entry with tooluse defaulted to False.
    For type=cluster: returns a deep copy with workers.use and handler.use replaced by their
    fully-resolved single-spec dicts (bounded at depth 1 by load validation).
...
def resolve_reviewer_override(registry: dict, name: str, *, reject_non_claude: bool) -> dict:
    ...
    A `--reviewer` override is a deliberate, one-off operator choice made on the command line to
    break a stalled review loop, so it is held to a narrower contract than the general-purpose
    `resolve()`: only single, model-bearing reviewers may be named, and (for the Agent-mode dispatch
    ...
    Args:
        ...
        reject_non_claude: When True, also require the resolved model to belong to a recognized
        Claude model family (checked via `_agent_dispatch.model_to_tier`).
        Agent-mode dispatch call sites pass True;
        direct-dispatch call sites pass False, since that path already dispatches non-Claude
        reviewers via ordinary config today and must keep doing so.

    Returns:
        The fully-flattened spec dict for `name`, unchanged from `resolve()`.

    Raises:
        ReviewerError: If `name` is unknown (propagated from `resolve()`), names a cluster reviewer,
            names a reviewer with no `model` key (e.g.
            the `test_stub` synthetic spec),
            or (when `reject_non_claude` is True) names a reviewer whose model family
...
resolve via registry.
    ...
    Returns None if reviewer is null or rounds is 0. Raises ReviewerError if the role or scope key
    is absent from cfg.
    """
...
def tier_rank(spec: dict) -> tuple[int, int] | None:
    ...
    Only type="single" + provider="claude" specs are comparable: cluster types and non-Claude
    providers (e.g.
    ...
    effort rank comes from spec["effort"], defaulting to 0 ("low") when the key is absent -- the
    shipped haiku/haiku_bulk entries carry no effort: field at all.

    Returns:
        (family_rank, effort_rank) tuple, or None if the spec is not comparable (wrong
        type/provider) or its model family is unrecognized.
    """
    import _agent_dispatch
...
def fixer_weaker_than_reviewer_warning(
    ...
    *,
    ...
    roles.fixer.model is a config key fully independent from roles.code-review.<scope>.reviewer, so
    an operator escalating the reviewer mid-task can silently leave the fixer on a weaker model.
    ...
    Returns:
        ...
        or None when either side is not comparable or the fixer is at least as strong as the
        reviewer.
    """
