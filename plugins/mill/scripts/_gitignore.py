...
Duplicate entries that already exist outside the markers are left alone.
...
# .wiki, .active, .portals are junction targets: real directories on Windows (git matches trailing-slash patterns against them fine) but symlinks on POSIX (git's trailing-slash dir-only patterns never match a symlink, even one that points at a directory).
# Dropping the trailing slash matches both.
...
def render_block(glob_entries: list[str]) -> str:
    ...
    Returns:
        Multi-line string starting with ``START`` and ending with ``END\\n``.
    """
    lines = [START]
...
def upsert(gitignore_path: Path, glob_entries: list[str]) -> bool:
    ...
    Renders the block via ``render_block(glob_entries)`` and delegates to ``_upsert_single``.
    ...
    Returns:
        ``True`` if the file was written (content changed), ``False`` if already byte-equal.
