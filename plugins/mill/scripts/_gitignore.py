...
# .wiki, .active, .portals are junction targets: real directories on Windows
# (git matches trailing-slash patterns against them fine) but symlinks on
# POSIX (git's trailing-slash dir-only patterns never match a symlink, even
# one that points at a directory). Dropping the trailing slash matches both.
