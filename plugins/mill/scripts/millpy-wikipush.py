...
When ``--leave-conflicts`` is passed (intended for skill/LLM use), the wiki is left in mid-rebase
state so the caller can resolve and continue.
Without the flag (manual operator use), the script aborts the rebase and resets the local commit so
the wiki returns to its pre-script state (unstaged edits intact, no commit, no garbage) — "no harm
done".
"""
