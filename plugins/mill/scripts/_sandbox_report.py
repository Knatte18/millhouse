...
def read(path: Path) -> dict[str, Any]:
    ...
    Steps:
        1. Open and parse ``path`` as JSON.
            Raise on file-not-found or a JSON syntax error.
        2. Require the parsed value to be a JSON object,
            and require its ``source`` field to equal ``"sandbox-report"`` exactly.
        3. Require ``items`` to be present and to be a JSON array.
            An empty
    ...
    # The top-level value must be a JSON object -- a bare array or string is not a valid sandbox-report file, and calling .get() on it would raise an uncaught AttributeError instead of a clear validation error.
    ...
    # Reject any file that isn't explicitly stamped as a sandbox-report -- this is much more likely to be a wrong-file-passed mistake than something to recover from silently.
    ...
    # Validate every item up front and track refs seen so far -- a duplicate ref would corrupt the consumed-items tracking inside mill-triage-to-tasks, so it is rejected outright.
