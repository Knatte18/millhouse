...
active task in Home.md, filters out worktrees that already have a VS Code window open, then shows a
unified prompt: ``<Enter>`` spawns a new task and opens it, a number opens the listed worktree, or
``q`` quits.

Usage:
    python millpy-vscode.py [--new | --slug <slug>] [--list] [--filter-open]

    --new Spawn a new task and open it without showing the picker. --slug <slug> Skip the picker and
    open the worktree for this slug. --list Print the list of active worktrees without launching.
    --filter-open Filter out worktrees that already have a VS Code window open. --new and --slug are
    mutually exclusive.
...
def _build_code_argv(worktree_path: Path) -> list[str]:
    ...
    On Windows, delegates resolution to cmd.exe so that ``code.cmd`` is found via the full
    interactive PATH (including WindowsApps), which is not inherited by Python subprocesses launched
    from debugpy or non- interactive shells (see discussion.md § debugpy-path).
...
def main(argv: list[str] | None = None) -> int:
    ...
    ``_spawn_core.discover_active_worktrees``, filters out worktrees that already have a VS Code
    window open, then presents a unified prompt.
