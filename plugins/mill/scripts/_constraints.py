...
Some projects ship a top-level ``CONSTRAINTS.md`` listing invariants the implementer must respect
(e.g. "touch no files under ``vendored/``").
...
Hub-root resolution uses ``git rev-parse --show-toplevel`` so the helper works from any subfolder of
the hub.
Scripts invoked deep inside ``plugins/mill/scripts/`` get the same answer as scripts run from the
top level.
...
def read_if_exists(cwd: Path | None = None) -> str | None:
    ...
    ``cwd`` is the directory from which ``git rev-parse --show-toplevel`` is run — defaults to the
    current working directory.
    Passing an explicit ``cwd`` is useful for tests that want to point at a scratch hub without
    chdir'ing the calling process.
    ...
    Returns:
        ...
        or ``None`` when either the directory isn't inside a git repo or ``CONSTRAINTS.md`` does not
        exist at its toplevel.
        Callers that want to distinguish "no repo" from "no file" should call ``_hub_root``
        directly.
    """
    root = _hub_root(cwd)
    if root is None:
