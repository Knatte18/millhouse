...
# Match uppercase-identifier tokens wrapped in angle brackets.
# The leading-uppercase constraint avoids eating angle-brackets from prose or HTML that happens to appear inside a template body.
...
def render(template_path: Path, values: dict[str, str]) -> str:
    ...
    Raises:
        KeyError: The template referenced one or more tokens not present in ``values``.
            The message lists all missing tokens sorted alphabetically.
    """
    # Read the template as UTF-8.
    # Templates ship as part of the plugin so encoding is controlled;
    # we don't accept cp1252 fallbacks.
    text = _strip_leading_comment(template_path.read_text(encoding="utf-8"))

    # Accumulate every missing token name so the error message can report all of them at once.
    # The regex callback cannot raise directly without halting substitution mid-string.
    missing: list[str] = []
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        if name not in values:
            missing.append(name)
            # Preserve the original ``<TOKEN>`` in the output so a human reading a failed render can see where each hole was.
