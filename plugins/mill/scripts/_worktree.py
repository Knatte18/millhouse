...
mill-spawn creates a new worktree for every claimed task.
...
def create(branch: str, target: Path, cwd: Path) -> None:
    ...
    Runs ``git worktree add -b <branch> <target>`` with ``cwd`` set to the caller's repo root.
    The parent directory of ``target`` must exist;
    the worktree path itself must NOT exist (git would refuse otherwise).

    Args:
        branch: New branch name, e.g. ``"hanf/fix-widget"``.
            Passed verbatim;
...
def move(old: Path, new: Path, cwd: Path) -> None:
    ...
    Args:
        ...
        new: Absolute path where the worktree should live afterward.
            Must not already exist (git refuses otherwise).
        cwd: Directory from which to invoke ``git`` — must be inside the target repository,
            and must NOT be ``old``.
...
def copy_millhouse(src: Path, dst: Path, exclude: set[str]) -> None:
    ...
    for item in src.iterdir():
        if item.name in exclude:
            continue
        # Skip junctions/symlinks — the new worktree creates its own via _junction.create.
        # copy_millhouse should never follow a junction because that would pull the wiki clone (or worse, the previous task's active dir) into the new worktree.
...
def list_worktrees(cwd: Path) -> list[dict[str, str | None]]:
    ...
    Runs ``git worktree list --porcelain`` and parses its output into a list of dicts.
    ...
    The main worktree (first block) is included.
    Callers filter if needed.
    Returns ``[]`` for empty output (bare repo with no worktrees is unusual but not an error).
