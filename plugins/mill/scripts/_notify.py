...
v1 mill had a ``notify`` entrypoint that never quite landed.
v2 flips the architecture: we own a tiny API surface,
and the backend is swapped out by config.
Skills (mill-go, mill-plan, mill-abandon, …) call :func:`notify` with a structured event;
...
def _load_backend():
    ...
    Raises NotifyError if the module is missing or does not expose ``BACKEND`` + ``send``.
    """
...
def notify(event: str, detail: str, **context) -> None:
    ...
    Args:
        ...
        **context: arbitrary serialisable metadata (slug, batch name, round number, etc.).
            Backends render this as JSON or a key=value trail;
            callers should not embed it into ``detail`` themselves.

    Delivery failures are logged to stderr and swallowed.
    A broken toast daemon must never crash the orchestrator.
    """
    try:
