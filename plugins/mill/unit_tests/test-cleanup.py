...
def test_cleanup_plan_has_no_orphan_baseline_dirs_field() -> None:
    """CleanupPlan no longer carries an orphan_baseline_dirs field.

    Batch 2 deleted `_verify_baseline.py`'s transient-worktree checkout, so a stray
    `.scratch/verify-baseline-*` directory is now simply ignored by cleanup rather than reaped.
    """
    plan = CleanupPlan(to_remove_done=[], to_remove_abandoned=[], to_reset_home=[], to_report=[])
    assert not hasattr(plan, "orphan_baseline_dirs"), (
        "CleanupPlan must no longer carry an orphan_baseline_dirs field"
    )
    try:
        CleanupPlan(
            to_remove_done=[], to_remove_abandoned=[], to_reset_home=[], to_report=[],
            orphan_baseline_dirs=["should not be accepted"],
        )
        raise AssertionError("CleanupPlan(orphan_baseline_dirs=...) unexpectedly succeeded")
    except TypeError:
        pass
    print("PASS CleanupPlan — orphan_baseline_dirs field removed")
...
def main() -> int:
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            wt.mkdir(parents=True)
            (wt / "_mill").mkdir()
            (wt / "_mill" / "status.md").write_text(_make_status_md("abandoned"), encoding="utf-8")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            wt.mkdir(parents=True)
            (wt / "_mill").mkdir()
            (wt / "_mill" / "status.md").write_text(_make_status_md("implementing"), encoding="utf-8")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Status.md lives in _mill/ subdir (hub root for in-place tasks).
            # worktree_path = hub_root for in-place.
            (hub_root / "_mill").mkdir()
            (hub_root / "_mill" / "status.md").write_text(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Status.md in _mill/ subdir (hub root for in-place).
            (hub_root / "_mill").mkdir()
            (hub_root / "_mill" / "status.md").write_text(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            wt.mkdir(parents=True)
            # Write status.md in _mill/ only — no root status.md
            task_dir = wt / "_mill"
            task_dir.mkdir()
            ...
            print("PASS test_build_plan_reads_task_status_md — _mill/status.md primary path")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Write _mill/status.md with done + feature-branch
            task_dir_ip = hub_root_ip / "_mill"
            task_dir_ip.mkdir()
            ...
            assert "feature-branch" in checkout_calls_ip[0], (
                f"Expected checkout of 'feature-branch' (_mill/status.md), got: {checkout_calls_ip[0]}"
            )
            ...
            print("PASS test_apply_inplace_record_reads_task_status_md — read_parent_branch and _read_phase resolve to _mill/")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            wt.mkdir(parents=True)
            (wt / "_mill").mkdir()
            (wt / "_mill" / "status.md").write_text(_make_status_md("pr-pending"), encoding="utf-8")
        ...
        test_cleanup_plan_has_no_orphan_baseline_dirs_field()
