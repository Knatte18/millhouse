...
def main() -> int:
    ...
    # --- (12b) merge_tasks rejects empty upsert without mutating store ---
    try:
        tmp_dir = tempfile.mkdtemp()
        try:
            db_path = Path(tmp_dir) / "tasks.json"
            ...
            store.upsert_task({"slug": "keep-a"})
            store.upsert_task({"slug": "keep-b"})

            raised = False
            try:
                store.merge_tasks(remove_slugs=["keep-a", "keep-b"], upsert={})
            except ValueError:
                raised = True
            assert raised, "merge_tasks must raise on empty upsert"

            # Critical: nothing was removed because validation ran first.
            assert store.get_task("keep-a") is not None, "keep-a must survive failed merge_tasks"
