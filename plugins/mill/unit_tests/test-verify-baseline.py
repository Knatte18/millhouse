...
Case 1 regresses #615/#620: the module-wide verify baseline's transient `git worktree add` failed with "Filename too long" on deep-path Windows repos because `core.longpaths` was not set for that throwaway checkout, silently disabling the baseline gate.
This case asserts the `git worktree add` argv `compute_baseline` builds always carries `-c core.longpaths=true` immediately after the `-C <git_root>` pair and before the `worktree` token -- the exact shape the fix in `_verify_baseline.py` produces.

Case 2 mitigates #629: a long `.scratch/verify-baseline-<uuid4().hex>/` prefix (32 hex characters) could itself push a deep-fixture Windows repo over MAX_PATH.
This case asserts the transient-worktree directory basename `compute_baseline` builds matches `verify-baseline-<12 hex chars>` -- the shortened `uuid.uuid4().hex[:12]` slice, not the full 32-character string.

Follows the monkeypatch/in-memory fixture style of `test-worktree.py`: no real git is invoked. `_subprocess_util.run` is monkeypatched to fabricate a successful `rev-parse` result and to capture the `worktree add` argv;
`_run_verify_in` is stubbed to return `(0, "")` so `compute_baseline` short-circuits to "clean" on the first verify;
`_junction.create` and `_worktree.remove_safe` are stubbed to no-ops since no real filesystem worktree is ever created.

Cases (b)/(c) cover `compute_batch_baselines`'s basic multi-command computation directly against a mocked `checkout_path`: (b) confirms two distinct commands each get their own, independent (non-aliased) signature list keyed by name;
(c) confirms a command with zero recognized FAIL-marker lines on both runs maps to `[]` (present, not an absent dict key).

Cases (d)/(e) cover `compute_batch_baselines`'s union-of-two-runs corroboration and mixed-cwd dependency-linking orchestration: (d) confirms a signature that only reproduces on one of the two runs still ends up in the union (neither run's set silently overwrites the other's);
(e) exercises `_link_dependency_dirs` called at two distinct resolved target paths against one shared mocked `checkout_path` (mirroring how a future shared-checkout orchestrator calls it once per distinct effective-cwd fragment), then `compute_batch_baselines` with commands resolving to each of those two paths via their `cwd_override` entries.
"""
...
def _run_compute_baseline_capturing_worktree_add(tmp: str) -> tuple[str, list[list[str]]]:
    """
    Run `compute_baseline` against a fully-mocked git/subprocess layer and return its result plus every `git worktree add` argv it issued.

    `_subprocess_util.run` is monkeypatched to fabricate a successful `rev-parse` result and to capture the `worktree add` argv;
    `_run_verify_in` is stubbed to return `(0, "")` so `compute_baseline` short-circuits to "clean" on the first verify;
    `_junction.create` and `_worktree.remove_safe` are stubbed to no-ops since no real filesystem worktree is ever created.
    """
...
def _case_b_independent_signature_lists() -> None:
    """
    Case (b): two distinct commands each get their own, independent (non-aliased) signature list keyed by name.
    """
...
def _case_c_zero_failures_returns_empty_list() -> None:
    """
    Case (c): a command with zero recognized FAIL-marker lines on both runs maps to [] (present, not an absent dict key).
    """
    checkout_path = Path("/fake/checkout")
    project_root = Path("/fake/project")
...
def _case_d_union_of_two_runs() -> None:
    """
    Case (d): a signature that only reproduces on one of the two runs still ends up in the union -- neither run's set silently overwrites the other's.
    """
    checkout_path = Path("/fake/checkout")
    project_root = Path("/fake/project")
    call_count = {"n": 0}
...
def _case_e_mixed_cwd_dependency_linking() -> None:
    """
    Case (e): `_link_dependency_dirs` called at two distinct resolved target paths against one shared checkout (mirroring how a future shared-checkout orchestrator calls it once per distinct effective-cwd fragment), then `compute_batch_baselines` with commands resolving to each of those two paths via their `cwd_override` entries -- confirming each command runs at its own cwd within the one shared checkout and dependency dirs are linked at both resolved paths.
    """
