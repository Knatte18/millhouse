...
Covers the #589 fix: `_scan()` must distinguish a genuine YAML parse failure in a SKILL.md's
frontmatter block (a `FrontmatterParseError`, reported with the underlying `yaml.YAMLError` message)
from the unrelated case of a SKILL.md that has no `---`-delimited frontmatter block at all (reported
as "missing frontmatter").
Both cases still drop the skill from the scan result -- this test only asserts the two failure modes
are diagnosable via distinct stderr messages, not that either becomes tolerated.

Loads millpy-skills-index.py via importlib.util.spec_from_file_location (its hyphenated filename
cannot be `import`ed directly -- same pattern used by test-abandon.py) and calls `_scan(repo_root)`
directly against a fixture tree built with `safe_temp_dir()`, with no subprocess or real git
involved.
"""
...
def _load_skills_index_module():
    ...
    Uses `mill_skills_index` (not `__main__`) so the `if __name__ == "__main__"` block at the bottom
    of the script does not fire on import.
    """
