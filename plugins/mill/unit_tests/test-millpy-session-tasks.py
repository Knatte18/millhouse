...
def _run(repo: Path, cfg: dict, slug_effect) -> tuple[int, str, str]:
    ...
    with (
        ...
        patch("mill_session_tasks.resolve_short_name", return_value="HUBSHORT"),
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        errors += _check(rc == 0 and "my-task:start" in text, "task worktree renders slug names")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        errors += _check(rc == 0 and "HUBSHORT:start" in text, "hub worktree renders short name")
