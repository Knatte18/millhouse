...
# The card requires these to still be present and unchanged -- output_path is purely additive.
...
def _mock_paths_and_reviewers(project_root: Path, briefs_dir: Path):
    ...
    # briefs_dir is where write_brief's real implementation writes the brief + unlinks any stale .out.md;
    # must be a real, writable tempdir path.
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            # Only the plan CLI's (unskipped) validator step calls resolve_path in the prepare branch;
            # a fixed tempdir stands in for plan_dir.
...
def _assert_success_envelope_shape(review_type: str) -> bool:
    ...
    # skip it here since the success-envelope shape is orthogonal to validator behaviour (the validator-failure envelope has its own test).
