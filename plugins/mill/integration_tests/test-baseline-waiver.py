...
`_mill/discussion.md`'s Testing section calls for real-git coverage of the
whole `--stage baseline` / `--stage finalize` per-batch waiver mechanism
(batches 3-6 of this plan): a fixture task whose one batch's `verify:`
command has a pre-existing failure unrelated to the batch's own work,
confirming `--stage baseline` captures that failure's signature and
`--stage finalize` waives it on replay while still catching a genuinely
new, distinct failure introduced later.

Layout mirrors `test-abandon.py` and `test-verify-baseline.py`:

    <container>/wiki.git               bare "remote" for the wiki clone
    <container>/wiki                   wiki clone; task record seeded via
                                        `wiki._client.upsert_task` (the
                                        TinyDB-backed store, not a
                                        hand-written Home.md, is the wiki
                                        daemon's actual source of truth)
    <container>/hub                    hub repo (main branch), also the
                                        git-worktree source
    <container>/wts/<slug>             the fixture task's linked worktree,
                                        on branch impl/<slug>

`millpy-implement.py --stage baseline` and `--stage finalize` are invoked
as real subprocesses (mirroring production dispatch) with `PYTHONPATH` set
to this repo's own `plugins/mill/scripts` directory (and `CLAUDE_PLUGIN_ROOT`
removed from the child environment) so the fixture always exercises the
worktree's own module code, never a stale plugin-cache copy.

The fixture batch's `verify:` command is a small tracked Python script
(`verify_check.py`) at the worktree root, which starts out printing exactly
one `FAILED ...`-shaped line and exiting 1 -- a "pre-existing" failure
present on the parent branch (`main`) from the very first commit.

Three steps:

    1. `--stage baseline` against the fixture worktree: confirms the
       batch's `verify_baseline_failures` field in `status.md` is captured
       as a non-empty list containing the pre-existing failure's signature.
    2. Mutate `verify_check.py` to ALSO print a second, distinct `FAILED
       ...
       keeping the original failure; commit the mutation; invoke `--stage
    ...
    3. Revert `verify_check.py` back to producing only the original
       failure; commit the revert; re-invoke `--stage finalize` with a
       fresh success `--agent-output`. Confirms the CLI's printed JSON now
       reports `status: "success"` (the pre-existing-only failure set is a
       subset of the baseline and is waived).
...
def _cli_env() -> dict:
    """
    Env for every `millpy-implement.py` subprocess invocation: `PYTHONPATH`
    pinned to this worktree's own scripts dir and `CLAUDE_PLUGIN_ROOT`
    removed, so template/registry resolution falls back to the source tree
    next to `_config.py` (this worktree) rather than any installed plugin
    cache -- see CLAUDE.md's "Task-worktree path for source verification"
...
def _setup_fixture(container: Path) -> tuple[Path, Path, Path, str]:
    ...
    # Bare wiki + clone. The wiki daemon's actual source of truth is the
    # TinyDB-backed tasks.json, not Home.md -- seed the task record via the
    # real `wiki._client.upsert_task` API (same call production code such as
    # millpy-add.py makes) so `_marker.slug_from_branch`'s
    # `list_tasks_brief` lookup finds it.
    ...
    # Hub repo: main branch tip carries the pre-existing failure from the
    # very first commit, plus the fixture's mill plan/status skeleton.
    hub.mkdir()
    _git_init(hub)
    ...
    # Worktree-local .millhouse stub, mirroring the hub's config so path
    # resolution succeeds when the CLI runs with cwd=wt_path.
...
def main() -> int:
    ...
    try:
        ...
        # --stage baseline wrote verify_baseline_failures directly into status.md
        # without committing (production's mill-go commits this as part of its own
        # housekeeping); commit it here so the in-scope dirty-tree gate at finalize
        # time sees a clean tree.
