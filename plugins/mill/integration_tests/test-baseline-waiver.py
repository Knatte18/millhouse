...
Layout mirrors `test-abandon.py` and `test-verify-baseline.py`:
    ...
    <container>/wiki wiki clone;
        task record seeded via `wiki._client.upsert_task` (the TinyDB-backed store, not a
            hand-written Home.md, is the wiki daemon's actual source of truth)
...
`millpy-implement.py --stage baseline` and `--stage finalize` are invoked as real subprocesses
(mirroring production dispatch) with `PYTHONPATH` set to this repo's own `plugins/mill/scripts`
directory (and `CLAUDE_PLUGIN_ROOT` removed from the child environment) so the fixture always
exercises the worktree's own module code, never a stale plugin-cache copy.

The fixture batch's `verify:` command is a small tracked Python script (`verify_check.py`) at the
worktree root, which starts out printing exactly one `FAILED ...`-shaped line and exiting 1 -- a
"pre-existing" failure present on the parent branch (`main`) from the very first commit.
...
Three steps:
    ...
    3. Revert `verify_check.py` back to producing only the original failure;
        ...
        Confirms the CLI's printed JSON now reports `status: "success"` (the pre-existing-only
            failure set is a subset of the baseline and is waived).
...
def _cli_env() -> dict:
    """
    Env for every `millpy-implement.py` subprocess invocation: `PYTHONPATH` pinned to this
    worktree's own scripts dir and `CLAUDE_PLUGIN_ROOT` removed, so template/registry resolution
    falls back to the source tree next to `_config.py` (this worktree) rather than any installed
    plugin cache -- see CLAUDE.md's "Task-worktree path for source verification" and this module's
    own docstring.
    """
    env = os.environ.copy()
