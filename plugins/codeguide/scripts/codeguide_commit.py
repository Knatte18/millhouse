...
Sibling mode — stage AND commit files inside the sibling repo, which has its own git history
independent of the target repo::

    python codeguide_commit.py --mode sibling --sibling-anchor <path> --file <path> [--file <path> ...]
    -m "<msg>"

The caller (codeguide-update) already holds ``mode`` and ``sibling_anchor`` from its own
``resolve.py`` call.
Those are passed explicitly so this helper does NOT re-run resolve.py (which would be fragile if cwd
differs from the repo root and couples commit-time behavior to import-time side effects).
