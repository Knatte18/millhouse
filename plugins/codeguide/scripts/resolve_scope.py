...
----------------------
1. **No-arg** (``args`` is empty): detect the parent branch via the three-step fallback below
    (optionally overridden by ``--parent``/``parent=``), then emit the union of ``<parent>..HEAD``
    (committed files) with the current diff (staged + unstaged).
2. **Time arg** (single token matching ``^\d+[hdw]$``, case-insensitive): use ``git log --since="N
    hour|day|week ago" --name-only --pretty=format:`` and collect unique non-empty file paths.
3. **Single-token ref arg**: any single token that resolves as a git ref via ``git rev-parse
...
Parent detection ----------------
1. Try ``git symbolic-ref --short refs/remotes/origin/HEAD`` — strip the ``origin/`` prefix from the
output (e.g. ``origin/main`` → ``main``).
...
Public API ---------- CLI: ``python resolve_scope.py [--parent <ref>] [<args>]`` stdout —
newline-separated absolute paths (empty output is valid) stderr — last non-empty line is a JSON
summary ``{"mode", "parent", "base_branch", "included_committed", "included_diff"}`` exit — 0 unless
not in a git repo

Function: ``enumerate_scope(args, cwd=None, parent=None) -> (list[Path], dict)`` ``args`` — the
    positional argument list (strings from ``$ARGUMENTS.split()``) ``cwd`` — working directory
    override for tests (defaults to ``os.getcwd()``) ``parent`` — optional base-branch override
    consulted only in no-arg mode;
    falls back to git-native detection when it doesn't resolve Returns — ``(absolute_paths_deduped,
        summary_dict)``
"""
...
def _ref_resolves(toplevel: pathlib.Path, ref: str) -> bool:
    ...
    Uses `git rev-parse --verify --quiet <ref>^{commit}` to determine if the ref can be resolved
    locally.
...
def _resolve_ref_token(toplevel: pathlib.Path, token: str) -> str | None:
    ...
    Strips a literal trailing ``..HEAD`` suffix (the shape mill-merge-in's checkpoint range
    produces) before checking, since git itself cannot resolve ``<ref>..HEAD`` as a single
    commit-ish.
    Any other token containing ``..`` (a genuine range) is passed through unstripped — it will
    simply fail the rev-parse check below, per the "literal ..HEAD suffix stripping only" design
    decision.

    Returns:
        The candidate ref string (with the ``..HEAD`` suffix already stripped, if present) when it
        resolves to a commit, else None.
    """
    suffix = "..HEAD"
