...
- **Creates:** (none)
- **Requirements:**
  - Each Python test has a module-level `SCRATCH = HUB / ".millhouse" / "scratch"` (or equivalent `SCRATCH = HUB / ".millhouse/scratch"`). Replace with `SCRATCH = HUB / ".scratch"`. Keep the path ABSOLUTE — all tests use it as an absolute fixture-root.
  - `test-bootstrap.ps1`: holds a PowerShell variable pointing at `.millhouse/scratch`. Replace with `.scratch`. Verify the test file still runs in PowerShell 5 (the environment convention from global CLAUDE.md).
