...
- **Requirements:**
  ...
  - Content (tight — three bullets, no prose preamble other than a one-line heading intent):
    ...
    ```
  - Update "Repo layout pointers" in the same edit: the line `- .millhouse/ in working clones is gitignored local state; .millhouse/wiki is a junction to the shared wiki repo.` gains a sibling line for `.scratch/`: `- .scratch/ in working clones is gitignored scratch-only state; not propagated to worktrees.`
