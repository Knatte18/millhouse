...
| **_Sidebar.md** | `wiki/_Sidebar.md` | `templates/_Sidebar.md` | `templates/_Sidebar.schema.md` | mill-setup, mill-add, mill-merge, mill-abandon, mill-groom (all regenerate) |
| **proposal.md** | `wiki/proposal-<slug>.md` (flat namespace at wiki root) | — (free-form markdown) | `templates/proposal.schema.md` (minimal) | mill-add (when `--proposal-body` given), mill-groom |
...
The wiki landing page IS the task list. Each task is a `## <Title> [<slug>]` heading with a short free-text body.
...
## <Human-readable title> [<slug-in-kebab-case>]

<short description — ideally one paragraph, max ~150 words>

## <Another title> [[<another-slug>]](proposal-<another-slug>)

<short summary — full background lives in proposal-<slug>.md at wiki root>
```

**Rules (enforced by `Home.schema.md`):**

- One `## <Title> [<slug>]` heading per task. Title is free prose; slug is the bracketed kebab-case ID.
- Slug: kebab-case, unique within Home.md, matches `[a-z][a-z0-9-]*`. Title is free prose.
- When a task has a long-form proposal at `proposal-<slug>.md` (wiki root, flat namespace — see below), the bracketed slug becomes a link: `[[<slug>]](proposal-<slug>)`. The double-bracket syntax keeps the brackets visible in rendered output while making the slug clickable.
- When a task has no proposal, the slug stays as plain `[<slug>]`.
- One regex parses both forms: `^##\s+(.+?)\s+\[\[?([a-z][a-z0-9-]*)\]?\](?:\([^)]+\))?\s*$` — group 1 = title, group 2 = slug.
- Body: free-form markdown, but should be short. Long descriptions are extracted to `proposal-<slug>.md` (see below).
...
### Why flat namespace for proposals (not `proposals/<slug>.md`)

GitHub Wiki does not reliably render pages in subdirectories — clicking a link to `proposals/foo` typically yields the raw `.md` view, not a rendered wiki page. Verified empirically (2026-04-19). Proposal files therefore live at wiki root with a `proposal-` prefix that disambiguates them from other wiki pages and lets the sidebar generator filter them with a glob.

### _Sidebar.md

Persistent navigation panel rendered on every wiki page by GitHub Wiki. Auto-generated from Home.md and the set of `proposal-*.md` files at wiki root.
...
- [<title>](proposal-<slug>)   <!-- linked when proposal exists -->
- <title>                       <!-- plain when no proposal -->
```

Regenerated by `_sidebar.py` (M1.3) on every wiki-mutating command — initially `mill-add` and `mill-setup`. Layer 04 commands (`mill-merge`, `mill-abandon`, `mill-groom`) trigger regeneration too when they land.
...
- Lives at wiki root as `proposal-<slug>.md` (flat namespace — see "Why flat namespace" above)
- Markdown, no required schema (free-form)
- Referenced from Home.md by the bracketed slug becoming a link: `## <Title> [[<slug>]](proposal-<slug>)`
