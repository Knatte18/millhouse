...
- Decision: Add a `SESSION_ID` render token to `implementer-brief.md`. Replace the prose `<this-session-id>` placeholder in both the success and stuck JSON examples with `<SESSION_ID>`. Replace the instruction "read from `--session-id` flag" with "use the UUID shown in the example above — it was injected when this brief was rendered". In `millpy-implement.py`, add `"SESSION_ID": session_id` to the token dict passed to `_render.render()`.
