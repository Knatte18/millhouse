...
- **(A) Fix `_paths.resolve_wiki_path` line 303** so it reads `.millhouse/config.local.yaml` from the *hub* (cwd), not from the git toplevel. Two-step read: stub at `git_toplevel / .millhouse / config.local.yaml` yields `hub_relative_path`, then full config at `hub / .millhouse / config.local.yaml` yields `paths.wiki:`. When `hub_relative_path = "."` the two reads collapse to one file — no overhead.
...
- **(A) Bootstrap-stub for subfolder-install discovery.** When `hub_relative_path != "."`, spawn (and any other producer of new worktrees) writes a tiny `worktree_path / .millhouse / config.local.yaml` containing exactly `hub_relative_path: <subpath>` and nothing else. Full hub state (the real `config.local.yaml`, `active.slug.md`, junctions, etc.) lives at `worktree_path / hub_subpath / .millhouse/`. The stub is purely a discovery anchor for outside-the-worktree consumers (terminal/vscode/mill-cleanup) that don't yet know `hub_subpath` for the worktree they're inspecting. **When `hub_relative_path = "."` no stub is written** — the standard-layout `.millhouse/` at the worktree root already serves both roles.
- **(A) Update every discovery / outside-the-worktree reader** to honor the stub:
  - `_spawn_core.discover_active_worktrees` ([_spawn_core.py:150-184](plugins/mill/scripts/_spawn_core.py#L150-L184)): instead of reading `entry / .millhouse / active.slug.md` directly, read `entry / .millhouse / config.local.yaml` for `hub_relative_path` (default `"."`), resolve `hub = entry / hub_subpath`, then read `hub / .millhouse / active.slug.md`.
  - `_load_config(wiki_path, selected_path)` from terminal/vscode: same two-step. `selected_path / .millhouse / config.local.yaml` for `hub_relative_path` → resolve `hub` → read `hub / .millhouse / config.local.yaml` for the rest. (When `hub_subpath = "."`, single read.)
  - `millpy-cleanup` and any other script that walks worktrees: same protocol.
- **(A) Spawn / worktree dst handling for subfolder-install.** Read `hub_relative_path:` from the *source* `.millhouse/config.local.yaml` (resolved via the same two-step protocol), then place the destination full hub state (`.millhouse/`, `.vscode/`, hub junctions, `_setup.create_hub_links` target_root) at `worktree_path / hub_subpath / ...` so the new worktree mirrors the source layout. Write the stub at `worktree_path / .millhouse / config.local.yaml` whenever `hub_subpath != "."`. Portal junction target stays `worktree_path` (terminal/vscode adjust at launch via the stub-then-real-config protocol).
...
- **(A) `_config.load_config` parameter rename + protocol awareness.** Rename the third parameter from `git_root` to `worktree_root` (or `hub_dir`) to make intent explicit. Implement the two-step stub-then-real-config read internally so callers don't need to know about the stub. Update all call sites to pass `_paths.resolve_hub_path()` (current worktree) or the equivalent destination path.
- **(A) Unit tests** for `_config.load_config` (renamed parameter, two-step read, stub presence/absence); for `_paths.resolve_hub_path`; for `_paths.resolve_wiki_path` two-step read; for `_spawn_core.discover_active_worktrees` (subfolder-install layout with stub); for `millpy-spawn`'s subfolder-install dst behavior (assert real hub state lands at `worktree_path / hub_subpath / .millhouse`, stub at `worktree_path / .millhouse`, terminal launch_path resolves correctly). Update existing `test-millpy-claim.py`, `test-millpy-color.py`, `test-millpy-spawn.py`, `test-millpy-worktree.py`, `test-spawn-core.py` for the new path semantics.
...
- **Decision:** Read `hub_relative_path:` from the source `.millhouse/config.local.yaml` at spawn time. Compute `dest_hub = worktree_path / hub_subpath`. Place full hub state (`.millhouse/`, `.vscode/`, hub junctions, `create_hub_links` `target_root`, and `tokens["HUB_PATH"]` / `tokens["CWD_PATH"]`) at `dest_hub`. Portal entry stays at `worktree_path`. When `hub_subpath != "."`, ALSO write a bootstrap stub at `worktree_path / .millhouse / config.local.yaml` containing only `hub_relative_path: <subpath>` (see next decision).
...
### Bootstrap stub for outside-the-worktree discovery

- **Decision:** For subfolder-install (`hub_relative_path != "."`), spawn writes a tiny `worktree_path / .millhouse / config.local.yaml` containing exactly `hub_relative_path: <subpath>` and nothing else. Full hub state lives at `dest_hub / .millhouse/`. Discovery functions (`_spawn_core.discover_active_worktrees`, `_load_config` reads from terminal/vscode, `millpy-cleanup` worktree walks) use a two-step read: stub yields `hub_subpath`, then the real config / `active.slug.md` are at `worktree / hub_subpath / .millhouse/`. For standard layout (`hub_subpath = "."`) no stub is written; the single `.millhouse/` at worktree root serves both roles. Internal helpers (`_config.load_config`, `_paths.resolve_wiki_path`) implement the two-step read so callers don't need to know about the stub.
- **Rationale:** Terminal/vscode/cleanup scan `<container>/wts/<slug>/...` for markers without knowing per-worktree `hub_subpath` — that's the chicken-and-egg the discussion review surfaced. Three options were on the table: (a) deep-scan / glob for `.millhouse/`, (b) bootstrap stub at worktree root, (c) move `hub_relative_path` to the wiki shared config. (a) is fragile and slow. (c) is most elegant but architecturally invasive (changes mill-setup, all configs, doc trail) and ties hub_subpath to the wiki repo (which is fine in practice but harder to revert). (b) is the smallest viable change: one extra file per task worktree (only when needed), discovery logic gains one extra read, no architectural shift. The stub is named, structural, and documented — distinct from the unintended `.millhouse/`-at-git-root the original bug describes.
- **Rejected:** Deep scan (option a — slow, fragile to nested test fixtures). Move `hub_relative_path` to wiki shared config (option c — too invasive for this task; revisit if the stub proves cumbersome). Skip the stub and let terminal/vscode break for subfolder-install (rejected — half-done fix).

### Two-step read in `resolve_wiki_path`

- **Decision:** `_paths.resolve_wiki_path(git_toplevel)` becomes stub-aware. Step 1: read `git_toplevel / .millhouse / config.local.yaml` for `hub_relative_path` (default `"."`). Step 2: compute `hub = git_toplevel / hub_subpath`. Step 3: read `hub / .millhouse / config.local.yaml` for `paths.wiki:`. Step 4: fall back to `_sibling.resolve_path("wiki", main_root)` if no override. When `hub_subpath = "."`, steps 1 and 3 read the same file; one read is cached. Update the docstring to remove the obsolete claim that "the local config file is read from `git_toplevel` (correct — each worktree carries its own `.millhouse/`)" — that's only true for standard layout post-fix.
- **Rationale:** Same chicken-and-egg as terminal/vscode. The two-step protocol unifies all consumers under one logic path.
- **Rejected:** Pass `worktree_root` parameter (caller would need to know `hub_subpath` already — same bootstrap problem); single-read with hub_subpath baked into the stub (works but conflates structural and configuration concerns).
...
| `plugins/mill/scripts/_paths.py` | `resolve_wiki_path` line 303 + docstring lines 293-297 | reads from `git_toplevel`; needs two-step (stub-then-real) read; obsolete docstring claim about correctness |
| `plugins/mill/scripts/_spawn_core.py` | `discover_active_worktrees` lines 175-184 | reads `entry / .millhouse / active.slug.md` directly; needs two-step stub-aware read |
| `plugins/mill/scripts/millpy-cleanup.py` | any worktree-walking code path | same two-step protocol if it scans worktrees by marker |
...
- **Cases:**
  1. `hub_relative_path = "."` (the existing default) → `.millhouse/`, `.vscode/`, hub junctions, `create_hub_links` target_root, `_build_tokens` HUB_PATH/CWD_PATH all at `worktree_path`. NO bootstrap stub written. (Regression test for the standard layout.)
  2. `hub_relative_path = "src/Models"` → full hub state at `worktree_path / "src/Models"`. Bootstrap stub at `worktree_path / ".millhouse" / "config.local.yaml"` containing only `hub_relative_path: src/Models`. (Subfolder-install assertion.)
  3. Portal junction in container/portals/<slug>/ points at `worktree_path` regardless of `hub_relative_path`. With the stub, simulated terminal/vscode discovery (`_spawn_core.discover_active_worktrees` + `_load_config(wiki, selected_path)` + `resolve_hub_relative_path`) yields `launch_path == worktree_path / hub_subpath` for both layouts. (Confirms launch path is correct, not just the portal target.)
...
- **Q (review round 1, GAP):** How does terminal/vscode discover worktrees in subfolder-install when `.millhouse/` is no longer at the worktree root? **A:** Bootstrap stub. Spawn writes `worktree_path / .millhouse / config.local.yaml` with only `hub_relative_path: <subpath>` whenever `hub_subpath != "."`. Discovery uses a two-step read: stub for `hub_subpath`, then real config / `active.slug.md` at `worktree / hub_subpath / .millhouse/`. Standard layout (`hub_subpath = "."`) writes no stub; the single `.millhouse/` at worktree root serves both roles. `_config.load_config` and `_paths.resolve_wiki_path` implement the two-step internally so callers don't need to know about the stub.
- **Q (review round 1, NOTE):** Why was `_paths.resolve_wiki_path` line 303 in the affected-files table but absent from the In-scope list with no fix description? **A:** Oversight. Now in scope explicitly: two-step stub-aware read, docstring update to drop the obsolete "correct — each worktree carries its own `.millhouse/`" claim.
