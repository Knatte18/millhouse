...
- Decision: `_paths.resolve_wiki_path(git_toplevel)` halts with the same `SystemExit` message as `resolve_git_root` when `git_toplevel.name == "wiki"` or when `git_toplevel` equals the resolved wiki path. This is checked at the top of the function before any config reading.
- Rationale: Belt-and-suspenders matches the proposal phrasing. `resolve_wiki_path` is sometimes called from helpers that did not go through `resolve_git_root` (e.g. `_marker.slug_from_branch` callers in test code). Catching the wiki-cwd case here too means there is no path from a wiki-cwd into config loading.
- Rejected: Trusting upstream guards (works in production but leaves test code and future callers exposed).
...
- The unit test must use raw regex matches, not `re.match` — `re.search` per line. Skip the test file itself to avoid self-match on the regex source. Skip files that are explicitly documentation of the anti-pattern: `CLAUDE.md` (root) and the eight SKILL.md notes — easiest via a tiny in-test allowlist of those file paths, OR (preferred) the regex matches `cd .wiki/ && git` not `cd .wiki/` so the documentation lines (which use the full anti-pattern as a quoted example) still match. To keep the test enforceable, use an explicit file allowlist scoped to `CLAUDE.md` and the eight SKILL.md files; everything else is denied.
