...
**Existing tests to retire:** every test currently calling `_active.write(...)`. Either replace the call with `_make_task_worktree(...)` (when the test exercises a real worktree flow) or delete the test (when it was purely testing the marker module's IO contract — those have no remaining surface).
