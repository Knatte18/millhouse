...
- Decision: Modify `mill-ghissues-to-tasks/SKILL.md` Step 3 in two ways.
  ...
  The skill body's "Rules" section gains two bullets: "Fold targets must be in an unlocked phase. `_tasks_md.LOCKED_FOLD_PHASES` is the source of truth — never duplicate the tuple." and "Fold-in always appends a `- Sources: #N — <issue title>` bullet to the target body via `_tasks_md.append_to_body`. The Home.md output of a fold-in is identical between this skill and `/mill-fold`."
