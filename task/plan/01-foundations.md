...
- **Requirements:**
  ...
  - `_test_registry.py` also exposes `write_to(wiki_root: Path, **overrides) -> Path` which calls `make_minimal_registry(**overrides)`, serialises the dict to YAML via `yaml.safe_dump`, writes the result to `wiki_root / "reviewers.yaml"`, and returns the written path. Used by every flow-test fixture in batch 2 to satisfy `_reviewers.load(wiki_root)`'s file-must-exist contract.
