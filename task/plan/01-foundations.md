...
- **Requirements:**
  ...
  - `resolve_role(cfg: dict, registry: dict, role: str, scope: str) -> dict | None`: defensively reads `cfg["roles"][role][scope]` via a `.get()` chain. Concretely: `if role not in cfg.get("roles", {}) or scope not in cfg["roles"][role]: raise ReviewerError(f"Missing roles.{role}.{scope} in config")`. Then read `reviewer` and `rounds` from the subsection. If `reviewer is None` OR `rounds == 0`, return `None`. Otherwise call `resolve(registry, reviewer)` and return the result. The defensive guard prevents `KeyError` from bypassing the API scripts' `except ReviewerError` handler when an overlay or partial config omits the parent keys.
...
- **Requirements:**
  ...
  - `_test_registry.py` also exposes `write_to(wiki_root: Path, **overrides) -> Path`. Body: first `wiki_root.mkdir(parents=True, exist_ok=True)` (fixtures typically assign `wiki_root = tmp_path / "wiki"` without creating the directory); then call `make_minimal_registry(**overrides)`, serialise the dict to YAML via `yaml.safe_dump`, write the result to `wiki_root / "reviewers.yaml"`, and return the written path. Used by every flow-test fixture in batch 2 to satisfy `_reviewers.load(wiki_root)`'s file-must-exist contract.
