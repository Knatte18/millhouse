...
Create `plugins/mill/skills/mill-finalize/SKILL.md` — a new end-of-task orchestration skill. It reads `git.require_pr_to_base` and `git.base_branch` from the deep-merged config, compares the parent branch to the base branch, and routes to either the PR path or the direct path. The PR path runs mill-merge-in, updates status.md, does the cleanup commit, pushes, calls /git-pr, flips Home.md to [pr-pending], and halts. The direct path delegates entirely to /mill-merge.

This batch has no file dependencies on other batches (creates a new file from scratch), so it is the root of the DAG.

## Cards

### Card 1: Create plugins/mill/skills/mill-finalize/SKILL.md
...
- **Requirements:**
  ...
  Create `plugins/mill/skills/mill-finalize/SKILL.md` with the exact content specified below. Every section is mandatory; no heading-only skeletons.

  **Frontmatter (YAML, with `---` delimiters as it is a SKILL.md):**
  ```
  ---
  name: mill-finalize
  ...
  **Header comment** (immediately after frontmatter):
  ```
  # mill-finalize
  ...
  **PR Steps section** (only the PR path; direct mode is a one-liner above):
  ```
  ## PR Steps
...
- **Commit:** `feat(mill-finalize): add mill-finalize orchestration skill`

## Batch Tests

No runnable test surface — this batch creates a SKILL.md file only. Correctness is verified by the plan reviewer (reading the file against discussion.md decisions) and at runtime when mill-go invokes the skill.
