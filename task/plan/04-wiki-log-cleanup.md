...
- **Requirements:** Perform a one-time cleanup of the orphan log in the wiki repo. Execute the following steps from the task worktree (so `_paths.resolve_git_root()` succeeds and is NOT inside the wiki):
  ...
  The implementer may execute steps 1–7 by writing a short inline `python -c "..."` block invoked via the `Bash` tool. Use the standard plugin-cache invocation per CLAUDE.md's `Conventions worth carrying` rule: `PYTHONPATH="${CLAUDE_PLUGIN_ROOT}/scripts" uv run --project "${CLAUDE_PLUGIN_ROOT}" python -c "..."`. CLAUDE.md documents a single fallback (and only because this task runs inside the millhouse source repo itself): if `${CLAUDE_PLUGIN_ROOT}` is unset/empty in the current shell, fall back to `PYTHONPATH="plugins/mill/scripts" uv run --project plugins/mill python -c "..."` against the source tree. Either approach uses the helper-based path (`wiki_lock` + `sync_pull`) because the lockfile is honoured automatically. The implementer MUST NOT `cd` into the wiki directory at any point — the same anti-pattern this whole task exists to prevent.
