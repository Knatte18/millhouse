...
- **Requirements:**
  ...
  - Imports: `argparse`, `sys`, `re`, `pathlib.Path`, plus mill helpers `_gh_issues`, `_sidebar`, `_tasks_md`, `_wiki`, and `from _paths import resolve_git_root, resolve_wiki_path`.
...
- **Requirements:**
  ...
  - Add module-level helpers (above `main()`):
    ...
    - `def _patch_resolve_paths(wiki_path: Path) -> tuple[callable, callable]` swaps `_paths.resolve_git_root` and `_paths.resolve_wiki_path` for closures that return `wiki_path`. Returns the original callables so the caller can restore them via `try`/`finally`. Direct module-attribute reassignment — no monkeypatch fixture.
