...
- **Creates:** none
- **Deletes:** none
- **Requirements:** In `_spawn_core.write_initial_status` (lines 682–743 of `_spawn_core.py`), after the existing `git commit` call (lines 736–742) and its returncode check, add a third `_subprocess_util.run` call: `result = _subprocess_util.run(["git", "-C", str(worktree_path), "push", "--set-upstream", "origin", branch])` followed by the same `if result.returncode != 0: raise RuntimeError(f"git push --set-upstream origin {branch} failed: {result.stderr.strip()!r}")` shape used by the existing add/commit blocks. Place the push immediately before the `return status_abs` line. Then in `test-spawn-core.py`, modify `_make_git_repo` (around line 138) so it ALSO sets up a tempdir-bare remote and configures `origin` — extend the helper using the existing `_make_wiki` bare-remote pattern (`test-spawn-core.py:81-135`): (a) create a `<tmp>/repo-bare/` directory, (b) `git -C <bare> init --bare`, (c) `git -C <repo> remote add origin <bare>`. Keep the helper's existing return value (the working repo path). In `test_write_initial_status` (around line 372), AFTER the `_make_git_repo` call and BEFORE the `write_initial_status` call, add `subprocess.run(["git", "-C", str(repo), "checkout", "-b", "hanf/task-one"], check=True, capture_output=True)` so the working tree is on the task branch when push fires — this mirrors the production flow where mill-spawn's `_worktree.create` checks out the task branch before `write_initial_status` runs. After these fixture changes the test passes because the push succeeds against the bare remote. Add a third test `test_write_initial_status_push_failure_raises_runtime_error` that uses a working repo with NO origin remote (build a stripped helper or skip the bare-remote setup), checks out `hanf/task-one` the same way, and asserts `RuntimeError` is raised with `"git push --set-upstream origin"` in the message. Register the new test in the `tests = [...]` list near the bottom of the file. Verify all three spawn-status tests pass after the change.
