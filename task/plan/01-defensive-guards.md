...
- **Creates:** none
- **Deletes:** none
- **Requirements:** After the existing `return Path(result.stdout.strip())` site in `_paths.resolve_git_root()`, transform the function so it (a) computes `repo_root = Path(result.stdout.strip())` into a local variable, (b) runs the name-check fast path (`repo_root.name == "wiki"` → raise `SystemExit` with the literal error message from the `error-message-text` Shared Decision, substituting `{wiki_path}` with `str(repo_root)`), (c) attempts the path-equality check by calling `resolve_wiki_path(repo_root)` inside a `try/except (Exception, SystemExit):` block — `resolve_wiki_path` may raise `SystemExit` per card 1's guard, and `SystemExit` inherits from `BaseException` (not `Exception`), so a plain `except Exception:` would let the inner halt propagate and break card 5 test case 4. Use the explicit `(Exception, SystemExit)` tuple form, NOT `except BaseException:` — the tuple form correctly catches the intended pair while still letting genuine interrupts (`KeyboardInterrupt`, `GeneratorExit`) propagate. On success, raise `SystemExit` when `repo_root.resolve()` equals `wiki_path.resolve()` (use `Path.samefile` when both paths exist on disk; fall back to `Path.resolve() == Path.resolve()` when either does not exist) substituting `{wiki_path}` with `str(wiki_path)`; on caught `(Exception, SystemExit)` from `resolve_wiki_path` (config missing, card-1 nested halt, etc.), silently swallow — the name-check above already protects this path. Return `repo_root` only after both checks pass.
...
- **Requirements:** Add the following test cases to `plugins/mill/unit_tests/test-paths.py`, following the existing `main()` function's `try/except AssertionError` style (each `_check_…` block prints `PASS:` on success, accumulates failures via `failures.append`). Use `unittest.mock.patch` on `_subprocess_util.run` for `resolve_git_root` cases. Cases to add:
    ...
    4. Name check fires before nested-halt from `resolve_wiki_path` can propagate — patch `_paths.resolve_wiki_path` to raise `SystemExit("nested-halt")` and exercise both sub-cases sequentially under the same patch:
       - Non-wiki sub-case: patch `_subprocess_util.run` to return `_make_run_result(stdout="/tmp/not-wiki\n")`. Assert the outer call returns `Path("/tmp/not-wiki")` (the `(Exception, SystemExit)` swallow in card 2 absorbs the inner `SystemExit("nested-halt")` and the function falls through to the normal return).
       - Wiki sub-case: patch `_subprocess_util.run` to return `_make_run_result(stdout="C:/Code/millhouse/wiki\n")`. Assert the outer `SystemExit` is raised and its message contains `"cwd is inside wiki"` — NOT the inner `"nested-halt"`. The name check (step (b) in card 2) must fire BEFORE `resolve_wiki_path` is called, so the nested halt cannot propagate.
