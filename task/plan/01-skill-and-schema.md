...
- **Requirements:**
  ...
  - Read the production wiki config at the absolute path `C:/Code/millhouse/wiki/config.yaml`. Note: production may or may not already contain a `merge:` block (template-vs-production drift is pre-existing and out of scope for this task). Do NOT make any assumption about which sections are present.
  - Identify the last existing top-level section in the production file (whichever block is closest to end-of-file — e.g. `groom:`, `merge:`, `notify:`, or another). Append the byte-for-byte identical commented `verify:` section IMMEDIATELY AFTER that last existing block, preserving any trailing newline behavior of the existing file (do not introduce extra blank lines beyond what the template uses). Do NOT add or remove any existing section as part of this card — the only addition is the commented `verify:` block at the bottom.
