...
- **Requirements:**
  ...
  - **Migration-warning test in `test-review-common.py`:** add a new test function (or extend an existing one) that constructs a `mill_dir` with a `config.local.yaml` containing a top-level `review:` key (e.g. `{"review": {"code": {"rounds": 1}}}`), calls `_review_common.load_config(wiki_root, mill_dir)` with a valid wiki-side config, and asserts `sys.stderr` (captured via `io.StringIO` redirected from `sys.stderr` for the duration of the call, or via `contextlib.redirect_stderr`) contains a non-empty warning string that mentions both the overlay path and the orphaned key `review`. Also assert the merged cfg returned does NOT contain `cfg["review"]` after the merge (the new schema has no `review:` block, so the orphan branch sits at `cfg["review"]` from the deep-merge — the assertion is that the warning fires, not that the orphan is removed; see card 7's behaviour spec). Use a `tempfile.TemporaryDirectory` for the wiki + mill dirs.
