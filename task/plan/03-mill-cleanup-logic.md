...
- **Creates:** none
- **Deletes:** none
- **Requirements:** In `plugins/mill/scripts/millpy-cleanup.py`, add a fifth field to the `CleanupPlan` `@dataclass(frozen=True)` (currently lines ~40–46): `to_reap_pr: list[SlugRecord] = field(default_factory=list)`, declared as the LAST field (after `to_report`). The field MUST come last — placing a defaulted field before a non-defaulted field (e.g., before `to_reset_home` or `to_report`) raises `TypeError: non-default argument follows default argument` at class-definition time and breaks every import of the module. The `default_factory=list` default is mandatory so existing 7+ `CleanupPlan(...)` constructions in `test-cleanup.py` (which pass only the original four fields by keyword) continue to work; all existing callers use keyword arguments so adding `to_reap_pr` at the end is backward-compatible. Add `field` to the `from dataclasses import …` line at the top of the file (currently imports only `dataclass`). Update the `build_plan` `return CleanupPlan(...)` call site to pass `to_reap_pr=to_reap_pr` once card 13 declares the list. Update `_print_plan` (currently lines ~193–209) to print a new section between `to_remove_abandoned` and `to_report`: for each record in `plan.to_reap_pr`, print `f"REAP-PR:           {r.slug}  [worktree={r.worktree_path}, branch={r.branch}]"` (the print ORDER in the output is independent of the dataclass field order). Update the `"Nothing to do."` early-return guard to also check `plan.to_reap_pr` (line 194). Do not modify `build_plan` or `apply_plan` logic in this card; those changes land in cards 12–15.
...
- **Requirements:** In `plugins/mill/scripts/millpy-cleanup.py`, rewrite the `if phase == "done":` branch in `build_plan` (currently lines ~131–144). Replace the existing `parent_branch = _status.read_parent_branch(...)` + `git log --oneline {parent_branch}..{record.branch}` guard with the following logic. Read the Home.md marker for this slug from `marker_by_slug` (already computed at the top of the function as `{t.slug: t.phase for t in home_tasks}`):
  ...
  **Test cleanup (mandatory — stale tests otherwise break `verify:`).** In `plugins/mill/unit_tests/test-cleanup.py`, delete the four `guard-slug-1` through `guard-slug-4` test blocks: they mock `git log --oneline parent..branch` and assert against the substring `"unmerged commits"`, which no longer matches the new "archive tag absent" path. Cards 17–18 supersede their coverage with archive-tag-aware tests. Also update `_mock_branch_run` (top of file): remove the `"log" in argv and "--oneline" in argv` branch entirely (no production caller uses it anymore), AND remove the `log_has_commits: bool = False` parameter from the function signature (after deleting the guard-slug-1 through guard-slug-4 blocks, no caller passes it). Add a `"tag" in argv and "-l" in argv` branch returning `"archive/<slug>\n"` by default (card 17's tests will override this per-block as needed).
...
- **Requirements:** Add five new test blocks to `plugins/mill/unit_tests/test-cleanup.py`'s `main()` function, each following the file's existing `with tempfile.TemporaryDirectory() as tmp:` + `patch("mill_cleanup._subprocess_util.run", side_effect=...)` pattern. Each block ends with a `print("PASS ...")` statement.
  1. **`phase=done + home_marker=done + archive tag present → to_remove_done`** — the default tag behavior installed by card 12's `_mock_branch_run` (returning `"archive/<slug>\n"` for `["git", "-C", ..., "tag", "-l", ...]`) already covers this case — no extension to `_mock_branch_run` needed for this test. Build plan with `home_tasks=[_make_task("done-slug", "done")]` and status.md phase=`done`. Assert the record lands in `plan.to_remove_done`, NOT in `plan.to_report`.
...
- **Creates:** none
- **Deletes:** none
- **Requirements:** Add four new test blocks to `plugins/mill/unit_tests/test-cleanup.py`'s `main()` covering `apply_plan` PR-reap. Each follows the existing block pattern (`with tempfile.TemporaryDirectory() ...`, `with patch(...) ...`).
  1. **`apply_plan PR-MERGED → teardown executed + Home.md flipped to [done]`** — construct a `SlugRecord` for a fake worktree path. Patch `mill_cleanup._subprocess_util.run` so calls matching `["gh", "pr", "list", ...]` return stdout `'{"state": "MERGED", "mergeCommit": {"oid": "abc123"}, "number": 42}'`. Patch `mill_cleanup._subprocess_util.run` for `tag -l archive/<slug>` to return empty stdout (so the helper creates the tag). Patch `mill_cleanup._worktree.remove_safe` (NOT `_worktree.remove` — `_apply_worktree_record` calls the `remove_safe` variant) and `_junction.remove`. Build a `CleanupPlan(to_remove_done=[], to_remove_abandoned=[], to_reset_home=[], to_report=[], to_reap_pr=[record])` (note: `to_reap_pr` is the last positional field per card 11's field ordering — prefer keyword args throughout). Pre-create a Home.md with `[pr-pending]` for the slug. Call `apply_plan`. Assert: (a) at least one `git tag archive/<slug>` call was made; (b) the Home.md text on disk after the call shows `[done]` for the slug; (c) `_worktree.remove_safe` was called with the worktree path.
  ...
  4. **`apply_plan gh-pr-list-failed → early return, no state mutation`** — mock the `gh pr list` invocation to return `returncode=1` with non-empty stderr. Assert: (a) no `git tag` call was made; (b) no worktree-remove call was made; (c) Home.md was NOT mutated (still shows `[pr-pending]`); (d) a stderr line containing the slug was emitted (capture as in test 3). This covers Card 15's "On non-zero exit or empty stdout, print to stderr and return early without touching state." path.
