...
- **Creates:** none
- **Deletes:** none
- **Requirements:** Extend `plugins/mill/unit_tests/test-marker.py` with three new test functions verifying that `slug_from_branch` succeeds for the new and existing non-`active` phases. Add `test_slug_from_branch_ready_to_merge`, `test_slug_from_branch_pr_pending`, and `test_slug_from_branch_done`, each following the existing `test_slug_from_branch_happy_path` pattern but passing `phase="ready-to-merge"`, `phase="pr-pending"`, and `phase="done"` respectively to `_test_helpers._make_task_worktree`. Each must assert the returned slug equals `"foo"`. Remove or rewrite any existing test that asserts `MarkerError` is raised when phase is not `[active]` — search the file for assertions citing `phase=` in a `MarkerError` message; the relaxed helper no longer raises in that case. Retain the detached-HEAD and unknown-slug failure tests unchanged. Register the new tests in `main()` (if present) or in whatever runner pattern the file uses.
