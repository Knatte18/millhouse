...
- **Creates:** none
- **Deletes:** none
- **Requirements:** Extend `plugins/mill/unit_tests/test-marker.py` with five new test functions verifying that `slug_from_branch` succeeds for every non-`active` phase (which the relaxation now accepts). Add `test_slug_from_branch_ready_to_merge`, `test_slug_from_branch_pr_pending`, `test_slug_from_branch_done`, `test_slug_from_branch_phase_abandoned`, and `test_slug_from_branch_phase_none`, each following the existing `test_slug_from_branch_happy_path` pattern but passing `phase="ready-to-merge"`, `phase="pr-pending"`, `phase="done"`, `phase="abandoned"`, and `phase=None` respectively to `_test_helpers._make_task_worktree`. Each must assert the returned slug equals `"foo"` (the helper succeeds — no `MarkerError`). Remove or rewrite any existing test that asserts `MarkerError` is raised when phase is not `[active]` — search the file for assertions citing `MarkerError` in conjunction with `phase=`. Tests likely affected (rewrite all that exist): `test_slug_from_branch_phase_abandoned`, `test_slug_from_branch_phase_none`, AND `test_slug_from_branch_phase_done` — any one of these may currently assert `MarkerError` and must be replaced with a success variant (same name, opposite expectation, asserting the helper returns `"foo"`). Retain the detached-HEAD and unknown-slug failure tests unchanged. Register the new tests in `main()` (if present) or in whatever runner pattern the file uses.
