# Status

```yaml
phase: approved-fold-guard-allowlist
slug: ghissues-skill-fold-guard
branch: hanf/ghissues-skill-fold-guard
plan: _mill/plan
parent: main
task: Fix mill-ghissues-to-tasks to refuse fold-ins into done and deferred tasks
task_description: |
  Fix mill-ghissues-to-tasks to refuse fold-ins into done and deferred tasks
```

## Timeline

```text
discussing  '2026-06-15T12:31:38Z'
discussion-fix-r2  '2026-06-15T12:48:37Z'
discussed  '2026-06-15T12:49:11Z'
planning  '2026-06-15T12:56:27Z'
plan-fix-r1  '2026-06-15T12:59:06Z'
planned  '2026-06-15T12:59:37Z'
implementing  '2026-06-15T13:00:40Z'
approved-fold-guard-allowlist  '2026-06-15T13:05:49Z'
```

## Batches

```yaml
batches:
  - name: fold-guard-allowlist
    state: approved
    implementer_session: 7cbe5aa1-3e10-4b1b-944c-659dfed85a27
    start_sha: 28d571b7bce46672dc5f4e5d97000caab3b791e7
    commit_sha: e307ec9b33da7b9547368bec3ee67b6fcf2a9ab7
```
