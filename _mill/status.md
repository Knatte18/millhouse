# Status

```yaml
phase: done
slug: mill-merge-nested-layout-and-lock-bugs
branch: hanf/mill-merge-nested-layout-and-lock-bugs
plan: _mill/plan
parent: main
task: 'mill-merge/merge-in: nested-layout config resolution, stale locks, and rollback-target bugs'
task_description: |
  mill-merge/merge-in: nested-layout config resolution, stale locks, and rollback-target bugs
```

## Timeline

```text
discussing  '2026-08-21T06:51:41Z'
discussed  '2026-08-21T09:21:42Z'
planning  '2026-08-21T09:32:00Z'
plan-review-r1  '2026-08-21T09:40:11Z'
plan-fix-r1  '2026-08-21T09:40:11Z'
plan-fix-r2  '2026-08-21T09:49:51Z'
plan-review-r3  '2026-08-21T09:55:54Z'
planned  '2026-08-21T09:56:08Z'
implementing  '2026-08-21T09:56:38Z'
approved-mill-merge-push-and-lock  '2026-08-21T09:59:15Z'
approved-mill-merge-in-nested-cwd  '2026-08-21T10:01:42Z'
approved-parent-branch-liveness  '2026-08-21T10:04:12Z'
approved-config-caller-alignment  '2026-08-21T10:07:53Z'
holistic-reviewing  '2026-08-21T10:08:07Z'
holistic-fixing  '2026-08-21T10:10:47Z'
nits-fixed-holistic  '2026-08-21T10:12:23Z'
holistic-approved  '2026-08-21T10:12:30Z'
done  '2026-08-21T10:12:59Z'
```

## Batches

```yaml
batches:
  - name: mill-merge-push-and-lock
    state: approved
    implementer_session: 5164b161-d031-4e66-bc50-acea6bf35856
    start_sha: 22a384c56f8f7b4b0e4914b9e0a41b88f0e109cd
    commit_sha: 6eeb5bac60aab4d67f1dc65806c2147db0c78c44
  - name: mill-merge-in-nested-cwd
    state: approved
    implementer_session: 2ad1fac5-da57-4029-a61e-784c55c4909e
    start_sha: 6958293aececd6a3601261b256fe19e910fb679a
    commit_sha: d2b885c5e46107b6abc76d52b6e756e9cb98ed20
  - name: parent-branch-liveness
    state: approved
    implementer_session: dff120c8-cf0a-4bfe-b3a8-8d293c38cec5
    start_sha: 3f86d5e2b545fa264e140739d02371757ad65806
    commit_sha: d67cc1f4277a7acc3d08b4da393b426c60af4178
    verify_baseline_failures: []
  - name: config-caller-alignment
    state: approved
    implementer_session: f2f59972-1002-49be-a733-aadcb5e1322a
    start_sha: 6881b85899e738ccd277c43f3934a6de549e88d0
    commit_sha: d439ddea6eeab62143d0a124bd55f9008bdde32a
    verify_baseline_failures: []
```
