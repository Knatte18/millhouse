# Implementer Brief — mill-setup/wiki/docs/build-env: misc small bugs, round 3 / handoff-draft-order
...
- **Batch file (authoritative for this batch):** `/home/hanf/Code/millhouse/wts/mill-setup-wiki-doc-misc-r3/_mill/plan/02-handoff-draft-order.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/hanf/Code/millhouse/wts/mill-setup-wiki-doc-misc-r3/_mill/plan/02-handoff-draft-order.md`.
   - Commit the plan edit first (`plan: extend handoff-draft-order refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"5011547f-7154-487e-a5f6-ad0f1b9323c3"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"5011547f-7154-487e-a5f6-ad0f1b9323c3","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `5011547f-7154-487e-a5f6-ad0f1b9323c3` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"5011547f-7154-487e-a5f6-ad0f1b9323c3","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"5011547f-7154-487e-a5f6-ad0f1b9323c3"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
