# Implementer Brief — Fix config unknown-key warning on git namespace and commit _mill/briefs/ after dispatch / brief-commit-uniformity
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-config-and-brief-gaps\_mill\plan\02-brief-commit-uniformity.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\millhouse\wts\mill-config-and-brief-gaps\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\millhouse\wts\mill-config-and-brief-gaps`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-config-and-brief-gaps\_mill\plan\02-brief-commit-uniformity.md`.
   - Commit the plan edit first (`plan: extend brief-commit-uniformity refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"108aeeb8-384b-4890-904c-98157f08839a"}
```
...
**`session_id` MUST be exactly `108aeeb8-384b-4890-904c-98157f08839a` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"108aeeb8-384b-4890-904c-98157f08839a"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\mill-config-and-brief-gaps` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\millhouse\wts\mill-config-and-brief-gaps`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C C:\Code\millhouse\wts\mill-config-and-brief-gaps show <parent-branch>:<path>` to read it without changing cwd.
