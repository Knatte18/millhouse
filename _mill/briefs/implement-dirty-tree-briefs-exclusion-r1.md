# Implementer Brief — mill-go: baseline-stage timeout/cold-build cost and finalize dirty-tree false positive / dirty-tree-briefs-exclusion
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/millhouse/wts/mill-go-finalize-and-baseline-stage-bugs/_mill/plan/01-dirty-tree-briefs-exclusion.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/millhouse/wts/mill-go-finalize-and-baseline-stage-bugs/_mill/plan/01-dirty-tree-briefs-exclusion.md`.
   - Commit the plan edit first (`plan: extend dirty-tree-briefs-exclusion refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"75ca5303-31f0-4218-b71e-90951280e288"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"75ca5303-31f0-4218-b71e-90951280e288","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `75ca5303-31f0-4218-b71e-90951280e288` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"75ca5303-31f0-4218-b71e-90951280e288","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"75ca5303-31f0-4218-b71e-90951280e288"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
