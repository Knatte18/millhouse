...
You are an independent discussion reviewer for **Fix implement finalize correctness: mid-batch stop recovery, commit-count guard, and empty-commit detection**. Round **1**. Reviewer model: **opushigh**.
...
Read the discussion at `C:\Code\millhouse\wts\mill-implement-finalize-gaps\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
...
```
MILL_REVIEW_BEGIN
# Review: Fix implement finalize correctness: mid-batch stop recovery, commit-count guard, and empty-commit detection
