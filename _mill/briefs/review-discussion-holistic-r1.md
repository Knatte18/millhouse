...
You are an independent discussion reviewer for **Fix stale checkpoint safety, Go binary ephemeral allowlist, and bare agent-tier name trap**. Round **1**. Reviewer model: **opushigh**.
...
Read the discussion at `C:\Code\millhouse\wts\mill-scope-and-infra-gaps\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
