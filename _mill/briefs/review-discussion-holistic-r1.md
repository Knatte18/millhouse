...
You are an independent discussion reviewer for **Fix prepare-retry atomicity, partial-batch finalize routing, and envelope field parity**. Round **1**. Reviewer model: **opushigh**.
...
Read the discussion at `C:\Code\millhouse\wts\mill-pipeline-finalize-gaps\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
