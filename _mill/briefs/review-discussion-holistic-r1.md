...
You are an independent discussion reviewer for **Fix millpy-implement.py finalize to accept (or not require) --session-id/--start-sha**. Round **1**. Reviewer model: **opushigh**.
...
Read the discussion at `C:\Code\millhouse\wts\implement-finalize-session-id\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
