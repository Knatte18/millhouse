# Implementer Brief — mill-go/millpy-implement: Windows dotnet build-server file-lock races in verify/baseline stages / baseline-teardown-defense-in-depth
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene/_mill/plan/02-baseline-teardown-defense-in-depth.md`
...
Before implementing any cards, identify which cards are already committed: run `git -C /home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene log ..HEAD --oneline` and match each commit subject against the cards' `Commit:` messages.
When `` is empty, derive the range start via `git -C /home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene log --grep="^mill-go: start batch" -n 1 --format=%H`.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene/_mill/plan/02-baseline-teardown-defense-in-depth.md`.
   - Commit the plan edit first (`plan: extend baseline-teardown-defense-in-depth refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"261f9d10-6820-4378-92e8-bef69697ed5c"}
   ```
...
Determine the range start exactly as in "Resume-after-incomplete" above: use `` when non-empty, else `git -C /home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene log --grep="^mill-go: start batch" -n 1 --format=%H`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"261f9d10-6820-4378-92e8-bef69697ed5c","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `261f9d10-6820-4378-92e8-bef69697ed5c` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"261f9d10-6820-4378-92e8-bef69697ed5c","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"261f9d10-6820-4378-92e8-bef69697ed5c"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
