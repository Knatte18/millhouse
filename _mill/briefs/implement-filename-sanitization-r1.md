# Implementer Brief — Fix batch-name sanitization (colon/slash on Windows) and implementer skill loading / filename-sanitization
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\batch-name-and-skill-loading\_mill\plan\01-filename-sanitization.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\millhouse\wts\batch-name-and-skill-loading\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\millhouse\wts\batch-name-and-skill-loading`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\batch-name-and-skill-loading\_mill\plan\01-filename-sanitization.md`.
   - Commit the plan edit first (`plan: extend filename-sanitization refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"cb6f064b-cca1-4139-a8d3-f3a87588b521"}
```
...
**`session_id` MUST be exactly `cb6f064b-cca1-4139-a8d3-f3a87588b521` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"cb6f064b-cca1-4139-a8d3-f3a87588b521"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
**File paths are POSIX-style relative paths from `C:\Code\millhouse\wts\batch-name-and-skill-loading`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
