# Implementer Brief — Fix spawn lifecycle integrity, agent-mode async assumption, merge-in conflicts, and pre-existing failure validation / verify-and-implementer
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-spawn-and-workflow-integrity\_mill\plan\03-verify-and-implementer.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-spawn-and-workflow-integrity\_mill\plan\03-verify-and-implementer.md`.
   - Commit the plan edit first (`plan: extend verify-and-implementer refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"f53bf683-450c-4801-9774-68f33697486f"}
```
...
**`session_id` MUST be exactly `f53bf683-450c-4801-9774-68f33697486f` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"f53bf683-450c-4801-9774-68f33697486f"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
