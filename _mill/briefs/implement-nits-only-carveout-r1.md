# Implementer Brief — Fix agent-mode dispatch races and pipeline gaps / nits-only-carveout
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\agent-mode-gaps\_mill\plan\07-nits-only-carveout.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\agent-mode-gaps\_mill\plan\07-nits-only-carveout.md`.
   - Commit the plan edit first (`plan: extend nits-only-carveout refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"c7d48356-e26d-4534-bd18-049923bf9152"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c7d48356-e26d-4534-bd18-049923bf9152"}
```
...
**`session_id` MUST be exactly `c7d48356-e26d-4534-bd18-049923bf9152` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c7d48356-e26d-4534-bd18-049923bf9152"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
