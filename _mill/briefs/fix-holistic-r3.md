# Holistic Fixer Brief — Audit mill-start/mill-plan/mill-go for turn reduction
...
- **Holistic review file:** `/home/knatte/Code/millhouse/wts/turn-reduction-audit/_mill/reviews/20260919-121122-code-review-r3.md`
- **Plan overview:** `/home/knatte/Code/millhouse/wts/turn-reduction-audit/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/millhouse/wts/turn-reduction-audit`
...
```
/home/knatte/Code/millhouse/wts/turn-reduction-audit/_mill/plan/01-turn-reduction-audit-doc.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/millhouse/wts/turn-reduction-audit/_mill/reviews/20260919-121122-code-review-r3.md`.
...
- Overlapping line-range double-classifies mill-plan Plan Review lines: **Location:** `doc/turn-reduction-audit.md` (`## mill-plan` ? Phase: Plan Review), citing `mill-plan/SKILL.md:308-337` vs. `mill-plan/SKILL.md:316-329`
- `_status.append_phase` call-site counts don't match current source: **Location:** `doc/turn-reduction-audit.md:441-445` (the `## mill-go-base` re-derivation paragraph)
...
Run each from `/home/knatte/Code/millhouse/wts/turn-reduction-audit` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/millhouse/wts/turn-reduction-audit rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/millhouse/wts/turn-reduction-audit status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"5373a83a-02de-4a28-b310-35ca87a95b42"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `5373a83a-02de-4a28-b310-35ca87a95b42` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"5373a83a-02de-4a28-b310-35ca87a95b42"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
