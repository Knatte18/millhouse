# Implementer Brief — Miscellaneous small tooling and doc/template accuracy gaps / plan-overview-comment-fix
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/millhouse/wts/mill-misc-tooling-and-docs-gaps/_mill/plan/04-plan-overview-comment-fix.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/millhouse/wts/mill-misc-tooling-and-docs-gaps/_mill/plan/04-plan-overview-comment-fix.md`.
   - Commit the plan edit first (`plan: extend plan-overview-comment-fix refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"b1e31b82-2a3e-423e-912e-8b2670303e90"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"b1e31b82-2a3e-423e-912e-8b2670303e90"}
```
...
**`session_id` MUST be exactly `b1e31b82-2a3e-423e-912e-8b2670303e90` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"b1e31b82-2a3e-423e-912e-8b2670303e90"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
