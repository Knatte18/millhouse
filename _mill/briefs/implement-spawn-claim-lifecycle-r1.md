# Implementer Brief — Fix spawn lifecycle integrity, agent-mode async assumption, merge-in conflicts, and pre-existing failure validation / spawn-claim-lifecycle
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-spawn-and-workflow-integrity\_mill\plan\01-spawn-claim-lifecycle.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-spawn-and-workflow-integrity\_mill\plan\01-spawn-claim-lifecycle.md`.
   - Commit the plan edit first (`plan: extend spawn-claim-lifecycle refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"fe87e522-d190-40e2-8d53-8464dece5163"}
```
...
**`session_id` MUST be exactly `fe87e522-d190-40e2-8d53-8464dece5163` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"fe87e522-d190-40e2-8d53-8464dece5163"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
