# Implementer Brief — Agent-mode dispatch: envelope fields and session/runtime state are unreliable / effort-tier-implementer
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/millhouse/wts/mill-go-agent-dispatch-reliability-gaps/_mill/plan/02-effort-tier-implementer.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/millhouse/wts/mill-go-agent-dispatch-reliability-gaps/_mill/plan/02-effort-tier-implementer.md`.
   - Commit the plan edit first (`plan: extend effort-tier-implementer refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"feee4e28-d9f7-4485-9af7-c157739fbabd"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"feee4e28-d9f7-4485-9af7-c157739fbabd"}
```
...
**`session_id` MUST be exactly `feee4e28-d9f7-4485-9af7-c157739fbabd` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"feee4e28-d9f7-4485-9af7-c157739fbabd"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
