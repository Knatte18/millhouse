# Implementer Brief — Fix nested-hub path resolution and SKILL.md vs shipped-API mismatches / merge-skill-path-fixes
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-nested-hub-and-skill-sync\_mill\plan\02-merge-skill-path-fixes.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-nested-hub-and-skill-sync\_mill\plan\02-merge-skill-path-fixes.md`.
   - Commit the plan edit first (`plan: extend merge-skill-path-fixes refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"f8790c96-b403-400e-91ce-78fd103d90eb"}
```
...
**`session_id` MUST be exactly `f8790c96-b403-400e-91ce-78fd103d90eb` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"f8790c96-b403-400e-91ce-78fd103d90eb"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
