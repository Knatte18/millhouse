# Implementer Brief — Fix mid-batch stop recovery and fixer false-success in agent-dispatch mode / finalize-incomplete-core
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-agent-dispatch-gaps\_mill\plan\01-finalize-incomplete-core.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-agent-dispatch-gaps\_mill\plan\01-finalize-incomplete-core.md`.
   - Commit the plan edit first (`plan: extend finalize-incomplete-core refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"42e6f4af-42bd-4ce6-b99c-6710dc812201"}
```
...
**`session_id` MUST be exactly `42e6f4af-42bd-4ce6-b99c-6710dc812201` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"42e6f4af-42bd-4ce6-b99c-6710dc812201"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
