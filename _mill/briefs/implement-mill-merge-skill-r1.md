# Implementer Brief — Handle pre-closed and pre-merged PRs gracefully in mill-merge / mill-merge-skill
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-merge-pr-state-awareness\_mill\plan\03-mill-merge-skill.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\millhouse\wts\mill-merge-pr-state-awareness\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\millhouse\wts\mill-merge-pr-state-awareness`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-merge-pr-state-awareness\_mill\plan\03-mill-merge-skill.md`.
   - Commit the plan edit first (`plan: extend mill-merge-skill refs for <short reason>`) and push via the wiki.
...
**`session_id` MUST be exactly `33a89e54-42c9-4c2a-8b08-9415e9e262ae` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"33a89e54-42c9-4c2a-8b08-9415e9e262ae"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\mill-merge-pr-state-awareness` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\millhouse\wts\mill-merge-pr-state-awareness`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
