# Implementer Brief — mill-finalize/mill-merge corrupt or mishandle _mill/status.md and task_dir on stacked branches / integration-test-coverage
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption/_mill/plan/04-integration-test-coverage.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption/_mill/plan/04-integration-test-coverage.md`.
   - Commit the plan edit first (`plan: extend integration-test-coverage refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"f54263a2-fc47-446c-b366-fafd0a4d7958"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"f54263a2-fc47-446c-b366-fafd0a4d7958"}
```
...
**`session_id` MUST be exactly `f54263a2-fc47-446c-b366-fafd0a4d7958` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"f54263a2-fc47-446c-b366-fafd0a4d7958"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
