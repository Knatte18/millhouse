# Implementer Brief — Agent-mode dispatch: envelope fields and session/runtime state are unreliable / permission-allowlist
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/millhouse/wts/mill-go-agent-dispatch-reliability-gaps/_mill/plan/06-permission-allowlist.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/millhouse/wts/mill-go-agent-dispatch-reliability-gaps/_mill/plan/06-permission-allowlist.md`.
   - Commit the plan edit first (`plan: extend permission-allowlist refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"b56d2387-0a65-4ca5-b5b4-7d94d1f339e1"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"b56d2387-0a65-4ca5-b5b4-7d94d1f339e1"}
```
...
**`session_id` MUST be exactly `b56d2387-0a65-4ca5-b5b4-7d94d1f339e1` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"b56d2387-0a65-4ca5-b5b4-7d94d1f339e1"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
