# Implementer Brief — Add first-class Moves/Renames field to plan cards for rename-heavy batches / templates-skill-config
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-plan-rename-moves\_mill\plan\03-templates-skill-config.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-plan-rename-moves\_mill\plan\03-templates-skill-config.md`.
   - Commit the plan edit first (`plan: extend templates-skill-config refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c5e41d63-de22-4d4b-84ee-badc7d2b52ac"}
```
...
**`session_id` MUST be exactly `c5e41d63-de22-4d4b-84ee-badc7d2b52ac` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c5e41d63-de22-4d4b-84ee-badc7d2b52ac"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
