# Implementer Brief — Fix agent-dispatch cwd guard, implementer self-termination, and false-success contracts / hub-cwd-resolution
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-implementer-and-dispatch-quality\_mill\plan\01-hub-cwd-resolution.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-implementer-and-dispatch-quality\_mill\plan\01-hub-cwd-resolution.md`.
   - Commit the plan edit first (`plan: extend hub-cwd-resolution refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"2db17510-f8e5-4a58-ad08-4605d2ad7e2b"}
```
...
**`session_id` MUST be exactly `2db17510-f8e5-4a58-ad08-4605d2ad7e2b` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"2db17510-f8e5-4a58-ad08-4605d2ad7e2b"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
