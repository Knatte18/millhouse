# Implementer Brief — Fix nested-hub path resolution and SKILL.md vs shipped-API mismatches / merge-integration-test
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-nested-hub-and-skill-sync\_mill\plan\03-merge-integration-test.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-nested-hub-and-skill-sync\_mill\plan\03-merge-integration-test.md`.
   - Commit the plan edit first (`plan: extend merge-integration-test refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"48691dc3-8fd9-406e-8a0a-d9b42d36501a"}
```
...
**`session_id` MUST be exactly `48691dc3-8fd9-406e-8a0a-d9b42d36501a` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"48691dc3-8fd9-406e-8a0a-d9b42d36501a"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
