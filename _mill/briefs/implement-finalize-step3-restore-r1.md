# Implementer Brief — mill-finalize/mill-merge corrupt or mishandle _mill/status.md and task_dir on stacked branches / finalize-step3-restore
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption/_mill/plan/02-finalize-step3-restore.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption/_mill/plan/02-finalize-step3-restore.md`.
   - Commit the plan edit first (`plan: extend finalize-step3-restore refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"f762958c-8f25-4f13-8a74-c19ea1ad0a8a"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"f762958c-8f25-4f13-8a74-c19ea1ad0a8a"}
```
...
**`session_id` MUST be exactly `f762958c-8f25-4f13-8a74-c19ea1ad0a8a` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"f762958c-8f25-4f13-8a74-c19ea1ad0a8a"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
