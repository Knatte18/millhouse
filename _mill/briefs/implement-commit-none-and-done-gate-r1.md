# Implementer Brief — mill-go-base / mill-plan documentation gaps / commit-none-and-done-gate
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/millhouse/wts/mill-go-plan-doc-gaps/_mill/plan/02-commit-none-and-done-gate.md`
- **Overview (for `## Shared Decisions` only):** `/home/knatte/Code/millhouse/wts/mill-go-plan-doc-gaps/_mill/plan/00-overview.md`
- **Worktree cwd:** `/home/knatte/Code/millhouse/wts/mill-go-plan-doc-gaps`
...
Before implementing any cards, identify which cards are already committed: run `git -C /home/knatte/Code/millhouse/wts/mill-go-plan-doc-gaps log ..HEAD --oneline` and match each commit subject against the cards' `Commit:` messages.
When `` is empty, derive the range start via `git -C /home/knatte/Code/millhouse/wts/mill-go-plan-doc-gaps log --grep="^mill-go: start batch" -n 1 --format=%H`.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/millhouse/wts/mill-go-plan-doc-gaps/_mill/plan/02-commit-none-and-done-gate.md`.
   - Commit the plan edit first (`plan: extend commit-none-and-done-gate refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"e8092f32-dc81-4434-b51b-ee5ac8a67b7f"}
   ```
...
**Pre-report self-check (mandatory before emitting success JSON):** Run `git -C /home/knatte/Code/millhouse/wts/mill-go-plan-doc-gaps status --porcelain --untracked-files=no`.
...
Determine the range start exactly as in "Resume-after-incomplete" above: use `` when non-empty, else `git -C /home/knatte/Code/millhouse/wts/mill-go-plan-doc-gaps log --grep="^mill-go: start batch" -n 1 --format=%H`.
Run `git -C /home/knatte/Code/millhouse/wts/mill-go-plan-doc-gaps log <range-start>..HEAD --oneline` and match commit subjects against the batch file's `## Cards` `Commit:` messages to get an exact count -- Commit: none cards are never expected to appear in this log;
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"e8092f32-dc81-4434-b51b-ee5ac8a67b7f","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `e8092f32-dc81-4434-b51b-ee5ac8a67b7f` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"e8092f32-dc81-4434-b51b-ee5ac8a67b7f","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"e8092f32-dc81-4434-b51b-ee5ac8a67b7f"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Use `git -C /home/knatte/Code/millhouse/wts/mill-go-plan-doc-gaps` for commits;
do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `/home/knatte/Code/millhouse/wts/mill-go-plan-doc-gaps`.**
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/millhouse/wts/mill-go-plan-doc-gaps show <parent-branch>:<path>` to read it without changing cwd.
