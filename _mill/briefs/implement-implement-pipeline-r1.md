# Implementer Brief — Fix prepare-retry atomicity, partial-batch finalize routing, and envelope field parity / implement-pipeline
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-pipeline-finalize-gaps\_mill\plan\01-implement-pipeline.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-pipeline-finalize-gaps\_mill\plan\01-implement-pipeline.md`.
   - Commit the plan edit first (`plan: extend implement-pipeline refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"9705d416-d8a0-45dd-9231-2a2d05587ace"}
```
...
**`session_id` MUST be exactly `9705d416-d8a0-45dd-9231-2a2d05587ace` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"9705d416-d8a0-45dd-9231-2a2d05587ace"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
