# Implementer Brief — Fix millpy-review-plan validator gaps and resolve_ref_paths path-doubling / ref-path-resolution
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\review-plan-and-ref-paths\_mill\plan\01-ref-path-resolution.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\review-plan-and-ref-paths\_mill\plan\01-ref-path-resolution.md`.
   - Commit the plan edit first (`plan: extend ref-path-resolution refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c5f9a08a-43a9-4562-a8e6-2f1fd3b5624c"}
```
...
**`session_id` MUST be exactly `c5f9a08a-43a9-4562-a8e6-2f1fd3b5624c` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c5f9a08a-43a9-4562-a8e6-2f1fd3b5624c"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
