# Implementer Brief — Add Go skill package (build, test, comments) / Create Go plugin files
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\golang-skills\_mill\plan\01-create-go-plugin-files.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\millhouse\wts\golang-skills\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\millhouse\wts\golang-skills`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\golang-skills\_mill\plan\01-create-go-plugin-files.md`.
   - Commit the plan edit first (`plan: extend Create Go plugin files refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"31e4d9b1-fe10-47bc-8cae-efb589227e96"}
```
...
**`session_id` MUST be exactly `31e4d9b1-fe10-47bc-8cae-efb589227e96` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"31e4d9b1-fe10-47bc-8cae-efb589227e96"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
