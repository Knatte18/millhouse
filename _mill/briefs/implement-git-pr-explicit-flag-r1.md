# Implementer Brief — Fix daemon health-check race, finalize env-var delivery, skills-index drop, and encoding crash / git-pr-explicit-flag
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-infra-and-tooling-fixes\_mill\plan\02-git-pr-explicit-flag.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-infra-and-tooling-fixes\_mill\plan\02-git-pr-explicit-flag.md`.
   - Commit the plan edit first (`plan: extend git-pr-explicit-flag refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"4d74d41a-9160-4877-8a94-d0e55cf16053"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"4d74d41a-9160-4877-8a94-d0e55cf16053"}
```
...
**`session_id` MUST be exactly `4d74d41a-9160-4877-8a94-d0e55cf16053` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"4d74d41a-9160-4877-8a94-d0e55cf16053"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
