# Implementer Brief — Add first-class Moves/Renames field to plan cards for rename-heavy batches / validator-move-checks
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-plan-rename-moves\_mill\plan\02-validator-move-checks.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-plan-rename-moves\_mill\plan\02-validator-move-checks.md`.
   - Commit the plan edit first (`plan: extend validator-move-checks refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"5b3aaf0b-7bc9-49ae-aed0-49d36fe0dd79"}
```
...
**`session_id` MUST be exactly `5b3aaf0b-7bc9-49ae-aed0-49d36fe0dd79` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"5b3aaf0b-7bc9-49ae-aed0-49d36fe0dd79"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
