# Implementer Brief — Fix nit-enforcement gate marker gaps, NIT-dispatch wording, implementer liveness probe, and Haiku false-completion / nits-only-envelope-threading
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-go-nit-gate-and-dispatch-gaps\_mill\plan\01-nits-only-envelope-threading.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-go-nit-gate-and-dispatch-gaps\_mill\plan\01-nits-only-envelope-threading.md`.
   - Commit the plan edit first (`plan: extend nits-only-envelope-threading refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"d03eb0c2-7b9a-4bc1-91a7-b6c15887d505"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"d03eb0c2-7b9a-4bc1-91a7-b6c15887d505"}
```
...
**`session_id` MUST be exactly `d03eb0c2-7b9a-4bc1-91a7-b6c15887d505` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"d03eb0c2-7b9a-4bc1-91a7-b6c15887d505"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
