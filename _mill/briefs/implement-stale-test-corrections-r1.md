# Implementer Brief — Fix pre-existing unit-test failures, CRLF cleanliness false-positive, and review false-BLOCKING on Go / stale-test-corrections
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-unit-test-and-signal-accuracy\_mill\plan\03-stale-test-corrections.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-unit-test-and-signal-accuracy\_mill\plan\03-stale-test-corrections.md`.
   - Commit the plan edit first (`plan: extend stale-test-corrections refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c40e7adb-b17a-4bcb-8e92-9ef12479fab4"}
```
...
**`session_id` MUST be exactly `c40e7adb-b17a-4bcb-8e92-9ef12479fab4` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c40e7adb-b17a-4bcb-8e92-9ef12479fab4"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
