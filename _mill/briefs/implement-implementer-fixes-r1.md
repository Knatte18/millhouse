# Implementer Brief — millpy-implement finalize/resume fixes and the red integration suites / implementer-fixes
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/millhouse/wts/implement-recovery-and-integration-tests/_mill/plan/01-implementer-fixes.md`
- **Overview (for `## Shared Decisions` only):** `/home/knatte/Code/millhouse/wts/implement-recovery-and-integration-tests/_mill/plan/00-overview.md`
- **Worktree cwd:** `/home/knatte/Code/millhouse/wts/implement-recovery-and-integration-tests`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/millhouse/wts/implement-recovery-and-integration-tests/_mill/plan/01-implementer-fixes.md`.
   - Commit the plan edit first (`plan: extend implementer-fixes refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"cb38b6c5-5b75-4d7d-b056-b08660ac0f74"}
   ```
...
**Pre-report self-check (mandatory before emitting success JSON):** Run `git -C /home/knatte/Code/millhouse/wts/implement-recovery-and-integration-tests status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"cb38b6c5-5b75-4d7d-b056-b08660ac0f74","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `cb38b6c5-5b75-4d7d-b056-b08660ac0f74` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"cb38b6c5-5b75-4d7d-b056-b08660ac0f74","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"cb38b6c5-5b75-4d7d-b056-b08660ac0f74"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Use `git -C /home/knatte/Code/millhouse/wts/implement-recovery-and-integration-tests` for commits;
do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `/home/knatte/Code/millhouse/wts/implement-recovery-and-integration-tests`.**
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/millhouse/wts/implement-recovery-and-integration-tests show <parent-branch>:<path>` to read it without changing cwd.
