# Implementer Brief — Fix nit-enforcement gate, Windows verify false-positive, reviewer oscillation, and scope-violation handling / windows-verify-gate
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-review-and-verify-quality\_mill\plan\01-windows-verify-gate.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-review-and-verify-quality\_mill\plan\01-windows-verify-gate.md`.
   - Commit the plan edit first (`plan: extend windows-verify-gate refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"41ca123c-0b3a-43ed-85e2-eb1551569c1e"}
```
...
**`session_id` MUST be exactly `41ca123c-0b3a-43ed-85e2-eb1551569c1e` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"41ca123c-0b3a-43ed-85e2-eb1551569c1e"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
