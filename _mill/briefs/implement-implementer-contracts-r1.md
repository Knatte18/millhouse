# Implementer Brief — Fix agent error recovery, implementer/review false-success contracts, VS Code watcher, and plan-validator Deletes / implementer-contracts
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-agent-and-implement-contracts\_mill\plan\05-implementer-contracts.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-agent-and-implement-contracts\_mill\plan\05-implementer-contracts.md`.
   - Commit the plan edit first (`plan: extend implementer-contracts refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"1fc4dd80-d3ea-4db7-aede-96d5fe5a03d6"}
```
...
**`session_id` MUST be exactly `1fc4dd80-d3ea-4db7-aede-96d5fe5a03d6` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"1fc4dd80-d3ea-4db7-aede-96d5fe5a03d6"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
