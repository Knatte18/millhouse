# Implementer Brief — Handle pre-closed and pre-merged PRs gracefully in mill-merge / cleanup-refactor
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-merge-pr-state-awareness\_mill\plan\02-cleanup-refactor.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-merge-pr-state-awareness\_mill\plan\02-cleanup-refactor.md`.
   - Commit the plan edit first (`plan: extend cleanup-refactor refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"be33ebed-dfb0-49fd-bad0-5f2da80bc275"}
```
...
**`session_id` MUST be exactly `be33ebed-dfb0-49fd-bad0-5f2da80bc275` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"be33ebed-dfb0-49fd-bad0-5f2da80bc275"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
