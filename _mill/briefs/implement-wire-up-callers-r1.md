# Implementer Brief — Fix codeguide-update scope resolution on stacked branches and resolve_scope.py branch-name parsing / wire-up-callers
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\codeguide-scope-resolution-fixes\_mill\plan\03-wire-up-callers.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\codeguide-scope-resolution-fixes\_mill\plan\03-wire-up-callers.md`.
   - Commit the plan edit first (`plan: extend wire-up-callers refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"be34051f-bb44-4a4a-9277-f0e3f0038460"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"be34051f-bb44-4a4a-9277-f0e3f0038460"}
```
...
**`session_id` MUST be exactly `be34051f-bb44-4a4a-9277-f0e3f0038460` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"be34051f-bb44-4a4a-9277-f0e3f0038460"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
