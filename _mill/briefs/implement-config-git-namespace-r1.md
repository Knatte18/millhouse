# Implementer Brief — Fix config unknown-key warning on git namespace and commit _mill/briefs/ after dispatch / config-git-namespace
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-config-and-brief-gaps\_mill\plan\01-config-git-namespace.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-config-and-brief-gaps\_mill\plan\01-config-git-namespace.md`.
   - Commit the plan edit first (`plan: extend config-git-namespace refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"deadb076-fc48-4696-a8f9-7e2b8497b737"}
```
...
**`session_id` MUST be exactly `deadb076-fc48-4696-a8f9-7e2b8497b737` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"deadb076-fc48-4696-a8f9-7e2b8497b737"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
