# Implementer Brief — Split mill-ghissues-to-tasks into source adapter + source-agnostic analysis / triage-to-tasks-skill
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-ghissues-source-adapter\_mill\plan\02-triage-to-tasks-skill.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-ghissues-source-adapter\_mill\plan\02-triage-to-tasks-skill.md`.
   - Commit the plan edit first (`plan: extend triage-to-tasks-skill refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"386e51b4-32a6-4a81-a6e7-a77331fbcfc2"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"386e51b4-32a6-4a81-a6e7-a77331fbcfc2"}
```
...
**`session_id` MUST be exactly `386e51b4-32a6-4a81-a6e7-a77331fbcfc2` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"386e51b4-32a6-4a81-a6e7-a77331fbcfc2"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
