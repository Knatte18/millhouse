# Implementer Brief — Fix agent error recovery, implementer/review false-success contracts, VS Code watcher, and plan-validator Deletes / vscode-watcher-exclude
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-agent-and-implement-contracts\_mill\plan\02-vscode-watcher-exclude.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-agent-and-implement-contracts\_mill\plan\02-vscode-watcher-exclude.md`.
   - Commit the plan edit first (`plan: extend vscode-watcher-exclude refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"f3cc4eb4-ec06-4610-bc1b-eff057f8992e"}
```
...
**`session_id` MUST be exactly `f3cc4eb4-ec06-4610-bc1b-eff057f8992e` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"f3cc4eb4-ec06-4610-bc1b-eff057f8992e"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
