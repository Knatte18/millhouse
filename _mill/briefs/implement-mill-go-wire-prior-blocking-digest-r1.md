# Implementer Brief — mill-go: quality-gate coverage gaps (NIT-fix regressions, missing lint gate) / mill-go-wire-prior-blocking-digest
...
- **Batch file (authoritative for this batch):** `/home/hanf/Code/millhouse/wts/mill-go-quality-gate-gaps/_mill/plan/03-mill-go-wire-prior-blocking-digest.md`
- **Overview (for `## Shared Decisions` only):** `/home/hanf/Code/millhouse/wts/mill-go-quality-gate-gaps/_mill/plan/00-overview.md`
- **Worktree cwd:** `/home/hanf/Code/millhouse/wts/mill-go-quality-gate-gaps`
- **Wiki path (for plan-edit commits if needed):** `/home/hanf/Code/millhouse/wiki`
- Round: **1**
...
Before implementing any cards, identify which cards are already committed: run `git -C /home/hanf/Code/millhouse/wts/mill-go-quality-gate-gaps log ..HEAD --oneline` and match each commit subject against the cards' `Commit:` messages.
When `` is empty, derive the range start via `git -C /home/hanf/Code/millhouse/wts/mill-go-quality-gate-gaps log --grep="^mill-go: start batch" -n 1 --format=%H`.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/hanf/Code/millhouse/wts/mill-go-quality-gate-gaps/_mill/plan/03-mill-go-wire-prior-blocking-digest.md`.
   - Commit the plan edit first (`plan: extend mill-go-wire-prior-blocking-digest refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"00d00f60-09c2-4e6e-b62c-24d617a8bfa9"}
   ```
...
- Before reporting any failure as "pre-existing" or "unrelated to my changes", confirm the failure reproduces on the parent branch `hanf/mill-merge-in-recompute-baseline-crash`:
  - Run `git log hanf/mill-merge-in-recompute-baseline-crash..HEAD -- <files in the failure's import/dependency chain>`.
    If a same-task commit touches those files, the failure is NOT pre-existing -- fix it.
  - Or run `git show hanf/mill-merge-in-recompute-baseline-crash:<path>` to inspect the parent's version of the failing file.
    If the failure does not exist on the parent, it is in-scope: fix it, or escalate `logic` -- never label it "pre-existing verify".
  - If `hanf/mill-merge-in-recompute-baseline-crash` is empty (the token renders as an empty string), skip the parent-reproduction check entirely and treat the failure as in-scope.
...
**Pre-report self-check (mandatory before emitting success JSON):** Run `git -C /home/hanf/Code/millhouse/wts/mill-go-quality-gate-gaps status --porcelain --untracked-files=no`.
...
Determine the range start exactly as in "Resume-after-incomplete" above: use `` when non-empty, else `git -C /home/hanf/Code/millhouse/wts/mill-go-quality-gate-gaps log --grep="^mill-go: start batch" -n 1 --format=%H`.
Run `git -C /home/hanf/Code/millhouse/wts/mill-go-quality-gate-gaps log <range-start>..HEAD --oneline` and match commit subjects against the batch file's `## Cards` `Commit:` messages to get an exact count -- Commit: none cards are never expected to appear in this log;
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"00d00f60-09c2-4e6e-b62c-24d617a8bfa9","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `00d00f60-09c2-4e6e-b62c-24d617a8bfa9` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"00d00f60-09c2-4e6e-b62c-24d617a8bfa9","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"00d00f60-09c2-4e6e-b62c-24d617a8bfa9"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
- `verify` — `verify:` still failing after 2 self-fix attempts.
  Before using this type, you MUST verify the failure is NOT pre-existing by checking `hanf/mill-merge-in-recompute-baseline-crash` (see `## Verify` above).
  Only use `verify` when you have confirmed the failure is not pre-existing OR when `hanf/mill-merge-in-recompute-baseline-crash` is empty.
...
Use `git -C /home/hanf/Code/millhouse/wts/mill-go-quality-gate-gaps` for commits;
do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `/home/hanf/Code/millhouse/wts/mill-go-quality-gate-gaps`.**
...
- **If you need a file from the parent:** use `git -C /home/hanf/Code/millhouse/wts/mill-go-quality-gate-gaps show <parent-branch>:<path>` to read it without changing cwd.
