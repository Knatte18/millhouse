# Implementer Brief — Split mill-ghissues-to-tasks into source adapter + source-agnostic analysis / skills-index-regen
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-ghissues-source-adapter\_mill\plan\05-skills-index-regen.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-ghissues-source-adapter\_mill\plan\05-skills-index-regen.md`.
   - Commit the plan edit first (`plan: extend skills-index-regen refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"4e9890d9-49c4-447a-93d0-ba6404996e62"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"4e9890d9-49c4-447a-93d0-ba6404996e62"}
```
...
**`session_id` MUST be exactly `4e9890d9-49c4-447a-93d0-ba6404996e62` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"4e9890d9-49c4-447a-93d0-ba6404996e62"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
