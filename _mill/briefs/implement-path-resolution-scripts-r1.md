# Implementer Brief — Fix nested mill layout paths, whole-repo formatter drift, and stacked-branch PR cleanup / path-resolution-scripts
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-external-repo-infra\_mill\plan\01-path-resolution-scripts.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-external-repo-infra\_mill\plan\01-path-resolution-scripts.md`.
   - Commit the plan edit first (`plan: extend path-resolution-scripts refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"927a2b4f-a90d-4d83-ad31-56fab2588e61"}
```
...
**`session_id` MUST be exactly `927a2b4f-a90d-4d83-ad31-56fab2588e61` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"927a2b4f-a90d-4d83-ad31-56fab2588e61"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
