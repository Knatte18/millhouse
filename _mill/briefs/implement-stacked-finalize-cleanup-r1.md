# Implementer Brief — Fix wiki push upstream, cleanliness gate, mojibake, container config, and stacked-branch finalize / stacked-finalize-cleanup
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-infra-and-path-fixes\_mill\plan\05-stacked-finalize-cleanup.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-infra-and-path-fixes\_mill\plan\05-stacked-finalize-cleanup.md`.
   - Commit the plan edit first (`plan: extend stacked-finalize-cleanup refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"746464df-77cb-4f0e-966d-b4ad020d8793"}
```
...
**`session_id` MUST be exactly `746464df-77cb-4f0e-966d-b4ad020d8793` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"746464df-77cb-4f0e-966d-b4ad020d8793"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
