# Implementer Brief — _plan_validate.py: path-reference heuristic false positives (round 3) + run() docstring drift / heuristic-gaps
...
- **Batch file (authoritative for this batch):** `/home/hanf/Code/millhouse/wts/mill-plan-validate-heuristic-gaps-3/_mill/plan/01-heuristic-gaps.md`
- **Overview (for `## Shared Decisions` only):** `/home/hanf/Code/millhouse/wts/mill-plan-validate-heuristic-gaps-3/_mill/plan/00-overview.md`
- **Worktree cwd:** `/home/hanf/Code/millhouse/wts/mill-plan-validate-heuristic-gaps-3`
...
Before implementing any cards, identify which cards are already committed: run `git -C /home/hanf/Code/millhouse/wts/mill-plan-validate-heuristic-gaps-3 log ..HEAD --oneline` and match each commit subject against the cards' `Commit:` messages.
When `` is empty, derive the range start via `git -C /home/hanf/Code/millhouse/wts/mill-plan-validate-heuristic-gaps-3 log --grep="^mill-go: start batch" -n 1 --format=%H`.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/hanf/Code/millhouse/wts/mill-plan-validate-heuristic-gaps-3/_mill/plan/01-heuristic-gaps.md`.
   - Commit the plan edit first (`plan: extend heuristic-gaps refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"b22b550f-0516-4933-b2b7-017f36e759a6"}
   ```
...
**Pre-report self-check (mandatory before emitting success JSON):** Run `git -C /home/hanf/Code/millhouse/wts/mill-plan-validate-heuristic-gaps-3 status --porcelain --untracked-files=no`.
...
Determine the range start exactly as in "Resume-after-incomplete" above: use `` when non-empty, else `git -C /home/hanf/Code/millhouse/wts/mill-plan-validate-heuristic-gaps-3 log --grep="^mill-go: start batch" -n 1 --format=%H`.
Run `git -C /home/hanf/Code/millhouse/wts/mill-plan-validate-heuristic-gaps-3 log <range-start>..HEAD --oneline` and match commit subjects against the batch file's `## Cards` `Commit:` messages to get an exact count -- Commit: none cards are never expected to appear in this log;
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"b22b550f-0516-4933-b2b7-017f36e759a6","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `b22b550f-0516-4933-b2b7-017f36e759a6` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"b22b550f-0516-4933-b2b7-017f36e759a6","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"b22b550f-0516-4933-b2b7-017f36e759a6"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Use `git -C /home/hanf/Code/millhouse/wts/mill-plan-validate-heuristic-gaps-3` for commits;
do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `/home/hanf/Code/millhouse/wts/mill-plan-validate-heuristic-gaps-3`.**
...
- **If you need a file from the parent:** use `git -C /home/hanf/Code/millhouse/wts/mill-plan-validate-heuristic-gaps-3 show <parent-branch>:<path>` to read it without changing cwd.
