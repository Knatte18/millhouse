# Implementer Brief — Fix mill-merge-in stale ref check, PowerShell 5.1 ConvertTo-Json, and _status str-path crash / worktree-asarray
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-script-fixes\_mill\plan\02-worktree-asarray.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-script-fixes\_mill\plan\02-worktree-asarray.md`.
   - Commit the plan edit first (`plan: extend worktree-asarray refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"a64acd16-f4e1-40a9-bc86-94b101d35ae4"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"a64acd16-f4e1-40a9-bc86-94b101d35ae4"}
```
...
**`session_id` MUST be exactly `a64acd16-f4e1-40a9-bc86-94b101d35ae4` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"a64acd16-f4e1-40a9-bc86-94b101d35ae4"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
