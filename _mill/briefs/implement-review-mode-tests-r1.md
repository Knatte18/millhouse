# Implementer Brief — Track _mill/briefs/ instead of gitignoring them / review-mode-tests
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\track-task-briefs\_mill\plan\03-review-mode-tests.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\track-task-briefs\_mill\plan\03-review-mode-tests.md`.
   - Commit the plan edit first (`plan: extend review-mode-tests refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"bdb9224c-8cdc-40a7-8b84-6e9d4930366a"}
```
...
**`session_id` MUST be exactly `bdb9224c-8cdc-40a7-8b84-6e9d4930366a` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"bdb9224c-8cdc-40a7-8b84-6e9d4930366a"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
