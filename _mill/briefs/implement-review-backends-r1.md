# Implementer Brief — Add first-class Moves/Renames field to plan cards for rename-heavy batches / review-backends
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-plan-rename-moves\_mill\plan\04-review-backends.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-plan-rename-moves\_mill\plan\04-review-backends.md`.
   - Commit the plan edit first (`plan: extend review-backends refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c9490a3d-749d-4af8-ae06-c5f94fc1f397"}
```
...
**`session_id` MUST be exactly `c9490a3d-749d-4af8-ae06-c5f94fc1f397` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c9490a3d-749d-4af8-ae06-c5f94fc1f397"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
