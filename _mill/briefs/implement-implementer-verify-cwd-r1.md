# Implementer Brief — Fix nested-hub-layout path resolution bugs across scope violations and review CLIs / implementer-verify-cwd
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Commit the plan edit first (`plan: extend implementer-verify-cwd refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"153539bb-43ab-4203-9265-d9c53e6f26d8"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"153539bb-43ab-4203-9265-d9c53e6f26d8"}
```
...
**`session_id` MUST be exactly `153539bb-43ab-4203-9265-d9c53e6f26d8` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"153539bb-43ab-4203-9265-d9c53e6f26d8"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
