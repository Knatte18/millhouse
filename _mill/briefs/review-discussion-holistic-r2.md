...
You are an independent discussion reviewer for **Fix mill-ghissues-to-tasks to refuse fold-ins into done and deferred tasks**. Round **2**. Reviewer model: **opushigh**.
...
Read the discussion at `C:\Code\millhouse\wts\ghissues-skill-fold-guard\_mill\discussion.md`. The discussion file is the authoritative scope. Read files referenced in `## Technical Context` to verify claims.
