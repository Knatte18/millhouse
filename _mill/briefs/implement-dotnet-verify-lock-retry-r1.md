# Implementer Brief — mill-go/millpy-implement: Windows dotnet build-server file-lock races in verify/baseline stages / dotnet-verify-lock-retry
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene/_mill/plan/01-dotnet-verify-lock-retry.md`
- **Overview (for `## Shared Decisions` only):** `/home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene/_mill/plan/00-overview.md`
- **Worktree cwd:** `/home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene`
...
**Resume-after-incomplete:** When `784d965bcd6cd65832d3c5944a6bbec322f9580e` is non-empty, you are being re-dispatched to finish a partially-completed batch.
Before implementing any cards, identify which cards are already committed: run `git -C /home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene log 784d965bcd6cd65832d3c5944a6bbec322f9580e..HEAD --oneline` and match each commit subject against the cards' `Commit:` messages.
When `784d965bcd6cd65832d3c5944a6bbec322f9580e` is empty, derive the range start via `git -C /home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene log --grep="^mill-go: start batch" -n 1 --format=%H`.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene/_mill/plan/01-dotnet-verify-lock-retry.md`.
   - Commit the plan edit first (`plan: extend dotnet-verify-lock-retry refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"4e041b01-ad5c-4a7d-9353-d5c99d731921"}
   ```
...
**Pre-report self-check (mandatory before emitting success JSON):** Run `git -C /home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene status --porcelain --untracked-files=no`.
...
Determine the range start exactly as in "Resume-after-incomplete" above: use `784d965bcd6cd65832d3c5944a6bbec322f9580e` when non-empty, else `git -C /home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene log --grep="^mill-go: start batch" -n 1 --format=%H`.
Run `git -C /home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene log <range-start>..HEAD --oneline` and match commit subjects against the batch file's `## Cards` `Commit:` messages to get an exact count -- Commit: none cards are never expected to appear in this log;
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"4e041b01-ad5c-4a7d-9353-d5c99d731921","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `4e041b01-ad5c-4a7d-9353-d5c99d731921` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
**On a `--resume-incomplete` re-dispatch specifically:** if you independently re-verify that every card's requirements are already satisfied by the existing commit(s) since `784d965bcd6cd65832d3c5944a6bbec322f9580e` and you make no new commit this turn, report `status: success` with `"already_complete": true` in the envelope, **in addition to** (not instead of) a `cards_done` array covering every card declared in this batch:

```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"4e041b01-ad5c-4a7d-9353-d5c99d731921","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"4e041b01-ad5c-4a7d-9353-d5c99d731921"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Use `git -C /home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene` for commits;
do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `/home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene`.**
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/millhouse/wts/mill-go-windows-buildserver-lock-hygiene show <parent-branch>:<path>` to read it without changing cwd.
