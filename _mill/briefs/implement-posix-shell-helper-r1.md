# Implementer Brief — Fix nested-junction teardown, Windows verify gate in merge-in, and review-plan --round threading / posix-shell-helper
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-junction-and-agent-gaps\_mill\plan\02-posix-shell-helper.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-junction-and-agent-gaps\_mill\plan\02-posix-shell-helper.md`.
   - Commit the plan edit first (`plan: extend posix-shell-helper refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"d965e1bd-7de4-4b57-94f3-5332c9c14db1"}
```
...
**`session_id` MUST be exactly `d965e1bd-7de4-4b57-94f3-5332c9c14db1` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"d965e1bd-7de4-4b57-94f3-5332c9c14db1"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
