# Implementer Brief — Fix mid-batch stop recovery and fixer false-success in agent-dispatch mode / fixer-brief-commit-guard
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-agent-dispatch-gaps\_mill\plan\03-fixer-brief-commit-guard.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-agent-dispatch-gaps\_mill\plan\03-fixer-brief-commit-guard.md`.
   - Commit the plan edit first (`plan: extend fixer-brief-commit-guard refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"09f22ace-ca48-45ff-93d6-3ba24b07b458"}
```
...
**`session_id` MUST be exactly `09f22ace-ca48-45ff-93d6-3ba24b07b458` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"09f22ace-ca48-45ff-93d6-3ba24b07b458"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
