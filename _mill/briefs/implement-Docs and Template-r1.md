# Implementer Brief — Fix implement finalize correctness: mid-batch stop recovery, commit-count guard, and empty-commit detection / Docs and Template
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-implement-finalize-gaps\_mill\plan\02-docs-and-template.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-implement-finalize-gaps\_mill\plan\02-docs-and-template.md`.
   - Commit the plan edit first (`plan: extend Docs and Template refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"69bce63f-c477-4c9c-8cd9-7e14e2b394d2"}
```
...
**`session_id` MUST be exactly `69bce63f-c477-4c9c-8cd9-7e14e2b394d2` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"69bce63f-c477-4c9c-8cd9-7e14e2b394d2"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
