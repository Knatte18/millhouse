# Implementer Brief — Handle pre-closed and pre-merged PRs gracefully in mill-merge / pr-state-helper
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-merge-pr-state-awareness\_mill\plan\01-pr-state-helper.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-merge-pr-state-awareness\_mill\plan\01-pr-state-helper.md`.
   - Commit the plan edit first (`plan: extend pr-state-helper refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"faaf6caf-c627-412b-be45-9e20d3999b93"}
```
...
**`session_id` MUST be exactly `faaf6caf-c627-412b-be45-9e20d3999b93` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"faaf6caf-c627-412b-be45-9e20d3999b93"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
