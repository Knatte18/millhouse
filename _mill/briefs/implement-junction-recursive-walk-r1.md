# Implementer Brief — Fix nested-junction teardown, Windows verify gate in merge-in, and review-plan --round threading / junction-recursive-walk
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-junction-and-agent-gaps\_mill\plan\01-junction-recursive-walk.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-junction-and-agent-gaps\_mill\plan\01-junction-recursive-walk.md`.
   - Commit the plan edit first (`plan: extend junction-recursive-walk refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"a13258d6-cbb3-432f-bb96-d27f00869cc5"}
```
...
**`session_id` MUST be exactly `a13258d6-cbb3-432f-bb96-d27f00869cc5` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"a13258d6-cbb3-432f-bb96-d27f00869cc5"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
