# Implementer Brief — Batch verify/baseline/completeness gates produce false positives or time out / finalize-timeout-guidance-generalization
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/millhouse/wts/mill-go-batch-verify-baseline-reliability/_mill/plan/05-finalize-timeout-guidance-generalization.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/millhouse/wts/mill-go-batch-verify-baseline-reliability/_mill/plan/05-finalize-timeout-guidance-generalization.md`.
   - Commit the plan edit first (`plan: extend finalize-timeout-guidance-generalization refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"636aecfc-416a-4ade-ae47-0f235275b94a"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"636aecfc-416a-4ade-ae47-0f235275b94a"}
```
...
**`session_id` MUST be exactly `636aecfc-416a-4ade-ae47-0f235275b94a` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"636aecfc-416a-4ade-ae47-0f235275b94a"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
