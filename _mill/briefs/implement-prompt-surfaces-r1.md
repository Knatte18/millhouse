# Implementer Brief — Explore and adopt Claude Code fork-agents in mill orchestration / prompt-surfaces
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\explore-fork-agent-opportunities\_mill\plan\03-prompt-surfaces.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\explore-fork-agent-opportunities\_mill\plan\03-prompt-surfaces.md`.
   - Commit the plan edit first (`plan: extend prompt-surfaces refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"971cadd3-eeee-4fbf-befa-84f67e3f2f72"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"971cadd3-eeee-4fbf-befa-84f67e3f2f72"}
```
...
**`session_id` MUST be exactly `971cadd3-eeee-4fbf-befa-84f67e3f2f72` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"971cadd3-eeee-4fbf-befa-84f67e3f2f72"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
