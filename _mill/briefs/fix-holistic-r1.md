# Holistic Fixer Brief — mill-plan SKILL.md: entry, phase-gate, finalize, and re-entry path gaps
...
- **Holistic review file:** `/home/hanf/Code/millhouse/wts/mill-plan-skilldoc-and-logic-bugs/_mill/reviews/20260813-092648-code-review-r1.md`
- **Plan overview:** `/home/hanf/Code/millhouse/wts/mill-plan-skilldoc-and-logic-bugs/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/hanf/Code/millhouse/wts/mill-plan-skilldoc-and-logic-bugs`
...
```
/home/hanf/Code/millhouse/wts/mill-plan-skilldoc-and-logic-bugs/_mill/plan/01-skill-doc-and-logic-fixes.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/hanf/Code/millhouse/wts/mill-plan-skilldoc-and-logic-bugs/_mill/reviews/20260813-092648-code-review-r1.md`.
...
Run each from `/home/hanf/Code/millhouse/wts/mill-plan-skilldoc-and-logic-bugs` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/hanf/Code/millhouse/wts/mill-plan-skilldoc-and-logic-bugs rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/hanf/Code/millhouse/wts/mill-plan-skilldoc-and-logic-bugs status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"e786c522-74d1-4b10-838b-ddbef4e5c48f"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `e786c522-74d1-4b10-838b-ddbef4e5c48f` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"e786c522-74d1-4b10-838b-ddbef4e5c48f"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
