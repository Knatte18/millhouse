# Holistic Fixer Brief — mill-plan: entry-gate wait for upstream mill-start misses discussion-gap-fix-r{N} and races the pinning commit
...
- **Holistic review file:** `/home/knatte/Code/millhouse/wts/mill-plan-entry-gate-wait-trigger-gaps/_mill/reviews/20260918-180746-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/millhouse/wts/mill-plan-entry-gate-wait-trigger-gaps/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/millhouse/wts/mill-plan-entry-gate-wait-trigger-gaps`
...
```
/home/knatte/Code/millhouse/wts/mill-plan-entry-gate-wait-trigger-gaps/_mill/plan/01-entry-gate-wait-fixes.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/millhouse/wts/mill-plan-entry-gate-wait-trigger-gaps/_mill/reviews/20260918-180746-code-review-r1.md`.
...
Run each from `/home/knatte/Code/millhouse/wts/mill-plan-entry-gate-wait-trigger-gaps` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/millhouse/wts/mill-plan-entry-gate-wait-trigger-gaps rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/millhouse/wts/mill-plan-entry-gate-wait-trigger-gaps status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c80a4136-042f-4706-b10d-497d4c6dccc6"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `c80a4136-042f-4706-b10d-497d4c6dccc6` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c80a4136-042f-4706-b10d-497d4c6dccc6"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
