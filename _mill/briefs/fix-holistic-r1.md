# Holistic Fixer Brief — Split mill-ghissues-to-tasks into source adapter + source-agnostic analysis
...
- **Holistic review file:** `C:\Code\millhouse\wts\mill-ghissues-source-adapter\_mill\reviews\20260701-041942-code-review-r1.md`
- **Plan overview:** `C:\Code\millhouse\wts\mill-ghissues-source-adapter\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\millhouse\wts\mill-ghissues-source-adapter`
...
```
C:\Code\millhouse\wts\mill-ghissues-source-adapter\_mill\plan\01-contract-adapters.md
C:\Code\millhouse\wts\mill-ghissues-source-adapter\_mill\plan\02-triage-to-tasks-skill.md
C:\Code\millhouse\wts\mill-ghissues-source-adapter\_mill\plan\03-report-to-tasks-skill.md
C:\Code\millhouse\wts\mill-ghissues-source-adapter\_mill\plan\04-ghissues-to-tasks-trim.md
C:\Code\millhouse\wts\mill-ghissues-source-adapter\_mill\plan\05-skills-index-regen.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\millhouse\wts\mill-ghissues-source-adapter\_mill\reviews\20260701-041942-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\millhouse\wts\mill-ghissues-source-adapter` via Bash. If a verify command fails:
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C C:\Code\millhouse\wts\mill-ghissues-source-adapter rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`). Before reporting `success`, confirm HEAD now differs from that recorded baseline; never report `success` when HEAD equals the baseline (no new commit was made). Run `git -C C:\Code\millhouse\wts\mill-ghissues-source-adapter status --porcelain --untracked-files=no`. If it shows ANY tracked modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"6dd3e074-1206-47c8-9b1c-867357169dc2"}
```
...
**`session_id` MUST be exactly `6dd3e074-1206-47c8-9b1c-867357169dc2` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"6dd3e074-1206-47c8-9b1c-867357169dc2"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\mill-ghissues-source-adapter` for commits; do not `cd`.
...
- **If you need a file from the parent:** use `git -C C:\Code\millhouse\wts\mill-ghissues-source-adapter show <parent-branch>:<path>` to read it without changing cwd.
