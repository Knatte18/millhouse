# Holistic Fixer Brief — Handle pre-closed and pre-merged PRs gracefully in mill-merge
...
- **Holistic review file:** `C:\Code\millhouse\wts\mill-merge-pr-state-awareness\_mill\reviews\20260630-154534-code-review-r1.md`
- **Plan overview:** `C:\Code\millhouse\wts\mill-merge-pr-state-awareness\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\millhouse\wts\mill-merge-pr-state-awareness`
...
```
C:\Code\millhouse\wts\mill-merge-pr-state-awareness\_mill\plan\01-pr-state-helper.md
C:\Code\millhouse\wts\mill-merge-pr-state-awareness\_mill\plan\02-cleanup-refactor.md
C:\Code\millhouse\wts\mill-merge-pr-state-awareness\_mill\plan\03-mill-merge-skill.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\millhouse\wts\mill-merge-pr-state-awareness\_mill\reviews\20260630-154534-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\millhouse\wts\mill-merge-pr-state-awareness` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"95d1ba0a-e336-4114-90c0-391afbc9ba65"}
```
...
**`session_id` MUST be exactly `95d1ba0a-e336-4114-90c0-391afbc9ba65` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"95d1ba0a-e336-4114-90c0-391afbc9ba65"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\mill-merge-pr-state-awareness` for commits; do not `cd`.
...
- **If you need a file from the parent:** use `git -C C:\Code\millhouse\wts\mill-merge-pr-state-awareness show <parent-branch>:<path>` to read it without changing cwd.
