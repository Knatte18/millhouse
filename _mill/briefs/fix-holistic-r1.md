# Holistic Fixer Brief — Fix drift-guard false positive and mill-start missing task body/brief
...
- **Holistic review file:** `C:\Code\millhouse\wts\mill-skill-and-tooling-gaps\_mill\reviews\20260630-045556-code-review-r1.md`
- **Plan overview:** `C:\Code\millhouse\wts\mill-skill-and-tooling-gaps\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\millhouse\wts\mill-skill-and-tooling-gaps`
...
```
C:\Code\millhouse\wts\mill-skill-and-tooling-gaps\_mill\plan\01-drift-guard-and-millstart-fix.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\millhouse\wts\mill-skill-and-tooling-gaps\_mill\reviews\20260630-045556-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\millhouse\wts\mill-skill-and-tooling-gaps` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"d473d1ed-0122-4e5f-9d4c-ab1036aa9c0b"}
```
...
**`session_id` MUST be exactly `d473d1ed-0122-4e5f-9d4c-ab1036aa9c0b` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"d473d1ed-0122-4e5f-9d4c-ab1036aa9c0b"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\mill-skill-and-tooling-gaps` for commits; do not `cd`.
