# Holistic Fixer Brief — compute_baseline: use the task worktree's own pre-edit state, not a parent-branch checkout
...
- **Holistic review file:** `/home/hanf/Code/millhouse/wts/baseline-uses-worktree-not-checkout/_mill/reviews/20260923-110349-code-review-r1.md`
- **Plan overview:** `/home/hanf/Code/millhouse/wts/baseline-uses-worktree-not-checkout/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/hanf/Code/millhouse/wts/baseline-uses-worktree-not-checkout`
...
```
/home/hanf/Code/millhouse/wts/baseline-uses-worktree-not-checkout/_mill/plan/01-status-baseline-fields.md
/home/hanf/Code/millhouse/wts/baseline-uses-worktree-not-checkout/_mill/plan/02-verify-baseline-core.md
/home/hanf/Code/millhouse/wts/baseline-uses-worktree-not-checkout/_mill/plan/03-implement-baseline-stage.md
/home/hanf/Code/millhouse/wts/baseline-uses-worktree-not-checkout/_mill/plan/04-implementer-gate-cleanup.md
/home/hanf/Code/millhouse/wts/baseline-uses-worktree-not-checkout/_mill/plan/05-merge-in-baseline-recompute.md
/home/hanf/Code/millhouse/wts/baseline-uses-worktree-not-checkout/_mill/plan/06-cleanup-orphan-removal.md
/home/hanf/Code/millhouse/wts/baseline-uses-worktree-not-checkout/_mill/plan/07-skill-docs-baseline.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/hanf/Code/millhouse/wts/baseline-uses-worktree-not-checkout/_mill/reviews/20260923-110349-code-review-r1.md`.
...
Run each from `/home/hanf/Code/millhouse/wts/baseline-uses-worktree-not-checkout` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/hanf/Code/millhouse/wts/baseline-uses-worktree-not-checkout rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/hanf/Code/millhouse/wts/baseline-uses-worktree-not-checkout status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"6a3adc3b-3ce3-451c-b9e6-a0a250769e9e"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `6a3adc3b-3ce3-451c-b9e6-a0a250769e9e` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"6a3adc3b-3ce3-451c-b9e6-a0a250769e9e"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
