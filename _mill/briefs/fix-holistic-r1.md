# Holistic Fixer Brief — Add first-class Moves/Renames field to plan cards for rename-heavy batches
...
- **Holistic review file:** `C:\Code\millhouse\wts\mill-plan-rename-moves\_mill\reviews\20260629-181743-code-review-r1.md`
- **Plan overview:** `C:\Code\millhouse\wts\mill-plan-rename-moves\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\millhouse\wts\mill-plan-rename-moves`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\millhouse\wts\mill-plan-rename-moves\_mill\reviews\20260629-181743-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\millhouse\wts\mill-plan-rename-moves` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"542e3b22-6fd0-416a-b3d8-3cc09e7cca6b"}
```
...
**`session_id` MUST be exactly `542e3b22-6fd0-416a-b3d8-3cc09e7cca6b` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"542e3b22-6fd0-416a-b3d8-3cc09e7cca6b"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\mill-plan-rename-moves` for commits; do not `cd`.
