# Holistic Fixer Brief — Track _mill/briefs/ instead of gitignoring them
...
- **Holistic review file:** `C:\Code\millhouse\wts\track-task-briefs\_mill\reviews\20260608-080302-code-review-r1.md`
- **Plan overview:** `C:\Code\millhouse\wts\track-task-briefs\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\millhouse\wts\track-task-briefs`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\millhouse\wts\track-task-briefs\_mill\reviews\20260608-080302-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\millhouse\wts\track-task-briefs` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"ca05ebf0-121a-4f7e-9460-1a2db16a9b7a"}
```
...
**`session_id` MUST be exactly `ca05ebf0-121a-4f7e-9460-1a2db16a9b7a` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"ca05ebf0-121a-4f7e-9460-1a2db16a9b7a"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
