# Holistic Fixer Brief — Fix discussion review round-cap, daemon cold-start, and nits-only no-op in finalize
...
- **Holistic review file:** `C:\Code\millhouse\wts\mill-review-and-finalize-gaps\_mill\reviews\20260701-043048-code-review-r1.md`
- **Plan overview:** `C:\Code\millhouse\wts\mill-review-and-finalize-gaps\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\millhouse\wts\mill-review-and-finalize-gaps`
...
```
C:\Code\millhouse\wts\mill-review-and-finalize-gaps\_mill\plan\01-discussion-review-round-cap-extension.md
C:\Code\millhouse\wts\mill-review-and-finalize-gaps\_mill\plan\02-wiki-cold-daemon-retry.md
C:\Code\millhouse\wts\mill-review-and-finalize-gaps\_mill\plan\03-nits-only-no-op-success.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\millhouse\wts\mill-review-and-finalize-gaps\_mill\reviews\20260701-043048-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\millhouse\wts\mill-review-and-finalize-gaps` via Bash. If a verify command fails:
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C C:\Code\millhouse\wts\mill-review-and-finalize-gaps rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`). Before reporting `success`, confirm HEAD now differs from that recorded baseline; never report `success` when HEAD equals the baseline (no new commit was made). Run `git -C C:\Code\millhouse\wts\mill-review-and-finalize-gaps status --porcelain --untracked-files=no`. If it shows ANY tracked modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"bf63bb69-3ac1-497b-9a81-8f1381528a44"}
```
...
**`session_id` MUST be exactly `bf63bb69-3ac1-497b-9a81-8f1381528a44` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"bf63bb69-3ac1-497b-9a81-8f1381528a44"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\mill-review-and-finalize-gaps` for commits; do not `cd`.
