# Holistic Fixer Brief — _verify_baseline.py transient worktrees can be orphaned when the task worktree is force-removed mid-computation
...
- **Holistic review file:** `/home/knatte/Code/millhouse/wts/verify-baseline-nested-worktree-orphan-risk/_mill/reviews/20260809-062338-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/millhouse/wts/verify-baseline-nested-worktree-orphan-risk/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/millhouse/wts/verify-baseline-nested-worktree-orphan-risk`
...
```
/home/knatte/Code/millhouse/wts/verify-baseline-nested-worktree-orphan-risk/_mill/plan/01-worktree-remove-safe-prune.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/millhouse/wts/verify-baseline-nested-worktree-orphan-risk/_mill/reviews/20260809-062338-code-review-r1.md`.
...
Run each from `/home/knatte/Code/millhouse/wts/verify-baseline-nested-worktree-orphan-risk` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/millhouse/wts/verify-baseline-nested-worktree-orphan-risk rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/millhouse/wts/verify-baseline-nested-worktree-orphan-risk status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"6171f11d-5824-4a89-9b14-989b5f776dba"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `6171f11d-5824-4a89-9b14-989b5f776dba` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"6171f11d-5824-4a89-9b14-989b5f776dba"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
