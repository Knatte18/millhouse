# Holistic Fixer Brief — mill-go2: fork-based dispatch reliability, shared-skill preloading, and catalog accuracy
...
- **Holistic review file:** `/home/knatte/Code/millhouse/wts/mill-go2-fork-dispatch-reliability/_mill/reviews/20260821-094342-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/millhouse/wts/mill-go2-fork-dispatch-reliability/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/millhouse/wts/mill-go2-fork-dispatch-reliability`
...
```
/home/knatte/Code/millhouse/wts/mill-go2-fork-dispatch-reliability/_mill/plan/01-fork-dispatch-reliability-fixes.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/millhouse/wts/mill-go2-fork-dispatch-reliability/_mill/reviews/20260821-094342-code-review-r1.md`.
...
Run each from `/home/knatte/Code/millhouse/wts/mill-go2-fork-dispatch-reliability` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/millhouse/wts/mill-go2-fork-dispatch-reliability rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made). Run `git -C /home/knatte/Code/millhouse/wts/mill-go2-fork-dispatch-reliability status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"26bc56a5-5235-49b4-abf6-23bcec7d91be"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `26bc56a5-5235-49b4-abf6-23bcec7d91be` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"26bc56a5-5235-49b4-abf6-23bcec7d91be"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
