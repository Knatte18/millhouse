# Holistic Fixer Brief — mill-go-base: remove subprocess/psmux dispatch branches
...
- **Holistic review file:** `/home/hanf/Code/millhouse/wts/mill-go-base-agent-dispatch-only/_mill/reviews/20260812-101501-code-review-r1.md`
- **Plan overview:** `/home/hanf/Code/millhouse/wts/mill-go-base-agent-dispatch-only/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/hanf/Code/millhouse/wts/mill-go-base-agent-dispatch-only`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/hanf/Code/millhouse/wts/mill-go-base-agent-dispatch-only/_mill/reviews/20260812-101501-code-review-r1.md`.
...
Run each from `/home/hanf/Code/millhouse/wts/mill-go-base-agent-dispatch-only` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/hanf/Code/millhouse/wts/mill-go-base-agent-dispatch-only rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made). Run `git -C /home/hanf/Code/millhouse/wts/mill-go-base-agent-dispatch-only status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"1c8b1a40-018d-4a3c-a576-3a4611477119"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `1c8b1a40-018d-4a3c-a576-3a4611477119` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"1c8b1a40-018d-4a3c-a576-3a4611477119"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
