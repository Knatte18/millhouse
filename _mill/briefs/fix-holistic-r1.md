# Holistic Fixer Brief — mill-plan autonomy guidance and validation gaps: fork scope violations, missing anti-pause guidance, no mechanical Context/Edits completeness check
...
- **Holistic review file:** `/home/knatte/Code/millhouse/wts/mill-plan-autonomy-and-validation-gaps/_mill/reviews/20260729-180549-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/millhouse/wts/mill-plan-autonomy-and-validation-gaps/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/millhouse/wts/mill-plan-autonomy-and-validation-gaps`
...
```
/home/knatte/Code/millhouse/wts/mill-plan-autonomy-and-validation-gaps/_mill/plan/01-context-completeness-check.md
/home/knatte/Code/millhouse/wts/mill-plan-autonomy-and-validation-gaps/_mill/plan/02-plan-guardrails-and-anti-pause.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/millhouse/wts/mill-plan-autonomy-and-validation-gaps/_mill/reviews/20260729-180549-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `/home/knatte/Code/millhouse/wts/mill-plan-autonomy-and-validation-gaps` via Bash. If a verify command fails:
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/millhouse/wts/mill-plan-autonomy-and-validation-gaps rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`). Before reporting `success`, confirm HEAD now differs from that recorded baseline; never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/millhouse/wts/mill-plan-autonomy-and-validation-gaps status --porcelain --untracked-files=no`. If it shows ANY tracked modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"6515154a-731d-40ef-9cc1-0d4eb0a94460"}
```
...
**`session_id` MUST be exactly `6515154a-731d-40ef-9cc1-0d4eb0a94460` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"6515154a-731d-40ef-9cc1-0d4eb0a94460"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C /home/knatte/Code/millhouse/wts/mill-plan-autonomy-and-validation-gaps` for commits; do not `cd`.
