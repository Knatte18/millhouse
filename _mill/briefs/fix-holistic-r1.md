# Holistic Fixer Brief — Extract a language-agnostic code-comments skill; add a general docstring-length ceiling, purpose-not-mechanism rule, mandatory file/module header, and prohibit measured-result/design-rationale narrative
...
- **Holistic review file:** `/home/knatte/Code/millhouse/wts/code-comments-skill-extraction/_mill/reviews/20260814-101016-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/millhouse/wts/code-comments-skill-extraction/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/millhouse/wts/code-comments-skill-extraction`
...
```
/home/knatte/Code/millhouse/wts/code-comments-skill-extraction/_mill/plan/01-shared-code-comments-skill.md
/home/knatte/Code/millhouse/wts/code-comments-skill-extraction/_mill/plan/02-workflow-routing-and-index.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/millhouse/wts/code-comments-skill-extraction/_mill/reviews/20260814-101016-code-review-r1.md`.
...
Run each from `/home/knatte/Code/millhouse/wts/code-comments-skill-extraction` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/millhouse/wts/code-comments-skill-extraction rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/millhouse/wts/code-comments-skill-extraction status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"6de9f241-0a67-4b27-b3d8-c11ca65d12ac"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `6de9f241-0a67-4b27-b3d8-c11ca65d12ac` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"6de9f241-0a67-4b27-b3d8-c11ca65d12ac"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
