# Holistic Fixer Brief — mill-finalize/mill-merge corrupt or mishandle _mill/status.md and task_dir on stacked branches
...
- **Holistic review file:** `/home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption/_mill/reviews/20260716-120842-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption`
...
```
/home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption/_mill/plan/01-parent-branch-identity.md
/home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption/_mill/plan/02-finalize-step3-restore.md
/home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption/_mill/plan/03-merge-slug-check-and-pathspec.md
/home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption/_mill/plan/04-integration-test-coverage.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption/_mill/reviews/20260716-120842-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `/home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption` via Bash. If a verify command fails:
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`). Before reporting `success`, confirm HEAD now differs from that recorded baseline; never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption status --porcelain --untracked-files=no`. If it shows ANY tracked modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"da700de5-064d-4ed8-b68e-a0d157262d79"}
```
...
**`session_id` MUST be exactly `da700de5-064d-4ed8-b68e-a0d157262d79` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"da700de5-064d-4ed8-b68e-a0d157262d79"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C /home/knatte/Code/millhouse/wts/mill-merge-stacked-branch-status-corruption` for commits; do not `cd`.
