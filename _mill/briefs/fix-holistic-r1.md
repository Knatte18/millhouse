# Holistic Fixer Brief — mill-merge: run the deterministic path as one script
...
- **Holistic review file:** `/home/knatte/Code/millhouse/wts/mill-merge-script/_mill/reviews/20260926-144314-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/millhouse/wts/mill-merge-script/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/millhouse/wts/mill-merge-script`
...
```
/home/knatte/Code/millhouse/wts/mill-merge-script/_mill/plan/01-merge-core-entry.md
/home/knatte/Code/millhouse/wts/mill-merge-script/_mill/plan/02-merge-core-squash.md
/home/knatte/Code/millhouse/wts/mill-merge-script/_mill/plan/03-merge-cli.md
/home/knatte/Code/millhouse/wts/mill-merge-script/_mill/plan/04-skill-rewrite.md
/home/knatte/Code/millhouse/wts/mill-merge-script/_mill/plan/05-cross-references.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/millhouse/wts/mill-merge-script/_mill/reviews/20260926-144314-code-review-r1.md`.
...
Run each from `/home/knatte/Code/millhouse/wts/mill-merge-script` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/millhouse/wts/mill-merge-script rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/millhouse/wts/mill-merge-script status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"9b46c8d8-7065-4e60-ba57-c056b9efdafe"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `9b46c8d8-7065-4e60-ba57-c056b9efdafe` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"9b46c8d8-7065-4e60-ba57-c056b9efdafe"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
