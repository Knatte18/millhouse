# Holistic Fixer Brief — Fix mill-ghissues-to-tasks to refuse fold-ins into done and deferred tasks
...
- **Holistic review file:** `C:\Code\millhouse\wts\ghissues-skill-fold-guard\_mill\reviews\20260615-130930-code-review-r1.md`
- **Plan overview:** `C:\Code\millhouse\wts\ghissues-skill-fold-guard\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\millhouse\wts\ghissues-skill-fold-guard`
...
```
C:\Code\millhouse\wts\ghissues-skill-fold-guard\_mill\plan\01-fold-guard-allowlist.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\millhouse\wts\ghissues-skill-fold-guard\_mill\reviews\20260615-130930-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\millhouse\wts\ghissues-skill-fold-guard` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"474db7c7-1c8b-4e95-88a8-a559970d940f"}
```
...
**`session_id` MUST be exactly `474db7c7-1c8b-4e95-88a8-a559970d940f` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"474db7c7-1c8b-4e95-88a8-a559970d940f"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\ghissues-skill-fold-guard` for commits; do not `cd`.
...
- **If you need a file from the parent:** use `git -C C:\Code\millhouse\wts\ghissues-skill-fold-guard show <parent-branch>:<path>` to read it without changing cwd.
