# Holistic Fixer Brief — _plan_validate.py: Batch Index/batch-file verify: drift, flattened-fence, and large-file-citation gaps
...
- **Holistic review file:** `/home/knatte/Code/millhouse/wts/plan-validate-batch-index-drift-and-misc-checks/_mill/reviews/20260904-103314-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/millhouse/wts/plan-validate-batch-index-drift-and-misc-checks/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/millhouse/wts/plan-validate-batch-index-drift-and-misc-checks`
...
```
/home/knatte/Code/millhouse/wts/plan-validate-batch-index-drift-and-misc-checks/_mill/plan/01-validator-checks.md
/home/knatte/Code/millhouse/wts/plan-validate-batch-index-drift-and-misc-checks/_mill/plan/02-validator-tests.md
/home/knatte/Code/millhouse/wts/plan-validate-batch-index-drift-and-misc-checks/_mill/plan/03-fix-table-docs.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/millhouse/wts/plan-validate-batch-index-drift-and-misc-checks/_mill/reviews/20260904-103314-code-review-r1.md`.
...
Run each from `/home/knatte/Code/millhouse/wts/plan-validate-batch-index-drift-and-misc-checks` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/millhouse/wts/plan-validate-batch-index-drift-and-misc-checks rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/millhouse/wts/plan-validate-batch-index-drift-and-misc-checks status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"70abb344-ca6d-4c8c-a5d7-d79a7e9c487e"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `70abb344-ca6d-4c8c-a5d7-d79a7e9c487e` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"70abb344-ca6d-4c8c-a5d7-d79a7e9c487e"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
