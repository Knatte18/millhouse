# Holistic Fixer Brief — mill-plan: Phase Plan Review's 4b/4c/4d re-validate gate says "7 kwargs", drops done_gate
...
- **Holistic review file:** `/home/knatte/Code/millhouse/wts/mill-plan-done-gate-kwargs-count-drift/_mill/reviews/20260919-113855-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/millhouse/wts/mill-plan-done-gate-kwargs-count-drift/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/millhouse/wts/mill-plan-done-gate-kwargs-count-drift`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/millhouse/wts/mill-plan-done-gate-kwargs-count-drift/_mill/reviews/20260919-113855-code-review-r1.md`.
...
Run each from `/home/knatte/Code/millhouse/wts/mill-plan-done-gate-kwargs-count-drift` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/millhouse/wts/mill-plan-done-gate-kwargs-count-drift rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/millhouse/wts/mill-plan-done-gate-kwargs-count-drift status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"8a0478d0-6ef7-4ded-b53b-e12312bd9e4a"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `8a0478d0-6ef7-4ded-b53b-e12312bd9e4a` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"8a0478d0-6ef7-4ded-b53b-e12312bd9e4a"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
