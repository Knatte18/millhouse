# Holistic Fixer Brief — Fix nested-junction teardown, Windows verify gate in merge-in, and review-plan --round threading
...
- **Holistic review file:** `C:\Code\millhouse\wts\mill-junction-and-agent-gaps\_mill\reviews\20260618-121722-code-review-r1.md`
- **Plan overview:** `C:\Code\millhouse\wts\mill-junction-and-agent-gaps\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\millhouse\wts\mill-junction-and-agent-gaps`
...
```
C:\Code\millhouse\wts\mill-junction-and-agent-gaps\_mill\plan\01-junction-recursive-walk.md
C:\Code\millhouse\wts\mill-junction-and-agent-gaps\_mill\plan\02-posix-shell-helper.md
C:\Code\millhouse\wts\mill-junction-and-agent-gaps\_mill\plan\03-review-round-autodiscovery.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\millhouse\wts\mill-junction-and-agent-gaps\_mill\reviews\20260618-121722-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\millhouse\wts\mill-junction-and-agent-gaps` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"fa8f3558-8b3b-4d0d-8acd-f6ce953bcbbd"}
```
...
**`session_id` MUST be exactly `fa8f3558-8b3b-4d0d-8acd-f6ce953bcbbd` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"fa8f3558-8b3b-4d0d-8acd-f6ce953bcbbd"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\mill-junction-and-agent-gaps` for commits; do not `cd`.
...
- **If you need a file from the parent:** use `git -C C:\Code\millhouse\wts\mill-junction-and-agent-gaps show <parent-branch>:<path>` to read it without changing cwd.
