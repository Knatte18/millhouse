# Holistic Fixer Brief — Fix stale checkpoint safety, Go binary ephemeral allowlist, and bare agent-tier name trap
...
- **Holistic review file:** `C:\Code\millhouse\wts\mill-scope-and-infra-gaps\_mill\reviews\20260629-172623-code-review-r1.md`
- **Plan overview:** `C:\Code\millhouse\wts\mill-scope-and-infra-gaps\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\millhouse\wts\mill-scope-and-infra-gaps`
...
```
C:\Code\millhouse\wts\mill-scope-and-infra-gaps\_mill\plan\01-bare-tier-aliases.md
C:\Code\millhouse\wts\mill-scope-and-infra-gaps\_mill\plan\02-go-artifact-allowlist.md
C:\Code\millhouse\wts\mill-scope-and-infra-gaps\_mill\plan\03-idempotent-checkpoint.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\millhouse\wts\mill-scope-and-infra-gaps\_mill\reviews\20260629-172623-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\millhouse\wts\mill-scope-and-infra-gaps` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"e2f00674-14ba-4039-be82-034ad29e28d9"}
```
...
**`session_id` MUST be exactly `e2f00674-14ba-4039-be82-034ad29e28d9` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"e2f00674-14ba-4039-be82-034ad29e28d9"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\mill-scope-and-infra-gaps` for commits; do not `cd`.
