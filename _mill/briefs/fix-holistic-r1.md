# Holistic Fixer Brief — Fix review finding-count parser, nested-layout brief path, verify cwd, process leaks, and missing done-gate
...
- **Holistic review file:** `C:\Code\millhouse\wts\mill-verify-and-layout-gaps\_mill\reviews\20260628-132317-code-review-r1.md`
- **Plan overview:** `C:\Code\millhouse\wts\mill-verify-and-layout-gaps\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\millhouse\wts\mill-verify-and-layout-gaps`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\millhouse\wts\mill-verify-and-layout-gaps\_mill\reviews\20260628-132317-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\millhouse\wts\mill-verify-and-layout-gaps` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"d4de6f66-6a32-40ec-b4f6-4f22c43d67b6"}
```
...
**`session_id` MUST be exactly `d4de6f66-6a32-40ec-b4f6-4f22c43d67b6` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"d4de6f66-6a32-40ec-b4f6-4f22c43d67b6"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\mill-verify-and-layout-gaps` for commits; do not `cd`.
