# Holistic Fixer Brief — mill-go-base: take the subagent report from the SubagentHandback message
...
- **Holistic review file:** `/home/knatte/Code/millhouse/wts/subagent-handback-report/_mill/reviews/20260926-092116-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/millhouse/wts/subagent-handback-report/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/millhouse/wts/subagent-handback-report`
...
```
/home/knatte/Code/millhouse/wts/subagent-handback-report/_mill/plan/01-agent-dispatch-core.md
/home/knatte/Code/millhouse/wts/subagent-handback-report/_mill/plan/02-sibling-skill-wording.md
/home/knatte/Code/millhouse/wts/subagent-handback-report/_mill/plan/03-script-wording.md
/home/knatte/Code/millhouse/wts/subagent-handback-report/_mill/plan/04-test-comments.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/millhouse/wts/subagent-handback-report/_mill/reviews/20260926-092116-code-review-r1.md`.
...
Run each from `/home/knatte/Code/millhouse/wts/subagent-handback-report` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/millhouse/wts/subagent-handback-report rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/millhouse/wts/subagent-handback-report status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"78640615-39b0-4aa1-ba08-d0db30701e28"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `78640615-39b0-4aa1-ba08-d0db30701e28` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"78640615-39b0-4aa1-ba08-d0db30701e28"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
