# Holistic Fixer Brief — Auto-name task sessions <slug>:<phase>
...
- **Holistic review file:** `/home/knatte/Code/millhouse/wts/session-naming/_mill/reviews/20260924-062507-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/millhouse/wts/session-naming/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/millhouse/wts/session-naming`
...
```
/home/knatte/Code/millhouse/wts/session-naming/_mill/plan/01-tasks-render.md
/home/knatte/Code/millhouse/wts/session-naming/_mill/plan/02-keybindings.md
/home/knatte/Code/millhouse/wts/session-naming/_mill/plan/03-session-tasks-cli.md
/home/knatte/Code/millhouse/wts/session-naming/_mill/plan/04-spawn-wiring.md
/home/knatte/Code/millhouse/wts/session-naming/_mill/plan/05-setup-docs.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/millhouse/wts/session-naming/_mill/reviews/20260924-062507-code-review-r1.md`.
...
Run each from `/home/knatte/Code/millhouse/wts/session-naming` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/millhouse/wts/session-naming rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/millhouse/wts/session-naming status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"6ae24113-a341-424d-b545-b27643b2a17f"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `6ae24113-a341-424d-b545-b27643b2a17f` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"6ae24113-a341-424d-b545-b27643b2a17f"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
