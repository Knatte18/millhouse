# Holistic Fixer Brief — Fix spawn lifecycle integrity, agent-mode async assumption, merge-in conflicts, and pre-existing failure validation
...
- **Holistic review file:** `C:\Code\millhouse\wts\mill-spawn-and-workflow-integrity\_mill\reviews\20260625-082332-code-review-r1.md`
- **Plan overview:** `C:\Code\millhouse\wts\mill-spawn-and-workflow-integrity\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\millhouse\wts\mill-spawn-and-workflow-integrity`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\millhouse\wts\mill-spawn-and-workflow-integrity\_mill\reviews\20260625-082332-code-review-r1.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\millhouse\wts\mill-spawn-and-workflow-integrity` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"760f67f2-1e5a-4e3f-836e-b5b0a71b85c7"}
```
...
**`session_id` MUST be exactly `760f67f2-1e5a-4e3f-836e-b5b0a71b85c7` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"760f67f2-1e5a-4e3f-836e-b5b0a71b85c7"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\mill-spawn-and-workflow-integrity` for commits; do not `cd`.
