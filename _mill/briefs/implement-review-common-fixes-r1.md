# Implementer Brief — Fix unit test suite failures, spurious review warning, and implementer verify-gate gaps / review-common-fixes
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-test-and-implementer-reliability\_mill\plan\01-review-common-fixes.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-test-and-implementer-reliability\_mill\plan\01-review-common-fixes.md`.
   - Commit the plan edit first (`plan: extend review-common-fixes refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"a9bb157c-c812-462d-b9c3-1de367ba6771"}
```
...
**`session_id` MUST be exactly `a9bb157c-c812-462d-b9c3-1de367ba6771` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"a9bb157c-c812-462d-b9c3-1de367ba6771"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
