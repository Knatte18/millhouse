# Implementer Brief — Fix agent error recovery, implementer/review false-success contracts, VS Code watcher, and plan-validator Deletes / review-discussion-nitcount
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-agent-and-implement-contracts\_mill\plan\03-review-discussion-nitcount.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\millhouse\wts\mill-agent-and-implement-contracts\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\millhouse\wts\mill-agent-and-implement-contracts`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-agent-and-implement-contracts\_mill\plan\03-review-discussion-nitcount.md`.
   - Commit the plan edit first (`plan: extend review-discussion-nitcount refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"0dce4179-6ac9-4c0e-9b4b-b46e7ce90110"}
```
...
**`session_id` MUST be exactly `0dce4179-6ac9-4c0e-9b4b-b46e7ce90110` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"0dce4179-6ac9-4c0e-9b4b-b46e7ce90110"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\mill-agent-and-implement-contracts` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\millhouse\wts\mill-agent-and-implement-contracts`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C C:\Code\millhouse\wts\mill-agent-and-implement-contracts show <parent-branch>:<path>` to read it without changing cwd.
