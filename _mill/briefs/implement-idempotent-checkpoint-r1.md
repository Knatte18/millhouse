# Implementer Brief — Fix stale checkpoint safety, Go binary ephemeral allowlist, and bare agent-tier name trap / idempotent-checkpoint
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-scope-and-infra-gaps\_mill\plan\03-idempotent-checkpoint.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-scope-and-infra-gaps\_mill\plan\03-idempotent-checkpoint.md`.
   - Commit the plan edit first (`plan: extend idempotent-checkpoint refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"807706dc-1b57-4757-8d76-51c7f96d1f52"}
```
...
**`session_id` MUST be exactly `807706dc-1b57-4757-8d76-51c7f96d1f52` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"807706dc-1b57-4757-8d76-51c7f96d1f52"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
