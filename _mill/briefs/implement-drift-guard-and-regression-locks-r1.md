# Implementer Brief — Fix nested-hub path resolution and SKILL.md vs shipped-API mismatches / drift-guard-and-regression-locks
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-nested-hub-and-skill-sync\_mill\plan\01-drift-guard-and-regression-locks.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\millhouse\wts\mill-nested-hub-and-skill-sync\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\millhouse\wts\mill-nested-hub-and-skill-sync`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-nested-hub-and-skill-sync\_mill\plan\01-drift-guard-and-regression-locks.md`.
   - Commit the plan edit first (`plan: extend drift-guard-and-regression-locks refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"b825910f-d782-42ed-acf9-7626ac6c1642"}
```
...
**`session_id` MUST be exactly `b825910f-d782-42ed-acf9-7626ac6c1642` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"b825910f-d782-42ed-acf9-7626ac6c1642"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\mill-nested-hub-and-skill-sync` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\millhouse\wts\mill-nested-hub-and-skill-sync`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
