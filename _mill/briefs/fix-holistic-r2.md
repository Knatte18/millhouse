# Holistic Fixer Brief — mill-merge/merge-in: squash non-ff rejection, stale-worktree logic, parent-branch detection, conflict resolution
...
- **Holistic review file:** `/home/hanf/Code/millhouse/wts/mill-merge-and-merge-in-bugs/_mill/reviews/20260813-063617-code-review-r2.md`
- **Plan overview:** `/home/hanf/Code/millhouse/wts/mill-merge-and-merge-in-bugs/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/hanf/Code/millhouse/wts/mill-merge-and-merge-in-bugs`
- **Wiki path:** `/home/hanf/Code/millhouse/wiki`
- Round: **2**

Batch plan files (for `verify:` commands):

```
/home/hanf/Code/millhouse/wts/mill-merge-and-merge-in-bugs/_mill/plan/01-parent-liveness-module.md
/home/hanf/Code/millhouse/wts/mill-merge-and-merge-in-bugs/_mill/plan/02-mill-merge-skill-fixes.md
/home/hanf/Code/millhouse/wts/mill-merge-and-merge-in-bugs/_mill/plan/03-mill-merge-in-and-conflict-brief.md
/home/hanf/Code/millhouse/wts/mill-merge-and-merge-in-bugs/_mill/plan/04-integration-tests.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/hanf/Code/millhouse/wts/mill-merge-and-merge-in-bugs/_mill/reviews/20260813-063617-code-review-r2.md`.
...
- mill-merge-in's rebind reuses a status_path derivation batch 2 explicitly calls an anti-pattern: **Location:** `plugins/mill/skills/mill-merge-in/SKILL.md:17` vs `plugins/mill/skills/mill-merge/SKILL.md:111`
...
Run each from `/home/hanf/Code/millhouse/wts/mill-merge-and-merge-in-bugs` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/hanf/Code/millhouse/wts/mill-merge-and-merge-in-bugs rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/hanf/Code/millhouse/wts/mill-merge-and-merge-in-bugs status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"de17f9f3-03cf-4154-8fd8-0026ce8140ba"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `de17f9f3-03cf-4154-8fd8-0026ce8140ba` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"de17f9f3-03cf-4154-8fd8-0026ce8140ba"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
