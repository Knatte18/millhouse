# Holistic Fixer Brief — mill-plan: review-round cap and skip-check threading bugs
...
- **Holistic review file:** `/home/knatte/Code/millhouse/wts/mill-plan-review-round-cap-and-skip-check-threading/_mill/reviews/20260904-090103-code-review-r2.md`
- **Plan overview:** `/home/knatte/Code/millhouse/wts/mill-plan-review-round-cap-and-skip-check-threading/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/millhouse/wts/mill-plan-review-round-cap-and-skip-check-threading`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/millhouse/wts/mill-plan-review-round-cap-and-skip-check-threading/_mill/reviews/20260904-090103-code-review-r2.md`.
...
- `phase: blocked` row's `--approve`/`--revise` text is dead prose: **Location:** `plugins/mill/skills/mill-plan/SKILL.md:78` vs `:117-138` ("### Entry: resuming after a max-rounds block")
- Live round-cap override not wired at any concrete dispatch site: **Location:** `plugins/mill/skills/mill-plan/SKILL.md:316` vs dispatch sites `:417-421`, `:457-475` (esp. `:461`), `:494-495`, `:501-514` (esp. `:505`)
...
Run each from `/home/knatte/Code/millhouse/wts/mill-plan-review-round-cap-and-skip-check-threading` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/millhouse/wts/mill-plan-review-round-cap-and-skip-check-threading rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/millhouse/wts/mill-plan-review-round-cap-and-skip-check-threading status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"5c69da32-dc53-4ae8-9b0c-a6c56124ce54"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `5c69da32-dc53-4ae8-9b0c-a6c56124ce54` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"5c69da32-dc53-4ae8-9b0c-a6c56124ce54"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
