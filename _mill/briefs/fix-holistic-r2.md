# Holistic Fixer Brief — _plan_validate context-completeness: further false-positive/false-negative gaps, round 3
...
- **Holistic review file:** `/home/hanf/Code/millhouse/wts/plan-validate-context-completeness-round3-gaps/_mill/reviews/20260923-131409-code-review-r2.md`
- **Plan overview:** `/home/hanf/Code/millhouse/wts/plan-validate-context-completeness-round3-gaps/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/hanf/Code/millhouse/wts/plan-validate-context-completeness-round3-gaps`
...
```
/home/hanf/Code/millhouse/wts/plan-validate-context-completeness-round3-gaps/_mill/plan/01-context-completeness-resolution-and-tokenization-rework.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/hanf/Code/millhouse/wts/plan-validate-context-completeness-round3-gaps/_mill/reviews/20260923-131409-code-review-r2.md`.
...
- Two symbol-branch exemption tests not migrated per card 5's own audit criterion: **Location:** `plugins/mill/unit_tests/test-plan-validate.py:4431-4466` (`test_check_context_completeness_symbol_prohibition_marker_exempt`) and `:4469-4504` (`test_check_context_completeness_symbol_citation_marker_exempt`)
...
Run each from `/home/hanf/Code/millhouse/wts/plan-validate-context-completeness-round3-gaps` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/hanf/Code/millhouse/wts/plan-validate-context-completeness-round3-gaps rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/hanf/Code/millhouse/wts/plan-validate-context-completeness-round3-gaps status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"7851ee03-e714-462a-a36a-21c4cd5e161e"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `7851ee03-e714-462a-a36a-21c4cd5e161e` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"7851ee03-e714-462a-a36a-21c4cd5e161e"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
