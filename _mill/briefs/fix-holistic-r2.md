# Holistic Fixer Brief — Fix millpy-review-plan validator gaps and resolve_ref_paths path-doubling
...
- **Holistic review file:** `C:\Code\millhouse\wts\review-plan-and-ref-paths\_mill\reviews\20260615-121959-code-review-r2.md`
- **Plan overview:** `C:\Code\millhouse\wts\review-plan-and-ref-paths\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\millhouse\wts\review-plan-and-ref-paths`
...
```
C:\Code\millhouse\wts\review-plan-and-ref-paths\_mill\plan\01-ref-path-resolution.md
C:\Code\millhouse\wts\review-plan-and-ref-paths\_mill\plan\02-validator-git-root-threading.md
C:\Code\millhouse\wts\review-plan-and-ref-paths\_mill\plan\03-review-plan-cli.md
C:\Code\millhouse\wts\review-plan-and-ref-paths\_mill\plan\04-skill-doc-update.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `C:\Code\millhouse\wts\review-plan-and-ref-paths\_mill\reviews\20260615-121959-code-review-r2.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\millhouse\wts\review-plan-and-ref-paths` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"35412aa7-78ba-4e6b-b074-a4be9fc486ec"}
```
...
**`session_id` MUST be exactly `35412aa7-78ba-4e6b-b074-a4be9fc486ec` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"35412aa7-78ba-4e6b-b074-a4be9fc486ec"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\review-plan-and-ref-paths` for commits; do not `cd`.
...
- **If you need a file from the parent:** use `git -C C:\Code\millhouse\wts\review-plan-and-ref-paths show <parent-branch>:<path>` to read it without changing cwd.
