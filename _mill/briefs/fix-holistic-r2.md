# Holistic Fixer Brief — mill-go's one-shot pre-batch-1 baseline can't cover a task's own later per-batch-baseline capability
...
- **Holistic review file:** `/home/knatte/Code/millhouse/wts/mill-go-per-batch-baseline-preflight-gap/_mill/reviews/20260805-184859-code-review-r2.md`
- **Plan overview:** `/home/knatte/Code/millhouse/wts/mill-go-per-batch-baseline-preflight-gap/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/millhouse/wts/mill-go-per-batch-baseline-preflight-gap`
...
```
/home/knatte/Code/millhouse/wts/mill-go-per-batch-baseline-preflight-gap/_mill/plan/01-self-hosting-detection-helper.md
/home/knatte/Code/millhouse/wts/mill-go-per-batch-baseline-preflight-gap/_mill/plan/02-per-batch-baseline-recapture-hook.md
```

## Before reading any finding

Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/millhouse/wts/mill-go-per-batch-baseline-preflight-gap/_mill/reviews/20260805-184859-code-review-r2.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `/home/knatte/Code/millhouse/wts/mill-go-per-batch-baseline-preflight-gap` via Bash. If a verify command fails:
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/millhouse/wts/mill-go-per-batch-baseline-preflight-gap rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`). Before reporting `success`, confirm HEAD now differs from that recorded baseline; never report `success` when HEAD equals the baseline (no new commit was made). Run `git -C /home/knatte/Code/millhouse/wts/mill-go-per-batch-baseline-preflight-gap status --porcelain --untracked-files=no`. If it shows ANY tracked modification, commit it via the `git-commit` skill (or report `stuck_type: logic`) -- never report `success` with an uncommitted tracked change.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"f3e7ca95-fd87-468e-baf8-d6b0cd26b91c"}
```
...
**`session_id` MUST be exactly `f3e7ca95-fd87-468e-baf8-d6b0cd26b91c` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"f3e7ca95-fd87-468e-baf8-d6b0cd26b91c"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C /home/knatte/Code/millhouse/wts/mill-go-per-batch-baseline-preflight-gap` for commits; do not `cd`.
