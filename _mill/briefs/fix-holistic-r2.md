# Holistic Fixer Brief — Fix nit-enforcement gate, Windows verify false-positive, reviewer oscillation, and scope-violation handling
...
- **Holistic review file:** `C:\Code\millhouse\wts\mill-review-and-verify-quality\_mill\reviews\20260623-095550-code-review-r2.md`
- **Plan overview:** `C:\Code\millhouse\wts\mill-review-and-verify-quality\_mill\plan\00-overview.md`
- **Worktree cwd (use for git and verify):** `C:\Code\millhouse\wts\mill-review-and-verify-quality`
...
Load the **mill-receiving-review** skill before reading any finding in `C:\Code\millhouse\wts\mill-review-and-verify-quality\_mill\reviews\20260623-095550-code-review-r2.md`. This is non-negotiable.
...
After all fixes are committed, run every non-null `verify:` command from every batch plan file listed above, in the order listed. Run each from `C:\Code\millhouse\wts\mill-review-and-verify-quality` via Bash. If a verify command fails:
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c269120b-8e1a-48c0-b6fe-4a281baec8e2"}
```
...
**`session_id` MUST be exactly `c269120b-8e1a-48c0-b6fe-4a281baec8e2` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c269120b-8e1a-48c0-b6fe-4a281baec8e2"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\mill-review-and-verify-quality` for commits; do not `cd`.
