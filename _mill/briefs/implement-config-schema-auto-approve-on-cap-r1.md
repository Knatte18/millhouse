# Implementer Brief — Auto-approve on review-round cap / config-schema-auto-approve-on-cap
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/millhouse/wts/review-cap-auto-approve/_mill/plan/01-config-schema-auto-approve-on-cap.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/millhouse/wts/review-cap-auto-approve/_mill/plan/01-config-schema-auto-approve-on-cap.md`.
   - Commit the plan edit first (`plan: extend config-schema-auto-approve-on-cap refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"c5df5282-9478-45ea-9c65-6d992a3dcbf6"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"c5df5282-9478-45ea-9c65-6d992a3dcbf6","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `c5df5282-9478-45ea-9c65-6d992a3dcbf6` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"c5df5282-9478-45ea-9c65-6d992a3dcbf6","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"c5df5282-9478-45ea-9c65-6d992a3dcbf6"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
