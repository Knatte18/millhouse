# Implementer Brief — Fix agent-dispatch prepare stage to emit namespaced subagent_type / Namespace constants
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\agent-dispatch-namespace-fix\_mill\plan\01-namespace-constants.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\millhouse\wts\agent-dispatch-namespace-fix\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\millhouse\wts\agent-dispatch-namespace-fix`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\agent-dispatch-namespace-fix\_mill\plan\01-namespace-constants.md`.
   - Commit the plan edit first (`plan: extend Namespace constants refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"3d9471bc-b910-4c6c-80de-9f2921ac24b0"}
```
...
**`session_id` MUST be exactly `3d9471bc-b910-4c6c-80de-9f2921ac24b0` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"3d9471bc-b910-4c6c-80de-9f2921ac24b0"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\agent-dispatch-namespace-fix` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\millhouse\wts\agent-dispatch-namespace-fix`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C C:\Code\millhouse\wts\agent-dispatch-namespace-fix show <parent-branch>:<path>` to read it without changing cwd.
