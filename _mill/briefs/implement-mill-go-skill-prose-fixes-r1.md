# Implementer Brief — Fix nit-enforcement gate marker gaps, NIT-dispatch wording, implementer liveness probe, and Haiku false-completion / mill-go-skill-prose-fixes
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-go-nit-gate-and-dispatch-gaps\_mill\plan\02-mill-go-skill-prose-fixes.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-go-nit-gate-and-dispatch-gaps\_mill\plan\02-mill-go-skill-prose-fixes.md`.
   - Commit the plan edit first (`plan: extend mill-go-skill-prose-fixes refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"8c8ff1bb-d3d4-47b2-83ac-ff288e8f993a"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"8c8ff1bb-d3d4-47b2-83ac-ff288e8f993a"}
```
...
**`session_id` MUST be exactly `8c8ff1bb-d3d4-47b2-83ac-ff288e8f993a` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"8c8ff1bb-d3d4-47b2-83ac-ff288e8f993a"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
