# Implementer Brief — Fix mill-merge-in stale ref check, PowerShell 5.1 ConvertTo-Json, and _status str-path crash / status-path-typeerror
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-script-fixes\_mill\plan\03-status-path-typeerror.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-script-fixes\_mill\plan\03-status-path-typeerror.md`.
   - Commit the plan edit first (`plan: extend status-path-typeerror refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"1b335ea0-ae9a-44eb-910e-50efca3e292e"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"1b335ea0-ae9a-44eb-910e-50efca3e292e"}
```
...
**`session_id` MUST be exactly `1b335ea0-ae9a-44eb-910e-50efca3e292e` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"1b335ea0-ae9a-44eb-910e-50efca3e292e"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
