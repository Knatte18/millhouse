# Implementer Brief — Fix pre-existing unit-test failures, CRLF cleanliness false-positive, and review false-BLOCKING on Go / review-dispatch-fixes
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-unit-test-and-signal-accuracy\_mill\plan\01-review-dispatch-fixes.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-unit-test-and-signal-accuracy\_mill\plan\01-review-dispatch-fixes.md`.
   - Commit the plan edit first (`plan: extend review-dispatch-fixes refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"d1b58a43-d5e2-4ffb-8e81-d880ec512306"}
```
...
**`session_id` MUST be exactly `d1b58a43-d5e2-4ffb-8e81-d880ec512306` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"d1b58a43-d5e2-4ffb-8e81-d880ec512306"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
