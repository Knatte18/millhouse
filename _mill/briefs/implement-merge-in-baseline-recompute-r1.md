# Implementer Brief — Fix agent-mode dispatch races and pipeline gaps / merge-in-baseline-recompute
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\agent-mode-gaps\_mill\plan\05-merge-in-baseline-recompute.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\agent-mode-gaps\_mill\plan\05-merge-in-baseline-recompute.md`.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"f369999a-bd87-451b-b51a-96e25b1e29d0"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"f369999a-bd87-451b-b51a-96e25b1e29d0"}
```
...
**`session_id` MUST be exactly `f369999a-bd87-451b-b51a-96e25b1e29d0` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"f369999a-bd87-451b-b51a-96e25b1e29d0"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
