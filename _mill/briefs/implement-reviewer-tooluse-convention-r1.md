# Implementer Brief — Track _mill/briefs/ instead of gitignoring them / reviewer-tooluse-convention
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\track-task-briefs\_mill\plan\01-reviewer-tooluse-convention.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\millhouse\wts\track-task-briefs\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\millhouse\wts\track-task-briefs`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\track-task-briefs\_mill\plan\01-reviewer-tooluse-convention.md`.
   - Commit the plan edit first (`plan: extend reviewer-tooluse-convention refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"d89cf7a5-311d-445a-97aa-a09e26ac9dfc"}
```
...
**`session_id` MUST be exactly `d89cf7a5-311d-445a-97aa-a09e26ac9dfc` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"d89cf7a5-311d-445a-97aa-a09e26ac9dfc"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C C:\Code\millhouse\wts\track-task-briefs` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `C:\Code\millhouse\wts\track-task-briefs`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C C:\Code\millhouse\wts\track-task-briefs show <parent-branch>:<path>` to read it without changing cwd.
