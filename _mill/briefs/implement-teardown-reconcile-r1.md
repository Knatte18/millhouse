# Implementer Brief — Fix spawn lifecycle integrity, agent-mode async assumption, merge-in conflicts, and pre-existing failure validation / teardown-reconcile
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-spawn-and-workflow-integrity\_mill\plan\02-teardown-reconcile.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-spawn-and-workflow-integrity\_mill\plan\02-teardown-reconcile.md`.
   - Commit the plan edit first (`plan: extend teardown-reconcile refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"32f923b7-c5e8-45c4-ad8a-b077c1026edc"}
```
...
**`session_id` MUST be exactly `32f923b7-c5e8-45c4-ad8a-b077c1026edc` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"32f923b7-c5e8-45c4-ad8a-b077c1026edc"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
