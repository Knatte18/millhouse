# Implementer Brief — Add mill-quick: skip-review pipeline for simple tasks / mill-quick-skill
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/millhouse/wts/mill-quick/_mill/plan/01-mill-quick-skill.md`
- **Overview (for `## Shared Decisions` only):** `/home/knatte/Code/millhouse/wts/mill-quick/_mill/plan/00-overview.md`
- **Worktree cwd:** `/home/knatte/Code/millhouse/wts/mill-quick`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/millhouse/wts/mill-quick/_mill/plan/01-mill-quick-skill.md`.
   - Commit the plan edit first (`plan: extend mill-quick-skill refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"ab5e5b87-18a6-41b6-b027-17e06ecb133e"}
   ```
...
**`session_id` MUST be exactly `ab5e5b87-18a6-41b6-b027-17e06ecb133e` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"ab5e5b87-18a6-41b6-b027-17e06ecb133e","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"ab5e5b87-18a6-41b6-b027-17e06ecb133e"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Available: Read, Edit, Write, Bash, Grep, Glob, Skill. Banned: TodoWrite, WebFetch, WebSearch. Use `git -C /home/knatte/Code/millhouse/wts/mill-quick` for commits; do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `/home/knatte/Code/millhouse/wts/mill-quick`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/millhouse/wts/mill-quick show <parent-branch>:<path>` to read it without changing cwd.
