# Implementer Brief — Fix stale checkpoint safety, Go binary ephemeral allowlist, and bare agent-tier name trap / bare-tier-aliases
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-scope-and-infra-gaps\_mill\plan\01-bare-tier-aliases.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-scope-and-infra-gaps\_mill\plan\01-bare-tier-aliases.md`.
   - Commit the plan edit first (`plan: extend bare-tier-aliases refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"7b987173-d78b-46f3-9879-b699b3bd742d"}
```
...
**`session_id` MUST be exactly `7b987173-d78b-46f3-9879-b699b3bd742d` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"7b987173-d78b-46f3-9879-b699b3bd742d"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
