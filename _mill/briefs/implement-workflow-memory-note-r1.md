# Implementer Brief — Fix agent-dispatch cwd guard, implementer self-termination, and false-success contracts / workflow-memory-note
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-implementer-and-dispatch-quality\_mill\plan\04-workflow-memory-note.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-implementer-and-dispatch-quality\_mill\plan\04-workflow-memory-note.md`.
   - Commit the plan edit first (`plan: extend workflow-memory-note refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"77ef7128-3ee9-45bb-8c80-e7e092f12926"}
```
...
**`session_id` MUST be exactly `77ef7128-3ee9-45bb-8c80-e7e092f12926` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"77ef7128-3ee9-45bb-8c80-e7e092f12926"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
