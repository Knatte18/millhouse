# Implementer Brief — Fix millpy-review-plan validator gaps and resolve_ref_paths path-doubling / validator-git-root-threading
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\review-plan-and-ref-paths\_mill\plan\02-validator-git-root-threading.md`
- **Overview (for `## Shared Decisions` only):** `C:\Code\millhouse\wts\review-plan-and-ref-paths\_mill\plan\00-overview.md`
- **Worktree cwd:** `C:\Code\millhouse\wts\review-plan-and-ref-paths`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\review-plan-and-ref-paths\_mill\plan\02-validator-git-root-threading.md`.
   - Commit the plan edit first (`plan: extend validator-git-root-threading refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"e3332b44-0666-4387-943e-00fecf460d53"}
```
...
**`session_id` MUST be exactly `e3332b44-0666-4387-943e-00fecf460d53` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"e3332b44-0666-4387-943e-00fecf460d53"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
**File paths are POSIX-style relative paths from `C:\Code\millhouse\wts\review-plan-and-ref-paths`.** Never flatten path separators into underscores. `plugins/mill/scripts/_config.py` is a file at `plugins/mill/scripts/` named `_config.py` -- not a file named `plugins_mill_scripts_config.py` at the worktree root. When in doubt, verify with `Read` before writing.
