# Implementer Brief — Fix wiki push upstream, cleanliness gate, mojibake, container config, and stacked-branch finalize / review-warning-ascii
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-infra-and-path-fixes\_mill\plan\01-review-warning-ascii.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-infra-and-path-fixes\_mill\plan\01-review-warning-ascii.md`.
   - Commit the plan edit first (`plan: extend review-warning-ascii refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"bef00b5f-04ed-4fe4-a8b6-f2b94a474bcd"}
```
...
**`session_id` MUST be exactly `bef00b5f-04ed-4fe4-a8b6-f2b94a474bcd` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"bef00b5f-04ed-4fe4-a8b6-f2b94a474bcd"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
