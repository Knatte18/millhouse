# Implementer Brief — Explore and adopt Claude Code fork-agents in mill orchestration / contract-helpers
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\explore-fork-agent-opportunities\_mill\plan\01-contract-helpers.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\explore-fork-agent-opportunities\_mill\plan\01-contract-helpers.md`.
   - Commit the plan edit first (`plan: extend contract-helpers refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"a50219e8-7daa-402c-bb4d-2ba060b6a6c4"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"a50219e8-7daa-402c-bb4d-2ba060b6a6c4"}
```
...
**`session_id` MUST be exactly `a50219e8-7daa-402c-bb4d-2ba060b6a6c4` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"a50219e8-7daa-402c-bb4d-2ba060b6a6c4"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
