# Implementer Brief — Fix plan validator Moves-target gap, code-review backtick parser, and mill-start encoding crash / mill-start-utf8-fix
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-plan-and-start-gaps\_mill\plan\03-mill-start-utf8-fix.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-plan-and-start-gaps\_mill\plan\03-mill-start-utf8-fix.md`.
   - Commit the plan edit first (`plan: extend mill-start-utf8-fix refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_done":N,"cards_remaining":M,"session_id":"83cd2d5c-fb68-4999-b66a-e57384da6d5d"}
   ```
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"83cd2d5c-fb68-4999-b66a-e57384da6d5d"}
```
...
**`session_id` MUST be exactly `83cd2d5c-fb68-4999-b66a-e57384da6d5d` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"83cd2d5c-fb68-4999-b66a-e57384da6d5d"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
