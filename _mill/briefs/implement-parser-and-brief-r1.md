# Implementer Brief — Fix review finding-count parser, nested-layout brief path, verify cwd, process leaks, and missing done-gate / parser-and-brief
...
- **Batch file (authoritative for this batch):** `C:\Code\millhouse\wts\mill-verify-and-layout-gaps\_mill\plan\01-parser-and-brief.md`
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `C:\Code\millhouse\wts\mill-verify-and-layout-gaps\_mill\plan\01-parser-and-brief.md`.
   - Commit the plan edit first (`plan: extend parser-and-brief refs for <short reason>`) and push via the wiki.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"76815769-8e42-4d5a-88e7-7526949fc0e7"}
```
...
**`session_id` MUST be exactly `76815769-8e42-4d5a-88e7-7526949fc0e7` (the UUID shown in the example above — it was injected into this brief when it was rendered). Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"76815769-8e42-4d5a-88e7-7526949fc0e7"}
```
**Do not wrap the JSON in a code block. Output it as a bare line — no backticks, no fence. Anything other than a bare JSON line is treated as `stuck_type: logic`.**
