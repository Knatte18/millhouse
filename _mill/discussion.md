...
**In:**
- `plugins/mill/skills/mill-merge-in/SKILL.md` Step 1 (no-op check) and Step 3 (merge command): fetch-then-compare-against-origin, with local-ref fallback when the parent branch has no upstream.
...
- `_status.py`'s public API list is documented in its module docstring (lines 18-30). All fourteen `status_path`-taking public functions are listed there; `_write_batches` (line 606) is a private helper (leading underscore) and should NOT get the guard applied redundantly if it's always called from an already-guarded public function — verify this at plan time by checking its callers.
