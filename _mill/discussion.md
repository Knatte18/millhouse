...
- Decision: Implement #987's suggested option (c) — document the `git show HEAD~1:<status_path>` recovery path as a fallback for the ordinary `done`-phase direct-squash flow, placed at `### 5. Direct squash`, immediately before the Direct-squash bash block that contains the `git -C <parent-path> commit -m "<cached_task>"` line that consumes it — rather than option (a) (move the caching instruction next to Step 4) or option (b) (restate the reminder inline at Step 4).
