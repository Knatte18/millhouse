...
- Decision: add a unit test under `plugins/mill/unit_tests/` that walks the shipped skill and template trees and fails when a file issues a load directive naming `mill:conversation` without also naming `mill:prose` **ahead of it**. Order is enforced, not merely co-presence: a load directive naming `mill:conversation` first and `mill:prose` afterwards fails the test.
- Rationale for enforcing order: `conversation` builds on `prose` and its own body assumes those rules are already active, so loading it first means its opening rules execute against an unloaded base — the same mechanism that produced this task's gap, one step subtler. All four sites that already have the directive right (`mill-start`, `mill-plan`, `mill-go-base`, `mill-pool`) use the identical wording "Load `mill:prose`, then `mill:conversation`", and the `handoff-first-line` Decision adopts that exact wording specifically so all five match. Enforcing co-presence only would let a fifth site invert the order and still pass, with nothing to flag it. The cost is that the pattern must locate both names and compare positions rather than test membership.
...
- Note on interaction with the order check: the order comparison in the `convention-test` Decision applies only within a matched load directive. A file that issues a correct directive and *elsewhere* mentions `mill:conversation` referentially is judged on the directive alone, so a later bare mention cannot invert the comparison and produce a false failure.
