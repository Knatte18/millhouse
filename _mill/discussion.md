...
- Step 7's missing `_notify.notify`/builder-lock-release (present in `### Blocked` but absent from step 7) is **not** added — #809 only reported the `ValueError` precondition bug, not a missing-notification gap. **Known residual inconsistency** (deliberately deferred, not an oversight): after this task, `## Holistic code review` will contain three distinct halt-mutation shapes — 3.5/3.6 (inline `set_blocked`+commit+notify+lock-release, new), 1187/1190/1195 (redirect via `set_blocked`+commit+`"go to *Blocked*"`), and step 7 (still no notify, no lock-release at all, and the builder lock stays held on halt). This three-way split is a real, pre-existing inconsistency in the file that this task narrows (by fixing 3.5/3.6/step-7's specific bugs) but does not fully unify — unifying all holistic halt paths onto one shape is a larger structural change than any of the four source issues asked for, and is left as a follow-up candidate.
...
- Decision: Add explicit state mutation to `mill-go/SKILL.md` before each of steps 3.5's and 3.6's `halt with BLOCKED: ...` sentences, using `_status.set_blocked` (not the two-call `append_phase`+`update_field` pattern), followed by commit, invoking the holistic cleanup block, `_notify.notify("mill-go.blocked", ...)`, and builder-lock release — mirroring the generic `### Blocked` section (lines 853–862) and the sibling holistic escalation branches (lines 1187, 1190, 1195) that already spell this out for their own halts.
  ...
  **Known pre-existing inconsistency at item 5 (not introduced or fixed by this task):** step 3.6's own enclosing gate (line 1133: "When ... `fallback_reviewer` is not null AND ...") wraps items 1-5, but item 5's own condition (line 1148: "If `fallback_reviewer is None` AND a rate-limit was detected on both 3.5 passes") directly contradicts that enclosing "not null" gate — as literally structured, item 5 appears unreachable, since 3.5's own text (line 1127) already says a `None` `fallback_reviewer` means "3.6 does NOT apply" at all. This is a structural bug in `mill-go/SKILL.md` that predates this task and is not one of the four source issues (#810/#809/#792/#806) — fixing the gate/condition mismatch itself is out of scope here. The new mutation-sequence text above still lands at item 5's current textual position exactly as written; whether item 5 is ever actually reachable is left as a follow-up candidate, parallel to the step-7 residual-gap callout in Scope/Out.

- Rationale: #810 reported exactly this gap, and the issue's own suggested fix names the missing sequence explicitly ("add the same explicit state-mutation sequence ... to step 3.5's halt branch, and to the corresponding 3.6 rate-limit-fallback-exhausted halts"). **Corrected precedent claim** (an earlier draft of this decision inaccurately said lines 1187/1190/1195 "already spell out" an inlined notify+lock-release sequence — they do not): those three sibling lines actually do `_status.append_phase(status_path, "blocked", ...)` + commit + the literal words `"and go to *Blocked*"` — i.e. they redirect to the shared `### Blocked` section (853-862) for `_notify.notify`/lock-release/message, rather than inlining it. `mill-merge/SKILL.md:81` and `mill-start/SKILL.md:315` (the `set_blocked` precedent cited above) likewise stop at `set_blocked`+commit+halt, with **no** notify/lock-release at all — those skills don't hold a builder lock the way mill-go does. Given that, 3.5/3.6 deliberately **inline** `set_blocked`+commit+cleanup+notify+lock-release rather than redirecting to `### Blocked`, because: (a) `### Blocked`'s "Tell the user: 'Batch X blocked with reason Y'" message is batch-scoped (references `batch_name`), which doesn't exist for the holistic scope, and (b) redirecting would drop the custom `reviews[].error` detail 3.5/3.6's existing halt messages already surface to the user — a real behavior siblings 1187/1190/1195 don't need to preserve (their halt is just "go to *Blocked*" with no custom per-error detail of their own). This inlined shape is therefore a new addition to the file, not a copy of an existing pattern — justified on its own terms, not by false precedent.
...
- Rejected: matching 1187/1190/1195's literal wording instead of using `set_blocked` — those sibling lines actually read `set batch state -> blocked, blocked_reason: "..."` (prose, `set_batch_field`-shaped, mirroring the literal `_status.set_batch_field(status_path, batch_name, "state"/"blocked_reason", ...)` calls at lines 625-626) plus an explicit `_status.append_phase(status_path, "blocked", ...)` call — not `update_field`. Rejected regardless: holistic scope has no `batch_name` to key a `set_batch_field`-shaped call on (per #810's own incident report), so that prose pattern doesn't transfer to 3.5/3.6 even on its own terms; `set_blocked` is the correct task-level (not batch-level) helper for holistic scope. This reused per-batch prose at 1187/1190/1195 is itself an unrelated, unflagged latent ambiguity in those sibling lines — out of scope for this task's four source issues.
- Rejected: redirecting 3.5/3.6 to the shared `### Blocked` section (mirroring 1187/1190/1195's literal `"go to *Blocked*"` idiom) instead of inlining — rejected per (a)/(b) above: the generic Blocked message is batch-scoped and would lose the `reviews[].error` detail these two halts specifically need to surface.
