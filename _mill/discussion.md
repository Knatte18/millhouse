...
- **#315** `parse_verdict` fails when the reviewer prepends a prose summary before the structured `# Review:` + yaml block. The scanner expects a leading yaml block and emits `ERROR: Could not parse verdict: no \`\`\`yaml block found.` The CLI then exits 1 *without* a JSON envelope, breaking mill-go's step 3.5 ERROR-only-retry path.
...
- `_review_common.py` — `.ipynb`-aware reader for `bulk_files` / `bulk_files_with_diff` (#308); `parse_verdict` rewrite that locates the yaml block anywhere in the response (#315 part 1); audit/affirm UTC timestamps in `write_review_file` (#317).
- `_review_code.py` (and `_review_plan.py` for parity) — wrap `parse_verdict` failures into a JSON envelope so the CLI emits `verdict: "ERROR"` instead of bare exit-1 (#315 part 2). Ensure every save path routes through `write_review_file` and never bypasses it (#316).
...
- **Decision:** Rewrite `parse_verdict` to scan the entire response for the first `` ```yaml `` opening fence (with closing `` ``` ``). Do NOT require it to be the first non-whitespace content. Reviewer prose preamble is harmless context; the canonical verdict signal is the fenced block. If multiple yaml blocks exist, the FIRST one wins (deterministic; matches what reviewers actually emit — they put the verdict block right after the prose summary).
- **Rationale:** Reviewers occasionally write a one-paragraph summary like "Review written to … Verdict is …" before the structured `# Review:` heading. The current "must be leading" scan rejects this. Searching from the top still picks up the canonical block — the loose scan never alters semantics for well-behaved reviewers.
- **Rejected:** (a) Strip-and-retry — adds round-trip; reviewer might re-add the same preamble. (b) Switch to JSON output schema — requires reviewer template + prompt rewrite; out of scope.

### parse_verdict-error-envelope

- **Decision:** When `parse_verdict` raises `ReviewError`, the backend (`_review_code.run`, `_review_plan.run`, `_review_discussion.run`) catches it inside the existing post-LLM block, writes the raw response to disk via `write_review_file` (still useful for operator inspection), and returns a `ReviewResult` with `verdict="ERROR"` (top-level) and `reviews=[{"scope": ..., "verdict": "ERROR", "file": <path>, "error": <str(exc)>}]`. CLI exits 0 (a structured ERROR is a normal envelope, not a crash).
