...
### RC-wiki-git -- fixtures that seed via wiki.upsert_task must initialise the wiki dir as a git repo with a local bare origin

- Decision: Add a shared helper `_test_helpers.init_wiki_repo(wiki_path: Path) -> None` that (1) `mkdir`s `wiki_path` (if not present), (2) `git init`s the directory, (3) sets `user.email` / `user.name` via `git config`, (4) creates an empty initial commit, (5) creates a sibling bare repo (e.g. `wiki_path.parent / f"{wiki_path.name}.git"`) and adds it as `origin`, (6) runs `git push --set-upstream origin <default-branch>`. Every fixture that calls `wiki.upsert_task` invokes `init_wiki_repo(wiki_path)` *before* the first `upsert_task` call. `wiki.upsert_task` internally calls `_render_and_commit_all` → `commit_push`, which runs `git add` / `git commit` / `git push origin <branch>` on `wiki_path`; without the init + bare-origin pair, `commit_push` raises `WikiPushError` and the fixture aborts before any assertion runs.
- Rationale: Production wikis are always git clones with a real remote; the V3 client assumes this and there is no "in-memory" mode. Aligning the fixture with that assumption is the smallest change that keeps the V3 client untouched. The test-fold.py fixture already does an equivalent setup (`plugins/mill/unit_tests/test-fold.py:75-82` runs `git -C <wiki_path> branch --show-current` then `git push --set-upstream origin <branch>` -- so `init_wiki_repo` is the extracted form of that pattern, not a new mechanism). The TinyDB write in `Store.upsert_task` happens *before* `commit_push`, so the row lands in `tasks.json` even if push fails -- but `_client.upsert_task` re-raises `WikiPushError` unconditionally, so a fixture that does not handle the exception aborts. Initialising the wiki as a proper git repo avoids the exception entirely and keeps fixture code free of try/except wrapping the V3 API.
- Rejected: (a) Wrap `upsert_task` calls in `try/except WikiPushError: pass` -- relies on undocumented "TinyDB-write-before-push" ordering, which is an internal implementation detail; if `_render_and_commit_all` is later reordered, fixtures break silently. (b) Make `_client.upsert_task` swallow `WikiPushError` -- changes production semantics for a test-only need. (c) Hand-write `tasks.json` JSON files -- couples tests to TinyDB internals.
...
**Test infrastructure to add (two helpers):**
...
- `plugins/mill/unit_tests/_test_helpers.py` -- add `init_wiki_repo(wiki_path: Path) -> None`. Initialises `wiki_path` as a git repo, sets `user.email` / `user.name`, creates an empty initial commit, creates a sibling bare repo as `origin`, pushes the initial commit. Pattern extracted from `test-fold.py:75-82`. Required before any `wiki.upsert_task` call from a fixture (RC1-seed, RC2).
...
- `plugins/mill/unit_tests/test-fold.py` -- import `_safe_rmtree`, replace the `shutil.rmtree(td.name, ignore_errors=True)` call at line 97 with `_safe_rmtree.safe_rmtree(Path(td.name), allowed_root=Path(td.name), ignore_errors=True)`. Only one callsite exists in the file; line 95 is the explanatory comment.
...
- **Q:** `wiki.upsert_task` triggers `commit_push` -- fixtures with a bare-directory wiki path will fail with `WikiPushError`. Swallow the exception, or init the wiki as a real git repo? **A:** Init as a real git repo with a local bare origin (`_test_helpers.init_wiki_repo`). Swallowing relies on undocumented internal ordering of `Store.upsert_task` (TinyDB write) vs `commit_push`; init aligns the fixture with production reality (V3 wiki is always a git clone with a remote) and matches the existing `test-fold.py` setup pattern.
