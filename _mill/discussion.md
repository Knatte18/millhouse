...
- Decision: mill-go Holistic step 1 (Crash-recovery) becomes a three-way branch:
  1. Review file present → use it, skip CLI (current behaviour).
  2. No review file AND no `bg-*-review-code-holistic-r{H}.log` exists in `.scratch/` → proceed to step 2 (normal CLI fire path).
  3. No review file AND a bg log exists for round H → call `_bg.is_bg_worker_alive(log_path)`. If alive → poll the log until `[mill-bg] EXIT` appears, then resume at step 4 (parse JSON, branch on verdict). If dead → proceed to step 2 (fire fresh CLI), logging `[mill-go] previous holistic round H bg worker died (pid=N); re-firing CLI`. Step 1 wording in `SKILL.md` is updated to enumerate these three branches explicitly.
