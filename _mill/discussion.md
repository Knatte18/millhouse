...
- `mill-go/SKILL.md` — 11 hits: delete sync_pull + health_check-via-_wiki + wiki_lock + write_commit_push + set_phase_at; replace health check with `_client.health_check`; update Board discipline
...
- `mill-fold/SKILL.md` — 4 hits: remove `_tasks_md.LOCKED_FOLD_PHASES` reference (this constant lives in `_tasks_md` which is gone — replace with `_client`-based phase check), update write_commit_push
