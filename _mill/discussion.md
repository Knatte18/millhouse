...
**Strand B — agents.yaml inheritance.** `wiki/agents.yaml` lister 15 entries i dag (g25flash, g25flash_tool, g25pro, g25pro_tool, g3flash_preview, g3flash_preview_tool, haiku, opushigh, opusmax, opusmedium, sonnethigh, sonnetmax, sonnetmax_tool, sonnetmedium). Hvert tool-variant duplikat-spesifiserer `provider`/`model`/`effort` fra base. Break-even for inheritance er rundt 10+ entries — vi har passert det. `extends: <base>` lar `sonnetmax_tool` arve fra `sonnetmax` og kun override `tooluse: true`.
