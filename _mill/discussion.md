...
- **Bug 2 (`_go_build_tag_retiering_stuck`):** extend `test-implementer-common.py`. TDD candidate: a fixture with `project_root/plugins/foo/go.mod` (minimal valid content) and `project_root/plugins/foo/bar.go`, tag-transitioned (added or removed `//go:build`) the same way existing cases 66a/66b do, asserting via a `cwd`-capturing variant of `_go_gate_mock` that the mocked `go build` call used `cwd=project_root/plugins/foo` and pattern `./...` (not `cwd=project_root` + `./plugins/foo/...`). Also cover the fallback case: same transition but with no nested `go.mod` present anywhere under `project_root/plugins/foo` up to `project_root` — assert behavior is byte-identical to the pre-fix case (cwd=project_root, pattern=`./plugins/foo/...`), to confirm the single-module common case is unaffected.
