...
- Add a Go row (marker `go.mod`) to `workflow.md`'s Language Detection table, routing to `@golang:golang-build`, `golang-comments`, `golang-testing` — matching the Python/C# rows' shape (see `workflow-md-go-row` Decision; corrected from an earlier draft that falsely claimed this routing already existed).
...
- Rationale: review-driven correction. An earlier discussion draft claimed this routing already existed as justification for leaving `workflow.md` untouched; it does not — the table has only Python and C# rows, and `golang-comments` is referenced nowhere else in that file. Without this row, the new `**Load the `code-comments` skill first.**` line being added to `golang-comments` has no table-driven trigger to ever fire for a Go project. Operator confirmed the recommended fix.
- Rejected: leaving it as a documented pre-existing gap, out of scope — rejected because it would leave this task's own change (the Step-0 load line) practically unreachable for the one language whose skill most needs the shared content applied.
