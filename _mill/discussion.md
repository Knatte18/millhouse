...
- `plugins/mill/scripts/_status.py` — delete `get_baseline_parent_sha` / `set_baseline_parent_sha`; add whatever clearing helper Decision `merge-in-batch-baseline-staleness` needs to drop every batch's `verify_baseline_failures` (`set_batch_field(..., None)` if the existing writer already expresses removal, otherwise a `clear_batch_verify_baselines` mirroring `clear_module_verify_baseline`).
...
- `module-wide-verdict-source`: a batch whose `verify:` string and cwd match the module-wide command runs once, not twice, and receives the module-wide run's signature set.
...
- **Q:** (r2) Does `baseline_verify_timeout_minutes`'s comment need rewriting when `baseline_prepare_cmd` is deleted? **A:** [auto-pick] Yes — it enumerates the deleted key by name in both the hub config and the template. **Why:** Leaving it would leave the surviving key documenting a key that no longer exists.
