...
- Add `Skill` to the `## Tools` "Available" line in **both** per-batch brief templates so the brief is internally consistent with the injected "load these skills" directive (the session is already dispatched with `Skill`).
...
### brief-tools-skill (review GAP)

- Decision: Add `Skill` to the `## Tools` "Available" line in **both** `templates/implementer-brief.md` and `templates/fixer-batch-brief.md`. Document the new `<LANGUAGE_SKILLS>` token in each template's leading HTML-comment token list.
- Rationale: Both brief `## Tools` sections currently read `Available: Read, Edit, Write, Bash, Grep, Glob` (no `Skill`). The injected non-optional "load these skills" directive contradicts that list, and a weak Haiku reader may obey the brief's own tool list over the directive — re-creating #483. The session **is** dispatched with `Skill` (the `mill-implementer` agent def lists it in its `tools:`), so adding it makes the brief consistent with reality. Documenting the token in the comment guards against the silent-no-op failure mode (`_render.render` raises `KeyError` only for template tokens missing from `values`; an extra `values` key with no matching token is silently ignored, so a token added to only one template injects nothing with no error).
- Rejected: Leaving the Tools sections unchanged and relying on the directive + agent-def Skill access alone — the brief still contradicts itself, which is the exact ambiguity #483 is trying to remove.

### directive-builder-home (review NOTE)

- Decision: Place the directive builder (`language_skills_directive(batch_file: Path) -> str`) in **`_agent_dispatch.py`**, importing `parse_batch_refs` from `_review_common`. Both `millpy-implement.py` and `millpy-fix.py` import it from there.
- Rationale: `_agent_dispatch.py` already owns brief construction (`write_brief`) and is the natural home for brief-content helpers. This does widen `_agent_dispatch`'s dependency surface (it transitively pulls `_review_common` → `_render`/`_reviewers`/`_config`), but the round-1 review confirmed **no import cycle** exists today — neither `_review_common` nor `_reviewers` imports `_agent_dispatch`. Pinning the home here (rather than deferring to mill-plan) keeps the import-direction decision explicit and reviewable.
- Rejected: (a) Leaving the home to mill-plan — review NOTE asked for an explicit choice since it widens a dependency surface. (b) Duplicating a second extension-detection parser instead of reusing `parse_batch_refs` — needless duplication.
...
- **`plugins/mill/templates/implementer-brief.md`** (`## Tools` at line ~98) and **`plugins/mill/templates/fixer-batch-brief.md`** (`## Tools` at line ~90) — (a) add the `<LANGUAGE_SKILLS>` token in a clear early section (e.g. just before "Implementation discipline" / "Fix discipline"); (b) add `Skill` to the `## Tools` "Available" line; (c) document the `<LANGUAGE_SKILLS>` token in each template's leading HTML-comment token list. `_render.render` strips the leading HTML comment but silently ignores a `values` key with no matching template token — so the token must be present in BOTH templates or its injection is a silent no-op (covered by the Testing assertions).
...
- **Q:** (review r1 GAP) Brief `## Tools` sections omit `Skill`, contradicting the injected directive — how to resolve? **A:** Add `Skill` to the "Available" line in both per-batch brief templates + document the `<LANGUAGE_SKILLS>` token in each leading comment.
- **Q:** (review r1 NOTE) Where does the directive builder live? **A:** `_agent_dispatch.py`, importing `parse_batch_refs` from `_review_common`; confirmed no import cycle.
- **Q:** (review r1 NOTE) Guard against `<LANGUAGE_SKILLS>` silently no-op'ing? **A:** Injection tests assert the directive appears in BOTH the implementer and fixer briefs.
