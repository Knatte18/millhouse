...
- **mill-go SKILL routing:** document a first-class **warm-session resume** recovery for `stuck_type: incomplete` in agent mode — `SendMessage` the still-warm implementer agent (keyed on the **agent ID the Agent tool returns at dispatch**, see "Warm-resume mechanism" below) to finish the remaining cards and emit the JSON report, then re-run finalize on the resumed output. In subprocess/psmux mode, `incomplete` does **not** reuse the skip-to-cleanliness branch — it auto-`--resume`s the implementer once (see the `subprocess-incomplete-resume` decision).
...
- **No change to subprocess/psmux dispatch mechanics** beyond adding an `incomplete` route that auto-`--resume`s the implementer once (the warm-`SendMessage` recovery is agent-mode-specific; subprocess/psmux use the existing `--resume` re-dispatch instead). `incomplete` deliberately does **not** fall through to the `commits_made > 0` "skip to cleanliness gate" branch.
...
- Decision: In subprocess/psmux dispatch, `stuck_type: incomplete` auto-`--resume`s the implementer **once** (the existing `running`-state resume re-invokes `millpy-implement.py <batch_name> --resume`), and only escalates if the resume still yields `incomplete`. It must NOT route into the `commits_made > 0` "skip to cleanliness gate" branch. Under `pipeline.autonomous_mode: true` the same one-auto-resume applies; a still-`incomplete` second result blocks the batch (`blocked_reason: "incomplete after resume"`).
- Rationale: The existing `commits_made > 0` autonomous auto-pick is "skip to cleanliness gate ... as if the implementer had reported success" (mill-go SKILL ~line 411-412). For a provably-partial batch that auto-accepts unfinished work — the exact #574 false-success `incomplete` was introduced to prevent. `incomplete` must finish the work (resume), not accept it.
- Rejected: Reusing the `commits_made > 0` handling verbatim (auto-ships partial batches in autonomous mode). Blocking immediately without a resume attempt (wastes the recoverable warm/partial state).
