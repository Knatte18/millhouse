...
- **Decision:** record the cold-fallback event on the fallback path only, emitting **both** a notification and a committed status row immediately before the cold retry:
  `_notify.notify("<VARIANT_LABEL>.fork-fallback", f"implementer {batch_name}", slug=slug)` and
  `_status.append_phase(status_path, f"fork-fallback-{batch_name}", _timestamp.now_utc_iso())`,
