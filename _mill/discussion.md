...
- Decision: In each of the three prose sites (4b line 565, 4c line 581, 4d line 597), change the kwarg count from 7 to 8 and add `done_gate` to the enumerated argument list. The kwarg *set* and `done_gate`'s value expression come from Phase: Plan's line 251/282; the `skip_checks` argument keeps 4b/4c/4d's own existing `skip_checks=plan_skip_checks` naming (their pre-existing text, correctly reusing Plan Review's own persisted-frontmatter variable per its Path Setup) rather than line 251's plain `skip_checks` — line 251 itself reads `skip_checks` with no `=` binding, since it's Phase: Plan's own scope, not Plan Review's. The corrected enumeration at each site: `root`, `git_root`, `wiki_root`, `skip_checks=plan_skip_checks`, `parent_branch`, `max_cards_per_batch`, `max_batch_context_tokens`, `done_gate`.
...
- Grep the same file for `"done_gate"` post-fix, scoped to the four kwarg-enumeration sites (Phase: Plan's original at lines 251/282 plus the three corrected 4b/4c/4d sites) — must show +3 occurrences there relative to before the fix (one new `done_gate` mention per corrected site), not counting the unrelated pre-existing `done_gate` mentions elsewhere in the file (e.g. lines 240-242, outside the kwarg-enumeration sites).
...
- No `verify:` command is meaningful here since no `.py`/code file changes. `_plan_validate.py`'s `_check_verify_not_isolated` flags any non-`None` `verify:` command lacking a `PYTHONPATH=` prefix whenever the repo is a Python project (`plugins/mill/pyproject.toml` present, confirmed) — so bare `true` fails this check. The plan's batch must either omit `verify:` entirely (`command=None`, check never fires) or, if a command is wanted, use `PYTHONPATH= true` (literal, empty value, per CLAUDE.md's `## Script invocation` convention) — never bare `true`.
