...
**In:**
- `millpy-fold.py` -- replace the `LOCKED_FOLD_PHASES` guard with the unclaimed-only allowlist check.
...
- **`millpy-fold.py` guard** is at lines 97-102: `phase = target_task["status"]; if phase in LOCKED_FOLD_PHASES: raise SystemExit(...)`. `target_task` comes from `wiki.list_tasks_brief(wiki_path)` (line 92). The brief dict includes both `status` and `deferred` (verified at runtime; the docstring on `list_tasks_brief` is stale and omits `deferred` but the daemon returns it). `get_task` also returns `deferred`. So the new predicate can read `target_task.get("status")` and `target_task.get("deferred", False)` directly -- no new daemon round-trip.
...
- **Existing tests that encode the OLD (buggy) contract** (`plugins/mill/unit_tests/test-fold.py`): line 176-184 asserts `LOCKED_FOLD_PHASES == ("active","ready-to-merge","pr-pending")` (remove); line 411-434 "done phase accepts fold" (invert to refused); line 436-459 "abandoned phase accepts fold" (invert to refused). Locked-phase refusal tests (lines 265-352) stay green -- those phases are still refused under the allowlist, though the assertion should match the new message text. The import at line 21 (`from wiki import _client as wiki, LOCKED_FOLD_PHASES, WikiPushError`) must drop `LOCKED_FOLD_PHASES`.
...
- `CLAUDE.md` line ~45 currently reads: "No fold into `[active]`/`[ready-to-merge]`/`[pr-pending]` tasks. Phase tuple at `_tasks_md.LOCKED_FOLD_PHASES`." This must be rewritten to the allowlist rule and the dangling `_tasks_md.LOCKED_FOLD_PHASES` pointer removed.
...
- `status="active" | "ready-to-merge" | "pr-pending"` -> REFUSED. (Existing tests at lines 265-352; update expected message text to the new wording.)
