...
- `skills/mill-go/SKILL.md` — Entry Step 5 phase gate. The current text is:
  ...
  ```
  `_status.read_full` calls `_status.read` which checks `status_path.exists()` and raises `ValueError` when absent (not `FileNotFoundError`). Either way the crash signature is unhandled and aborts mill-go entry. The new block wraps with `if not status_path.exists(): <wiki fallback>`. Use the existing `from wiki import _client` import pattern already documented in mill-merge SKILL Step 5.
