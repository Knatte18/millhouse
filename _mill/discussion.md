...
- Rationale: `git worktree prune` only removes administrative entries whose linked working directory is verifiably missing — it is safe to call unconditionally and idempotently, including under concurrency (a sibling task's still-live nested worktree has a directory that still exists, so prune never touches it). This orchestrator routinely tears down sibling task worktrees in parallel, so simultaneous `git worktree prune` calls (or a prune racing a concurrent `worktree add`) from parallel `remove_safe` invocations are a real, not hypothetical, scenario; any resulting lock contention degrades gracefully via the already-decided prune-failure-is-non-fatal behavior (see `prune-failure-is-non-fatal` below) — a prune that loses a race just warns and the caller's already-succeeded worktree removal is unaffected. Fixing `remove_safe` covers every current and future caller uniformly. Independently confirmed a second call site with the identical exposure: `millpy-implement.py:354-426`'s shared per-batch baseline checkout uses the same `_verify_baseline._checkout_parent_branch` + `remove_safe`-in-`finally` pattern. A registry-based fix scoped to `_verify_baseline.py` would miss that second call site entirely.
...
- **TDD candidate:** `unit_tests/test-worktree.py`, extending the existing `remove_safe` coverage. All current `remove_safe` tests (lines 141-300) mock `_subprocess_util.run`; the file's real-git-repo pattern (`_git_init` + `create`/`move`/`remove`) covers the separate `remove()` function, not `remove_safe`. The new nested-worktree scenario below is `remove_safe`'s first real-git test, not an extension of an existing real-git pattern for this function.
...
- Existing mocked tests fall into two groups after the restructure: (1) `Permission denied`, `is in use`, `Invalid argument`, and the generic-`WorktreeError` case all raise before reaching any prune call — unaffected by the restructure. (2) `test-worktree.py:246-263` ("is not a working tree", path exists) and `:286-300` (path absent) already mock a `git worktree prune` call via `side_effect=[mock_result, mock_prune]` and exit without raising — these already exercise the current fallback-branch prune call and will exercise the restructured trailing prune call instead; their two-call mock ordering (`git worktree remove` result, then `git worktree prune` result) must survive the restructure unchanged. The rmtree-fallback `PermissionError` test (lines 265-284) raises before reaching prune, in the first group.
