...
- Six new exemptions in the **path branch** of `_check_context_completeness`: gitignored, out-of-repo/absolute, directory-intent, forward cross-card `Creates:`, non-dependency negation phrasing, contrast-citation phrasing, quoted-material (fenced/blockquote), plus one explicit escape marker.
...
**Signature compatibility.** A test asserting that `_check_context_completeness` still accepts its current positional argument list without the new creates→card-number keyword (defaulting to no forward exemptions) — this is what keeps the existing direct-call tests valid.
