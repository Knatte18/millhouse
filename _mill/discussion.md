...
- Update `_check_all_files_touched_mismatch`'s two runtime error-message strings (`_plan_validate.py:1192-1193` and `:1203-1204`) to name Move targets alongside Edits:/Creates:, matching the module docstring (`:29-30`, which already correctly says "Edits:/Creates:/Moves: targets"). This is a message-text change only — the check logic (which already includes Move targets in `cards_set`, lines 1170-1182) is unchanged.
...
**Out:**
- No change to `_check_all_files_touched_mismatch`'s validator **logic** itself — its behavior (requiring Move targets) is the deliberate, already-shipped design from issue #494; only the docs (including its own error-message strings, see In) that describe it are wrong.
...
- Rationale: The multi-line sub-bullet form is documented and tested as one-path-per-bullet (`test-review-common.py:834-847`'s "multi-line bullet form returns both paths" test uses exactly one backtick token per sub-bullet line, establishing the design-intent precedent directly). Taking the leading token only formalizes that intent and eliminates the phantom-token failure mode without affecting any passing test. This is defense-in-depth alongside (not a replacement for) plan-validate Check 6 `_check_ref_not_backtick_path`, which already rejects the same multi-backtick sub-bullet shape at `--stage prepare` time but cannot catch edits made to batch files after that one-time gate (see Problem #2's reconciliation note) — `parse_batch_refs` is the only check still in a position to act once a batch file has been edited post-approval.
  (Note: the Moves-exclusion regression test at `test-review-common.py:3260-3289` describes `reads-not-backtick-path` — a different check — and `parse_batch_refs` is blind to `Moves:` headers entirely via field dispatch, so that test does not itself exercise the Context/Edits/Creates/Deletes path being fixed here; it is not cited as precedent for this decision.)
...
- `_plan_validate.py:_check_all_files_touched_mismatch` (line 1144) is the validator; `compute_moves_union` (referenced line 1181) is the existing helper that already separates Move sources from targets — no new helper needed, this confirms the validator's check **logic** is correct and only docs (template, SKILL fix-table) plus the validator's own two error-message strings need to change (message text, not logic).
- `_plan_validate.py:1192-1193` and `:1203-1204` are the two error-message strings to update to name Move targets, matching the already-correct module docstring at `:29-30` ("Edits:/Creates:/Moves: targets").
...
- Existing test at `test-review-common.py:834-847` ("multi-line bullet form returns both paths") is the precedent establishing one-path-per-sub-bullet as design intent; the fix is consistent with, not contradictory to, that test. (The Moves-exclusion regression test at `:3260-3289` covers a different check, `reads-not-backtick-path` — `parse_batch_refs` is blind to `Moves:` headers via field dispatch and that test does not exercise the fixed code path; see the Decision section's note.)
...
- **`_check_all_files_touched_mismatch` / templates:** the template/SKILL fix-table wording is a doc-only change verified by inspection (matches the validator's actual `cards_set` computation: Edits + Creates + Move targets). The two error-message string updates are a code change — if an existing unit test asserts the old message text verbatim, update that assertion to match the new Move-targets-inclusive wording; otherwise no new test is needed since the check's pass/fail logic is unchanged.
...
- **Cross-check with plan-validate Check 6 (layered-defense note):** also add or extend a `_check_ref_not_backtick_path` test in `test-plan-validate.py` (or wherever its existing tests live) asserting the *same* repro sub-bullet is flagged with a `reads-not-backtick-path` error at plan-validate time. `_check_ref_not_backtick_path` has two distinct guards that can each fire on this shape — `len(bt_matches) > 1` (multi-backtick count, line 1073) and the separate single-token "prose alongside backtick path" guard (lines 1095-1107) — so assert only that a `reads-not-backtick-path` error fires for the repro, without pinning which guard triggers it, so the test stays robust to either. This documents explicitly, in the test suite, that the two parsers are intentionally layered — Check 6 catches the shape at first approval, `parse_batch_refs` catches it again if the batch file is edited afterward — rather than leaving that relationship implicit.
