...
- Decision: At the start of Step 1, run `git fetch origin <parent-branch>` and record the resolved merge ref in a shell variable, e.g.:
  ```bash
  if git fetch origin "<parent-branch>"; then
    MERGE_REF="origin/<parent-branch>"
  else
    echo "[mill-merge-in] note: fetch of origin/<parent-branch> failed (branch not pushed?) -- comparing against local ref"
    MERGE_REF="<parent-branch>"
  fi
  git log HEAD.."$MERGE_REF" --oneline
  ```
  Step 3's merge command changes from `git merge <parent-branch>` to `git merge "$MERGE_REF"`, reusing the exact variable Step 1 resolved — this is the explicit mechanism that threads the ref choice from Step 1 to Step 3 (they are separate inline-bash blocks in the SKILL.md, so the variable must be carried across them; both blocks execute in the same shell session within one skill invocation). If the fetch fails (e.g. `<parent-branch>` is a sibling task branch never pushed to origin — a real case in mill's DAG-dependency model), fall back to today's behavior: diff and merge against the local `<parent-branch>` ref, printing the one-line note shown above.
- Rationale: The reported bug is that the local ref can be stale relative to origin because the same branch name is checked out unpulled in a sibling worktree. Fetching updates `origin/<parent-branch>` regardless of what's checked out locally, so it works even for the reported scenario. Using the same `$MERGE_REF` for the actual merge (not just the no-op check) avoids re-introducing the same bug one step later — a fetch-only fix that still merges the stale local branch would mean Step 1 correctly reports "there are new commits" while Step 3 merges old content.
...
- **No-op guarantee impact:** SKILL.md's "## No-op guarantee" section currently promises "this skill touches nothing: no checkpoint, no verify, no codeguide-update, no output side effects" and frames Step 1 as a "cheap exit" `mill-merge` depends on for every call. An unconditional `git fetch` is still a **read-only, non-mutating** network call — no local ref, working tree, checkpoint, or branch state changes as a result of the fetch alone, so the "touches nothing" guarantee about local *state* still holds. Two things do change and must be reflected in the section's rewritten wording: (1) it is no longer a zero-network-cost check (every call now does a fetch, accepted as necessary since there is no way to detect a stale local ref without asking origin); (2) the "no output side effects" clause is no longer exactly true on the fetch-failure path — the fallback note (`[mill-merge-in] note: fetch of origin/<parent-branch> failed...`) can print even when the end result is a no-op. Update the section to state plainly: no local mutation occurs on the no-op path; a network fetch always occurs; a one-line note may print on fetch failure even when there's nothing to merge.
...
- **`mill-merge-in` no-op check:** `test-merge.py`'s `_setup_trio` fixture (line 76) currently gives the hub/code repo **no `origin` remote at all** — only the wiki gets a bare remote (`git -C <wiki_path> push origin main`, lines 178/287). As written, `git fetch origin <parent>` would fail against this fixture, meaning only the fallback (local-ref) path is reachable, not the actual fetch-and-compare-origin path the fix exists to exercise. Extend `_setup_trio` to also create a bare "origin" remote for the hub repo (mirroring the existing wiki pattern: `git init --bare` a sibling dir, `git remote add origin <bare-path>`, initial `push origin main`) so the integration test can genuinely drive the fetch-succeeds branch. Update the Step 1 replica (~lines 446-453) to `git fetch origin <parent>` then diff against `origin/<parent>` against this now-real remote. A second, smaller scenario — fetch failing because the parent branch was never pushed to origin — is worth a lightweight assertion too (e.g. fetching a branch name never pushed to the new bare origin and confirming the fallback note path), but is not required if it adds significant fixture complexity; mill-plan decides based on cost.
