...
**Out:**
- The "stale-worktree edge" ambiguity path (`_inplace.prompt_stale_worktree`, and its callers in `millpy-cleanup.py:427` and `mill-merge/SKILL.md`'s own Entry Step 1 stale-worktree-edge block at `SKILL.md:23`) — this is a deliberately separate ambiguity-resolution flow (branch matches the active task's recorded branch AND a worktree directory already exists at the canonical path) that neither #735 nor #736 touches. Its own canonical-path assumption is out of scope here.
...
- Decision: Replace `_inplace.is_inplace`'s body with a git-topology comparison: in-place iff `git_root` is the same directory as `_paths.resolve_main_worktree_root(git_root)`. Use `git_root.samefile(main_root)`, falling back to `git_root.resolve() == main_root.resolve()` on `OSError` — the same fallback pattern already used in `_paths.resolve_git_root` (see `_paths.py` around line 145-150) for comparing worktree paths. `slug` and `cfg` stay in the function signature for API compatibility with all three existing call sites (`_paths.py:433`, `millpy-cleanup.py:434`, `mill-merge/SKILL.md:21`) and the structural signature test in `test-inplace.py`, but no longer participate in the check — the docstring must say so explicitly.
- Rationale: `_paths.resolve_main_worktree_root(git_root)` already exists and does exactly the git-verifiable check issue #735's "Expected" section asks for (comparable to `git worktree list --porcelain` reasoning, but via pygit2's common-dir resolution, matching the codebase's existing convention). No circular-import risk: `_paths.py` only imports `_inplace` lazily inside a function body (`_paths.py:418`), so `_inplace.py` importing `resolve_main_worktree_root` from `_paths` at module level (alongside the existing `resolve_worktrees_dir` import) is safe. All three existing call sites (`_paths.resolve_active_worktree`, `millpy-cleanup._resolve_inplace_mode`, and `mill-merge/SKILL.md:21`'s Entry Step 1) only ever invoke `is_inplace` after already confirming the checkout's current branch matches the slug in question — combined with git's own invariant that a branch cannot be checked out in two worktrees simultaneously, "is `git_root` the main worktree" is unambiguous and equivalent to "is this task's branch checked out in-place" for all three.
- Rejected: Hybrid approach (keep path-existence as primary signal, consult topology only when path-existence says "in-place"). In #735's actual repro (no directory at the canonical path), "path-existence says in-place" is always true, so gating on it before consulting topology reduces to the pure-topology check anyway — the hybrid adds a second source of truth and a conditional without changing the outcome for the scenario the bug report describes. Rejected for unnecessary complexity, not because it fails to fix #735.
...
- Callers of `is_inplace` (unchanged call sites, behavior changes transparently):
  ...
  - `plugins/mill/skills/mill-merge/SKILL.md:21` (Entry Step 1: `mode_inplace = _inplace.is_inplace(slug, git_root, cfg)`) — this is the exact call whose misdetection produces #735's symptom. Its branch==slug precondition holds even more directly than the other two: `slug` here comes from `_marker.task_data(git_root, wiki_path, cfg)`, which derives the slug from `git_root`'s own current branch in the first place.
...
- **`plugins/mill/unit_tests/test-cleanup.py`** — the SKILL.md-level behavior needs no changes, but `_resolve_inplace_mode`'s own coverage gains a genuinely new live branch and needs a new test. In `_resolve_inplace_mode` (`millpy-cleanup.py:423-434`), `is_inplace` is only reached after the caller has already computed `worktree_dir = resolve_worktrees_dir(cfg, hub_root) / slug` and confirmed `.is_dir()` is `False` — under the *old* implementation, `is_inplace` recomputed that identical path-existence check internally and was therefore guaranteed to return `True` there; the `return ("worktree", "")` fallback at line 437 was dead code, unreachable from this call site. The new topology check removes that tautology, making line 437 reachable for the first time. Add a `test-cleanup.py` case for `_resolve_inplace_mode` that patches only `_inplace.resolve_main_worktree_root` (not `_resolve_inplace_mode` itself, unlike every existing test in that file) to confirm both outcomes (`"inplace"` and `"worktree"`) are actually reachable post-fix. The one existing site referencing `_inplace` directly (`mill_cleanup._inplace.prompt_stale_worktree`, line ~702) exercises the separate stale-worktree edge and still short-circuits before reaching `is_inplace` — unaffected, no change needed there.
- **Step 5 guard** — `plugins/mill/integration_tests/test-merge.py` (~593-611) already exercises Step 5's restore sequence end-to-end, but it deliberately dodges this exact bug rather than covering it: its own comment states hub has no `_mill/` at all on `main` yet, and a bare `git checkout -- <pathspec>` "fails ... independent of the worktree-mode bug this scenario targets" — so the test seeds a trivial `_mill/status.md` on the hub, committed before the squash, specifically so the restore commands "have something real to act on." No existing scenario covers "parent tracks nothing at `task_dir`," which is precisely the case #736 reports and the common worktree-mode case per the Problem section. Add a new scenario (or a variant of the existing one with the seeded `status.md` dropped) asserting the guarded checkout no longer halts when the parent's `HEAD` has nothing at `task_dir`.
...
- **Q:** (Review r1 gap) Does the existing `test-merge.py` integration harness already cover Step 5's "parent tracks nothing at `task_dir`" case? **A:** [auto-pick] No — it deliberately seeds a hub-side `status.md` specifically to avoid hitting this exact bug (its own comment says so). Testing section now requires a new/modified scenario with the seed dropped. **Why:** the discussion previously framed this as speculative ("check... add if missing") when the harness's own comment already confirms the gap definitively.
- **Q:** (Review r1 gap) Does the caller inventory for `is_inplace` cover every call site the Decision's rationale relies on? **A:** [auto-pick] No — `mill-merge/SKILL.md:21`'s own Entry Step 1 call was missing from both the Technical context caller list and the Decision rationale's "both existing callers" count. Added as a third call site. **Why:** that SKILL.md call is the exact one producing #735's symptom; omitting it from the inventory feeding the correctness argument was a factual gap, not just an oversight.
- **Q:** (Review r1 gap) Does `millpy-cleanup._resolve_inplace_mode`'s call to `is_inplace` get test coverage for both outcomes after the topology rewrite? **A:** [auto-pick] Not yet — under the old implementation that branch's `is_inplace` call was tautological (guaranteed `True`), so no test ever needed to exercise the `False` outcome; the rewrite makes it live for the first time. Testing section now requires a new `test-cleanup.py` case patching only `_inplace.resolve_main_worktree_root`. **Why:** "no changes needed" was correct for existing tests but incomplete — it missed that new coverage is needed for a branch that was previously unreachable.
- **Q:** (Review r2 gap) Is `_paths.resolve_active_worktree` actually a caller of `prompt_stale_worktree`, as the Scope › Out section claimed? **A:** [auto-pick] No — verified against `_paths.py:378-450`: `resolve_active_worktree` only calls `_inplace.is_inplace`, never `prompt_stale_worktree`. The real third caller is `mill-merge/SKILL.md:23`'s own Entry Step 1 stale-worktree-edge block. Out section corrected. **Why:** an incorrect caller inventory in a scope-boundary statement risks mill-plan reasoning about the wrong code path when respecting the "out of scope" boundary.
