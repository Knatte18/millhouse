...
- Decision: acquire `<parent-path>/.scratch/merge.lock` (three lines: pid, ISO-8601 UTC Z timestamp, child branch) after the merge-in check passes, immediately before Step 5, and never hold it across a stop.
  ...
  otherwise poll every 10 s up to 5 min, then `halt` with the holder's three lines in `reason`.
...
- Decision: every step stays idempotent so a plain re-run after any `halt` resumes correctly, relying on existing mechanisms:
  ...
  - Squash: `Already up to date` or `nothing to commit` skips push.
...
- Decision: new `SKILL.md` (target ~100 lines) contains: role line and cross-worktree invariants;
  the script invocation in cache form (`PYTHONPATH="${CLAUDE_PLUGIN_ROOT}/scripts" "$MILL_PYTHON" "${CLAUDE_PLUGIN_ROOT}/scripts/millpy-merge.py" [resume args]`);
  a branch table on `status` / `action`;
  the two callback procedures;
...
- TDD candidates in `_merge.py`:
  ...
  - Lock: fresh acquire, stale overwrite, wait-then-clear, timeout halt; released on success, on Step 5 halt, and on exception.
  ...
  - Idempotency: task dir absent skips cleanup commit; squash "nothing to commit" skips push.
