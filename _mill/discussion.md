...
- Decision: add a new clause-scoped exemption function, `_is_cross_card_ownership_exempt(lowered_line, token_start, token_end)`, reusing the existing `_clause_bounds` helper (same helper `_is_non_dependency_negation_exempt` and `_is_contrast_citation_exempt` already use). It searches the token's own clause (not the whole line — a card can legitimately make more than one unrelated claim per Requirements: line) for a regex matching `\b(?:batch|card)\s+\d+\s+(?:'s\s+)?(?:owns?|fixes?|corrects?|rewrites?|addresses?|handles?|resolves?|covers?|edits?|updates?)\b`. The verb set mirrors both issues' exact wording ("fixes", "owns", "corrects", "rewrites") plus close synonyms already implied by the same ownership-attribution idiom, following the same "spell out the inflected forms by hand" convention `_PROHIBITION_VERB_FORMS` uses.
- Rationale: both issues share one idiom — "<batch|card> <N> <ownership-verb> ... `path`" — regardless of which side of the token the phrase falls on, so a clause-wide regex search (not a positional before/after split like the negation exemption) is the right granularity, matching `_is_contrast_citation_exempt`'s own clause-wide-substring style rather than `_is_non_dependency_negation_exempt`'s stricter positional style.
...
- **Ownership exemption:**
  ...
  - `test_check_context_completeness_dirty_ownership_separate_clause_not_exempted` — the ownership phrase and the token in different clauses of the same line (comma/period-separated) must NOT be exempt, mirroring the existing `..._dirty_contrast_citation_separate_sentence` test's guard.
