...
- Amending the tool-use instruction prose at all five artefact-assembly sites to tell the reviewer to resolve each listed path against the root stated in `## Path roots` (see the `tool-use-mode-relative` decision -- this is what makes the relative tool-use list correct without depending on the reviewer subprocess's cwd).
...
- Decision: the tool-use `read_list` / `batch_list` / `Overview:` / `Batch:` lines are relativized too, exactly like the bulk-mode manifest. **Correctness does not rest on the reviewer's cwd.** The `## Path roots` header states the absolute root, and the tool-use instruction text is amended to tell the reviewer to resolve each listed path *against the root stated in `## Path roots`* -- i.e. to join the stated absolute root onto the relative path before calling `Read`. An absolute path passed to `Read` works regardless of cwd, so this removes the cwd dependency entirely instead of relying on it.
- Rationale: the earlier draft of this decision claimed `_llm_claude.run_tool_use` forwards a `cwd` to the reviewer subprocess. That is **false** and was corrected here. Verified: `run_tool_use()` (`_llm_claude.py:481-514`) has no `cwd` parameter and passes none to `_invoke()`; only `run_implementer()` (line 525) does. `_invoke()` (line 326) accepts `cwd` but defaults it to `None`, and `_reviewer_single.run()` (lines 71-78) builds its kwargs without `cwd`. So under the subprocess/psmux path the reviewer subprocess simply **inherits the ambient Python-process cwd**. That cwd does happen to be the task worktree in practice -- `millpy-bg.py` validates it and refuses to run from a non-task worktree (`millpy-bg.py:151-170`), and under agent-mode dispatch the reviewer subagent inherits the orchestrator's worktree cwd -- but nothing in the review code path *sets* it, so it is an ambient invariant, not a guarantee this feature may depend on for correctness. Making the instruction join the stated absolute root makes the tool-use list correct even if that invariant is ever broken, while still removing the repeated prefix from the listing itself. Leaving tool-use absolute while the manifest immediately above it is relative would also be actively confusing -- the two lists name the same files.
- Consequence for the plan: the tool-use instruction strings at the four `_review_plan.py` sites and the one `_review_code.py` site (the "Then read the source files listed under ..." / "Read the overview and every batch file above..." sentences) must be amended to carry the resolve-against-stated-root instruction. This is a small prose edit at each site, not a new mechanism.
- Rejected: (a) keeping absolute paths in tool-use mode only -- inconsistent with the manifest directly above it, and forfeits the whole saving in tool-use reviews; (b) plumbing `cwd=project_root` through `run_tool_use` / `_reviewer_single.run` / `_invoke` to make the ambient invariant explicit -- a real improvement, but it changes reviewer *dispatch* behaviour, which is outside a display-only task's blast radius, and it would still leave the agent-mode path (which never touches `_llm_claude`) relying on inherited cwd. Worth its own task; noted here so the option is not lost.
...
- **Q:** Should tool-use `read_list` / `batch_list` also go relative, given the reviewer must `Read` those paths itself? **A:** [auto-pick] Yes, relative, with the `## Path roots` header stating the absolute root once. **Why:** a relative manifest sitting directly above an absolute read-list would be confusing. **Amended after round 4:** the original "tool-use reviewers run with cwd at the task worktree" justification was source-contradicted (`run_tool_use` forwards no `cwd`); the decision now instructs the reviewer to join the stated absolute root onto each relative path, so it no longer depends on cwd at all. See `tool-use-mode-relative`.
...
- **Q:** Round-4 review showed the `tool-use-mode-relative` rationale rested on a false claim (`_llm_claude.run_tool_use` forwarding `cwd`). Correct the rationale and keep depending on ambient cwd, plumb `cwd` through the reviewer dispatch, or remove the cwd dependency? **A:** [auto-pick] Remove the dependency: state the absolute root in `## Path roots` and instruct the reviewer to resolve listed paths against it. **Why:** an absolute path passed to `Read` works whatever the cwd is, so the listing can stay relative without the correctness of tool-use reviews resting on an invariant nothing in this code path sets. Plumbing `cwd` through `run_tool_use` / `_reviewer_single.run` / `_invoke` would change reviewer dispatch behaviour -- outside a display-only task -- and would still not cover the agent-mode path, which never touches `_llm_claude`; it is recorded as a separate follow-up rather than folded in here.
