...
- **Decision:** Extend `clean_ephemeral_scope_violations`'s allowlist:
  - Add suffix **`.exe`** to the existing suffix set (`.test`, `.test.exe`, `.prof`, `.cover` + basename `coverage.out`). This auto-cleans `sandbox.exe` — the reported case and the Windows-primary platform mill runs on.
...
- **#567 (SKILL prose; validated by inspection + a focused shell-level check):** no Python module owns step 2, so coverage is via the skill text. The plan should include a deterministic check (e.g. an integration-style note or a tiny scripted repro in `integration_tests/` if one fits the existing harness) demonstrating: create checkpoint, re-run, confirm `$CHK` now points at the current pre-merge HEAD and the run does not error. If no clean automated hook exists, document the manual repro from the issue in the plan and rely on the prose guard. TDD candidate: only #565 and #571 are pure-code; #567 is doc/behavioral.
