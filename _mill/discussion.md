...
- Decision: at each converted site, the escalation runs before the site's `set_blocked` / `set_batch_field(..., "blocked")` / commit, so `retry` and `approve` have nothing to undo.
  ...
  *Blocked* is also entered from four per-batch cleanliness-gate halts (out-of-scope untracked file mid-batch, dead parent branch, unresolvable parent diff, dirty tree), which record their own state before jumping there.
  Those stay user-only, matching the handoff dirty-tree/out-of-scope gates: *Blocked* gets a fourth parameter `escalate` (true only when entered from a `### Stuck escalation` bullet, false from every cleanliness gate), and the escalation step runs only when `escalate` is true.
  The cleanliness-gate entries keep their current record-then-jump order and pass `escalate: false`, so *Blocked* skips both the escalation and the recording step for them (their state is already recorded).
  *Blocked* takes three recording parameters so every stuck bullet's current recording behaviour survives unchanged:
...
- Rationale: none of `millpy-fix.py`'s existing flags (`--scope`, `--stage`, `--review-file`, `--round`, `--nits-only`, `--prior-blocking`, `--agent-output`, `--start-sha`, `--session-id`) carries free text into the brief, so a CLI-dispatched fixer has no guidance slot;
...
- Decision: these sites escalate;
  ...
  mill-plan step 5's "Non-progress check" halt (`non-progress round {N}`, a stable planner/reviewer disagreement — the step's own text says user intervention is required, and resolving it means overruling one side, which stays the operator's call);
