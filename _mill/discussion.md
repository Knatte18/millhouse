...
- **#1022** — a card's `Requirements:` legitimately names a path to say *another* card/batch owns it
  ...
  This is a cross-batch handoff statement, not a dependency the card itself reads. The issue's own
  suggested fix names the idiom as "'batch N fixes `path`', 'batch N owns `path`'" — "fixes" is
  verbatim from the reproduction sentence, "owns" verbatim from this suggested-fix phrasing.
...
- **#984** (two sub-cases, one of which overlaps #1022's shape):
  ...
  - A cross-reference to another numbered card's file ("for the same reason card 23 corrects one
    ...
    different verbs ("corrects"/"rewrites" instead of "fixes").
...
- Decision: add a new exemption function, `_is_cross_card_ownership_exempt(lowered_line)`, mirroring `_is_prohibition_exempt`'s **line-wide** (not clause-scoped) search granularity: it searches the *whole* `Requirements:` line for a regex matching `\b(?:batch|card)\s+\d+\s+(?:'s\s+)?(?:<verb-form>)\b`, independent of the token's own position, where `<verb-form>` is built from a new `_OWNERSHIP_VERB_FORMS` dict, hand-spelled exactly like `_PROHIBITION_VERB_FORMS` (base/3rd-person/past/gerund per verb, e.g. `own → own, owns, owned, owning`; `rewrite → rewrite, rewrites, rewrote, rewriting, rewritten`, mirroring `write`'s own irregular 5-form entry): `own`, `fix`, `correct`, `rewrite`, `address`, `handle`, `resolve`, `cover`, `edit`, `update`. Two of the four issue-quoted verbs — "fixes" and "corrects"/"rewrites" — are simple present/3rd-person forms in the source text, so the past/gerund forms are not independently attested there, but the hand-spelling convention this decision claims to follow requires generating them anyway, exactly as `_PROHIBITION_VERB_FORMS` already does for verbs whose issue-motivating quote was likewise only ever seen in one inflected form.
...
- Decision: add a new single-marker line-wide exemption, `_is_illustrative_output_exempt(lowered_line)` — structurally a one-set presence gate like the `_CITATION_MARKERS` check, not a two-set AND-pairing like `_is_prohibition_exempt`'s negation-word-plus-verb-form gate — testing a new `_OUTPUT_VERB_FORMS` dict, hand-spelled with the same base/3rd-person/past/gerund shape as `_PROHIBITION_VERB_FORMS`: `emit → emit, emits, emitted, emitting`; `render → render, renders, rendered, rendering`; `output → output, outputs, outputted, outputting`; `print → print, prints, printed, printing`; `display → display, displays, displayed, displaying`. The line-wide gate fires whenever any of these forms appears anywhere on the same `Requirements:` line as the token, independent of position, exactly like `_is_prohibition_exempt`'s existing gate — including a past-tense phrasing ("the answer emitted the bare `README.md`") that the reported `#985` sentence itself doesn't exercise but the mechanism should still cover, consistent with the hand-spelling convention.
- Rationale: `#985`'s exact sentence ("an answer ... emitting the bare `` `README.md` ``, never `` `./README.md` ``") describes a function's rendered *output value*, not a file it reads — "emitting"/"rendering"/"outputting" is the idiom that marks a backtick token as an illustrative output literal rather than a read dependency, the same semantic class `_CITATION_MARKERS`'s existing "signature inlined" / "no file read needed" entries already carve out for a different idiom. Reusing `_is_prohibition_exempt`'s line-wide *granularity* (a single-set presence test, not its two-set AND-pairing) keeps the new function's shape consistent with precedent and needs no new clause-boundary logic.
- Rejected: adding "the bare" as a `_CITATION_MARKERS` substring — too narrow (only catches this one phrasing) and semantically misleading as a "citation" marker when the real signal is the output-verb idiom; rejected in favor of the single-marker verb-form mechanism above, which generalizes to any output-describing verb.
...
- **Ownership exemption:**
  ...
  - `test_check_context_completeness_clean_ownership_synonym_addresses` — `` "batch 4 addresses `y.py`." `` — exercises one of the extended close-synonym verbs (not one of the four issue-sourced verbs), mirroring how `..._prohibition_marker_new_verbs` exercises more than one verb form for exemption 1.
  - `test_check_context_completeness_clean_ownership_past_tense` — `` "batch 8 fixed `x.cs`." `` — exercises the hand-spelled past-tense form, guarding the `_OWNERSHIP_VERB_FORMS` table actually has one (not just base/3rd-person).
  - `test_check_context_completeness_clean_ownership_possessive` — `` "batch 8's fix touches `x.cs`." `` — exercises the regex's optional `'s` group.
...
- **Illustrative-output exemption:**
  ...
  - `test_check_context_completeness_clean_illustrative_output_past_tense` — `` "the answer emitted the bare `README.md`." `` — exercises the hand-spelled past-tense form, guarding that `_OUTPUT_VERB_FORMS` actually has one (not just base/3rd-person/gerund).
