...
- Tighten markers to Go-aware patterns: `--- FAIL` (go-test per-test), `FAIL\t` / bare `FAIL` summary line, `panic:`, `build failed`. Keep case-insensitive matching but match the structured go-test failure tokens, not the substring `"fail"`.
...
- **Go false-BLOCKING** — add a `test-implementer-common.py` case: win32 + cleanup-race signature + benign go-test output containing a `"fail"` substring (e.g. `ok  pkg/failover  0.1s`) and no structured `--- FAIL`/`FAIL\t` -> resolves to **success** (benign). Keep cases 24/24b/25/26 green (real `--- FAIL` / bare FAIL summary still -> stuck/verify).
