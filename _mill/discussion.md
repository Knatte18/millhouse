...
- Guard form (pin this — do not improvise): `if [ -d <worktree>/_mill/briefs ]; then git -C <worktree> add _mill/briefs/; fi`. This is the uniform shape; the planner must use it rather than `git add --ignore-unmatch _mill/briefs/` or pathspec-magic (`':(exclude)'`). `--ignore-unmatch` is **rejected** — it silently swallows the no-match and is non-uniform with the existing mill-go/mill-plan commit steps (which add `_mill/briefs/` unconditionally because a brief always exists there).
