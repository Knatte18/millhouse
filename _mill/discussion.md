...
- Shared helper(s) in `_finalize_cleanup.py` for checkpoint naming and pr-notes stashing, with unit tests.
...
- Decision: in `mill-finalize` Step 3, before either cleanup branch runs, if `<task_dir>/pr-notes.md` exists and is non-empty, copy it to `<worktree>/.scratch/pr-notes.md` (gitignored, so it survives `git rm -r` and is never committed).
  In Step 5, `git-pr` is invoked as today; `git-pr` Step 9 gains: if `<repo_root>/.scratch/pr-notes.md` exists, append its contents to the generated body under a `## Reviewer notes` heading, then delete the scratch file after `gh pr create` succeeds (also after the REST fallback).
  `git-pr` reads that file only when it exists, so non-task and non-finalize uses are unaffected.
...
- Decision: add `checkpoint_branch_name(branch: str) -> str` to `_finalize_cleanup.py` (`"mill-checkpoint-" + branch.replace("/", "-")`, matching the merge-in skill's `tr '/' '-'`) and `delete_checkpoint_branch(hub_root, branch)` (runs `git -C <hub_root> branch -D <name>`; a missing branch is success, other failures are logged non-fatally like the existing `branch -D` handling).
  Call it from `_apply_worktree_record`, `_apply_inplace_record`, and the branch-deleting part of `_apply_pr_reap_record`, right after the task-branch deletion.
...
- Stash helper for pr-notes (if implemented as a script helper rather than inline skill text): present file copied, absent file no-op, empty file no-op, re-run with scratch copy already present and source gone keeps the copy.
