...
- **Gap B:** Make brief-committing uniform across all orchestrators that dispatch briefs, following the existing "everything in `_mill/` gets committed by the orchestrator" pattern. Concretely:
  ...
  - **mill-merge-in** (`SKILL.md`): on the conflict-resolution success path, stage `_mill/briefs/` before `git -c core.editor=true merge --continue` so the `merge/conflicts` and `merge/verify-fix` briefs land in the merge commit.
...
- Decision: Replace the commented `git:` block in `plugins/mill/templates/mill-config.yaml` (currently lines ~80-82) with a real block:
  ...
  ```
  Keep the surrounding explanatory comments. The validator (`walk_unknown_keys`) uses the template dict as the schema, so the three subkeys become "known," and any other `git.*` key (a typo, an unknown future key) still correctly warns.
...
- Decision: Where a commit step may run **without** any brief having been written (mill-start Handoff reached via the review-skip path when `rounds: 0` or `reviewer: null`, and the `--auto` halt-commits), guard the add so a missing `_mill/briefs/` does not fail the commit: e.g. `if [ -d <worktree>/_mill/briefs ]; then git -C <worktree> add _mill/briefs/; fi`, or otherwise only include the pathspec when the directory exists.
- Rationale: `git add _mill/briefs/` errors with "did not match any files" if the directory does not exist. In mill-start's post-review commits (4b, 5) and mill-merge-in's conflict path, a brief always exists by construction, so an unguarded add is fine there; only the skip/halt paths need the guard.
...
- **`plugins/mill/templates/mill-config.yaml`** — the schema-of-record. The `git:` example block is currently commented out (the source of the warning). Keys consumed elsewhere: `git.require_pr_to_base` + `git.base_branch` are read by `mill-merge/SKILL.md` (steps ~30-31); `git.parent-branch` is read by `git-pr/SKILL.md` (step ~88) from `.millhouse/config.yaml`.
- **Brief writers** (call `_agent_dispatch.write_brief` / `_implementer_common.emit_prepare`):
  ...
  - `millpy-merge-in-subagent.py` (roles `merge/conflicts`, `merge/verify-fix`) → **mill-merge-in** ✗ **(the gap)**. The merge is finalized with `git -c core.editor=true merge --continue` (SKILL line ~48), which commits the index but not the untracked brief.
...
- **Q:** mill-merge-in's conflict sub-agent also writes uncommitted briefs (outside the originally-reported CLI list) — include it? **A:** Yes — fix mill-merge-in too (stage `_mill/briefs/` before `merge --continue` on success), for full consistency with "everything in `_mill/` gets committed."
