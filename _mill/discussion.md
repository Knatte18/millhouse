...
| `merge_tasks(wiki_path, *, remove_slugs: list[str], upsert: dict, set_phase: tuple[str, str \| None] \| None = None)` | `dict` (the upserted task) | Atomic op replacing `_wiki.wiki_lock` + read-modify-write windows in `_spawn_core.multi_select_groom_then_claim`. All operational args are **keyword-only**. `set_phase` is `(slug, phase)`; the source annotation is `tuple[str, str \| None]` though the standalone `set_phase()` function accepts `id_or_slug: int \| str` — pass a string slug to be safe. Verified `_client.py:289–331`. |
