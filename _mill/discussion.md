...
- Decision: fix the finalize side (authoritative; never the prepare side, even if the reproduction surprises).
  ...
  Fix: exclude a path when it lies under the briefs directory; the source (the `task_dir` argument `_in_scope_dirty_stuck` already receives, or `_paths.resolve_task_path`) is chosen by the plan writer from what `task_dir` actually holds in the nested layout, verified by the reproduction test.
  A candidate computation is `(project_root-relative form of _paths.resolve_task_path(project_root, "_mill/briefs/"))`, with a `/_mill/briefs/` path-component match as fallback when that resolution is unavailable.
