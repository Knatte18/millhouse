...
- `plugins/mill/scripts/_review_code.py:220` — replace `resolve_path("status.md", slug)` with `_paths.status_path(project_root, cfg)`. Both vars are in scope.
- `plugins/mill/scripts/_spawn_core.py:716,720` — replace `worktree_path / "_mill" / "status.md"` and `"_mill/status.md"` with `_paths.status_path(worktree_path, cfg)`. The function `_write_initial_status` (around line 681) needs `cfg` in its signature if it isn't already there — confirm during planning.
- `plugins/mill/scripts/millpy-spawn.py:165` — dry-run print: replace `worktree_path / '_mill' / 'status.md'` with `_paths.status_path(worktree_path, cfg)`.
- `plugins/mill/scripts/millpy-claim.py:216` — dry-run print: replace `resolve_hub_path() / '_mill' / 'status.md'` with `_paths.status_path(resolve_hub_path(), cfg)`.
