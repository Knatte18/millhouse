...
- Decision: Replace both callsites at `plugins/mill/unit_tests/test-fold.py:95,97` with `_safe_rmtree.safe_rmtree(Path(td.name), allowed_root=Path(td.name), ignore_errors=True)`. Import `_safe_rmtree` at top of file alongside the existing scripts-path setup. Do **not** add `test-fold.py` to `ALLOWED_FILES` in `test-no-direct-rmtree.py`. With RC1 in place, the daemon-lock fallback path the wrapper exists to handle should rarely fire, but `ignore_errors=True` keeps the existing safety net.
...
- The eight RC1 files (`test-marker.py`, `test-millpy-spawn.py`, `test-review-cli.py`, `test-review-code-flow.py`, `test-review-common.py`, `test-review-discussion-flow.py`, `test-review-plan-flow.py`, `test-setup-hub-links.py`) -- audit fixtures; any that trigger daemon work via `wiki.upsert_task` / `wiki.list_tasks_brief` / similar must call `wait_for_daemon_exit(wiki_path)` before tempdir teardown. Several of these tests do NOT call wiki APIs directly but still fail because some helper they share with the bg-launcher pattern starts a daemon. Plan-time investigation step: grep each failing test for wiki/daemon entry points and add the wait where needed.
