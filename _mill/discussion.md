...
- `plugins/mill/scripts/_status.py` — add `append_fork_fallback_log`, a timeline-style audit-append helper mirroring the existing `append_recovery_log` and `append_inferred_success_log`. Required by Decision `fork-fallback-status-marker`; see that decision for why `append_phase` cannot be used.
...
- `plugins/mill/unit_tests/` — a unit test for `append_fork_fallback_log` (extend the existing `_status` test file rather than adding a new one; see Testing).
...
- **Any Python behaviour change beyond the one new `_status` helper.** `millpy-implement.py`, `_agent_dispatch.py`, and `templates/implementer-brief.md` are all unchanged, and the brief is rendered identically for forked and cold dispatch. `_status.py` gains `append_fork_fallback_log` and nothing else — no existing function's behaviour changes. (This entry originally claimed `_status.py` was untouched too; review round 1's BLOCKING finding on `append_phase` overwriting `phase:` forced the new helper.)
...
- **Rationale:** the override point is defined as "follow it instead of the default `Agent()` call below" (`mill-go-base/SKILL.md:238-242`), which scopes the override to step 3 alone. The design then assumes a fork returns an `agentId` and delivers a completion `<task-notification>` exactly as a cold agent does, so step 4's `TaskOutput` liveness probe and step 6.5's warm `SendMessage` resume work unmodified.
- **That assumption is an inference, not a confirmed fact — state it as such in the variant file.** No spike or document confirms the fork dispatch's notification and `agentId` shape. `plugins/mill/docs/harness-tool-contracts.md` spiked the `Agent` tool generally, not `subagent_type: "fork"` specifically, and the base is careful to mark what *was* spiked — `mill-go-base/SKILL.md:182` flags its two-notification-shape claim as "confirmed by a live spike... not assumed". The only supporting evidence here is indirect: `mill-start/SKILL.md:184` shows `SendMessage` addressing a fork, which implies some addressable handle exists. This is the assumption a real `/mill-go2` run falsifies first, and if it is wrong, steps 4 and 6.5 need explicit fork-specific handling instead of inheriting the base's unchanged. A cheap pre-implementation spike — dispatch one trivial fork, observe whether an `agentId` and a `<task-notification>` arrive in the documented shapes — would retire the risk, and the plan may schedule one as its first card. The sibling task records the same assumption the same way, so both variants stay honest about it.
...
- **Decision:** the fork override applies to the initial implement dispatch (`### 1. Implement`, `mill-go-base/SKILL.md:553-572` — the heading is at `:553`, and `:572` is the line inside it that routes to the Agent-mode dispatch pattern) and to the transient one-retry re-dispatch. It does **not** apply to step 6.5.2's `--resume-incomplete` re-dispatch, which stays cold. Step 6.5.1's warm `SendMessage` resume is unaffected either way: it addresses the already-running agent by `agentId`, which works identically for a fork.
...
- **Decision:** no new config key. `mill-go2/SKILL.md` documents that its own driver-session model becomes the effective implementer model, and that `roles.implementer.model` (currently `sonnetmedium` in this hub) no longer applies under fork. The documentation must state that **effort** is lost alongside model: `_agent_dispatch.resolve_subagent_type` encodes the resolved alias's effort tier as a `subagent_type` suffix (`sonnetmedium` → `mill:mill-implementer-medium`), and each per-tier agent-definition file under `plugins/mill/agents/` pins a fixed `effort:` in its own frontmatter. A fork inherits the driver's effort, so both halves of the tier assignment are lost, not just the model half.
- **Tier count — do not copy the base's stale number.** `mill-go-base/SKILL.md:255` says "six per-tier agent-definition files" and enumerates only medium/high/max. That is superseded. `_agent_dispatch.EFFORT_TIERED_SUBAGENT_TYPES` (`_agent_dispatch.py:68`) is `{low, medium, high, xhigh, max}` — **5 tiers** — and `plugins/mill/agents/` holds **10 tiered files** (`mill-implementer-{low,medium,high,xhigh,max}.md` plus the five reviewer equivalents), alongside the two untiered base definitions. Prefer stating this generically in `mill-go2/SKILL.md` (e.g. "the per-tier agent-definition files under `plugins/mill/agents/`") so the variant carries no count that can go stale a second time. Correcting the base's own stale sentence is out of scope for this task and is not required by anything here.
...
- **Decision:** record the cold-fallback event on the fallback path only, emitting **both** a notification and a committed audit row immediately before the cold retry:
  `_notify.notify("<VARIANT_LABEL>.fork-fallback", f"implementer {batch_name}", slug=slug)` and
  `_status.append_fork_fallback_log(status_path, batch_name, _timestamp.now_utc_iso())`,
  followed by `git -C <worktree> add <status_path> && git -C <worktree> commit -m "<VARIANT_LABEL>: fork-fallback for implementer {batch_name}"`.
  The fork path itself gets no marker.
- **`append_fork_fallback_log` is a new helper this task adds** to `plugins/mill/scripts/_status.py`, mirroring the existing `append_recovery_log` (`:1067`) and `append_inferred_success_log` (`:1175`) byte-for-byte in shape: it lazily creates a dedicated `## Fork-fallback log` section on first call, appends one row per call inside the existing fenced block, never rewrites or drops a prior row, and writes its timestamp through `_yaml_writer.quote_scalar`. It must not touch the top-level `phase:` field.
- **Why not `_status.append_phase`** (the original form of this decision, corrected after review round 1): `append_phase` overwrites the top-level `phase:` yaml field (`_status.py:468`, documented at `:431`), which is the exact field mill-go-base's Entry phase-table lookup reads (`mill-go-base/SKILL.md:108-121`). A value of `fork-fallback-{batch_name}` matches none of the widened-matching literals or regexes at `SKILL.md:125-133` — the set is `{implementing, reviewing, fixing, self-resolved-verify-logic, holistic-approved}` plus `^approved-.*$`, `^reviewing-.*-r\d+$`, `^fixing-.*-r\d+$`, `^holistic-reviewing$` — so it falls through to the table's `any other` row, which surfaces and halts instead of resuming. A crash or interrupt in the window between the fallback commit and the cold retry's own next phase write would therefore make the task unresumable. Beyond the resume bug, the marker is an *audit* event and not a *routing state*: the batch is still `implementing` while the fallback happens, so overwriting `phase:` destroys live routing information. The base already pays that cost once — `self-resolved-verify-logic` overwrites `phase:` for an audit-only event and then needs six lines of `read_batches` disambiguation (`SKILL.md:148-153`) to reconstruct the routing it destroyed. The dedicated-section helpers exist precisely so later events do not repeat that.
- **Rejected fix (the reviewer's second option): adding `^fork-fallback-.*$` to the widened-matching regex set.** It would make the phase value recognized, but it is a *behavioural* edit to shared base machinery rather than the prose-only edit Decision `correct-why-not-fork-in-base` scopes; it would require defining a routing rule for the new phase; it would still destroy the real `phase:` value; and it would collide with the sibling task, which needs the same regex on the same list and would conflict on landing.
- **Cross-task note:** the sibling `mill-go2-fork-fixer`'s Decision `record-the-fallback` specifies `_status.append_phase(status_path, f"fork-fallback-fix-{scope}-r{N}", ...)` and has the identical defect for the identical reason — that value matches none of the widened set either. Whichever task lands first should add `append_fork_fallback_log`; the second should reuse it rather than re-adding it.
- **Rationale:** how often a forked implementer dies is the single most valuable measurement this experiment produces, so it needs both halves: `_notify` for live visibility during the run, and a committed status row for durability once the session ends. Forking is mill-go2's documented default, so a marker on the *success* path carries no information; only the fallback is worth recording. Both helpers are already used throughout the base — `_notify.notify("<VARIANT_LABEL>.holistic-fallback", …)` (`:1205`) is direct precedent for a fallback-event notification, and the free-form phase-name shape matches existing usage such as `f"approved-{batch_name}"` (`:805`). Committing the row immediately matches the base's own pattern for `self-resolved-verify-logic` and `self-resolved-terminal-dirt`. This is deliberately identical in shape to the sibling task `mill-go2-fork-fixer`'s Decision `record-the-fallback` (`mill-go2-fork-fixer/_mill/discussion.md:157-171`), so the two roles' fork-failure rates stay directly comparable and uniformly monitorable.
- **Naming divergence from the sibling, deliberate:** the fixer's marker is `f"fork-fallback-fix-{scope}-r{N}"` and carries a round number; this one is `f"fork-fallback-{batch_name}"` and does not. That is structurally justified rather than an oversight: per Decision `cold-fallback-on-dead-fork` at most one cold fallback can occur per batch for the implementer, whereas the fixer dispatches per review round and so needs `r{N}` to disambiguate. Adding a round number here would name a dimension that cannot vary.
- **Rejected:** (a) `_notify` only — lost once the session ends, which is exactly when the measurement is read; (b) status row only — no live signal during a run, and an unexplained divergence from the sibling task measuring the same thing; (c) marking both the fork and fallback paths — doubles the byte cost to record a non-event; (d) no marker at all — leaves the PoC's most interesting outcome invisible.
...
- **Cross-task note (this decision deliberately diverges from the sibling task).** `mill-go2-fork-fixer`'s Decision `no-base-edits` takes the opposite position, arguing that naming a variant in the base "violates the parameterization intent" that `test-mill-go-variants.py`'s `_check_parameterization_lock` protects. That rationale does not survive inspection and is not followed here: the lock bans exactly three *hardcoded literal families* from the base — `"mill-go: `, `_notify.notify("mill-go.`, and `[mill-go]` — and says nothing about prose. The base already names both variants in prose in its own frontmatter (`mill-go-base/SKILL.md:3`: "Loaded by /mill-go and /mill-go2, which bind the variant contract this skill reads"), so a prose cross-reference is established practice rather than a violation. Two consequences for whoever plans this task: the correction here is safe against the lock, and once it lands, `no-base-edits`'s assertion that "`Why not fork?`'s three disqualifiers remain accurate as written" goes stale. That staleness is documentation-only and cannot produce a merge conflict in either landing order, because the fixer task never edits `mill-go-base/SKILL.md`.
...
The task has two test surfaces with very different characters. The SKILL.md override is instructions an LLM orchestrator follows, so conformance assertions are the only executable verification available for it. The new `_status.append_fork_fallback_log` helper is ordinary Python and gets ordinary unit tests.

**`append_fork_fallback_log` — the genuine TDD candidate**, tested in the existing `plugins/mill/unit_tests/test-status.py` (extend it; it already owns `_status` coverage, and `append_recovery_log` / `append_inferred_success_log` tests there are the models to copy). Scenarios that must be covered:

- Lazy section creation: first call on a `status.md` with no `## Fork-fallback log` section creates it, with a properly terminated fenced block.
- Append-only on subsequent calls: a second call adds a row without rewriting or dropping the first.
- `phase:` is untouched. This is the assertion that encodes the BLOCKING finding from review round 1 and must not be dropped — read the top-level yaml before and after and assert `phase` is byte-identical.
- Timestamp is written through `_yaml_writer.quote_scalar`, matching the quoted form the sibling helpers produce.
- Malformed input: heading present but its fenced block missing or unterminated raises `ValueError`, matching `append_inferred_success_log`'s documented behaviour.

**Second test surface: `plugins/mill/unit_tests/test-mill-go-variants.py`** (extend, do not create a new file — it already owns the variant contract and its `main()` aggregates check functions into a single PASS/FAIL summary).
...
- Helper batch: `PYTHONPATH= uv run --project plugins/mill python plugins/mill/unit_tests/test-status.py`
- Override batch: `PYTHONPATH= uv run --project plugins/mill python plugins/mill/unit_tests/test-mill-go-variants.py`
...
- **Q:** (review r1, BLOCKING) The fallback marker used `_status.append_phase`, which overwrites the top-level `phase:` field with a value the Entry phase-gate does not recognize — how is the audit row recorded without breaking resume? **A:** [auto-pick] Add `_status.append_fork_fallback_log`, a dedicated-section audit-append helper mirroring `append_recovery_log` and `append_inferred_success_log`. **Why:** it records the event without touching `phase:`, so no routing state is destroyed and no base edit is needed. The rejected alternative — adding `^fork-fallback-.*$` to the base's widened-matching set — is a behavioural edit to shared machinery, still destroys the real phase value, and would collide with the sibling task on the same list. This puts one new Python function in scope; the task is no longer SKILL-only.
- **Q:** (review r1, NIT) Is the "six per-tier agent-definition files" figure correct? **A:** [auto-pick] No — 5 tiers, 10 tiered files; state it generically instead. **Why:** the base's own `:255` sentence is stale, and copying a count into the variant would let it go stale a second time.
- **Q:** (review r1, NIT) Is the fork's `agentId` / `<task-notification>` contract confirmed? **A:** [auto-pick] No — mark it as an inference and name it as the first thing a real run falsifies. **Why:** only the general `Agent` tool was spiked, not `subagent_type: "fork"`; the base marks its own spiked claims explicitly, so an unmarked assumption here would read as equally confirmed.
- **Q:** How is fork-versus-cold-fallback made visible in the audit trail? **A:** [auto-pick] Record on the cold-fallback path only, emitting both `_notify.notify` and a committed audit row. **Why:** forking is the documented default and carries no information; the fallback is the event worth auditing, and it needs both halves — `_notify` for live visibility, the committed row for durability after the session ends. Shape matches the sibling task's `record-the-fallback` so the two roles' fork-failure rates stay comparable. (Revised from a row-only decision after orchestrator review `_mill/discussion-orch-review.md` flagged the unexplained divergence from the sibling.)
