...
### unclassified-or-unknown-class-folds-into-blocking

- Decision: a finding whose bracket carries no class (`### [BLOCKING] <title>`) or an unrecognised class (`### [BLOCKING:perf]`) is a reviewer defect.
  It is handled by extending `count_unrecognized_severity_findings()` to the class axis: the finding is counted into the blocking bucket regardless of its stated severity, is exempt from the ceiling (it cannot be demoted, since its class is unknown), and a one-line warning goes to stderr.
  The review still lands and the round still counts.
- Rationale: this is the house pattern, already chosen deliberately for off-vocabulary *severity* labels for exactly this reason — fail conservative and keep going, so a malformed heading can never cause an auto-approve round to silently ignore a real finding.
  Extending it to classes costs one regex group and reuses the existing warning path.
- Rejected: hard-failing the review with `verdict: ERROR` and letting the existing ERROR-only retry re-dispatch (a literal reading of "it's an error", but it burns a full re-review round per malformed heading and, on a second consecutive occurrence, blocks the task outright);
  splitting the two cases so unknown class errors and missing class warns.
...
- Rationale: in-flight task worktrees have discussion-review files on disk with `verdict: GAPS_FOUND`, and several code paths re-read historical review files — `_review_plan.py` lines 100–123 (recovery) and 770–793 (resume), and `_nit_gate.py` line 89.
  A hard cut would make those paths throw `ReviewError` on any task that started before this change landed.
- Rejected: removing `GAPS_FOUND` from the accepted set outright.
...
- `_review_common.py:1661` — `parse_blocking_count()`, pattern `^###\s+\[<severity>\]\s+`, MULTILINE, case-sensitive.
  ...
  It must also return class information, not just a count, since `finalize_scope()` now needs per-finding class to apply the ceiling.
- `_review_common.py:1719` — `count_unrecognized_severity_findings()`, pattern `^###\s+\[([A-Z0-9-]+)\]\s+`.
  Needs the same optional group plus the unknown-class check.
...
- Unknown-class folding: `[BLOCKING:perf]` and bare `[BLOCKING]` each land in the blocking bucket and are exempt from demotion.
...
- Mixed-format document — real markdown headings for one severity and a fenced `findings:` YAML block for the other — with an unknown class hidden in whichever mechanism the known labels did not use.
  This is the property `count_unrecognized_severity_findings()` was written to hold;
...
- **Q:** What happens when a reviewer emits a finding with no class? **A:** It is a defect, not a supported fallback — but it is handled the same conservative way off-vocabulary severities already are (folded into blocking, warned to stderr, review still lands) rather than by failing the round.
