...
- Fix `_llm_claude._build_argv`: skip `--allowedTools` when the string is empty; always emit `--disallowedTools "Edit,Write,Bash,NotebookEdit"` for `run_bulk` and `run_tool_use`. `run_implementer` argv unchanged (it needs Edit/Write/Bash).
...
- Extend `plugins/mill/unit_tests/test-llm-claude.py` to assert: `run_bulk` argv omits `--allowedTools` and contains `--disallowedTools Edit,Write,Bash,NotebookEdit`; `run_tool_use` argv contains `--allowedTools Read,Grep,Glob` AND `--disallowedTools Edit,Write,Bash,NotebookEdit`; `run_implementer` argv contains `--allowedTools Read,Edit,Write,Bash,Grep,Glob,Skill` AND does NOT contain `--disallowedTools`.
...
- Decision: In `_llm_claude._build_argv`, change the `--allowedTools <allowed_tools>` line to: `*(["--allowedTools", allowed_tools] if allowed_tools else [])`, then append `["--disallowedTools", "Edit,Write,Bash,NotebookEdit"]` only for callers where `allowed_tools` does NOT include `Edit`/`Write`/`Bash`/`NotebookEdit`. Concretely: add a new keyword-only `add_disallow: bool = False` parameter to `_build_argv`; pass `add_disallow=True` from `run_bulk` and `run_tool_use`, pass `add_disallow=False` (default) from `run_implementer`.
...
| `plugins/mill/scripts/_llm_claude.py` | Modify `_build_argv` (add `add_disallow: bool = False` kw param); update `run_bulk` / `run_tool_use` call sites to pass `add_disallow=True`; conditional `--allowedTools` emission when value is empty |
...
- **Q:** How to fix `--allowedTools ""`? **A:** [auto-pick] Drop the flag when value is empty; always add `--disallowedTools Edit,Write,Bash,NotebookEdit` for bulk + tool-use via a new `add_disallow=True` keyword. `run_implementer` argv unchanged. **Why:** Defense in depth — both allow-list and deny-list semantics are explicit. Proposal-aligned tool set. `--tools ""` interacts unpredictably with `--allowedTools` in tool-use mode.
