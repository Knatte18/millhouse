...
The rule applies to prose paragraphs; it does not apply inside table cells or other single-line-constrained contexts (e.g. blockquote lines, which already require a per-line `>` prefix under CommonMark) — those must stay on one line regardless of sentence count.
