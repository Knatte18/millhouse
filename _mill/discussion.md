...
- Existing tests in `test-fix-finalize.py`, `test-review-finalize.py`, and `test-mill-finalize-dispatch.py` cover the success and other-error paths for the finalize stage across the various CLIs — confirm the new missing-file test doesn't duplicate or conflict with any existing fixture in those files (a quick grep for `agent_output`/`agent-output` in each before adding the new test is sufficient; no broad refactor of those files is in scope).
