...
- No `verify:` command is meaningful for a plan batch here — mill-plan should set `verify: null` for these batches. This repo (a `uv`/Python project) has no markdown-lint gate configured, so there is no equivalent command to substitute; this is a fixed expectation, not something the plan writer needs to re-investigate.
