...
- `mill-plan/SKILL.md`, Step 1.5 fix table: rewrite the `verify-full-suite` row so its remedy directs to the check's own per-runner `message` field (already runner-correct — `-run` for Go, `--filter` for dotnet, `-k`/`--only` for run-all.py/pytest) instead of hardcoding the Python-only `-k`/`--only` phrasing; add routing for `batch: None` findings to the `## Shared Decisions` escape hatch.
