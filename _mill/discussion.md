...
- Decision: for both mill-go-base loops, when `auto_approve_on_cap: true` and the round-cap is exhausted with `REQUEST_CHANGES` still outstanding, run the same terminal actions the loop already runs on a normal `APPROVE`: per-batch → set batch state → `approved`, `_status.append_phase(status_path, f"approved-{batch_name}", ...)`; holistic → `_status.append_phase(status_path, "holistic-approved", ...)`. Each commit message notes the config-driven override (parallel to the existing `" (min_rounds/demoted-predicate not satisfied by round cap)"` suffix already used for the APPROVE-path implicit-approve-at-cap case), and each keeps its existing `_notify.notify(...)` call so the auto-approval is still observable, not silent.
...
- **mill-go-base** (`plugins/mill/skills/mill-go-base/SKILL.md`, per-batch Code Review section):
  ...
  - Step 5 "Max-rounds exhaustion" is the halt to wire: "After `roles.code-review.batch.rounds` rounds without APPROVE," sets batch state → `blocked`, halts. This fires only after step 4's fixer has already run for the last round.
...
- **Manual/integration verification** (documented, not necessarily automated in this task): a hub with `roles.code-review.holistic.rounds` set low (e.g. 1) and `auto_approve_on_cap: true`, run against a change that reliably draws a `REQUEST_CHANGES` verdict, should reach Handoff instead of a `blocked` status after the single round's fix pass — vs. the same setup with the flag absent/false, which should still halt as today. This is the acceptance scenario mill-plan should design its own batches' `## Batch Tests` around, per this repo's plan-quality conventions.
