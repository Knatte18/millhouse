...
- `plugins/mill/scripts/_parent_branch.py`: add `resolve_for_codeguide(hub_root) -> str | None` — a non-interactive, exception-swallowing wrapper around the existing `resolve()` that returns `None` when there's no mill worktree / no recorded parent, instead of raising or prompting.
- `plugins/mill/skills/git-commit/SKILL.md` Step 2: before invoking `codeguide-update`, call `resolve_for_codeguide`; if it returns a branch name, pass `--parent <branch>`; otherwise invoke exactly as today (no-arg).
...
- Rationale: `codeguide-update`'s `$ARGUMENTS` was never meant to carry a full git command — only a scope token/range. The "git diff" prefix was always wrong, independent of the ref-dispatch fix above. `resolve_scope.py` does not currently parse a `<ref>..HEAD` range syntax as a single token; the fix passes the checkpoint branch name as `$CHK..HEAD`, which — combined with the broadened ref dispatch — needs `resolve_scope.py` to also strip a trailing `..HEAD` suffix from a single token before the ref-resolution check (a token like `mill-checkpoint-foo..HEAD` doesn't resolve as-is via `rev-parse --verify`, but `mill-checkpoint-foo` does). See Technical context for the exact parsing rule.
...
- Decision: add `resolve_for_codeguide(hub_root: Path) -> str | None` to `plugins/mill/scripts/_parent_branch.py`. It resolves the status.md path from `hub_root` (`hub_root / "_mill" / "status.md"`, matching the config-driven path used elsewhere — `git-commit`'s SKILL.md instructions will pass the already-resolved `status_path` rather than recomputing it), calls `resolve(status_path, interactive=False)`, and returns `None` (never raises) on `ParentBranchError` or `FileNotFoundError`. Returns the branch name string otherwise.
- Rationale: `git-commit`'s SKILL.md is prose instructions for an assistant, not executable code — the "check status.md, resolve parent, swallow errors" logic needs to live in a unit-testable function, not be re-derived from scratch as inline prose every time the skill runs.
...
- **`resolve_scope.py`'s current dispatch** (see module docstring, lines 1–39): no-arg → time-form regex → `HEAD~`/`HEAD`/hex-SHA regex → explicit-paths fallback. The fix changes step 3 from a narrow regex to a live `git rev-parse --verify --quiet <token>^{commit}` check, and needs to handle a `<ref>..HEAD` suffix (used by the mill-merge-in caller) by checking whether the token contains `..` — if so, split at the last `..`, verify the left-hand side resolves as a ref, and use it as the head-rev token (mirroring what `_head_rev_scope` already does internally — it already builds `f"{token}..HEAD"` from a bare token, so a caller-supplied `<ref>..HEAD` needs the trailing `..HEAD` stripped before reaching `_head_rev_scope`, not appended twice).
