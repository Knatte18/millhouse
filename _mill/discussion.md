...
- `plugins/mill/scripts/_plan_validate.py`
  ...
  - The module docstring's check list (near the top, the `verify-full-suite —` line) and the `_check_verify_full_suite` docstring both describe the dotnet rule as "dotnet test without --filter" and must be updated to the new rule.
...
- `plugins/mill/unit_tests/test-plan-validate.py` (TDD candidate for #1118):
  ...
  - New dirty cases: bare `dotnet test`; `dotnet test MySolution.sln`; `dotnet test --nologo -c Release` (flags only, the `Release` value must not count as a target); `dotnet build X.sln && dotnet test` (flagged per segment).
...
- **Q:** Which `dotnet test` forms should `verify-full-suite` flag? **A:** [auto-pick] Only when there is no `--filter` AND either no positional target or a `.sln`/`.slnx` target, evaluated per shell segment. **Why:** a project target is already scoped; a solution target or bare invocation is the true full-suite run.
