...
- **Creates:** none
- **Deletes:** none
- **Requirements:** Keep module-level `CAS_RETRIES` declared (existing constant; bump to `5` so the existing integration-test pattern reads from the constant rather than a local literal) but the new mutating methods do NOT retry on it — the V3 `base_hash` CAS path is gone per card 5, and cross-host conflicts are handled inside the daemon's `commit_push` rebase retry. The constant lingers only to make the `test-wiki-e2e.py` migration trivial (`from wiki._client import CAS_RETRIES`) and to leave a single place to revisit if true client-driven CAS ever returns. Delete public functions `read` and `write_commit_push`. Add public functions matching the External interface section in the batch-scope above — every function takes `wiki_path: Path` as required first positional argument, mirroring the existing `read`/`write_commit_push` shape:
