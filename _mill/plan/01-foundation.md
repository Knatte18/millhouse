...
- **Creates:** none
- **Deletes:** none
- **Requirements:** Add a new test function `test_wiki_server_on_stop_closes_log_handlers` (registered in the file's main-runner list, following the existing PASS-line convention used by sibling tests in the file) that: (a) creates a tempdir wiki_path via `_test_helpers.safe_temp_dir()`, (b) instantiates `WikiServer(wiki_path, idle_timeout=1)` directly (does NOT call `.run()` -- the test exercises `on_stop` in isolation), (c) selects the test-instance's own handler defensively from the shared logger to avoid collisions with stale handlers left behind by earlier tests that also instantiated `WikiServer` in the same runner process -- use `our_handlers = [h for h in wiki_server._log.handlers if isinstance(h, logging.handlers.RotatingFileHandler) and Path(h.baseFilename).resolve() == (wiki_path / ".wiki-daemon.log").resolve()]` and assert `len(our_handlers) == 1` before capturing `handler = our_handlers[0]`, (d) calls `wiki_server.on_stop()`, (e) asserts that the captured handler's `stream` is None or `stream.closed` is True (RotatingFileHandler clears `stream` to None after close on some Python versions; both states are accepted) AND that `wiki_server._log.handlers` no longer contains the captured handler (use `handler not in wiki_server._log.handlers` rather than `== []` since other tests in the same process may have added handlers to the shared `"wiki-server"` logger). Print `PASS: WikiServer.on_stop closes log handlers and removes them from logger` on success. Do NOT use `subprocess` to spawn a real daemon -- in-process is sufficient and fast. No allowlist edit to `test-no-direct-rmtree.py` needed (`test-wiki-daemon.py` is already on the list).
