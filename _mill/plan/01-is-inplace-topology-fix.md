...
- **Requirements:**
  ...
  - `hub_root` is a bare, non-git `tmp_path / "hub"` directory. `_resolve_inplace_mode` (`millpy-cleanup.py:423`) calls `_paths.resolve_worktrees_dir(cfg, hub_root)` *before* the stale-worktree check, and `resolve_worktrees_dir` itself unconditionally calls `_paths.resolve_main_worktree_root(hub_root)` internally (`_paths.py:271`) — against a non-git directory this raises `_pygit2_util.GitOpsError`, which `_paths.resolve_main_worktree_root` converts to `SystemExit` (`_paths.py:246-248`), and `test-cleanup.py`'s `main()` only catches `AssertionError` (line 1790), so an unpatched call here crashes the whole script. Both Case A and Case B must therefore also patch `mill_cleanup._paths.resolve_main_worktree_root` with `return_value=hub_root` (mirroring the existing sibling test's pattern at test-cleanup.py:705) — this patch is independent of, and in addition to, the `mill_cleanup._inplace.resolve_main_worktree_root` patch below, since `millpy-cleanup.py` imports `_paths` and `_inplace` as two separate module references.
  - Case A (in-place): patch `mill_cleanup._paths.resolve_main_worktree_root` with `return_value=hub_root` AND `mill_cleanup._inplace.resolve_main_worktree_root` with `return_value=hub_root`; call `_resolve_inplace_mode(record, hub_root, wiki_path, cfg={})`; assert the result equals `("inplace", "impl/my-task")`.
  - Case B (worktree): patch `mill_cleanup._paths.resolve_main_worktree_root` with `return_value=hub_root` AND `mill_cleanup._inplace.resolve_main_worktree_root` with `return_value=<a tmp path distinct from hub_root>`; call `_resolve_inplace_mode(record, hub_root, wiki_path, cfg={})` again; assert the result equals `("worktree", "")`. Add a one-line comment noting this exercises `millpy-cleanup.py:437`'s fallback, which was unreachable before this task's topology rewrite (the old `is_inplace` recomputed the same path-existence check the caller had already confirmed `False`, so it always returned `True` at this call site).
