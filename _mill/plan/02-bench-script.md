...
- **Requirements:**
  ...
  **Artefact construction** — read fixtures once before the reviewer loop; call `render_prompt` inside the reviewer loop (because `reviewer_model` changes per reviewer):
  ...
  - DISCUSSION artefact: read `FIXTURES / "sample-discussion.md"`; build `discussion_artefact_section` as `"Evaluate the discussion document below.\n\n--- FILE: _mill/discussion.md ---\n" + discussion_text`. For each reviewer, render via `_review_common.render_prompt("review-discussion", task_title="Sample render module refactor", round=1, reviewer_model=reviewer_name, tool_rule=TOOL_RULE_BULK, artefact_section=discussion_artefact_section, constraints="(none)")`. Note: render inside the reviewer loop — `reviewer_model` changes per reviewer.
  - PLAN artefact: read `FIXTURES / "sample-plan" / "00-overview.md"` and `FIXTURES / "sample-plan" / "01-core.md"`; build `plan_artefact_section` concatenating `"Evaluate the plan below. All plan files and referenced source files are inlined.\n\n## Files included\n\n- `_mill/plan/00-overview.md`\n- `_mill/plan/01-core.md`\n\n"` followed by `"--- FILE: _mill/plan/00-overview.md ---\n" + overview_text + "\n\n--- FILE: _mill/plan/01-core.md ---\n" + batch_text`. For each reviewer, render via `_review_common.render_prompt("review-plan-holistic", ..., artefact_section=plan_artefact_section, ...)`. Note: render inside the reviewer loop — `reviewer_model` changes per reviewer.
  - CODE artefact: plan files + `FIXTURES / "sample-code.py"`; build `code_artefact_section` with all three files using the same `--- FILE: path ---` delimiter pattern, using path label `plugins/mill/scripts/_render.py` for `sample-code.py` (it represents the implemented `_render.py`). Add `"## Files included"` manifest listing all three paths. For each reviewer, render via `_review_common.render_prompt("review-code-holistic", ..., artefact_section=code_artefact_section, ...)`. Note: render inside the reviewer loop — `reviewer_model` changes per reviewer.
  ...
  **Per-run metrics**:
  ...
  - On `TimeoutExpired` / timeout: verdict `"TIMEOUT"`, findings 0, format_ok False (matches Shared Decision `timeout-300s`). On other exceptions: verdict `"ERROR"`, findings 0, format_ok False.
...
`verify:` runs the bench script with `--reviewers test_stub --types discussion` and checks exit code 0. The following are manual checks after the run: (1) results table has one row, (2) the row shows verdict APPROVE, findings 0, fmt True. These cannot be automated by the shell command alone. This exercises argument parsing, prompt rendering, reviewer dispatch, metric collection, and output formatting without any real LLM call.
