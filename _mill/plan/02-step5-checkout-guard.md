...
- **Requirements:**
  ...
  - Create and check out a new branch (e.g. `task/guard-test`) off `main`, add both a `_mill/status.md` file AND a `feature.txt` file (the intended production file — mirrors the flat-hub scenario's own `feature.txt` survival assertion at line 777-778), commit both together, then check back out to `main`. `main` now has no `_mill/` anywhere in its tree — the exact #736 case ("parent tracks nothing at `task_dir`"). `feature.txt` is required so the batch's later commit assertion below has a real staged change to land after `_mill/` is unstaged — a squash whose only diff is `_mill/status.md` would leave nothing staged once that path is reset out, and `git commit` would fail with "nothing to commit".
  - Run `git -C <repo> merge --squash task/guard-test` on `main` (mirrors line 712's `merge --squash` call). At this point both `_mill/status.md` and `feature.txt` are staged as new files (verified: a bare `git checkout -- "_mill"` immediately after squash, with no prior `reset`, restores from the *index* where the path IS present and succeeds — it does NOT reproduce the bug on its own).
  - **Repro:** run `git -C <repo> reset -q HEAD -- "_mill"` (`check=False`; assert `returncode == 0` — unstages `_mill/status.md`, matching HEAD's absence there), THEN run a bare, unguarded `git -C <repo> checkout -- "_mill"` (`check=False`); assert `returncode != 0` and that combined stdout+stderr contains `did not match any file` — proves this fixture reproduces #736 before proving the fix, mirroring both the real Step 5 command ordering (line 177 reset, then line 178 checkout) and the existing `#648` repro sub-step's own two-command structure (lines 721-728). This reset+bare-checkout pair touches neither `feature.txt`'s staged state nor the working tree on the checkout's failure, so no cleanup step is needed before the next assertion — `feature.txt` remains staged and `_mill/status.md` remains present on disk but now untracked.
  - **Fix:** run `git -C <repo> reset -q HEAD -- "_mill"` again (assert `returncode == 0` — already unstaged by the repro step above, so this is a true no-op, matching the `step5-checkout-guard` Shared Decision's verified reset behavior), then run the corrected guarded form exactly as Card 6 writes it in the skill — invoke it via `subprocess.run("git checkout -- _mill 2>/dev/null || true", shell=True, cwd=str(repo), capture_output=True, text=True)` (the `||` guard is shell syntax, so this one call needs `shell=True` unlike every other `_run()` call in this file, which passes an argv list) — and assert `returncode == 0`.
  - Commit the result: `git -C <repo> commit -m "Demo guarded merge"`; assert the commit succeeds (mirrors line 757's commit step for the `#648` scenario) and that `git -C <repo> show --stat HEAD` does NOT mention `_mill` — proving the guard both avoids halting AND still keeps `_mill/status.md` out of the squash commit, matching Step 5's actual purpose. `_mill/status.md` remains on disk as an untracked file (not committed, not deleted) — no assertion needed on that, it is incidental to what this scenario tests.
