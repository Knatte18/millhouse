...
- **Requirements:** In the `roles:` block, insert `min_rounds: 1` immediately after each of these 4 `rounds:` lines (leave every other `rounds:` line, including `plan-review.batch`'s at line 151, untouched — no `min_rounds` under `plan-review.batch`, per the discussion's `review-loop-min-rounds-and-demoted-predicate` Decision):
  ...
  **`mill-config.yaml` bootstrap justification:** `_plan_validate.py`'s `_check_wiki_config_mutation` does an exact-string membership test (`"mill-config.yaml" in writes`) against each card's raw `Edits:`/`Creates:` ref tokens, not a basename match — this card's `Edits:` token is the full path `plugins/mill/templates/mill-config.yaml`, which never equals the literal string `"mill-config.yaml"`, so the check does not actually fire on this card. Card 10 (bare `Edits: mill-config.yaml`) is the one that triggers it; this card's own bootstrap justification is documented here defensively, for consistency with Card 10 and in case the check is ever changed to a basename match — see Card 10's Requirements for the justification that governs the check's actual firing. Substantively: this is a key *addition* with default `1`, read via `.get("min_rounds", 1)` at every consuming site (Cards 11-13, shipping in this same plan) — no existing code path reads or depends on the key's absence, so the field is inert to every session that has not yet picked up Cards 11-13's SKILL.md changes, and becomes live only once those land, in the same plan. Safe mid-flight for any concurrently-running mill-start/mill-plan/mill-go session on another task worktree sharing this hub's `mill-config.yaml`: an old (pre-this-plan) session's `.get("rounds", ...)`-style reads never inspect `min_rounds` and are unaffected by its presence.
...
- **Requirements:** Mirror Card 9's 4 insertions in this repo's own hub `mill-config.yaml` (no inline comments in this file — insert bare `min_rounds: 1` lines only, matching this file's terser existing style):
  ...
  **This is the card the `wiki-config-mutation` validator check actually fires on** (its `Edits:` token is the bare `mill-config.yaml`, an exact match against `_check_wiki_config_mutation`'s `"mill-config.yaml" in writes` test — Card 9's own `Edits:` token, a longer full path under `templates/`, does not match and does not trigger the check). The substantive bootstrap justification is the same key-addition-with-safe-default reasoning given in Card 9's Requirements — record it once, citing this card, when applying the `wiki-config-mutation` validator skip.
