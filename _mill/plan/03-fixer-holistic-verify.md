...
- **Creates:** none
- **Deletes:** none
- **Requirements:** In `millpy-fix.py`'s holistic finalize path (where it currently passes `verify_cmd = None`), derive a verify command instead: resolve `plan_dir = _paths.resolve_task_path(_paths.resolve_hub_path(), "_mill/plan/")`, call `_plan_dag.iter_batch_verifies(plan_dir)` (returns non-null `(batch_name, verify_cmd)` pairs in DAG order), take the `verify_cmd` values, and join them with ` && ` to form the holistic `verify_cmd`. If the list is empty, keep `verify_cmd = None` (no gate). Pass the derived command to the existing `finalize_from_output` / `_run_verify_gate` flow exactly as the per-batch path does, so a non-zero combined exit demotes a self-reported `success` to `stuck/verify`. Join contract: `iter_batch_verifies` already filters null/missing verifies, so no dangling ` && ` is possible. Each survivor keeps its own `PYTHONPATH= ` prefix. ASCII-only.
