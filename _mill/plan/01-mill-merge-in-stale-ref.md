...
- **Requirements:**
  ...
  In the "## No-op guarantee" section, keep the existing sentence "When step 1 returns empty, this skill touches nothing: no checkpoint, no verify, no codeguide-update, no output side effects." verbatim (it remains true: the added `git fetch` is read-only and prints nothing), and append one new sentence immediately after it: "Step 1 always performs a network fetch (`git fetch origin <parent-branch>`) even when the result is a no-op; this is a deliberate cost of correctly detecting a stale local ref and is the only exception to the "touches nothing" guarantee." Do not change the second sentence of that section ("`mill-merge` depends on this...").
...
- **Requirements:**
  ...
  2. Immediately after the "Task branch + worktree with a single commit ahead of main" block (after line 221, `worktree` now one commit ahead of `hub`'s `main`), push the *worktree's* view of `main`'s current tip to the new origin so `origin/main` matches what the no-op check will compare against: no push needed here since `main` itself has not advanced past what was already pushed in step 1 — only confirm no additional push is required (the worktree's own branch, `test/<slug>`, is never pushed to origin; only `main` is).
