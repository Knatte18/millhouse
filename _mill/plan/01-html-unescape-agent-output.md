...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Add `import html` to `millpy-review-code.py`'s imports. In the `--stage finalize` branch (currently `raw_text = agent_output_path.read_text(encoding="utf-8")`), wrap the read: `raw_text = html.unescape(agent_output_path.read_text(encoding="utf-8"))`. Add a new test function `test_review_code_finalize_unescapes_html_entities` to `test-review-finalize.py`, modeled on the existing `test_review_code_finalize_no_prepare` mock-module setup (same `mock_modules` dict shape, same `importlib.util.spec_from_file_location` load pattern): write HTML-escaped text (e.g. `"Q&amp;A send &lt;guid&gt;"`) to `output_file`; mock `_review_code.finalize` with `unittest.mock.MagicMock(return_value=mock_result)` as the existing tests do; invoke `millpy_review_code.main(["--stage", "finalize", "--round", "1", "--agent-output", str(output_file)])`. **The unescape check must be the function's return value, not a bare `assert` inside the `try` block** — `test_review_code_finalize_no_prepare`'s pattern wraps the whole body in `except Exception: return True`, and its inner `except AssertionError` only special-cases the literal "prepare() must not be called" message before re-raising everything else, so any other `AssertionError` raised inside that `try` is swallowed by the outer handler and silently converted to a passing `True` — a bare `assert` on the unescaped text would never be able to fail. Instead, after the `main()` call, read `_review_code.finalize.call_args.args[2]` (the `raw_text` positional) and make the function's return value the boolean comparison itself, e.g. `return _review_code.finalize.call_args.args[2] == "Q&A send <guid>"` (or capture the mock's call args to a variable and return the comparison from outside the inner `try`, after the outer `except Exception` can no longer intercept it) — do not let a false comparison result manifest only as a swallowed `AssertionError`.
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Add `import html` to `millpy-review-discussion.py`'s imports. In the `--stage finalize` branch (currently `raw_text = agent_output_path.read_text(encoding="utf-8")`), wrap the read: `raw_text = html.unescape(agent_output_path.read_text(encoding="utf-8"))`. Add a new test function `test_review_discussion_finalize_unescapes_html_entities` to `test-review-finalize.py`, modeled on the existing `test_review_discussion_finalize_no_prepare` mock-module setup: write HTML-escaped text to `output_file`; mock `_review_discussion.finalize`; invoke `millpy_review_discussion.main(["--stage", "finalize", "--round", "1", "--agent-output", str(output_file)])`. **The unescape check must be the function's return value, not a bare `assert` inside the `try` block** — the existing model function's `except Exception: return True` outer handler swallows any `AssertionError` other than the literal "prepare() must not be called" one, silently converting a failed unescape check into a passing `True`. Instead, capture `_review_discussion.finalize.call_args.args[2]` (the `raw_text` positional per `finalize(cfg, slug, raw_text, round_n=..., ...)`) and make the function's return value the boolean comparison against the expected unescaped string, structured so a mismatch actually surfaces as `False` rather than being absorbed by the outer exception handler.
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Add `import html` to `millpy-review-plan.py`'s imports. In the `--stage finalize` branch (currently `raw_text = agent_output_path.read_text(encoding="utf-8")`), wrap the read: `raw_text = html.unescape(agent_output_path.read_text(encoding="utf-8"))`. Add a new test function `test_review_plan_finalize_unescapes_html_entities` to `test-review-finalize.py`, modeled on the existing `test_review_plan_finalize_no_prepare` mock-module setup: write HTML-escaped text to `output_file`; mock `_review_plan.finalize`; invoke `millpy_review_plan.main(["--stage", "finalize", "--round", "1", "--agent-output", str(output_file)])`. **The unescape check must be the function's return value, not a bare `assert` inside the `try` block** — the existing model function's `except Exception: return True` outer handler swallows any `AssertionError` other than the literal "prepare() must not be called" one, silently converting a failed unescape check into a passing `True`. Instead, capture `_review_plan.finalize.call_args.args[2]` (the `raw_text` positional per `finalize(cfg, slug, raw_text, scope=None, round_n=..., ...)`) and make the function's return value the boolean comparison against the expected unescaped string, structured so a mismatch actually surfaces as `False` rather than being absorbed by the outer exception handler.
