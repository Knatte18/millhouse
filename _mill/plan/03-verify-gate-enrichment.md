...
- **Requirements:**
  ...
  Update the function's docstring: it currently says "On non-zero return code, returns a stuck dict with stuck_type="verify" and reason set to the last 2000 characters of stdout+stderr" — this goes stale once the marker + up-to-20-extracted-lines enrichment ships. Reword to describe the new behavior: on truncation (output over 2000 chars), `reason` is an omitted-content marker (byte count, plus up to 20 extracted earlier-failure summary lines when any are found) followed by the last 2000 characters; under 2000 chars, `reason` is the stripped output unchanged.
