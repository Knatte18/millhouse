...
- **Requirements:**
  ...
  - **Divergence-halt sub-scenario** — the parent worktree has a local-only commit not present on `origin/<parent_branch>` (an out-of-band state); the fast-forward must fail loudly instead of discarding it.
    ...
    2. On `parent_ff_halt`, commit a second file directly to local `main` WITHOUT pushing it — `parent_ff_halt`'s local `main` is now ahead of `origin_ff_halt/main` by one unpushed commit.
    3. Capture `local_before = git -C parent_ff_halt rev-parse main` and the working tree's file listing before the next step.
