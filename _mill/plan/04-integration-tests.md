...
- **Requirements:**
  ...
  - **Repro (old, buggy target) then fix (new, correct target)** — mirrors this file's existing `#648`/`#736` repro-then-fix pattern.
    ...
    3. Create `mill-checkpoint-demo` on `parent_rb` pointing at a THIRD, unrelated commit — check out a throwaway branch, commit an unrelated file, then `git -C parent_rb branch -f mill-checkpoint-demo <that-commit's-sha>` — representing the child worktree's own pre-merge-in history, which per the #824 bug report is unrelated to the parent's state. Capture `checkpoint_sha`. Check back out to `main` on `parent_rb` before the next step (mirroring Card 9's and Card 12's own "check back out to main" convention) — without this, steps 4-6's `reset --hard`/`rev-parse HEAD` operate on the throwaway branch instead of `main`, the parent worktree's actual branch in production, and the assertions would pass trivially regardless of which rollback target is used.
    4. **Repro:** run the OLD rollback command, `git -C parent_rb reset --hard mill-checkpoint-demo`. Assert `git -C parent_rb rev-parse HEAD` now equals `checkpoint_sha` — proving the old command lands the parent worktree's `main` on unrelated child history (the bug).
...
- **Requirements:**
  ...
  - **Sub-scenario (a): torn-down parent (archive tag present, chain-walk resolves).**
    ...
    2. Commit a cleanup commit on `test/parent-task` locally (do NOT push it — the branch is already deleted on origin, matching the real archive flow where the cleanup commit happens right before tagging): add a status file at the fixture-internal path _mill/status.md (no backticks — this is fixture content the test writes at runtime, not a project source file this card reads) with content `"```yaml\nphase: done\ntask: Parent task\nparent: main\n```\n"` (the fence is REQUIRED — `_parse_parent_from_yaml_text`, batch 1 Card 1, only enters `in_block` after a line exactly equal to `` ```yaml `` and reads `parent:`/`slug:` rows only while in that block; unfenced content parses as if `parent:` were entirely absent, exactly as `_setup_trio`'s and `_setup_nested_hub_scenario`'s existing status.md fixtures in this same file already wrap their own content), commit it, THEN append one more trivial commit ("chore: pre-merge cleanup" style, mirroring the real `_mill/` removal semantics is not required here — the test only needs the tag to point one commit AFTER the status.md-bearing commit). Tag the tip: `git -C repo_dead tag archive/parent-task test/parent-task`.
...
- **Requirements:**
  ...
  - Build `test/task-c`: branch off `main`, add a status file at the fixture-internal path _mill/status.md (no backticks — fixture content, not a project source file) with `"```yaml\nphase: done\ntask: Task C\nparent: release/1.0\n```\n"` (fence required — see Card 11's fixture-fence note; unfenced content parses as `parent:` absent), commit, add one more trivial commit on top, tag `archive/task-c` at the tip. Do not push `test/task-c` to origin (it is dead — same "never pushed" or "torn down" shape either way; the chain-walk only cares that `check_liveness` on it returns `False`, and it is not pushed here so that is guaranteed).
  - Build `test/task-b`: branch off `main`, add a status file at the fixture-internal path _mill/status.md (no backticks — fixture content, not a project source file) with `"```yaml\nphase: done\ntask: Task B\nparent: test/task-c\n```\n"` (fence required, same reason) (its parent is the STILL-DEAD `test/task-c`, not yet the live branch), commit, add one more trivial commit on top, tag `archive/task-b` at the tip. Do not push `test/task-b` to origin.
...
- **Requirements:**
  ...
  - Build two dead branches that point at each other, forming a cycle with no live endpoint ever reachable:
    - `test/cycle-x`: branch off `main`, add a status file at the fixture-internal path _mill/status.md (no backticks — fixture content, not a project source file) with `"```yaml\nphase: done\ntask: Cycle X\nparent: test/cycle-y\n```\n"` (fence required, same reason as Card 11), commit, add one more trivial commit on top, tag `archive/cycle-x` at the tip. Do not push.
    - `test/cycle-y`: branch off `main`, add a status file at the fixture-internal path _mill/status.md (no backticks — fixture content, not a project source file) with `"```yaml\nphase: done\ntask: Cycle Y\nparent: test/cycle-x\n```\n"` (fence required, same reason), commit, add one more trivial commit on top, tag `archive/cycle-y` at the tip. Do not push.
...
- **Requirements:**
  ...
  - Build `test/legacy-task`: branch off `main`, add the status file at the LEGACY fixture-internal path task/status.md (no backticks — fixture content, not a project source file; NOT the _mill/status.md path used by the other cards in this batch) with content `"```yaml\nphase: done\ntask: Legacy task\nparent: main\n```\n"` (fence required — see Card 11's fixture-fence note), commit, add one more trivial commit on top, tag `archive/legacy-task` at the tip. Do not push.
