...
- **Requirements:** In `finalize_scope` (def at line 2290), restructure the body currently at lines 2339-2352:
  ...
  ```
  so that `blocking_count`/`nit_count` are computed and the verdict is recomputed BEFORE `write_review_file` runs (moved up, values unchanged), a local `original_verdict` retains `parse_verdict`'s pre-recompute result, a local `demoted_any` records whether `apply_blocking_ceiling` actually demoted at least one finding THIS call, and — only when `demoted_any` is `True` AND the recomputed `verdict` differs from `original_verdict` — call `rewrite_verdict_token(raw_text, verdict)` and assign its result back to `raw_text` before `write_review_file` runs. Equivalent target shape:
  ```
      raw_text = apply_actual_model_override(raw_text, actual_model)
  ...
  ```
  **Why `demoted_any`, not `blocking_classes is not None`:** `resolve_blocking_classes` never returns `None` and every production call site already passes a non-`None` value (per this function's own docstring: "every production call site passes this explicitly"), so a `blocking_classes is not None` guard alone does NOT exclude the case this batch is explicitly scoped to leave untouched — "a reviewer stating the wrong verdict token for its own finding count," unrelated to any ceiling demotion. `demoted_any` is `True` only when `apply_blocking_ceiling` this call actually flipped at least one finding's `severity` from `BLOCKING` to `NIT` (setting `Finding.demoted = True` on it) — checking it AFTER `apply_blocking_ceiling` and BEFORE any re-read from disk means it can only be `True` from a live demotion this call performed, never from a stale `**Demoted-from:** BLOCKING` marker already present in `raw_text` from a prior round (freshly-authored reviewer output has no such marker; `extract_findings` only sets `demoted=True` on first extraction when a ceiling later demotes it, or on re-read of already-marked text — see Card 12's Requirements in `03-review-loop-min-rounds.md` for the same on-disk-marker distinction applied to a different call site). Combined with `verdict != original_verdict`, the rewrite now fires only when a demotion this call actually changed the recomputed verdict — never for an unrelated pre-existing mismatch between the reviewer's stated verdict and its own finding count. Do not change any other line in the function (the final `return` dict, the `effective_scope` computation, and the docstring's Args/Returns sections stay as-is except adding one sentence to the docstring noting the new verdict-token rewrite, mirroring how the existing docstring already documents the demoted-findings rewrite).
...
- **Requirements:** Extend `test-review-class-taxonomy.py`, using the existing `_finalize` helper (line 36) and `_verdict_yaml` helper (line 81) already in the file. Add a new fixture helper `_verdict_section(verdict: str = "REQUEST_CHANGES") -> str` returning a 2-line `## Verdict` body matching `review-output.schema.md`'s `### \`## Verdict\`` contract (heading, blank line, verdict token line, one-sentence summary line) — e.g. `f"## Verdict\n\n{verdict}\n<summary>\n"` — placed near the other `_verdict_yaml`/`_heading`/`_yaml_block` fixture helpers. Add new test functions (following this file's existing `def test_*() -> bool:` + `TESTS` list + docstring-comment-banner convention, not `test-review-plan-flow.py`'s inline style):
  ...
  - `test_verdict_token_unchanged_when_mismatched_without_demotion` — build raw text via `_verdict_yaml("APPROVE")` + `_verdict_section("APPROVE")` + `_heading("BLOCKING", "design", "genuinely blocking")` for `review_type="discussion"`, `blocking_classes=resolve_blocking_classes({}, "discussion", None)` (`design` IS in discussion's default ceiling, so nothing gets demoted — `demoted_any` is `False` — but the recomputed verdict is `REQUEST_CHANGES`, a genuine mismatch against the fixture's stated `APPROVE` that has nothing to do with ceiling activity); call `_finalize`; assert `result["verdict"] == "REQUEST_CHANGES"` (the returned envelope value is still correctly recomputed) BUT the written file's verdict lines (both the yaml field and the `## Verdict` token) remain byte-identical to the fixture's original `APPROVE` text — this is the case Card 7's `demoted_any` gate exists to leave untouched (the reviewer's own inconsistency is surfaced via the envelope, not silently patched over in the persisted file).
  - `test_verdict_token_rewritten_for_plan_and_code_types` — repeat the ceiling-flip case (same shape as the first new test) once for `review_type="plan"` (using `resolve_blocking_classes({}, "plan", "holistic")` and a finding class excluded from plan's ceiling per the existing `test_ceiling_table_plan` table, e.g. `"consistency"`) and once for `review_type="code"` using an **explicit custom** `blocking_classes` frozenset that excludes one class (e.g. `frozenset(RECOGNIZED_CLASSES) - {"consistency"}`) rather than `resolve_blocking_classes({}, "code", "holistic")` — `test_ceiling_table_code` (existing, unchanged) already asserts `DEFAULT_BLOCKING_CLASSES["code-review"]` equals all four `RECOGNIZED_CLASSES`, so no class is excluded by code review's real default and no ceiling-flip fixture is constructible from `resolve_blocking_classes` alone for this sub-case. Both variants assert the same yaml-field + `## Verdict`-token rewrite, since `finalize_scope` is shared across all three review types and the token-rewrite logic must be type-agnostic (per discussion.md's Testing section).
