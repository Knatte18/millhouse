...
- **Requirements:**
  ...
  - Apply the same `_as_path` rebinding to every other public function whose first parameter is `status_path` but has no guard today (e.g. `resume_batch`, `append_inferred_success_log`, `append_fork_fallback_log`, `append_fixer_fork_fallback_log`, `read_fixer_fork_fallback_log` — check each public `def` whose first parameter is `status_path`), so the whole public surface accepts `str` consistently.
