...
- **Creates:** none
- **Deletes:** none
- **Requirements:** In `test_implementer_parity_finalize_stage`, the fixture mock (`mock_subprocess_run`, around the class setup) returns a constant `stdout` (`"abc1234\n"`) for every `_subprocess_util.run`, so prepare records `start_sha = "abc1234"` and finalize's no-content-commit gate (`_forward_output` in `_implementer_common.py`) sees `git rev-parse HEAD == start_sha` and demotes the envelope to `stuck/logic`. Adjust the mock so the `git rev-parse HEAD` call observed during the finalize stage returns a SHA different from the prepare-time `start_sha`. Because `_subprocess_util.run` is invoked many times across prepare+finalize (rev-parse, `git add`, `git commit`, `git status`, terminal-dirt computation), do NOT use a flat positional `side_effect` SHA list — it is order-fragile and can desync the prepare `start_sha` from the finalize HEAD. Prefer a callable `side_effect` that inspects the git argv: return `abc1234` for `rev-parse HEAD` during prepare and a distinct SHA such as `def5678` for `rev-parse HEAD` during finalize (e.g. flip a flag when finalize begins), while returning sensible defaults for the non-rev-parse git calls. After the fix, `test_implementer_parity_finalize_stage` must assert `data["status"] == "success"` with `commit_sha` present and `session_id == "fake-session"`, and the LLM `run` must not be called. Do not change the production no-content-commit gate or any `_implementer_common.py` / `millpy-implement.py` code.
