...
- **Requirements:**
  ...
  In this hub's own `mill-config.yaml`, in its own `pipeline:` block, add the same key with the same comment, in the same relative position (immediately after `done_gate_baseline_preflight: false  # ...`, before `max_cards_per_batch: 10`), matching the CLAUDE.md "`mill-config.yaml` hub file and plugin template must stay in sync" convention:
