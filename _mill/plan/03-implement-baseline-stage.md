...
### Card 10: Add the preflight-precondition-guard helper
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Add a small shared helper `_baseline_exclusion_prefixes(project_root: Path, git_root: Path) -> tuple[str, str]` returning the `("_mill/", ".millhouse/")` exclusion prefixes re-anchored to the hub fragment (`project_root.relative_to(git_root)`, empty string in a flat layout, prepended to each). This is the single source of truth for the exclusion set — Card 11's dirt-warning helper calls it too (never re-deriving the re-anchoring inline), so the two checks cannot drift apart in practice, not just in prose.
  Add a new module-level function `_baseline_preflight_skip_reason(project_root: Path, git_root: Path, status_path: Path) -> str | None`, implementing Decision `preflight-precondition-guard`'s gating condition. Resolve `<parent>` via `_parent_branch.resolve(status_path, interactive=False)`; on any exception, return a skip reason string describing the unresolvable parent (do not raise). Run `git -C <git_root> merge-base HEAD <parent>`; a non-zero exit returns a skip reason describing the failed merge-base. Run `git -C <git_root> diff --name-only <merge_base_sha> HEAD` to get the set of changed paths since the merge-base. Call `_baseline_exclusion_prefixes(project_root, git_root)` for the exclusion prefixes. If any changed path falls outside both exclusion prefixes, return a skip reason string ("worktree is not pre-edit"); otherwise return `None` (precondition holds, capture may proceed). This function performs no capture itself and mutates nothing — it is a pure gate, called exactly ONCE per `--stage baseline` invocation, by `_run_baseline_stage` (Card 14) only, before either half attempts anything; neither `_run_module_wide_standalone` (Card 12) nor `_run_per_batch_baseline_standalone` (Card 13) calls this function itself — each instead receives the already-computed skip reason (or `None`) as a parameter from `_run_baseline_stage`, so the guard's two `git` calls never run twice in one invocation. Do NOT check `git status --porcelain` here — a dirty tracked file outside `_mill/`/`.millhouse/` is the ADVISORY case Card 11 covers separately; this gate's only load-bearing check is the merge-base-vs-HEAD diff (whether implementation work has been *committed* to this branch), per the "Why a dirty tree only warns" rationale in `_mill/discussion.md`'s `preflight-precondition-guard` Decision.
- **Commit:** `feat(millpy-implement): add baseline preflight precondition guard`

### Card 11: Add the preflight-dirt-warning helper
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Add two small helpers implementing Decision `preflight-dirt-warning`: one to take a `git -C <git_root> status --porcelain` snapshot (returning the raw modified-tracked-path list, e.g. `_porcelain_snapshot(git_root: Path) -> set[str]`), and one to compare a before/after pair and print an ASCII-only stderr warning naming any path that is newly dirty and falls outside the exclusion set (e.g. `_warn_new_dirt(before: set[str], after: set[str], exclusions: tuple[str, str]) -> None`). Each caller obtains `exclusions` by calling Card 10's `_baseline_exclusion_prefixes(project_root, git_root)` — never by re-deriving the hub-fragment re-anchoring inline — so this warning's exclusion set and the preflight guard's own exclusion set are structurally the same call, not two independently-maintained copies. This warning never blocks, never reverts, and never fails the stage — it is advisory only, reused verbatim by both halves (the module-wide half wraps its own before/after snapshot around its 1-2 verify runs; the per-batch half wraps its own before/after snapshot around the whole per-batch capture loop).
- **Commit:** `feat(millpy-implement): add preflight dirt-warning helper`

### Card 12: Rewrite _run_module_wide_standalone for the new compute_baseline signature
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Delete `_relative_cwd_fragment` (its only caller is rewritten below to resolve the absolute cwd directly instead) and `_pin_baseline_parent_sha` (its only caller is removed in Card 14) in full.
  Rewrite `_run_module_wide_standalone`, adding a new parameter `preflight_skip_reason: str | None` (the already-computed result of Card 10's `_baseline_preflight_skip_reason`, passed in by the caller — this function never calls that guard itself, per Card 10's "called exactly once, by `_run_baseline_stage`" requirement): keep the existing `_module_wide_skip_or_cached_payload` short-circuit unchanged (no module-wide verify configured, or already cached). Otherwise, if `preflight_skip_reason` is not `None`, print `{"stage": "baseline", "substage": "module_wide", "result": "skipped", "reason": preflight_skip_reason}` and return (this half's own tagged skip line, per Decision `preflight-precondition-guard`). Otherwise resolve `effective_cwd = module_wide_cwd_override if module_wide_cwd_override is not None else git_root` (mirroring `_run_verify_gate`'s own resolution exactly, per the "`git_root` vs `project_root`" gotcha in `_mill/discussion.md`). Take a before-porcelain snapshot (Card 11, using Card 10's `_baseline_exclusion_prefixes(project_root, git_root)` for the exclusion set), call `_verify_baseline.compute_baseline(effective_cwd, module_wide_verify_cmd, timeout_seconds=verify_timeout_seconds)`, take an after-porcelain snapshot and warn on new dirt (Card 11). Unpack `(result, signatures)`; persist both via `_status.set_module_verify_baseline(status_path, result)` and `_status.set_module_verify_baseline_signatures(status_path, signatures)`; print `{"stage": "baseline", "substage": "module_wide", "result": "computed", "value": result}` (unchanged shape — `signatures` is persisted to status.md, not added to this printed line). Keep the existing `except Exception` -> `{"result": "error", "reason": str(e)}` fallback around the `compute_baseline` call, but drop the now-obsolete `parent_branch = _parent_branch.resolve(...)` block above it — the preflight guard (now called once, upstream, by `_run_baseline_stage`) already resolved and validated the parent; this function no longer needs its own separate resolution.
  Return `(module_wide_verify_cmd, effective_cwd, signatures)` from this function when it actually computed a fresh result this invocation (the `"computed"` branch only), or `None` on every other branch (skipped/cached/error) — Card 14 threads this return value into Card 13 as `module_wide_pair_seed`.
- **Commit:** `refactor(millpy-implement): module-wide baseline capture uses new compute_baseline signature and preflight guard`

### Card 13: Add the eager per-batch capture function
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Add a new function `_run_per_batch_baseline_standalone(project_root: Path, git_root: Path, status_path: Path, plan_base: Path, timeout_seconds: float | None, preflight_skip_reason: str | None, module_wide_pair_seed: tuple[str, Path, list[str]] | None) -> None`, implementing the per-batch half of Decision `two-half-stage-ownership` plus `capture-set-equals-gate-set`, `per-batch-capture-driver`, `module-wide-verdict-source`'s seeding, and the key-presence idempotence rule. `preflight_skip_reason` is the already-computed result of Card 10's `_baseline_preflight_skip_reason`, passed in by the caller — this function never calls that guard itself (Card 10 requires it be called exactly once, by `_run_baseline_stage`). `module_wide_pair_seed`, when not `None`, is `(module_wide_verify_cmd, effective_cwd, signatures)` from a module-wide half that computed a fresh result *this same invocation* (used to seed `pair_cache` below); `None` when the module-wide half was skipped, errored, or already cached.
  Step 1 (deferred detection): call `_status.read_batches(status_path)`; if the list is empty (`## Batches` section absent or empty), print `{"stage": "baseline", "substage": "per_batch", "result": "deferred", "reason": "## Batches not yet seeded"}` and return — this branch is checked before `preflight_skip_reason` is even consulted (there is nothing to gate yet).
  Step 2 (preflight): if `preflight_skip_reason` is not `None`, print `{"stage": "baseline", "substage": "per_batch", "result": "skipped", "reason": preflight_skip_reason}` and return.
  ...
  Wrap the whole per-batch driver loop (step 6) in Card 11's before/after porcelain dirt-warning snapshot.
...
### Card 14: Rewrite _run_baseline_stage to drive both halves
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Rewrite `_run_baseline_stage`: delete the `baseline_prepare_cmd: str | None` parameter entirely (its only use today is `del plan_base, baseline_prepare_cmd  # unused`). `plan_base: Path` remains a parameter but becomes genuinely used: pass it through to Card 13's `_run_per_batch_baseline_standalone`.
  Call Card 10's `_baseline_preflight_skip_reason(project_root, git_root, status_path)` exactly ONCE here — this is the guard's single call site for the whole invocation, per Card 10's requirement that neither half calls it itself.
  Call `_run_module_wide_standalone(..., preflight_skip_reason=<the value above>)` (Card 12) first, capturing its return value as `module_wide_pair_seed` (either the `(module_wide_verify_cmd, effective_cwd, signatures)` tuple when it computed a fresh result this invocation, or `None` on every other branch). Then call `_run_per_batch_baseline_standalone(project_root, git_root, status_path, plan_base, verify_timeout_seconds, preflight_skip_reason=<the value above>, module_wide_pair_seed=<the value just captured>)` (Card 13).
  Keep the function's "never raises, always returns 0" contract and its module-level docstring's description of the two independent sub-steps, rewritten to describe the module-wide and per-batch halves instead of the module-wide-plus-`baseline_parent_sha`-pin pair it describes today, and to note that the preflight guard runs once here and is threaded to both halves rather than being called by either.
- **Commit:** `refactor(millpy-implement): _run_baseline_stage drives module-wide and per-batch halves`

### Card 15: Update main()'s --stage baseline branch
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** In `main()`'s `if args.stage == "baseline":` branch, delete the `baseline_prepare_cmd = pipeline_cfg.get("baseline_prepare_cmd")` line and drop the corresponding positional argument from the `_run_baseline_stage(...)` call (the signature no longer accepts it, per Card 14).
- **Commit:** `refactor(millpy-implement): drop baseline_prepare_cmd from --stage baseline call site`

### Card 16: Remove pipeline.baseline_prepare_cmd from the hub config
...
### Card 17: Remove pipeline.baseline_prepare_cmd from the config template
