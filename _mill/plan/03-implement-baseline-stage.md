...
- **Requirements:** Delete `_relative_cwd_fragment` (its only caller is rewritten below to resolve the absolute cwd directly instead) and `_pin_baseline_parent_sha` (its only caller is removed in Card 15) in full.
  Rewrite `_run_module_wide_standalone`, adding a new parameter `preflight_skip_reason: str | None` (the already-computed result of Card 11's `_baseline_preflight_skip_reason`, passed in by the caller — this function never calls that guard itself, per Card 11's "called exactly once, by `_run_baseline_stage`" requirement): keep the existing `_module_wide_skip_or_cached_payload` short-circuit unchanged (no module-wide verify configured, or already cached). Otherwise, if `preflight_skip_reason` is not `None`, print `{"stage": "baseline", "substage": "module_wide", "result": "skipped", "reason": preflight_skip_reason}` and return (this half's own tagged skip line, per Decision `preflight-precondition-guard`). Otherwise resolve `effective_cwd = module_wide_cwd_override if module_wide_cwd_override is not None else git_root` (mirroring `_run_verify_gate`'s own resolution exactly, per the "`git_root` vs `project_root`" gotcha in `_mill/discussion.md`). Take a before-porcelain snapshot (Card 12, using Card 11's `_baseline_exclusion_prefixes(project_root, git_root)` for the exclusion set), call `_verify_baseline.compute_baseline(effective_cwd, module_wide_verify_cmd, timeout_seconds=verify_timeout_seconds)`, take an after-porcelain snapshot and warn on new dirt (Card 12). Unpack `(result, signatures)`; persist both via `_status.set_module_verify_baseline(status_path, result)` and `_status.set_module_verify_baseline_signatures(status_path, signatures)`; print `{"stage": "baseline", "substage": "module_wide", "result": "computed", "value": result}` (unchanged shape — `signatures` is persisted to status.md, not added to this printed line). Keep the existing `except Exception` -> `{"result": "error", "reason": str(e)}` fallback around the `compute_baseline` call, but drop the now-obsolete `parent_branch = _parent_branch.resolve(...)` block above it — the preflight guard (now called once, upstream, by `_run_baseline_stage`) already resolved and validated the parent; this function no longer needs its own separate resolution.
  Return `(module_wide_verify_cmd, effective_cwd, signatures)` from this function when it actually computed a fresh result this invocation (the `"computed"` branch only), or `None` on every other branch (skipped/cached/error) — Card 15 threads this return value into Card 14 as `module_wide_pair_seed`.
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Rewrite `_run_baseline_stage`: delete the `baseline_prepare_cmd: str | None` parameter entirely (its only use today is `del plan_base, baseline_prepare_cmd  # unused`). `plan_base: Path` remains a parameter but becomes genuinely used: pass it through to Card 14's `_run_per_batch_baseline_standalone`.
  Call Card 11's `_baseline_preflight_skip_reason(project_root, git_root, status_path)` exactly ONCE here — this is the guard's single call site for the whole invocation, per Card 11's requirement that neither half calls it itself.
  Call `_run_module_wide_standalone(..., preflight_skip_reason=<the value above>)` (Card 13) first, capturing its return value as `module_wide_pair_seed` (either the `(module_wide_verify_cmd, effective_cwd, signatures)` tuple when it computed a fresh result this invocation, or `None` on every other branch). Then call `_run_per_batch_baseline_standalone(project_root, git_root, status_path, plan_base, verify_timeout_seconds, preflight_skip_reason=<the value above>, module_wide_pair_seed=<the value just captured>)` (Card 14).
