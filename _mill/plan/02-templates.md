...
- **Requirements:**
  ...
  Fix: remove the outer anonymous ` ``` ` fence entirely (line 35 and line 66). Keep the inner ` ```yaml ` block as-is (lines 38–43). The surrounding scaffold (lines 36–37: `# Review: ...` and blank line; lines 44–65: `## Findings`, finding format examples, `## Missing context`, `## Verdict`) should be shown as indented prose or wrapped in a single outer fence using a different fence style. The cleanest approach: remove both the outer opening ` ``` ` and the outer closing ` ``` `, and prefix the non-yaml example lines with `>` blockquote markers so the model can distinguish them from the yaml block without needing nested fences. If blockquote markers would be confusing in context, an alternative is to wrap the entire example in a ` ~~~markdown ` fence (tilde fences allow inner backtick fences without nesting conflicts). Either approach is acceptable; the key invariant is that the ` ```yaml ` block and its ` ``` ` closing fence must be the ONLY backtick fence in the output-format section (no anonymous outer fence that the model could mistake for the yaml block's closing fence).
