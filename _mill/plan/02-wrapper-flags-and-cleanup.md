...
- **Requirements:** Create `plugins/mill/unit_tests/test-claude-sub.py` modelled on `test-millpy-bg.py` (argv-driven CLI test pattern) and `test-llm-claude.py` (mocking style). Set `sys.path` to include the scripts dir, import `millpy-claude-sub` as a module (use `importlib.util` since the filename contains a hyphen — see `test-millpy-bg.py` for the pattern). Mock `_psmux.new_session`, `_psmux.set_history_limit`, `_psmux.send_keys`, `_psmux.load_buffer`, `_psmux.paste_buffer`, `_psmux.capture_pane`, `_psmux.list_sessions`, `_psmux.kill_session`, and the module-level functions `_wait_for_marker_in_pane`, `_wait_for_idle_prompt`. Default both wait-for-* functions to return True in each test's setup so create-session tests reach the post-launch happy path unless the test explicitly overrides one of them to return False to drive a specific error site (the per-test descriptions below name the overrides where they apply). Drive `main()` by setting `sys.argv` and feeding stdin via `io.StringIO` patched onto `sys.stdin`. Capture stdout/stderr to assert behaviour. Tests to implement:
  ...
  - **S5 — keep-alive true, error mid-call when wrapper owns session:** `--psmux-session new-name --keep-alive`; `list_sessions` returns `[]`; `new_session` succeeds; `_wait_for_marker_in_pane` (the CLAUDE_READY check) is mocked to return True so boot proceeds past Step 7; `_wait_for_idle_prompt` returns False (this fires after the claude-launch `send_keys` at Step 9, AFTER the wrapper owns the session). Assert `kill_session` IS called (error-path + `session_owned_by_us` true); `main()` returns 1.
