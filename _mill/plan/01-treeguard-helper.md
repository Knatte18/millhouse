...
- **Requirements:**
  ...
  - Implement `check_and_restore(worktree: Path, tracked_root: str = "_mill", *, git_root: Path | None = None) -> dict`:
    ...
    8. Compute `still_missing = set(deleted_paths) - set(restored_paths)`. If `still_missing` is non-empty — whether or not `restored_paths` is also non-empty (a partial restore, where the checkout succeeded for some paths but not others, is exactly as important to surface as a total failure) — print an ASCII-only diagnostic to stderr naming every path in `still_missing` and the subprocess's captured stderr. This diagnostic print is unconditional on `still_missing` being non-empty; it is not gated on `restored_paths` being empty.
    9. If `restored_paths` is empty (the checkout restored nothing at all), return `{"triggered": False, "restored_paths": [], "timestamp": None}` — a total restore failure must never be reported as `triggered: True`, since callers (`_status.append_recovery_log`) treat that field as "a restore actually happened" and would otherwise log a false success.
    10. Otherwise (`restored_paths` is non-empty, whether or not `still_missing` is also non-empty) return `{"triggered": True, "restored_paths": sorted(restored_paths), "timestamp": _timestamp.now_utc_iso()}` — a partial restore is still `triggered: True` and still reports exactly the paths that were actually restored (never the ones in `still_missing`), so `_status.append_recovery_log` logs only what genuinely happened; the stderr diagnostic from step 8 is what surfaces the still-missing paths, since `append_recovery_log`'s own row only ever records `restored_paths`.
...
- **Requirements:**
  ...
  - Implement every scenario from `_mill/discussion.md`'s Testing section, each as its own `with tempfile.TemporaryDirectory()` block:
    ...
    8. **Nested-hub layout rebases git-root-relative porcelain paths onto the hub before matching (regression case for round 2's GAP):** use a real nested-hub git fixture, not a mock of `_pygit2_util.status_porcelain` — mocking that boundary while `check_and_restore`'s own disk-existence check (Card 1 requirement 7) still runs against real paths produces a contradiction (a mocked-success restore that never actually creates the file on disk always fails the disk check, per round 4's GAP finding). Build the fixture for real: create a tempdir as `git_root`, create `hub_root = git_root / "hub"`, create the `_mill/` tree (`status.md`, `discussion.md`, `briefs/x.md`, `reviews/y.md`) under `hub_root`, then `git init` at `git_root` and commit the whole tree (including the `hub/` prefix) from there — do not mock anything for this scenario; `_pygit2_util.status_porcelain(hub_root, ...)` will genuinely discover the same repository (pygit2 walks up from `hub_root` to find `.git` at `git_root`) and genuinely return git-root-relative lines like `"hub/_mill/status.md"`, exercising the real rebase path end to end. Delete `hub_root / "_mill/status.md"` on disk (`unlink()`), call `check_and_restore(hub_root, "_mill", git_root=git_root)`, and assert: the file is restored on disk under `hub_root` (real content, real existence check); `result["restored_paths"] == ["_mill/status.md"]` (hub-relative, never `"hub/_mill/status.md"`); `result["triggered"] is True`. This is the literal regression case review round 2 flagged: an unrebased implementation would never match `"hub/_mill/status.md"` against bare `"_mill"` and would silently return `triggered: False`.
    ...
    10. **Partial restore is reported accurately, not silently upgraded to full success (regression case for round 3's GAP):** delete two tracked files (e.g. `_mill/status.md` and `_mill/reviews/y.md`, both `unlink()`, both status code `" D"`), then patch `_treeguard._subprocess_util.run` to return `unittest.mock.Mock(returncode=1)` while, inside the same patched call (e.g. via a `side_effect` function), actually restoring only one of the two files to disk from the seeded git history (simulating git's real per-pathspec partial-success behavior) and leaving the other still deleted. Call `check_and_restore` and assert: `result["triggered"] is True`; `result["restored_paths"]` contains only the one path that was actually restored (never the still-missing one); and something was printed to stderr identifying the still-missing path (capture stderr via `contextlib.redirect_stderr` or equivalent, matching how other tests in this suite capture output, and assert the still-missing path's name appears in it).
