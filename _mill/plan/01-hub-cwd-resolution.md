...
- **Creates:** none
- **Deletes:** none
- **Requirements:** In `main()`, replace `project_root = Path.cwd()` (line ~92) with `project_root = _paths.resolve_hub_path()` and set `mill_dir = project_root / ".millhouse"` accordingly. The existing `load_config(resolve_hub_path(), mill_dir)` call (line ~96) keeps working (now consistent with `project_root`). After `cfg` is loaded and before any status read is delegated into `_review_code`, add a guard: `try: _paths.require_status_path(project_root, cfg) except _paths.TaskHubError as e: print(str(e), file=sys.stderr); return 1`. This guarantees the same actionable error as the other two CLIs when run from a non-hub cwd, instead of a raw `ValueError` surfacing from `_review_code` internals. Do not otherwise change review dispatch behavior.
...
- **Creates:** none
- **Deletes:** none
- **Requirements:** Add unit tests for `_paths.require_status_path` following the existing `test-paths.py` fixture style (tempfile dirs, no real git required). Cover: (1) a project_root whose `_mill/status.md` does NOT exist raises `_paths.TaskHubError` and the exception message contains both the missing status path and the project_root; (2) a project_root whose `_mill/status.md` exists returns that path. Build a minimal `cfg` dict containing `paths.status_md` set to `"_mill/status.md"` (matching the config shape `status_path` reads). If `test-paths.py` uses a test-runner-discovered function/class convention, follow whatever pattern the file already uses (`run-all.py` discovery).
