...
- **Requirements:** Spot-clean each integration test file's V2 references. Integration tests are out of the verify gate per discussion, but cleaning them is in-scope per the "eliminate every trace of V2" framing.
  ...
  - `test-spawn.py:42` (`import _tasks_md`), `:206` (`_tasks_md.parse(...)`).
  - `test-spawn-units.py:23` (`import _tasks_md`), `:34-35` (uses `_tasks_md.Task(...)` builder). The fixture builder needs the dict-literal replacement; check the surrounding code for any `.slug`/`.title`/`.phase` attribute accesses and convert them to dict-key access.
