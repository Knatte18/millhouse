...
- **Requirements:**
  ...
  - `command_touches_wiki(command: str) -> bool`, implemented in this order (authoritative; the numbered discussion steps are out of order):
    1. Strip heredoc bodies: for each heredoc start (`<<`, `<<-`, optionally-quoted tag), drop every following line up to and including the terminator line (tag alone on a line, leading tabs allowed for `<<-`). Keep the text before the heredoc operator on the start line. A heredoc start is `<<` or `<<-` NOT part of `<<<` (a here-string is never a heredoc; its word is an ordinary argument), followed by a tag matching `[A-Za-z_][A-Za-z0-9_]*` optionally wrapped in single or double quotes. Strip a body only when a matching terminator line exists later in the text; an unterminated start strips nothing (fail closed), so a `<<` inside a quoted argument without a terminator cannot hide later text.
    ...
    6. If the command word is `echo` or `printf`: its plain arguments are text and are not tested, but every token that follows a redirect operator token (`>`, `>>`, `<`, `>|`, `&>`, or a fd-prefixed form such as `2>`) is tested by step 9, so writing to the junction through a redirect is denied.
...
- **Requirements:**
  ...
  - True: `echo x >` a junction file path, and `echo x >>` a junction file path (redirect targets are tested even for echo).
  - Heredoc edge cases: a `<<<` here-string word that is a junction path in a non-echo command is True; an unterminated heredoc start followed by a line naming a junction path is True (nothing stripped).
