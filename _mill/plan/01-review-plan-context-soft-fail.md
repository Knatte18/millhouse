...
- **Requirements:** In `_review_one_batch()` (def at line 132), replace the single-call ref resolution at lines 177-185:
  ...
  ```
  with a Context:-only split mirroring `_review_code.py`'s `prepare()` pattern at lines 266-290 (`context_only_refs`/`other_refs` built via two `parse_batch_refs(bp, fields=(...))` calls, then two `resolve_ref_paths` calls, only the `Context:`-only one passing `soft_fail_gitignored=True`): call `parse_batch_refs(batch_path, fields=("Context",))` for the soft-fail-eligible set and `parse_batch_refs(batch_path, fields=("Edits", "Creates", "Deletes"))` for the hard-fail set, resolve each separately (the `Edits:`/`Creates:`/`Deletes:` group keeping today's call shape and arguments exactly, the `Context:` group adding `soft_fail_gitignored=True`), then concatenate the two resolved lists into `reads` (same variable name, same final shape/order-independent list-of-Path semantics existing callers already consume). This function reads BOTH the plain list `raw_refs` (line 188: `[raw for raw in combined_creates if raw not in raw_refs]`, an `ancestors_on_disk` membership filter) AND `raw_refs_set` again later — keep both names bound after the split: `raw_refs_set = set(context_only_refs) | set(other_refs)` (the union of both field-groups' refs, preserving its existing full-unsplit-ref-set semantics) and either keep a combined `raw_refs` list (`context_only_refs + other_refs` or equivalent) or repoint line 188's `not in raw_refs` check at `raw_refs_set` instead (a membership test works identically against either a list or a set — the two names are interchangeable for that one check). Read the rest of `_review_one_batch()` (through its closing `return`) to confirm there is no other statement depending on `raw_refs`/`raw_refs_set` beyond this. `combined_creates` (built from `creates_union | moves_targets` — note this call site's local variable is named `moves_targets`, not `moves_targets_union` like the other three sites) is passed to both resolve calls unchanged.
...
- **Requirements:** In `prepare()` (def at line 336), per-batch scope branch, replace the single-call ref resolution at lines 408-416:
  ...
  ```
  with the identical split-and-resolve-twice pattern described in Card 1's Requirements (same `parse_batch_refs(..., fields=(...))` split, same two `resolve_ref_paths` calls with only the `Context:`-only one passing `soft_fail_gitignored=True`, same `reads` concatenation). This function reads BOTH the plain list `raw_refs` (line 419: `[raw for raw in combined_creates if raw not in raw_refs]`) AND `raw_refs_set` again later — apply Card 1's identical remedy: keep `raw_refs_set = set(context_only_refs) | set(other_refs)` populated, and either keep a combined `raw_refs` list or repoint line 419's check at `raw_refs_set`. This call site's local variable is `moves_targets_union` (not `moves_targets` — that name is specific to Card 1's site).
...
- **Requirements:** In `prepare()` (def at line 336), holistic scope branch, replace the ref-union-then-resolve block at lines 509-520:
  ...
  **Downstream reuse of `all_raw_refs` — do not drop it.** This branch reads the combined dict `all_raw_refs` TWICE more after this block, at (current) lines 523 and 535: `[r for r in combined_creates if r not in all_raw_refs]` (building `all_creates_on_disk`) and `[s for s in moves_sources_union if s not in all_raw_refs]` (building `holistic_moves_on_disk`) — both are plain membership checks against the full unsplit ref set, unrelated to the Context:/Edits: split. Keep a combined lookup available for both: either retain `all_raw_refs = {**all_context_refs, **all_other_refs}` (or `all_context_refs | all_other_refs` merged into one dict) alongside the two split unions, or repoint both downstream `not in all_raw_refs` checks at that same merged structure. A literal implementation that deletes `all_raw_refs` outright without this replacement raises `NameError` at both downstream sites.
...
- **Requirements:** In `run()` (def at line 660), holistic scope branch, replace the ref-union-then-resolve block at lines 906-917:
  ...
  ```
  with the identical split-union-then-resolve-twice pattern described in Card 3's Requirements (this is `prepare()`'s holistic branch's twin site, kept in sync for prepare/finalize symmetry per the discussion's `Technical context` note — same two-union construction, same two `resolve_ref_paths` calls, same `soft_fail_gitignored=True` scoped to the Context: union only, same `all_reads` concatenation). Apply Card 3's identical downstream-reuse caveat here too: this branch reads the combined `all_raw_refs` dict TWICE more after this block, at (current) lines 920 and 932 (`[r for r in combined_creates if r not in all_raw_refs]` and `[s for s in moves_sources_union if s not in all_raw_refs]`) — keep a combined lookup (`all_raw_refs = {**all_context_refs, **all_other_refs}` or equivalent) available for both, or repoint both checks at it.
