...
Rewrite `wiki/_render.py` to replace `group`-based bucketing with a `compute_layers` helper that derives letter assignments from the DAG, add `extended_title` and `render_order` helpers, and update `render()` to emit the new Home.md format (Someday section, Depends-on lines, no Unspecified section, corrected letter suffix behavior). Write `test-wiki-render.py` additions first (TDD). This batch is fully independent — `_render.py` imports nothing from `wiki/__init__` or `wiki/_store`.

## Cards

### Card 11: Add compute_layers
...
### Card 12: Add extended_title and render_order
