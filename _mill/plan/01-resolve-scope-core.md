...
- **Moves:** none
- **Requirements:**
  - Add Scenario 14: single non-hex branch-name token with a literal `..HEAD` suffix. Create a repo, commit `base.py` on `main`, checkout branch `mill-checkpoint-feature` at that commit (the checkpoint/base ref), then commit 1-2 more files (e.g. `a.py`, `b.py`) on top — either directly on `mill-checkpoint-feature` advancing it further is wrong; instead create the checkpoint branch first, then check out a NEW branch or stay on the same branch line and advance HEAD past the checkpoint point via new commits, so `mill-checkpoint-feature` marks the base and HEAD is its descendant. Then call `enumerate_scope(["mill-checkpoint-feature..HEAD"], cwd=tmp)`. Assert `summary["mode"] == "head-rev"` and the resulting path set equals the files added in the commits *between* `mill-checkpoint-feature` and HEAD (i.e. `a.py`, `b.py` — the files committed AFTER the checkpoint, not on it), mirroring Scenario 8's `HEAD~3` semantics literally (`<base>..HEAD` returns what changed after `<base>`, never `<base>`'s own state).
