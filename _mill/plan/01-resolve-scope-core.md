...
- **Moves:** none
- **Requirements:**
  - Add Scenario 14: single non-hex branch-name token with a literal `..HEAD` suffix. Create a repo, commit on `main`, checkout branch `mill-checkpoint-feature`, commit 1-2 files on it, checkout a separate branch off `mill-checkpoint-feature` (or stay on it and diff from a descendant), then call `enumerate_scope(["mill-checkpoint-feature..HEAD"], cwd=tmp)`. Assert `summary["mode"] == "head-rev"` and the resulting path set matches the files committed on `mill-checkpoint-feature`. Mirror existing Scenario 8's `HEAD~3` structure.
