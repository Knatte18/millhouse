...
- **Requirements:**
  - Mechanical substitution across four files: replace every call of the form `_paths.resolve_task_path(<wt-expr>, "_mill/status.md")` with `_paths.status_path(<wt-expr>, cfg)`. The replacements are:
    ...
    - `plugins/mill/scripts/millpy-cleanup.py` line 130 (inside the worktree iteration loop): `_paths.resolve_task_path(wt_path, "_mill/status.md")` -> `_paths.status_path(wt_path, cfg)`. Verify `cfg` is in scope.
    - `plugins/mill/scripts/millpy-cleanup.py` line 325 (inside `_remove_worktree`): `_paths.resolve_task_path(record.worktree_path, "_mill/status.md")` -> `_paths.status_path(record.worktree_path, cfg)`. Thread `cfg` through `_remove_worktree`'s signature if it is not already available there -- check the function signature first; if cfg is not present, prefer threading it as a keyword arg over loading it inside the function. Update the call site of `_remove_worktree` accordingly.
    - `plugins/mill/scripts/millpy-cleanup.py` line 353 (also inside `_remove_worktree`): `_paths.resolve_task_path(record.worktree_path, "_mill/status.md")` -> `_paths.status_path(record.worktree_path, cfg)`. Same `cfg` threading as line 325.
