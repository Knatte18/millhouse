...
- **Requirements:**
  - Mechanical substitution across four files: replace every call of the form `_paths.resolve_task_path(<wt-expr>, "_mill/status.md")` with `_paths.status_path(<wt-expr>, cfg)`. The replacements are:
    ...
    - `plugins/mill/scripts/millpy-cleanup.py` line 130 (inside `build_plan`'s active-worktree iteration loop): **leave unchanged.** Keep the existing `_paths.resolve_task_path(wt_path, "_mill/status.md")` call. `build_plan` does not receive `cfg` -- it is exercised directly by `plugins/mill/unit_tests/test-cleanup.py` with `tempfile`-backed wiki paths whose `wiki/config.yaml` is absent (`_config.load_config` returns `{}` against an absent file), and `_paths.status_path({}, ...)` would raise `KeyError`. Threading `cfg` through `build_plan`'s signature would force fixture rewrites across every `build_plan` call in `test-cleanup.py` (lines 203, 228, 250, 273, 292, 316, 331, 349). The discussion's uniformity decision applies to call sites that already have `cfg` in scope; `build_plan` does not. This call site is the documented exception; do not flag it as inconsistent.
...
**Coverage gap (cards 5 and 6):** the verify command does not directly exercise `millpy-inspect.py` or `millpy-status.py`'s status-path resolution -- the existing unit-test inventory has no `test-millpy-inspect.py` or `test-millpy-status.py`. A correct one-line substitution at `millpy-inspect.py:77` and `millpy-status.py:49` is verified only by reading the diff and (optionally) running `millpy-inspect` manually against the current worktree, which both bug fixes will visibly resurrect from `(no active tasks)` to a real dump. Adding thin unit tests for those two scripts is out of scope for this task per the discussion's testing decision; the bug-fix correctness rests on the substitution being mechanical (the helper itself is unit-tested in batch 1).
