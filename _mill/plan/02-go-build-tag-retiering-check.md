...
- **Deletes:** none
- **Moves:** none
- **Requirements:** In the explicit-success branch of `_forward_output`/`finalize_from_output` (the same branch batch 01's Card 4 already threads `card_ids`/`cards_done` through), call `_go_build_tag_retiering_stuck(project_root, start_sha, _gate_session_id)` immediately after the existing verify gate (`_run_verify_gates`) returns `None` (pass) and BEFORE the no-content-commit check and the completeness gate (`_batch_completeness_stuck`). If it returns a non-`None` stuck dict: attach `commit_sha` via the existing `_attach_commit_sha` helper (mirroring how the verify gate's own failure path attaches it), print the JSON, and return — short-circuiting before the no-content-commit and completeness gates fire for this call, exactly as the existing verify gate's own failure already does.
