...
- **Moves:** none
- **Requirements:**
  Add `_check_context_completeness(batch_files: list[Path], project_root: Path, root: str | None, creates_union: set[str], deletes_union: set[str], moves_targets: set[str], *, wiki_root: Path | None = None, git_root: Path | None = None) -> list[dict]` to `plugins/mill/scripts/_plan_validate.py`, placed immediately after `_check_plugin_manifest_context_missing` (after its closing `return errors` at line 1357) and before the `# Check 8 — all-files-touched-mismatch` section comment at line 1361. The `deletes_union`/`moves_targets` parameters mirror `_check_non_existent_path`'s existing parameter shape (line 662-668) for the same resolvability purpose (see below).
  ...
  **Line-range suffix stripping.** Before testing resolvability and before matching against the combined set (below), strip a trailing line-range suffix from the token using the module's existing `_RE_LINE_RANGE` pattern (line 94: matches `:NN-NN` at the end of a token) via `_RE_LINE_RANGE.sub("", token)`. Use this stripped form for the `resolve_existing_paths(...)` call, the `creates_union` membership test, and the combined-set membership test described below. Continue to use the ORIGINAL (unstripped) token in the emitted error dict's `"path"` field, so the message shows exactly what the Requirements: text wrote. Without this stripping, a line-range-suffixed citation (a style already used elsewhere in this codebase's plans, e.g. Card 2's own `` `plugins/mill/unit_tests/test-plan-validate.py:93-155` `` reference below) would never resolve as an existing path and would be silently — and wrongly — treated as a non-path token.
  ...
  For each path-candidate token surviving the prohibition-marker exemption, determine resolvability by calling `resolve_existing_paths([stripped_token], project_root, root, wiki_root=wiki_root, git_root=git_root)` (already imported into this module from `_review_common` at line 68). The token is resolvable when that call returns a non-empty list, OR `stripped_token in creates_union`, OR `stripped_token in deletes_union`, OR `stripped_token in moves_targets` — the same four-way OR-chain `_check_non_existent_path` already uses (lines 701-705) to decide whether a reference is a legitimate forward/cross-batch reference rather than a genuinely missing file; a plan-wide Move *target* that does not exist on disk yet (it appears only after the rename step) must still be treated as resolvable so a Requirements: reference to it can be checked against the card's own Context:/Edits:/Moves:-source set. A path-candidate token that is NOT resolvable under any of these four conditions is skipped silently (no finding) — this is the false-positive guard: `context-completeness` must never flag a token that is not actually a file reference (e.g. `` `response.json` `` as a JSON-body key), since the alternative (flagging it) would prescribe the actively harmful "add to Context:" remedy, which would itself then trip `non-existent-path` on the very next validator pass.
  ...
  Wire the new check into `run()`: add `errors.extend(_check_context_completeness(batch_files, project_root, effective_root, creates_union, deletes_union, moves_targets, wiki_root=wiki_root, git_root=git_root))` immediately after the existing `errors.extend(_check_plugin_manifest_context_missing(batch_files))` line (`_plan_validate.py:2275`) and before `errors.extend(_check_all_files_touched_mismatch(overview_path, batch_files))`. `deletes_union` and `moves_targets` are already computed earlier in `run()` (lines 2238 and 2241 respectively — `moves_targets` is the second element of the `moves_sources, moves_targets = compute_moves_union(plan_dir)` tuple) and threaded into `_check_non_existent_path`'s call already; no new computation is needed, only passing the existing local variables through.
