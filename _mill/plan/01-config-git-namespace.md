...
- **Requirements:** Under the "Git integration" comment header (~lines 76-82), replace the commented example block (the `# git:`, `#   require_pr_to_base: true`, `#   base_branch: main` lines) with a real, uncommented block:
  ...
  ```
  Keep the surrounding "Git integration" explanatory comments. `parent-branch` is **net-new** (the current comment has only `require_pr_to_base` and `base_branch` — this is an addition, not an uncomment). All three subkeys are **required**, not just `require_pr_to_base`/`base_branch`: once `git` becomes a known dict, `walk_unknown_keys` recurses into it, so an unregistered `parent-branch` would itself warn — and `git.parent-branch` is exactly the key loomyard sets that triggered #511. This registers `git` plus its three subkeys in the template dict that `_config.walk_unknown_keys` treats as the schema, so `[config] unknown key: git` no longer fires while any unregistered `git.*` subkey still warns. The three defaults (`null` / `false` / `main`) match existing fallbacks, so no hub behavior changes.
...
- **Creates:** none
- **Deletes:** none
- **Requirements:** **First, register the `git:` block in the `_setup_plugin_template` fixture** (the minimal template string at test-config.py ~L56-80 — it currently has spawn/roles only and NO `git:`). Append `git:\n  parent-branch: null\n  require_pr_to_base: false\n  base_branch: main\n` to that fixture string so the fixture schema matches the real registered template. This is **mandatory**: `_config.walk_unknown_keys` validates the merged config against this fixture, not the real template Card 1 edits; without the fixture `git:` block the positive test sees top-level `unknown key: git` (fail) and the negative test sees `unknown key: git` instead of `git.bogus-key` (fail), because `walk_unknown_keys` (L104-109) only recurses into a key when BOTH actual and template have it as a dict. Then add two test functions modeled on the existing `test_via_psmux_does_not_trigger_unknown_key_warning` (uses the `_setup_plugin_template` fixture, `_write_yaml`, `_git_init`, and patches `_config.resolve_plugin_template_path` + `_paths.resolve_wiki_path`, capturing `sys.stderr`):
...
`verify: PYTHONPATH= uv run --project plugins/mill python plugins/mill/unit_tests/run-all.py --only test-config.py` runs the entire `test-config.py` suite (including the two new tests). Scope is the single test file the batch edits. The positive test confirms the warning is gone for registered git keys; the negative test confirms typo-detection still works. The tests validate against the in-test `_setup_plugin_template` **fixture** template (a minimal string written by the test, NOT the real template), so Card 2 must register the `git:` block in that fixture for the tests to pass — Card 1's edit to the real template is what fixes production, and is mirrored into the fixture by Card 2.
