...
- **Deletes:** none
- **Moves:** none
- **Requirements:** In `_forward_output` and `finalize_from_output` (both in this file): rename each function's `card_count: int | None = None` parameter to `card_ids: set[int] | None = None`. Update all four call sites of `_batch_completeness_stuck` (the explicit-success path, plus the three no-JSON-inference fallback paths) and all four call sites of `_reclassify_verify_failure` (immediately preceding each of the above) to pass `card_ids=card_ids` instead of `card_count=card_count`. On the **explicit-success path only** (where `parsed: dict` from `_extract_status_json` is in scope): extract `cards_done = parsed.get("cards_done")` and `already_complete = bool(parsed.get("already_complete", False))`. Pass `cards_done=cards_done` to BOTH the `_reclassify_verify_failure` and `_batch_completeness_stuck` calls on that path (per Card 3, `_reclassify_verify_failure` DOES take `cards_done`). Pass `already_complete=already_complete` to the `_batch_completeness_stuck` call ONLY — per Card 3/Card 6, `_reclassify_verify_failure` does NOT take an `already_complete` parameter at all (passing it there would raise `TypeError`); do not add it to that call's arguments. On the **three no-JSON-inference fallback paths** (no parsed success JSON exists there — these paths run when no valid `status` JSON was found in the implementer's output): pass `cards_done=None` to both functions, and do not pass `already_complete` at all on these paths (there is nothing to self-report from, so these paths always use the absent-field fallback — this is correct and matches today's behavior for those paths, which never had a self-report mechanism to begin with). Because `finalize_from_output`'s and `_forward_output`'s `card_count` parameter is renamed to `card_ids` in this same edit, the two call sites in `millpy-implement.py` that currently pass both `card_count=card_count` and (since Card 1) `card_ids=card_ids` must drop the now-stale `card_count=card_count` keyword argument in this card's edit pass, or the renamed callee signature raises `TypeError: unexpected keyword argument 'card_count'`.
