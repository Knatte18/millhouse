...
- **Creates:** none
- **Deletes:** none
- **Requirements:** In `millpy-abandon.py` `main`, keep the local `_status.append_phase(..., "abandoned", ...)` + `git add` + `git commit -m "task: abandon {slug}"` steps, but replace the final `git push` (which currently pushes the abandon-marker commit to `origin/<branch>`) with `git -C <active_hub> push origin --delete <branch>`. Resolve `<branch>` via `_status.read_branch(status_path, cfg=cfg, slug=slug)` — NOT `info["branch"]`: `info` comes from `_status.read_status`, whose summary dict does not include `branch`. (`read_branch` returns the `branch:` value with a config-derived fallback; `cfg` and `slug` are already in scope.) Treat a non-zero exit whose stderr indicates the remote ref does not exist as success (idempotent); surface any other non-zero exit as an error. Update the success message so it no longer implies a marker commit was pushed. Add a test in `test-abandon.py` mocking `_subprocess_util.run` asserting the final git invocation is `push origin --delete <branch>` (not a bare `push`) and that a "remote ref does not exist" stderr is tolerated.
