...
- **Moves:** none
- **Requirements:**
  In the frontmatter block at the top of the file, add a new `argument-hint: "[--revise]"` line immediately after `description: ...` and before the closing `---` — matching the universal `name` → `description` → `argument-hint` ordering used by every other `argument-hint`-bearing `SKILL.md` in this repo (e.g. `mill-setup/SKILL.md`'s `argument-hint: "[--from-url <url>] [--branch <name>]"`, `mill-start/SKILL.md`'s `argument-hint: "[--auto]"`, `git-commit/SKILL.md`'s `argument-hint: "[--onmain] [message]"` — all placed after `description:`, never between `name:` and `description:`).
