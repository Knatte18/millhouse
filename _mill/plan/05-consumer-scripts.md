...
TDD order: write Card 22 (tests) first; implement Cards 23–26 to make them pass.

## Cards

### Card 22: Update test fixtures and add new consumer tests
...
### Card 24: _spawn_core.py: use render_order and extended_title in pickers
...
- **Creates:** none
- **Deletes:** none
- **Requirements:** Import `extended_title` and `render_order` from `wiki._render` at the top of `_spawn_core.py`. In `pick_task_single`: replace `_prompt_numbered(unmarked)` with `_prompt_numbered([t for t in render_order(tasks) if t["status"] is None])` — call `render_order` on the FULL `tasks` list to get correct layer assignment from the full set, then filter to unmarked in the result order. In `pick_task_single_or_multi`: apply the same pattern for the `unmarked` list passed to `_prompt_numbered_multi`. In `_prompt_numbered`: replace `t["title"]` with `extended_title(t)` in the `print(f"  {i}) ...")` line. In `_prompt_numbered_multi`: same replacement. `compute_layers` is called with `.get()` defaults so existing test fixtures that lack the new fields continue to work (missing fields default to `[]`/`False`/`False`).
- **Commit:** `feat(spawn): use render_order and extended_title in interactive pickers`

### Card 25: millpy-status.py: use extended_title for title display
...
- **Creates:** none
- **Deletes:** none
- **Requirements:** In `_build_rows`, add two new fields to each row: `"layer": home_tasks[slug].get("layer") if slug in home_tasks else None` and `"id": home_tasks[slug].get("id") if slug in home_tasks else None`. Add both to each `rows.append({...})` dict. In `_render_table`, update the TITLE column to show the layer suffix: replace the plain `title` cell value with `f"{title} [{layer}]"` when `row.get("layer")` is set and it is not `"__deferred__"` and not `"__done__"`. When `layer` is `None`, `"__deferred__"`, or `"__done__"`, show the plain `title`. Add `"layer"` as a valid `--sort` choice: `parser.add_argument("--sort", choices=["slug", "phase", "layer"], default="slug")`. When `sort_by == "layer"`, sort rows by `render_order` key order: define a helper `_layer_sort_key(row)` that maps `layer` to a tuple `(bucket_index, row.get("id") or 0)` using the canonical order `A < B < ... < Y < Z < __deferred__ < __done__ < None`.
- **Commit:** `feat(status): display layer suffix in TITLE column, add --sort layer option`

### Card 26: millpy-inspect.py: add home_layer to output
