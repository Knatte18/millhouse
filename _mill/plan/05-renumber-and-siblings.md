...
- **Requirements:**
  ...
  Apply this mapping to the list markers and to the bold step titles: step 2 becomes 1, 3 becomes 2, 4 becomes 3, 5 becomes 4, 6 becomes 5, 7 becomes 6; `4(a)`/`4(b)`/`4(c)` become `3(a)`/`3(b)`/`3(c)`; `6.5` becomes `5.5`, and its three numbered sub-items (warm `SendMessage` resume, the `--resume-incomplete` fallback, and After recovery) keep their `1.`/`2.`/`3.` local numbering.
  ...
  Two classes of reference inside the section must **not** shift, and both must be checked on every match before it is edited:
  - **A citation into another skill's numbering.**
  - **A same-file cross-namespace citation** — a reference that names a step belonging to one of the other three numbered-step namespaces card 21 enumerates, even though the sentence carrying it physically sits inside `## Agent-mode dispatch`.
    The confirmed instance is step 5's sentence beginning "The old behaviour made the orchestrator read the reviewer's entire final message", which cites `(see step 3 of "Code Review loop")` — namespace 3, and it must still read `step 3` afterwards.
    This is the collision the discussion's own `renumber-agent-mode-steps-with-namespace-scoped-sweep` Decision flags at line 332, and it is card 20's to protect: card 21's known-collision list names the same reference but card 21 is scoped to text *outside* this section and never reaches it.
  The general rule, which subsumes both: a reference shifts only when it names a step of the Agent-mode dispatch list itself.
  Location inside the section is not sufficient grounds to remap.
