...
- **Requirements:**
  ...
  - In step 2's "Subprocess/psmux branch — Background the CLI via `millpy-bg`" bash block (the one backgrounding `--slug review-discussion-r<N> -- "$MILL_PYTHON" ".../millpy-review-discussion.py"` with no trailing flags), add the same conditional: append ` --max-rounds <max_review_rounds + 1>` to the inner `millpy-review-discussion.py` invocation ONLY on the extension round, with a one-line note directly above the code block stating the condition (mirroring the wording used for step 3.5 below, for consistency). Per `mill-go/SKILL.md`'s "## Agent-mode dispatch" step 6, finalize is invoked with "the same standard arguments" as prepare — so in Agent-mode, `--max-rounds` (when present in `<args>`) is also passed to the finalize call. This is harmless and requires no special-casing: `millpy-review-discussion.py`'s `--stage finalize` branch never reads `args.max_rounds` (confirmed: `finalize()` has no `max_rounds` parameter), so the flag is silently accepted by argparse and ignored. Do not write logic to strip `--max-rounds` from the finalize call — none is needed.
