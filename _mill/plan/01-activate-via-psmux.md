...
- **Requirements:**
  ...
  3. Add `def _get_via_psmux_flag() -> bool:` placed immediately after `_has_mutating_tool`. Body:
     - Wrap the whole lookup in `try` / `except Exception` -- on any exception, `return False`.
...
- **Requirements:** Append new test cases to the existing `main()` body in `plugins/mill/unit_tests/test-llm-claude.py`, using the same `errors += 1` / `print("PASS: ...")` pattern as existing tests. Add `import re` and `import uuid` at top alongside the existing stdlib imports. Each new case mocks `_subprocess_util_mod.run` with `mock.patch.object` AND additionally monkey-patches `_llm_claude_mod._get_via_psmux_flag` to return either `True` or `False` per case (so no real config file is read). Required new cases, in order:
  ...
  11. `_get_via_psmux_flag()` fallback -- patch `_llm_claude_mod._paths.resolve_git_root` to raise `SystemExit("test")` (re-importing `_paths` inside the helper means the patch must target wherever the helper imports it; if necessary patch `sys.modules` or the local `_paths.resolve_git_root` symbol after the deferred import succeeds once). Call `_llm_claude_mod._get_via_psmux_flag()` directly; assert it returns `False` (no exception propagates). This guards the broad `try / except Exception` envelope.
