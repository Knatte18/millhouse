...
- **Deletes:** none
- **Moves:** none
- **Requirements:** In `plugins/mill/integration_tests/test-merge.py`'s flat-hub scenario, immediately before the existing `git -C str(hub) merge --squash` line (~line 614), add three new sub-scenarios covering the dirty-parent-worktree preflight check. Only the mid-Step-5-retry case needs a worktree and child branch to run a real `merge --squash`; the other two cases only need a hub-shaped git repo with one tracked file, so use a lighter fixture for those rather than three more full `_setup_trio()` spin-ups (each of which also builds a bare wiki repo, clone, child worktree, and OS junctions that these two cases never touch):
  1. **Independent uncommitted edit (lightweight fixture):** create a fresh temp directory `dirty_hub`, `git init` it, configure `user.email`/`user.name` (matching `_run`'s existing pattern), write and commit one tracked file (e.g. `README.md`, mirroring `_setup_trio`'s own `hub / "README.md"` seed at `test-merge.py:207-209`). Write an uncommitted change to that same file without staging or committing it. Run `git -C str(dirty_hub) status --porcelain --untracked-files=no` via `_run` and `_assert` the output is non-empty (`_assert(status_output.strip() != "", ...)`). Then `_assert` that the file's on-disk content still matches the uncommitted edit (proving nothing further ran that would have touched it) and that `git -C str(dirty_hub) log --oneline -n 1` does NOT show a squash-merge commit (proving the documented halt means the squash step is skipped, not attempted-and-rolled-back).
  2. **Mid-Step-5-retry (partially-applied squash):** this case needs a real child branch to squash, so use a fresh full `_setup_trio(<distinct container path>)` call, bound to distinct names `retry_hub, retry_wiki_path, retry_worktree, retry_slug` — never reusing the `hub`/`wiki_path`/`worktree`/`slug` identifiers already bound earlier in the same flat-hub `main()` scenario, which would silently repoint the existing scenario's later assertions (squash, archive tag, Home.md flip, from line ~614 onward) at the wrong trio (follow the file's own naming convention for extra scenario trios, e.g. `nested_hub` at line 791, `verify_hub_root` at line 881). `_setup_trio` returns `(hub, wiki_path, worktree, slug)` (verified at `test-merge.py:89,264` — one worktree, `slug` always the literal `"demo-merge"`). Derive `retry_child_branch = f"test/{retry_slug}"` (the branch `_setup_trio` creates the worktree on, per line 235) and run `git -C str(retry_hub) merge --squash retry_child_branch` (staging squash content) but do NOT run the subsequent `commit` — simulating a Step 5 that failed between `merge --squash` and `commit`. Run the same `git -C str(retry_hub) status --porcelain --untracked-files=no` and `_assert` the output is non-empty here too — proving the porcelain check flags this state identically to case 1 (which is exactly why `_mill/discussion.md`'s Decision gives the halt message two scenarios rather than trying to auto-distinguish them in the check itself).
  3. **Untracked-file negative check (lightweight fixture):** create a second fresh temp directory `untracked_hub` using the same lightweight `git init` + one-commit pattern as case 1 (do not reuse `dirty_hub`, since case 1 leaves it dirty by design). Write an untracked file (no `git add`) into `untracked_hub`, and `_assert` that `git -C str(untracked_hub) status --porcelain --untracked-files=no` output is empty for that file alone (proving the `--untracked-files=no` scoping choice — untracked noise must NOT trip this check).
