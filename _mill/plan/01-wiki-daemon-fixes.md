...
- **Requirements:**
  - Add new test cases following the existing `main()` + `ok()` / `fail()` shape in `test-wiki-daemon.py`. Three new cases, appended after the existing block (use the next available `(letter)` marker):
    1. **Stale-port-reuse case** — write a valid state file with `protocol_version` matching `PROTOCOL_VERSION`. Patch `wiki._client.socket.create_connection` to succeed (returns a `MagicMock` whose `.close()` is a no-op) AND patch `wiki._client._connect_send_recv` to raise `OSError`. Patch `wiki._client._spawn_server` to be a no-op. Patch `wiki._client._is_stale` to return `True`. Call `_ensure_daemon(wiki_path)`. Assert that the state file was unlinked AND `_spawn_server` was called.
    2. **Non-ok-health-response case** — same setup, but `_connect_send_recv` returns `{FIELD_OK: False, FIELD_ERROR: "test"}` instead of raising. Same expected outcome.
    3. **Successful-health case** — `_connect_send_recv` returns `{FIELD_OK: True}`. Assert `_ensure_daemon` returns the host/port/token tuple from the state file AND `_spawn_server` was NOT called.
  - Use `_test_helpers.safe_temp_dir` (already imported at line 25) for the temp wiki directory. Continue to use `shutil.rmtree(tmp, ignore_errors=True)` for the new cases — Batch 3 converts every rmtree in this file in one pass.
  - For patching, follow the `unittest.mock.patch` pattern (import via `from unittest.mock import patch, MagicMock`). The existing test file does not import mock; add the import at the top after the existing `from _test_helpers import safe_temp_dir` line.
...
- **Requirements:**
  ...
  - One new case for #384:
    3. **Orphan-deletion case** — seed the wiki dir with `proposal-old-slug.md` (any content) AND seed the store with one task whose slug is `new-slug` (no body, so render does NOT emit `proposal-new-slug.md`). Call `_handle_rerender({})`. After the call, assert `(wiki_path / "proposal-old-slug.md").exists()` is `False`. Use a spy on `wiki._sync.commit_push` and assert the call's `paths` argument contains `"proposal-old-slug.md"`.
