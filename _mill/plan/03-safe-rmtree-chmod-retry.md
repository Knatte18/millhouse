...
- **Requirements:**
  ...
  - In each of the three files, add `import _safe_rmtree` immediately after the existing `sys.path.insert(0, str(HUB / "plugins" / "mill" / "scripts"))` line (or wherever the other `import _foo` mill-script imports live in that file). Keep the `import shutil` line — `shutil` is still used elsewhere by the files (e.g. fixture setup). Verify after editing that no `shutil.rmtree` call remains in any of the three files (`grep -n "shutil\.rmtree" <file>` returns no matches).
