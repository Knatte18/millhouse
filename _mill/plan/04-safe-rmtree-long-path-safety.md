...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Add four new cases to `test-safe-rmtree.py::main()`, placed after the "top-level safe_rmtree entry-point window: outermost scandir vanishes" case and before the "non-container allowed_root does not crash" case, using this file's existing `# --- <description> ---` comment-banner convention (no lettered case names) and its `with tempfile.TemporaryDirectory() as tmp: tmp_path = Path(tmp); tree_dir = tmp_path / "tree"; tree_dir.mkdir()` setup pattern:
  ...
  - **`_is_reparse_point` routes its `os.path.isjunction`/`os.lstat` calls through `_long_path.to_extended`:** patch `_safe_rmtree._long_path.to_extended` with `MagicMock(return_value="LONGPATH-MARKER-REPARSE")`. Patch `_safe_rmtree.os.name` to `"nt"` (so the Windows branch of `_is_reparse_point` executes on the POSIX test runner) and patch `_safe_rmtree.os.path.isjunction` with a `side_effect` function that asserts its argument equals `"LONGPATH-MARKER-REPARSE"` and returns `False` — matching this file's existing `hasattr(os.path, "isjunction")` real-attribute assumption, so no separate `hasattr` patch is needed. Call `_safe_rmtree._is_reparse_point(tree_dir)` directly (no `safe_rmtree` wrapper call needed — this is a direct unit test of the helper). The `side_effect`'s internal assertion is what proves the marker was passed; also assert the call returns `False` (the mocked `isjunction` result passed through unchanged).
- **Commit:** `test(safe-rmtree): cover extended-length path usage in walker scandir, rmtree root, and _is_reparse_point`

## Batch Tests

`verify:` runs `test-safe-rmtree.py`, covering the file's pre-existing coverage plus the four new cases added by Card 8.
