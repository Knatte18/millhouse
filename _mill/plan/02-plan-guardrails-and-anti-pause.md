...
- **Moves:** none
- **Requirements:**
  In `plugins/mill/skills/mill-go/SKILL.md`'s "## Agent-mode dispatch" section, "**Agent-mode properties:**" bullet list (lines 173-179), add a new bullet immediately after the last existing bullet (the `incomplete` stuck-errors bullet, line 178): "- Waiting on a dispatch — either branch — is never a decision point: state in one sentence what you're waiting for, then wait. `AskUserQuestion` (and, under `mill:conversation`'s numbered-options rule, any equivalent operator prompt) is banned here absent an explicitly named escape hatch for the calling phase." Do not add this rule inside the shared dispatch steps (1-7) themselves — only this bullet list — since those steps are reused by non-review Implement/Fix/merge-in dispatch, where each phase's own stuck-escalation rules already govern waiting.
