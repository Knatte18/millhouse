...
- **Requirements:** Do NOT change the existing
  ...
  Every subsequent use of `project_root` (the `require_status_path` guard and the `prepare(...)` call's `project_root=project_root` keyword argument) resolves against the corrected worktree. This file's `briefs_dir = _paths.resolve_task_path(git_root, "_mill/briefs/")` line already reads `git_root`, not `project_root` — `git_root` (from `_paths.resolve_git_root()`) is cwd-based and does not have `resolve_hub_path()`'s escaping-fallback bug, so that line needs no change; re-verify this is still the case when editing (read the current line before assuming) and leave it untouched if so.
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** For each of the 3 test files listed under Edits, add one focused regression test matching that file's existing mocking conventions: mock `_paths.resolve_hub_path` to return a decoy directory simulating the "escaped to main worktree" failure mode (a tmpdir distinct from the fixture's real task worktree), and mock `_paths.resolve_active_hub` to return the fixture's real task worktree path, then assert that `briefs_dir` (`test-review-plan-flow.py` and `test-review-discussion-flow.py`) or `status_path` (whichever is the simplest observable surface for `test-review-code-flow.py`'s existing harness) ends up resolving under the real task worktree, not the decoy. The assertion should fail against the pre-Card-12-through-14 code (which would resolve everything under the decoy) and pass after. For `millpy-review-code.py` specifically, also add a regression test confirming `briefs_dir` (already correctly `git_root`-based, unchanged by Card 12) is unaffected by this batch's edits — i.e. still resolves correctly regardless of what `resolve_hub_path`/`resolve_active_hub` are mocked to return, since it never consults them. Do not weaken or remove any existing test in these 3 files; this card only adds new regression coverage.
