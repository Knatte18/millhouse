...
- **Requirements:** Create a `unittest.TestCase` test file that exercises the exit-code contract for all three reviewer CLIs without invoking a real LLM. Use `unittest.mock.patch` to stub every pre-launch dependency AND the backend's `run` function, then assert the contract for three paths:
  ...
  **Loading the CLI modules:** each CLI filename contains hyphens (`millpy-review-discussion.py`, `millpy-review-code.py`, `millpy-review-plan.py`), so a bare `import millpy-review-discussion` is a Python syntax error. Load each module via `importlib.util.spec_from_file_location("millpy_review_<X>", scripts_dir / "millpy-review-<X>.py")` followed by `spec.loader.exec_module(module)`. Cache the loaded module object so `main` and the patched module-level helpers (see "Pre-launch dependency patches" above) refer to the same instance. The `scripts_dir` resolves to `Path(__file__).parent.parent / "scripts"` from the test file's location.
