...
- **Requirements:** Create a new CLI script. Top-level structure: shebang, module docstring (one paragraph explaining purpose + invocation), imports (`pathlib.Path`, `sys`, `shutil`, `yaml`, plus local `_paths`, `_wiki`, `_subprocess_util`). Define a `main() -> int` returning the process exit code. `if __name__ == "__main__": sys.exit(main())`.
  ...
  4. **Config migration:**
     ...
     - Either way, when `wiki_config` exists, delete it from the wiki: inside `with _wiki.wiki_lock(wiki_path, slug="migrate-config"):`, run `_subprocess_util.run(["git", "-C", str(wiki_path), "rm", "config.yaml"])` then call `_wiki.write_commit_push(wiki_path, ["config.yaml"], "chore(migrate): remove wiki/config.yaml (moved to hub/mill-config.yaml)", slug="migrate-config")`. (Match the `write_commit_push` signature in the current codebase; consult `_wiki.py` if uncertain. The slug kwarg may not be required by every version of the helper.) Print `[migrate] wiki/config.yaml deleted and pushed`.
  5. **Agents migration:**
     ...
     - If `wiki_data == template_data` (deep equality): delete the wiki file via `git -C <wiki_path> rm agents.yaml` inside the same lock-block pattern + `write_commit_push` with message `chore(migrate): remove wiki/agents.yaml (moved to plugin template)`. Print `[migrate] wiki/agents.yaml identical to plugin template -- deleted and pushed`.
