...
### Card 2: Update test-status.py for the accessor changes
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Leave the existing "--- baseline_parent_sha tests ---" block and the `get_baseline_parent_sha`/`set_baseline_parent_sha` imports untouched in this card — per this batch's own Batch Scope note, the accessor pair itself is not deleted until batch 4, and its test coverage is deleted there too (batch 4's Card 26), alongside the function deletion, not here.
