...
- **Creates:** none
- **Deletes:** none
- **Requirements:** Replace the `wiki_lock` + `_tasks_md.append_entry` + `_wiki.write_commit_push` flow at lines 169 and 198. Preserve V2's duplicate-slug rejection: before the upsert, call `existing = wiki.get_task(wiki_path, args.slug)` and if `existing is not None`, `raise SystemExit(f"Slug {args.slug!r} already present in Home.md.")` (matching V2's exact error message text for grep continuity). Otherwise call `wiki.upsert_task(wiki_path, slug, title=..., brief=..., body=..., group=...)`. `wiki_path` is the local already resolved earlier in the file for the V2 `_wiki.write_commit_push` call — reuse it. The normal-add flow does not pass `status=...` (new tasks default to unmarked / `None`; phase transitions happen later via `wiki.set_phase`). Drop the `from _wiki import ...` and `from _tasks_md import ...` imports. Drop the `_sidebar` regeneration call if present — the daemon renders `_Sidebar.md` automatically. Drop any `LockBusy` exception handling — there is no advisory lock; let `WikiPushError` propagate. The argparse and prompt flow are untouched.
...
- **Requirements:** Line-by-line:
  ...
  - `:135` — replace `_tasks_md.append_to_body(...)` with `task = wiki.get_task(wiki_path, target_slug); wiki.upsert_task(wiki_path, target_slug, brief=((task["brief"] or "") + "\n" + fold_line))`. **Append to the `brief` field, not `body`** — the V3 renderer (batch 1 card 2) emits `brief` as the inline text under the Home.md slug line (which is where V2's `_tasks_md.append_to_body` visually landed), while `body` renders to `proposal-{slug}.md`. Writing to `body` would (a) hide the fold note from Home.md and (b) silently create a `proposal-{slug}.md` for any task that previously had no proposal. The full-dict shape (from `wiki.get_task`, which returns the full record) carries `brief`; if `task["brief"]` is `None` or missing, treat as empty string.
...
- **Requirements:** Line-by-line:
  ...
  - `multi_select_groom_then_claim` (function defined at `:446`; body roughly `:446-525` — the discussion's older citation of `groom_and_claim_merge` at `:488-514` is stale; the canonical name in current source is `multi_select_groom_then_claim`) — replace the `wiki_lock` + read-modify-write window inside the function body with a single atomic call: `result = wiki.merge_tasks(wiki_path, remove_slugs=[slug_a, slug_b, ...], upsert={"slug": merged_slug, "title": ..., "brief": ..., "body": ..., "group": ...}, set_phase=(merged_slug, "active"))`. The daemon's `OP_MERGE_TASKS` handler performs the remove + upsert + set_phase in one TinyDB transaction, then renders + commits once (per batch 1 card 5). A crash mid-sequence is impossible because all three sub-ops happen inside one DB transaction before the render or commit fires. No advisory-lock wrapper. No client-level retries. Cross-host non-fast-forward push is handled by `_sync.commit_push`'s one-rebase retry; permanent failure raises `WikiPushError` and propagates to the caller. The `has_proposal` field is computed by `Store.list_tasks_brief` from `body`, so callers do not pass it. The internal helper line citations `:493` and `:494` (originally for the old name) are now subsumed by the single `merge_tasks` call.
  ...
  - `claim_in_wiki` (function at `:624`, body roughly `:624-644`; older `:636-641` citation is stale) — replace the `wiki_lock` + Home.md read + `_tasks_md.claim` + write + `_wiki.write_commit_push` flow with a single `wiki.set_phase(wiki_path, slug, "active")` call. Drop the surrounding `with wiki_lock(...)` context manager.
  - **Attribute-to-dict-key conversion inside `_spawn_core.py`'s own function bodies.** Once `_tasks_md.Task` is gone, every `t.<attr>` access inside this file raises `AttributeError` at runtime — tasks are dicts now. Grep the file end-to-end for `t\.[a-z_]+` (the iteration variable in `pick_task_single`, `_prompt_numbered`, `_prompt_numbered_multi`, `pick_task_single_or_multi`, `prompt_merged_entry`, `multi_select_groom_then_claim`, `discover_active_worktrees`, and any other function bodies) and convert every `.phase`, `.slug`, `.title`, `.brief`, `.group`, `.status`, `.has_proposal` access to dict-key access (`t["phase"]`, `t["slug"]`, etc.). Also widen the regex to other iteration variable names (`task`, `entry`, `picked`, `chosen`, etc.) — anywhere a `Task` instance previously came from `_tasks_md.parse`, the new equivalent is a dict from `wiki.list_tasks_brief(wiki_path)` or `wiki.get_task(wiki_path, slug)`. Run the file's imports through a syntax sanity check after the rewrite (no remaining `_tasks_md.Task` references, no remaining `.phase`/`.slug`/etc. on these locals).
  - `:264` (docstring), `:292`, `:302`, `:421`, `:431` — remove the `[s]` (spawn-ready) phase fast-paths. After batch 1 card 3 lands, `_parse.py` parses `[s]` to `None`, so no task ever carries `phase == "s"` at runtime. Specifically: at lines 292 and 421 drop `"s"` from the `t.phase in (None, "s")` allowlists (becomes `t.phase is None`); at lines 302 and 431 delete the `fast = next((t for t in tasks if t.phase == "s"), None)` lookups and the subsequent branch that returns the fast-path task; update the docstring at line 264 to drop the `"s"` mention. Tasks are picked from the `phase is None` pool only.

  Drop the `_wiki` import. Drop any `LockBusy` exception handling (no client retries any more — let `WikiPushError` propagate to the caller, which surfaces it to the user; no special-case handling needed).
