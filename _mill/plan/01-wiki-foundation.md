...
- **Deletes:** none
- **Requirements:**
  Import `hashlib` from stdlib and `WikiPathError` from `wiki`. Define `class Store`: instance var `_cache: dict[str, tuple[str, str]]` (maps `rel_path -> (content, hash)`). Method `content_hash(content: str) -> str` — returns `hashlib.sha256(content.encode("utf-8")).hexdigest()`. Method `get(rel_path: str) -> tuple[str, str] | None` — returns `(content, hash)` or `None` on miss. Method `set(rel_path: str, content: str, hash: str) -> None` — stores the pair; hash must match `content_hash(content)`, raise `WikiPathError` if not (guard against caller-provided mismatched hash). Actually, drop the hash-match guard — the caller computes the hash and the store just stores what it's given (the server always uses `content_hash`); keep the store simple. Method `set(rel_path: str, content: str) -> None` — computes hash via `content_hash`, stores `(content, hash)`. Method `get(rel_path: str) -> tuple[str, str] | None`. Method `invalidate(rel_path: str) -> None` — removes the entry (no-op if absent). Method `invalidate_all() -> None` — clears the entire cache. No TTL, no size limit.
