...
- **Requirements:**
  ...
  - `SKILLS.md` is generated ("Do not edit by hand"): do not hand-edit it; regenerate it from the worktree root with `PYTHONPATH= uv run --project plugins/mill python plugins/mill/scripts/millpy-skills-index.py` after card 4's frontmatter has landed. The resulting diff must be exactly the `ask-parent` row replaced by an `ask-thread` row linking `plugins/mill/skills/ask-thread/SKILL.md` with the new frontmatter `description` (the script reproduces the current file byte-for-byte otherwise, checked at plan time).
