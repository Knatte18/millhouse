...
- **Creates:** none
- **Deletes:** none
- **Requirements:** First read the existing `test-config.py` to determine the test framework (unittest vs pytest) and the fixture pattern (tmp_path vs hand-rolled TemporaryDirectory). Match that style. Add these test cases (one function per case; descriptive names):
  ...
  - `test_fallback_to_wiki_config_yaml` -- repo has NO `mill-config.yaml`; sibling wiki has `config.yaml` with a distinctive value. Confirm the loaded config contains that value AND stderr emitted a fallback warning naming the wiki path.
  - `test_both_files_present_mill_wins` -- repo has `mill-config.yaml` with key `K=mill`; wiki has `config.yaml` with key `K=wiki`. Confirm loaded config has `K=mill` AND stderr emitted a "stale wiki/config.yaml" warning.
...
- **Creates:** none
- **Deletes:** none
- **Requirements:** First read existing `test-review-common.py` to confirm the test framework. Add cases parallel to card 14 covering: three-layer merge (`test_load_config_three_layer_merge`), env overrides (one consolidated test or six parallel tests -- match what was done in `test-config.py`), unknown-key warning, fallback to `wiki/config.yaml`, both-files-present. PLUS two cases specific to `_review_common.load_config`:
...
- **Requirements:** Read existing `test-reviewers.py` to determine framework and fixture style. Add these cases:
  ...
  - `test_legacy_wiki_fallback_when_no_plugin_template` -- monkeypatch `_resolve_plugin_template_path` to return a non-existent path; no `.millhouse/agents.local.yaml`; create a sibling wiki dir at the expected location with `agents.yaml` containing 2 entries. Confirm `load(hub_dir)` returns those entries AND stderr emitted a legacy-fallback warning.
