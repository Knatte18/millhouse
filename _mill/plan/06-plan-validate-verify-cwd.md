...
- **Deletes:** none
- **Moves:** none
- **Requirements:** `_check_verify_full_suite(batch_files: list[Path])` gains a `project_root: Path` parameter (matching `_check_verify_not_isolated`'s existing signature, which already has one). Both functions gain an `overview_path: Path` parameter. In both functions, replace the `verify = parsed.get("verify"); if verify is None or not isinstance(verify, str): continue` guard (applied per batch file) with a call that also accepts the mapping form: attempt `command, _cwd = _plan_dag.parse_verify_field(parsed, project_root, project_root)` (both roots the same is sufficient here — these checks only need the extracted command string, not the resolved cwd), and on a `ValueError` (malformed mapping) `continue` to the next batch **without emitting a finding** — Card 24's dedicated `_check_verify_malformed_cwd` is the sole reporter for malformed mappings, so these two functions must not duplicate that finding; otherwise apply the existing string checks (`PYTHONPATH=` prefix check, `run-all.py --only` filter check) to `command`. Extend both functions to also check the overview file's own `verify:` frontmatter (read via `overview_path`, previously batch-file-only) using the same `parse_verify_field` extraction and the same continue-on-`ValueError` rule, iterating it alongside the existing per-batch loop. Update `run()`'s two call sites (`_check_verify_not_isolated(batch_files, project_root)` and `_check_verify_full_suite(batch_files)`) to pass the new `overview_path` argument (and, for `_check_verify_full_suite`, the new `project_root` argument) — `run()` already has both `project_root` and `overview_path` as local variables in scope.
