...
- **Requirements:**
  ...
  5. Update `run_preflight`'s and `run_gate`'s docstrings. `run_preflight`'s docstring currently states "a non-zero exit is reported as `blocked` with the captured output" and "so both call sites (this pre-flight and the Handoff-time gate) treat the same `gate_cmd` identically" — both sentences live only in `run_preflight`'s own docstring; `run_gate`'s docstring instead says "Mirrors `run_preflight`'s subprocess-invocation shape and never-raise contract exactly -- including its stdout+stderr concatenation and 2000-character tail truncation on failure," referencing `run_preflight` by name rather than repeating those two sentences verbatim. Both functions' `Returns:` sections currently read `{"result": "blocked", "reason": <captured output, tail-truncated to 2000 chars>}` (this part IS identically shaped in both docstrings). Change both `Returns:` descriptions to state that on truncation, `reason` is an omitted-content marker (naming the omitted character count, plus up to 20 extracted failure-marker lines recovered from the omitted portion when any exist) followed by the last 2000 characters — not a bare tail. Also update `run_gate`'s "including its stdout+stderr concatenation and 2000-character tail truncation on failure" phrase (since truncation no longer means a bare tail) to note it mirrors `run_preflight`'s new enriched-truncation shape instead. `run_preflight`'s own "so both call sites ... treat the same `gate_cmd` identically" sentence is still accurate and needs no further change, since this card keeps both functions calling the identical `_build_truncated_reason` helper.
