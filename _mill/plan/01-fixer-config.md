...
- **Creates:** none
- **Deletes:** none
- **Requirements:** Add a new active (uncommented) top-level `roles:` block (the file currently has the whole `roles:` block commented out, but the deep-merge means the template's default is what propagates). Specifically, append a `roles:` mapping to the active section of the file with a single `fixer:` subsection whose only key is `model: haiku`. Place the new block immediately below the existing `pipeline:` block (around the file's `# Reviewer roles` divider comment) and BEFORE the `# Notifications` divider. Keep the existing commented-out documentation block for `roles:` intact -- it documents the full schema for operators. Do NOT modify any other key in the file. The end state is that `_config.load_config` returns a dict where `cfg["roles"]["fixer"]["model"] == "haiku"` for any hub that has not overridden it locally.
