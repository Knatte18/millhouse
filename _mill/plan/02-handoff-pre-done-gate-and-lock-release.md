...
- **Requirements:** Immediately after Card 3's "proceed to the fixer-dispatch check below before halting" sentence, add a new "fixer-dispatch check" step, replacing the section's final `BLOCKED: done gate failed — <reason>` halt with the following control flow (the halt itself moves to the end of this new logic, not immediately after the JSON parse):
  ...
  Append the sentence "Do NOT set `phase: done` when the gate fires; the task remains in its current phase so the operator can investigate the failure." as its own line immediately after EACH of the two halt messages above (step 2's still-blocked halt AND step 3's halt) — write it twice, once per branch, not once shared between them. This supersedes Card 3's original single-occurrence placement of the same sentence (Card 3 deletes it from its old position; this card is where both live copies land).
