...
- **Requirements:**
  ...
  3. **Step 2, subprocess/psmux branch.** Locate the blockquote "> **Before invoking `millpy-bg`**: verify `pwd` in the Bash terminal matches the task worktree..." that appears under "**Subprocess/psmux branch — Invoke the CLI as a subprocess:**", ahead of the round-cap-override reminder blockquote and the `--slug plan-review-r<N> --` bash invocation that follow it. Insert immediately before this blockquote, verbatim: "Run the discussion drift guard (see 'Discussion drift guard' above) now, before invoking `millpy-bg`."
  ...
  7. **Step 3.5, subprocess/psmux branch.** Locate the second occurrence of the blockquote "> **Before invoking `millpy-bg`**: verify `pwd` in the Bash terminal matches the task worktree..." — the one under Step 3.5's own "**Subprocess/psmux branch:**" heading, ahead of its own round-cap-override reminder blockquote and the `--slug plan-review-retry-r<N> --` invocation that follow it. Insert immediately before this blockquote, verbatim: "Run the discussion drift guard (see 'Discussion drift guard' above) now, before invoking `millpy-bg`."
