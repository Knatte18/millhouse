...
- **Requirements:** Add a `--module-wide-only` boolean CLI flag (argparse `action="store_true"`, default `False`) to `millpy-implement.py`'s `--stage baseline` argument handling, and thread it into `_run_baseline_stage` as a new `module_wide_only: bool` parameter.
  ...
  Add `test_stage_baseline_module_wide_only_skips_per_batch_loop` to `plugins/mill/unit_tests/test-verify-baseline.py` — first read the existing test functions in that file to identify its fixture conventions for invoking `--stage baseline` (config, worktree, status.md fixtures) and match them. The new test invokes the baseline stage with `--module-wide-only` against a fixture plan directory containing at least one batch file with a non-null `verify:` command, and a fixture `status.md` that has NO `## Batches` section at all (simulating the pre-`## Prepare` scenario) and asserts: (a) the call completes without raising; (b) the printed per-batch JSON line has `computed: []`, `cached: []`, `errored: {}`; (c) `_verify_baseline._checkout_parent_branch` is never invoked (spy/mock it per the file's existing test conventions, or assert no transient checkout directory is left behind) — this is the assertion that specifically catches the Case-B-fallthrough bug this fix closes, distinguishing "the loop body didn't run" from "the expensive shared-checkout setup didn't run either"; (d) when the fixture's config declares a module-wide verify command, the module-wide substage's own result is still written to `status.md`'s `module_verify_baseline:` field (confirming the module-wide half is genuinely unaffected by the flag).
