...
- **Requirements:**
  ...
  - Add `test_task_data_retries_on_cold_daemon()`: same mocking applied to `_marker.wiki.list_tasks_brief` and `_marker.wiki.health_check`, calling `task_data()` instead of `slug_from_branch()` directly. Note `task_data()` makes 3 total `list_tasks_brief` calls after Card 2's fix — 2 inside its own internal `slug_from_branch()` call (1 failure + 1 retry-success) plus a 3rd from `task_data()`'s own separate `_list_tasks_brief_with_retry()` call — so a naive 2-item `side_effect` list raises `StopIteration` on the third call (flagged by plan review round 3). Use a 3-item `side_effect` list (`[wiki.WikiStartupError("cold"), <real return value>, <real return value>]`) or an equivalent unbounded side_effect (e.g. an `itertools.repeat`-based callable) so the third call also succeeds. Assert `task_data()` still returns the correct `{"slug", "branch", "task_title"}` dict — proving `task_data()`'s independent call site is now routed through the same retry helper as `slug_from_branch()` (Card 2).
