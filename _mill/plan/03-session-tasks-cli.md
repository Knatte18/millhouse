...
- **Requirements:** Create `plugins/mill/scripts/millpy-session-tasks.py` in `millpy-color.py`'s style (module docstring with usage and exit codes, `main(argv: list[str] | None = None) -> int`, `if __name__ == "__main__": sys.exit(main())`).
  ...
  Any other failure (`SystemExit` from config/path resolution, `OSError`, `ValueError` from `_vscode_tasks`) is caught, reported as one ASCII line on stderr prefixed `[mill-session-tasks]`, and returns 1 with nothing written.
