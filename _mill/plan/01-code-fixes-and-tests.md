...
- **Requirements:**
  ...
  **Case 39 — Bug #557 (inference path, start-batch commit only, snapshot present → stuck/logic):**
  Use `_setup_fixture()`. Capture `pre_start_sha`. Call `_cleanliness.capture_snapshot(project_root, snapshot_path)` (or the equivalent function used by existing snapshot-present test cases) to write a valid snapshot of the clean repo state. Make the "mill-go: start batch test-batch" commit. Leave the tree clean. Call `_forward_output` with `output="no json here"` (non-JSON), `start_sha=pre_start_sha`, `snapshot_path=<the snapshot file>`. The inference path should fire (HEAD != pre_start_sha, tree clean, snapshot-present branch), hit the `_is_only_start_batch_commit` guard, and emit stuck/logic. Assert `"status": "stuck"` AND `"stuck_type": "logic"` AND that `"reason"` contains `"start-batch commit"` (or the exact phrase used in the guard's message). The reason assertion confirms the guard fired — not some other stuck/logic path. This exercises the snapshot-present clean-tree emit path (line ~851), NOT the no-snapshot path.
