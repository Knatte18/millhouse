...
- **Requirements:**
  ...
  - **Tests** in `test-implementer-common.py`:
    - #500 regression: a parsed `status:success` report passed to `_forward_output` where `start_sha` equals the current `HEAD` (i.e. NO new commit was made after `start_sha` was recorded) → assert the emitted JSON is `stuck` with `stuck_type: logic` and a reason naming the no-content-commit condition. For fixture setup, reuse the verify-gate parsed-success cases' scaffolding but do NOT add the extra commit those cases make to advance HEAD — leave `HEAD == start_sha` so the no-content-commit guard fires. (Verify must still PASS in this fixture, to prove the guard rejects a no-commit success independently of the verify gate.)
