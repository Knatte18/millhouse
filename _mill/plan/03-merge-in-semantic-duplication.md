...
- **Deletes:** none
- **Moves:** none
- **Requirements:** In the `## Instructions` section of `plugins/mill/templates/merge-in-conflict-brief.md`, add a new numbered instruction (becoming the new step 4) immediately after the existing step 3 ("combine both edits" for disjoint-region conflicts, currently the paragraph ending in "it does not discard either") and before the existing step 4 ("Run `git -C <PROJECT_ROOT> add <file>`"). Renumber the existing steps 4, 5, and 6 (`git add`; the modify/delete DU-conflict handling; the UD-conflict handling) to 5, 6, and 7 respectively, and update any in-file cross-reference to those step numbers if present. The new instruction: before keeping content from either side inside a conflict hunk, search the rest of the file (outside the hunk) for that same content, with two explicit branches:
