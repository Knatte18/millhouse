...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Modify `_check_non_existent_path` to accept the Move unions: add a `moves_targets` set to the suppression alongside `creates_union` (a downstream card editing a Move target must not raise non-existent-path) and treat Move sources via the same existence rule as general refs. Modify `_check_all_files_touched_mismatch` so `cards_set` includes Move **targets** (add `compute_moves_union(plan_dir)[1]`), keeping Move sources excluded — mirroring the existing exclusion of `Deletes:` tokens (issue #494). Update the function signatures and the `run()` call sites are updated in card 10.
