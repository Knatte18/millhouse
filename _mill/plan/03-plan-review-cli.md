...
`verify:` runs `test-review-plan-flow.py` (exercises `_review_plan.py`'s `prepare()`/`run()` directly — the functions Cards 5-6 change), plus `test-review-prepare-envelope.py` and `test-review-cli-error-envelope.py` (both load `millpy-review-plan.py` via `importlib` and exercise its `--stage prepare`/error-envelope contract — the CLI Card 7 changes), scoped via `run-all.py --only`. The new `reviewer_override`-specific test cases are added in the later `unit-tests` batch; this batch's own `verify:` is a regression check that these three existing files still pass unmodified after the signature/flag changes.
