...
- **Requirements:**
  ...
  - Add an entry under the existing module-docstring's "Public API:" section (the header block above the `from __future__ import annotations` line) describing `status_path(worktree_root, cfg)`. Match the prose style of `resolve_hub_relative_path`'s docstring entry (a short one-paragraph form with signature on the heading line). Note: `resolve_task_path` exists in `__all__` and as a function definition but does not currently have a Public API docstring entry -- do not look there for style.
