...
- **Requirements:**
  ...
  - Update the in-file comments and docstrings in `_worktree.py` that reference `shutil.rmtree` by name so they reference `_safe_rmtree.safe_rmtree` (or `_safe_rmtree` for short) instead. Replacements (verify current line numbers when editing; the descriptions below describe text, not line number):
    - The `remove_safe` docstring sentence "``shutil.rmtree`` -- safe NOW because junctions are already gone --" -> "``_safe_rmtree.safe_rmtree`` -- safe NOW because junctions are already gone --".
    - The `remove_safe` docstring sentence "``shutil.rmtree(ignore_errors=...)`` decision" -> "``_safe_rmtree.safe_rmtree(ignore_errors=...)`` decision".
    - The inline comment `# Long-path / not-a-working-tree fallback. Junctions are stripped, so shutil.rmtree is safe.` -> `# Long-path / not-a-working-tree fallback. Junctions are stripped, so _safe_rmtree is safe.`.
    - The log string `"[worktree] remove_safe: git failed; falling back to shutil.rmtree (junctions already stripped)"` -> `"[worktree] remove_safe: git failed; falling back to _safe_rmtree (junctions already stripped)"`. ASCII-only; the existing line break / formatting stays unchanged.
  - After editing, the source of `_worktree.py` must contain zero matches for the regex `shutil\.rmtree` (verifiable via `grep -E 'shutil\.rmtree' plugins/mill/scripts/_worktree.py`). The `import shutil` line stays because `shutil.copytree` / `shutil.copy2` are still used in `copy_millhouse`.
...
- **Requirements:**
  - The four files contain two different rmtree-call shapes; the migration handles both. For each file, locate every match of `shutil\.rmtree` and apply the appropriate transform:
    ...
    - **Shape B: `shutil.rmtree(str(tmpdir), ignore_errors=True)`** -- direct call inside a `try: ... finally:` block. Transform to `_safe_rmtree.safe_rmtree(tmpdir, allowed_root=tmpdir, ignore_errors=True)` (drop the redundant `str(...)` cast -- `safe_rmtree` accepts `Path` directly and binds its own internal path objects). Files using this shape:
      - `test-millpy-spawn.py:825` (one call in a `finally` block, inside `test_spawn_standard_layout_regression`).
      - `test-millpy-spawn.py:887` (one call in a `finally` block, inside `test_spawn_subfolder_install_destination_layout`).
      - `test-millpy-spawn.py:1002` (one call in a `finally` block, inside `test_spawn_discovery_round_trip_subfolder`).
  - In `test-millpy-spawn.py` specifically: each of the three test functions named above opens with a function-local `import shutil` line (currently at `:783`, `:837`, `:899`). After the migration replaces every `shutil.rmtree(...)` call inside the function body, those inline imports become dead code -- remove each one. Do NOT touch the unrelated `import shutil as _shutil` at `:647` (separate scope and aliased -- only remove if a grep confirms `_shutil` is also unreferenced after the edit). The module has no top-level `import shutil`; no module-level import change is needed. For each affected function, add a function-local `import _safe_rmtree` line in place of the removed `import shutil` (the file follows a per-function inline-import idiom; preserve that convention rather than adding a module-level import).
