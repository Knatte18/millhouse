...
- **Requirements:** Following the existing `resolve_active_worktree`/`resolve_active_hub` test block's style in `test-paths.py` (the M1/M1+sub/M2/M2+sub scenarios using `patch(...)` and `print("PASS: ...")`), add:
  1. **`skip_slug_validation=True`, in-place mode:** **do NOT reuse the existing M2 fixture as-is (plan-review round 4 BLOCKING finding)** — the existing `"resolve_active_worktree M2 — in-place returns git_root"` test's `git_root` is a bare `mkdir()`'d directory (no real git repo), which only works there because that test mocks `_marker.slug_from_branch` directly rather than exercising real git. Card 6's `skip_slug_validation=True` branch instead calls `_pygit2_util.current_branch(git_root)` against the real filesystem — against a bare directory this raises `GitOpsError` (caught → `branch=""`), the in-place check never matches, and the function falls through to `ActiveWorktreeNotFound` (`container_path/"wts"/slug` was never created by this fixture either). Build a **real** git repo at `git_root` instead, using `_test_helpers.init_minimal_git_repo(git_root, branch="main")` + `_test_helpers.checkout_new_branch(repo, "my-task")` (mirroring `_make_active_marker`'s pattern, applied directly to `git_root` rather than a separate worktree dir — check out the branch name equal to the slug itself, since this fixture's `cfg` has no `spawn.branch_prefix` key, so the cheap prefix-strip check compares the raw branch name against `slug` with an empty prefix). Then patch `_marker.slug_from_branch` to raise `AssertionError("daemon should not be called")` if invoked, call `resolve_active_worktree(..., skip_slug_validation=True)`, and assert it returns `git_root` without the patched `slug_from_branch` ever being called.
  2. **`skip_slug_validation=True`, worktree mode:** construct the same M1 (container-form worktree) fixture the existing `"resolve_active_worktree M1 (new sig)"` test uses (this one already uses `_make_active_marker`, i.e. a real git repo — no fixture change needed here), same daemon-must-not-be-called patch, call with `skip_slug_validation=True`, assert it returns `container_path / "wts" / slug` without the patched `slug_from_branch` being called.
  ...
  4. **`resolve_active_hub` threads the parameter:** one test calling `resolve_active_hub(..., skip_slug_validation=True)` on an in-place fixture built the same real-git-repo way as test 1 above (not the bare-directory M2 fixture), patched daemon-must-not-be-called, asserting the hub path resolves correctly.
