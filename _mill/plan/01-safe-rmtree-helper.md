...
- **Requirements:**
  ...
  - Monkeypatch target convention: every blacklist scenario patches `_safe_rmtree._paths.resolve_container_path` (NOT bare `_paths.resolve_container_path`) so the patch is scoped to the consumer module's namespace. Form: `with patch("_safe_rmtree._paths.resolve_container_path", return_value=container): ...`. This works because `_safe_rmtree.py` does `import _paths` (per Card 1's imports list), making `_safe_rmtree._paths` resolvable. The same applies to any scenario that needs to override blacklist construction.
  - Scenarios (each its own `with tempfile.TemporaryDirectory() as tmp:` block; each ends with a `print("PASS: <scenario>")` line):
    - `refuses on path == container root`: build the fake layout above, attempt `safe_rmtree(container, allowed_root=container)` inside a `with patch("_safe_rmtree._paths.resolve_container_path", return_value=container):` block, assert `SystemExit` is raised.
    - `refuses on path == container/wiki`: same patch; target `container / "wiki"`, assert `SystemExit`.
    - `refuses on path == container/portals`: same patch; target `container / "portals"`, assert `SystemExit`.
    - `refuses on path == container/wts/container`: same patch; target the main-repo worktree path, assert `SystemExit`.
    - `refuses on path ancestor of blacklist`: target `<tmp>` (which contains `container` and therefore the wiki); `allowed_root=<tmp>`; same patch (`_safe_rmtree._paths.resolve_container_path` returning `<tmp>/container`); assert `SystemExit`. Verifies the `resolved_path in entry.parents` ancestor check.
