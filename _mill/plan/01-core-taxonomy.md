...
- **Requirements:** Add `extract_findings(raw_text: str) -> list[Finding]` to `_review_common.py`, placed immediately after `count_unrecognized_severity_findings`.
  ...
  Results from the two scans are concatenated heading-scan-first and then deduplicated **across mechanisms only**: a title produced by the yaml scan is dropped when the heading scan already produced that title, and a title produced by the yaml scan is dropped when an earlier yaml entry already produced it.
