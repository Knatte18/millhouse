...
- **Requirements:** Add `apply_blocking_ceiling(findings: list[Finding], blocking_classes: frozenset[str]) -> list[Finding]` to `_review_common.py`.
  ...
  For each finding with `demoted is True`, keyed on its `title`:
  ...
  Match each heading exactly once, anchored at line start, and leave every line of `raw_text` not covered by (a) or (b) byte-identical.
