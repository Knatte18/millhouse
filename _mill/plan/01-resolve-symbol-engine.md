...
- **Requirements:** Change `_symbol_candidate_shape`'s return type from `str | None` to `tuple[str, str | None] | None`: `None` when the token is not symbol-shaped (unchanged condition), else a 2-tuple of `(search_key, qualifier)` where `search_key` is exactly what the function returns today (the bare identifier, or a dotted pair's trailing segment) and `qualifier` is `segments[0]` when `len(segments) == 2` (today's dotted case) else `None` (today's bare-identifier case). Do not change which tokens qualify — the existing `qualifies()` helper and the `_RE_SYMBOL_SHAPE` gate are unchanged.
  ...
  `_check_context_completeness` has two separate edit sites for this card, not one contiguous region — an early shape-gate call and a later, distinct resolution block:

  1. The early shape-gate call (inside `if not is_path_shaped:`, before the four shared exemption checks that follow it): today it does `search_key = _symbol_candidate_shape(token)` then `if search_key is None: continue`. Change this to unpack the new tuple: `shape_result = _symbol_candidate_shape(token)`, `if shape_result is None: continue`, `search_key, qualifier = shape_result`.
  2. The later resolution code, in the separate `else:` arm that runs after `is_path_shaped` is tested a second time (this arm holds the `search_cache` lookup, the `_resolve_symbol_files` call, the `len(matches) != 1` guard, and `matches[0].relative_to(producing_root)`): every use of `search_key` there keeps using the `search_key` string unpacked at site 1 above, exactly as before — `_resolve_symbol_files` itself is not touched by this card and takes no `qualifier` argument. The new `qualifier` variable from site 1 is consumed here (see the disambiguation logic below), carried from site 1 to site 2 the same way `search_key` already is today.
