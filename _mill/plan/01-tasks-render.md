...
- **Requirements:** Create `plugins/mill/scripts/_vscode_tasks.py`, mirroring `_vscode.py`'s module-docstring style (public-API list, caller-decides-when-to-write note) and loading its template through `_render.render`.
  ...
  - `render_tasks(name: str, sessions_cfg: dict | None = None) -> str` validates `name` (non-empty, no `"`, backslash or control characters, else `ValueError`), resolves sessions, builds one command per `TASK_SPECS` entry, JSON-escapes each command (the inner text of `json.dumps(command)`), and substitutes the tokens `<CMD_START>`, `<CMD_START_AUTO>`, `<CMD_START_ORCH>`, `<CMD_PLAN>`, `<CMD_GO>`, `<CMD_QUICK>` into the template.
