...
### Card 1: Add new op constants and ERR_VALIDATION to wiki/__init__.py
...
### Card 2: Add WikiValidationError and bump PROTOCOL_VERSION
...
### Card 3: Update test-wiki-protocol.py for new version and symbols
...
- **Creates:** none
- **Deletes:** none
- **Requirements:** Write these test cases in `test-wiki-protocol.py` (TDD: add them before the implementation, confirm they pass after): (a) `PROTOCOL_VERSION == 3` (update any existing assertion from `== 2`); (b) `OP_SET_DEPS == "set_deps"` exists and is importable; (c) `OP_MIGRATE_DEPS == "migrate_deps"` exists and is importable; (d) `ERR_VALIDATION == "validation_error"` exists and is importable; (e) `WikiValidationError` is a subclass of `WikiError` and can be raised and caught as `WikiError`.
