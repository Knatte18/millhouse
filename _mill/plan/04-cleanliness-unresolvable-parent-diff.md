...
- **Moves:** none
- **Requirements:**
  1. In `_parent_diff_names` (`plugins/mill/scripts/_cleanliness.py:113-130`): change the return-type annotation from `-> list[str]` to `-> list[str] | None`. Change the `if result.returncode != 0:` branch's `return []` (line 129) to `return None`. Update the stderr warning message and the docstring to describe the diff as "unresolvable" (unknown) rather than "empty" — the log line currently reads `f" exited {result.returncode} -- treating parent diff as empty"`; change the trailing clause to `" -- parent diff is unresolvable (unknown, not empty)"`.
...
- **Requirements:**
  ...
  2. Add a new **CTD-6** block, inserted immediately after the existing CTD-5 block (the "`compute_terminal_dirt: absolute task_dir is relativized...`" case, ending around line 451, right before the PDN-1 block): patch `_cleanliness._parent_diff_names` to `return_value=None` (mirroring the mocking convention every CTD-\* case already uses at its own `with unittest.mock.patch("_cleanliness._parent_diff_names", return_value=...)` line) and call `compute_terminal_dirt(Path(tmp), Path("_mill"), "main")`; assert the result `is None`. `print("PASS: compute_terminal_dirt: unresolvable parent diff -> None")`.
  3. Add a new **ROOD-5** block, inserted immediately after the existing ROOD-4 block (the "`revert_out_of_scope_drift: file in parent-diff owned set but outside task_dir is in-scope`" case): patch `_cleanliness._parent_diff_names` to `return_value=None` and call `revert_out_of_scope_drift(Path(tmp), Path("_mill"), "main")` (reuse the same tempfile git fixture setup the ROOD-\* cases already share); assert the returned tuple is `([], None)` — `reverted_paths == []` and `remaining_in_scope_lines is None`. `print("PASS: revert_out_of_scope_drift: unresolvable parent diff -> ([], None), nothing reverted")`.
